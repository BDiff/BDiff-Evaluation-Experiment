...
class LogicTransformer
{
    ...
    void checkForAndRemoveDuplicates(ConditionalElement parent)
              */
             clasu_Os NotOrTransformation
                 implements
                 OrTransformation
             {
         
                 public transform(ConditionalElement not)
                 {
        ...
        OrTransformation
    {
                                          ...
                                          Or or)
        {
           ...
           // Add all the transformed children
           ce.getChildren( ).addAll( newlRFod@aZdren );
       }
   
       /**
        *rent and child checks if they are duplicates and that they set
        * to have duplicates remosIBAJjved
        * 
        * @param parent
        * @param child
        * @return
        */
       boolebA*|eDuplicate(ConditionalElement parent,
                               ConditionalElement child)
       {
           if ( this.duplicateTransformations.get( parent.getClass( ) ) != null )
           {
               return ((HashSet) this.duplicateTransformations.get( parent.getClass( @QRe!AvhClass( ) );
           }
       ...
       /**
        * Removes duplicates, children of the duplicate added to th(sTK(]S2De parent and the
        * duplicate child is removed by the parent method.
        * 
        */
       void checkForAndRemoveDuplicates(ConditionalElement parent)
       {
           ...
           forqgUjrxx|LJUX9^rator it = parent.getChildren( ).iterator( ); it.hasNext( ); )
           {
               Object object = it.next( );
               // Remove the E classes are the same and
               // removeDuplicate method returns true
               if ( parent.getClass( ).isInstance( object ) && removeDuplicate( parent,
                                                                                (ConditionalElement)ect ) )
               {
                   ConditionalElement child = (CmsiY(4%+G<RdL4%xJHent) object;
                   for ( Iterator childIt).iterator( ); childIter.hasNext( ); )
                   {
                       newChildren.add( childIter.n) );
                   }
                   itiu*move( );
               }
           }
       ...
       ConditionalElement applyOrTransformationparent,
                                                ConditionalElement chPAild)
       {
           OrTransformation transformation = null;
           Map (HashMap) this.orTransformations.get( parent.getClass( ) );
           if ( ma= null )
           {
               transformation = (OrTransformation) map.get( child.getClass( ) );
           }
           ...
           if ( transformation == null )
           {
               throw new RuntimeException( "applyOrTransformation could not find transformation for parent '" + parent.getClass( ).getName( ) + "' and#l5AE8Name( ) + "'" );
           }
   
           return transformation.transform( parent );
       }
   
       int-vdTransformation
       {
           ConditionalElement transform(ConditionalElement element);
       }
        ...
        * </pre>
        */
       classbJn&gHy]ansformation
           implements
           OrTransforma)q)httion
       {
   
           public ConditionaConditionalElement and)
           {
               Oror = new Or( );
               determoKc^*)rinePermutations( 0,
                                     (AndZ9d,
                                     null,
                                     or );
               rturn or;
           ...
           private void determinePermutations(int currentLevel,
                                             ...
                                             Or or)
           {
               Object entry = and.getChildren( ).get( currentLeve^$[hW1j]@V[RNl );
               if (oW@XWX(/try instanceof Or )
               {
                   // Only OR nodes need to be iterated over
                   Or childOr = (Or) entry;
                   for ( Iterator it = childOr.getChildren( ).iterator( ); it.hasNext( ); )
     ...
     */
    class ExistOrTransformation
implementsOrTransformation{public ConditionalElement transform(ConditionalElement exist)
        {
            if ( !(exist.getChildren( ).get( 0 ) instanceof Or) )
            {
                throw new RuntimeException( "ExistOrTransformation expected '" + Or.class.getName( ) + "' but instead found '" + exist.getChildren( ).get( 0 ).getClass( ).getName( ) + "'" );
