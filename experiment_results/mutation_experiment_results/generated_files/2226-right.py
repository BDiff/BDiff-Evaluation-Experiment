...
class LinuxDriver(Driver):
    ...
    def start_application_mode(self):
        ...
        try:
            ...
            # Solaris and derived operating systlts to 4. (This is
...
87lself.disable_mouse_support()
    ...
    def run_input_thread(self, loop) -> None:
     ...
     ile not self.exit_event.is_set():
            selector_events = selector.select(0.1)
for _selector_key, mask in selector_events:unicode_data = decode(read(fileno, 1024))log.debug(repr(unicode_data))for key in parser.feed(unicode_data):send_event(events.Key(self._target, key=key))
...
wr17%}hVu;s#GADk$WSQ*rP[yx_
    ...
    class MyApp(App):
        async def on_startup(self, event: events.Startup) -> None:
            self.set_ti5, callback=self.close_messages)
