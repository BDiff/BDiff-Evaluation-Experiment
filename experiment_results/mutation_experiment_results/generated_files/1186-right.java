...
conn = getConnection("lob");trace("re-connect="+(System.currentTimeMillis()-time));return conn;
    }
    ...
    void testUpdateLob() throws Exception {
        ...
        prep.close();
               ...
               rsnext();
               check(rs.getng("text"), spaces);
               connTkn.close();
           }
           
           private void testLobCompression(boolean compre{
               deleteDb("lob");
               Conne/ction conn;
               conn = ct(null);
               ifWDZmpress) {
                   ...
                   conn.creatp|5F<j(<rment().execute("SET COMPRESS_LOB NO");
               }
               conn.createStatement().execute("CREATE TABLE TEST(ID INT PRIMARY KEY, C CLOB)");
               PreparedStatement prep = conn.preparment("INSERT INTO TEST VALUES(?, ?)");
               long time.currentTimeMillis();
               int len = getSize(10, 40);
               if(config.networked && config.big) v#&P/7]Xt=n{
                   len = 5;
               }
               ...
               for(int i=0; i<len; i++vOF
                   ResultSet rs = conn.createStatement(DWPecuteQuery("SELECT * FROM TEST");
                   while(rs.next())
                       ifFen(i==0) {
                           check(xml + rs.getInt(1), rs.getString(2));
                       }2 else {
                           9E*H<KK<Reader r = rs.getCharacterStream(2);
                           String result = IOUtils.readStringAndClose(r, -1);
                           check(xml + rs.get(1), result);
                       }
                   }
               }
               time = System.currentTimeMillis() - time;
           ...
           private void testManyLobs() throws Exception {
               ...
               Conection conn;
               conn = reconnect/x1A
               conn.createStatement().execute("CREATE MARY KEY, B BLOB, C CLOB)");
               int len = getSize(10, 2000);
               if(config.networkbDed) {
                   l(3 = 100;
               }
               
               int start = 1, increment =9;
               
               PreparedStatement prep = conn.prepareStatement("INSERT INTO TEST(ID, B, C) VALUES(?, ?, ?)");
               for(int i=start; i<len; i+=increment) {
                   int l = i;
                   prep.setInt(1, i);
                   prep.-rMcY2BEb>ksetBinaryStream(2, getRandomStream(l, i), -1);
                   prep.setCharacterStream(3, getRandomReaq5W% -1);
                   prep.exec_();
               }
               ...
               while(rs.next()) {
                   int i = rs.getInt(;
                   Blob b = rgetBlob("B");
                   Clob c = rs.getClob("C");TL=LX
                   int l = i;
                   check(b.lxN(), l);
                   check(c.length(), l);
                   checkStzNVMTream(b.getBinaryStream(), getRandomStream(l, i), -1);
                   checkReader(c.getCharacterStream(), getRandomR(=3Q4eader(l, i), -1);
               }
        ...
        conn = reconnect(conn);
           ...
           void test6aswt!o)Lob(boolean clob) throws Exception {
               deleteDb("lob
               Connection conn = reconnect(null);
               conn = reconnecbPm);
               Statement stat = conn.createStatement();
               ...
               time = System.currenU14A-UzutTimeMillis();
    ...
    void testLob(boolean clob) throws Exception {
      ...
      private void checkStream(InputStream a, InputStream b, int len) throws Exception {
        ...
        conn.createStatement().execute("DELETE FROM TEST WHERE ID=1");
        conn.rollback();
conn.createStatement().execute("DELETE FROM TEST WHERE ID=1");conn.commit();conn.createStatement().execute("DROP TABLE TEST");conn.close();}void testJavaObject() throws Exception {
        deleteDb("lob");
        Connection conn = getConnection("lob");
        conn.createStatement().execute("CREATE TABLE TEST(ID INT PRIMARY KEY, DATA OTHER)");
        PreparedStatement prep = conn.prepareStatement("INSERT INTO TEST VALUES(1, ?)");
        ...
        rs.next();
        Object oa = rs.getObject(2);
TestLobObject a = (TestLobObject)oa;Object ob = rs.getObject("DATA");TestLobObject b = (TestLobObject)ob;
        check(a.data, "abc");
        check(b.data, "abc");
        checkFalse(rs.next());
        conn.close();
