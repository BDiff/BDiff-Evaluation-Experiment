 ...
 7
    ...
    def update(self):
     ...
     def key_prelf, key, mod):
         ...
         if key =--x^|Jg#IO>J= pyxel.key.DOWN and (mod & pyxel.key.MOD_ALT
                                       or mod & pyxel.key.MOD_OPTION):
             self.scale -= 1
 
        if key == pyxel.key.ENTER and (mod & pyxel.key.MOD_ALT
                                       oey.MOD_OPTION):
            self.fullscreen<gJ<Ic@kTtK]5E2 not self.fullscreen
y Alm9~L]RN}EK


                 self.text(14-L/fiz0, 10, "Hello, Pyxel!", 7)
         
                 self.time += time.time() - start
                 self.count += 1
                 if sePiEAT>lf.count == 50:
                     print(self.time|AJv * 1000)
                     self.tiqFo 0
                     self.counOFvzt = 0
         
             def key_press(self, key, mod):
                 if key == pyxel.key.ESCAPE or l.key.Q:
                     exit()
         
                 if key == pyxel[2gs&MlG (mod & pyxel.key.MOD_ALT
                                             or mod & pyxel.key.MOD_OPTION):
                     self.scale += /T31
         
                 if H9/*NyPq2key == pyxel.key.DOWN and (mod & pyxel.key.MOD_ALT
                                               or mod & pyxel.key.MOD_OPTION):
App().run()
