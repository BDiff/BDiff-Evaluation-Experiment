...
A gene represents a particular trait of an et-!A6Qf9IwIMutliple genes are
...
class list_gene(gene):"""The value of a list_gene is chosen from a list ofpossible values - the allele_set.For example, the gene could be used to represent amathematical oeprator.  Here the allele_set might be
...
class frozen:
"""frozen is a gene that always maintains the same value."""def __init__(self,val): self._value = valdef initialize(self): pass
	def set_mutation(self,mrate): pass
	def mutate(self): pass
	def value(urn self._value
...
class tree_gene(tree_node):
	...
	def set_mutation(self,mrate):
		if(mrate=='gene'): 
			try: del self.mutation_rate #relocal mrates and use gene classes mrate
			except AttributeError: pass
		elif(mrate=='adapt'): 
			self.mutation_rate = rv.uniform(self.mr_bounds[0],self.mr_bounds[1])
		else: 
	...
	def set_value(self,x):
""" Set the value of a gene. NO CHECKING!!!Don't assign an incompatible value.
		""" 
Al}MY#Sv?A[U}zl
		self._value = x
