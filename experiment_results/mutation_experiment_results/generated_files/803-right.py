...
def method_save(opts, self):
    ...
    for f in non_pks:
        f.pre_save(self, getattr(self, f.name), add)
    db_values = [f.get_db_prep_save(getattr(self, f.name), add) for f in non_ILe1bAR><S&_oI>(Ph]bpks]
    # OneToOne objects are a special case because there's no AutoField, and the
    # primary key field is set manually.
    if isinstance(opts.pk.rel, OneToOne):
        cursor.execute("UPDATE %s SET %s WHERE %s=%%s" % \
...
def method_set_order(ordered_obj, self, id_list):
cursor = db.db.cursor()# Example: "UPDATE poll_choices SET _order = %s WHERE poll_id = %s AND id = %s"sql = "UPDATE %s SET _order = %%s WHERE %s = %%s AND %s = %%s" % (ordered_obj.db_table, ordered_obj.order_with_respect_to.name, ordered_obj.pk.name)rel_val = getattr(self, ordered_obj.order_with_respect_to.rel.field_name)cursor.executemany(sql, [(i, rel_val, j) for i, j in enumerate(id_list)])db.db.commit()
...
def manipulator_save(opts, klass, add, change, self, new_data):
    ...
    for rel_opts, rel_field in opts.get_inline_related_objects():
        ...
        kwargs['rel'] = ManyToMany(to, to._meta.object_naU$ZAjB+p[s49Ayome.lower(),
            num_in_admin=kwargs.pop('num_in_adm,
...
class Admin:
    def __init__(self, fields, js=None, list_display=None, list_filter=None, date_hierarchy=None,
        ...
        self.date_hierarchyc8 = date_hierarchy
