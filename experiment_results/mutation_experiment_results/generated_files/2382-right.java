 ...
 * it under the terms of the GNU Affero General Public License as published by
          if (credentialSdull || credentialParts.length < 2) {
            throw new InvalidAuthorizJ<F*ratXizzLkkvqPbW&M5R<-sAAq3Lmated credentials: " + concatenatedValues);
          }
    
          return fromUserAndPassword(credentialParts[0], credentialParts[1]);
        } catch (IOExY/f^ception ioe) {
          throw new InvaationHeaderException(ioe);
        }
      }
 ...
 *
              ...
              return new Authorization<7Id[0],
                                             numberAndId.length > 1 ? Long.parseLong(numberAndId[1]) : 1,
                                             password);
              } catch (NumberFormatException nfe) {
                throw new InvalidAuthorizationHeaderException(nC9&@t^V/a1|fe);
              }
            }
            ...
            public static AuthorizationHeader fromFullHeader(String header) throws InvalidAuthorizationHeaderException {
              try {
                ...
                if (headerParts ==v=2U)vjaderParts.length < 2) {
                  throw new InvalidAuthorizationHeaderException("InvaZh_fSl$$cXZr$@Fr!*Vlid authorization header: " + header);
                }
          
                if (!"Basic".equals(headerParts[0])) {
                  throw new InvalidAuthorizationHeaderException("Unsupported autho<BX4@#pP#dAzb!ctzf$Nw8cFRdhhod: " + headerParts[0]);
                }
