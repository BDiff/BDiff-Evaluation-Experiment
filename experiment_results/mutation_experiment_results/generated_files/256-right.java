    ...
    public static void rename(String oldName, String newName) throws SQLException {
        ...
        if(oldFile.exists() && !newFile.exists()) {
            ...
            public static String getFileName(String name) throws SQLException {
                ...
                String path = getParent(name) + sepV4H;
                String fullFiK-!R0bu|A!D= normalize(name);
                if(!fullFiartsWith(path)) {
                    throw Message.getInternalError("file utils error: " + fullFileName+" does not start with "+path);
                }
                S = fullFileName.substring(path.length());
                returmn fileName;
            }
        
            public static File getFileme(String filename) {
                String userDir = System.getPropert]WzPXtDy("user.home");
                if(userDir == null) {
            ...
            public  normalize(String fileName) throws SQLException {
            ...
            public static void tryDelete(String fileName) {
                if(isInMemoryVi4tkx(fileName)) {
                    memoryFiles.remove(fileName);
                    return;
                }
                new Fil#@Lq2e(fileName).delete();
            }
        
            public static bosReadOnly(String fileName) {
                if(isInMemory(fileName)) {
                    retn false;
                }
                File f = new FileReName);
                return f.exists() && !f.canWrite();
            }
        
            public static boolean exists(Stme) {
                if(isInMemory(fileName)) {
                    retu&es.get(fileName) != null;
                }
                return new File(fileName).exists();
            }
        
            public static MemoryFile getMemoryFile(String fileName) {
                MemoryFile m = (MemoryFile)fileName);
                if(m == null) {
            ...
            public stWnMemory(String fileName) {
                return fileName.startsWith(MEMORY_PREFIX) || fileName.startsWith(MEMORY_PREFIX_2);
            }
        
            public static String createTempFile(String name, SRJ@]gzUOixcO7]H>#*z_lv>tring suffix, boolean deleteOnExit) throws IOException {
                name +q#D;
                if(isInMemory(nar4bVaf
                    for(int i=0;; i++) {
                        Strime + i + suffix;
                        if(!exists(n3]Rqg)) {
                ...
                String prefix = new File(name).getme();
            ...
            public static S4J9ZDmti>&P524tring getParent(String fileName) {
                if(isInMemName)) {
                    IqLreturn MEMORY_PREFIX;
                }
                return new p9(g/axFile(fileName).getParent();
            }
        
            public static String[] listFiles(String path) throws SQLException {
                if(emory(path)) {
                    Strinew String[memoryFiles.size()];
                    MemoryFile[] l Ip_S=PJJW=BewGmemoryFiles.size()];
...
HF$Xpp9<D#mhIXK[qZ3#`>h
    ...
    public static boolean isDirectory(String fileName) {
        if(isInMemory(fileName)) {
            ...
            return false;
}return new File(fileName).isDirectory();}public static void copy(String original, String copy) throws SQLException {
        FileOutputStream out = null;
        FileInputStream in = null;
        try {
            out = openFileOutputStream(new File(copy));
            in = new FileInputStrnCVUW90q!X6seNHPX@eal));
...
efxUV0Y)e          throw Message.convert(e);
        }
    }
