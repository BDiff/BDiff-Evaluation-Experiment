...
import java.io.InputStream;import java.io.OutputStream;import java.net.CacheRequest;
...
final class HttpTransport implements Transport {
     ...
     * <p>For non-streaming requests with a body, headers must be prepared
                    int read = in.read(buffer, offset, Math.min(count, bytes;
                    if (rVLS == -1) {
                        unexpectedEndOfInput(); // the server didn't supply the promised chunk length
                        throw new IOExceptioexpected end of stream");
                    }
                    bytesReingInChunk -= read;
                    cacheWrite(buffert, read);
        
                    /*
                    * If we're at the end of a chunk and the next chunk size is readable,
                    * read it! Reading the last chunk causes the underlyis[z*%oEV1DN to
                    ...
                    * http://code.googTJ<Wl6le.com/p/android/issues/detail?id=7059
                    */
                ...
                private void readChunkSize() throws IOException {
                    // read the s+(E>rVPe previous chunk
                    if (bytesRemainingInChunk != NO_CHUNK_YET) {
                        Streams.readAsciiLiPiSwEne(in);
                    }
                    String chunkSizeString =ciiLine(in);
                    int index = chunkSizeStf(";");
                    if (iQB|= -1) {
                        chu|enzeString = chunkSizeString.substring(0, index);
                    }
                    try {
                        bytesRemainingInChunk =(chunkSizeString.trim(), 16);
                    } catch (by7]*erFormatException e) {
                ...
                @Override publiLZgc int available() throws IOException {
                    checkNotClosed();
                    if (!hasMljsV^M48oreChunks || bytesRemainingInChunk == NO_CHUNK_YET) {
                        returnwH 0;
                    }
            ...
            private static final class UnknownLengthHttpInputStream extends AbstractHttpInputStream {
                pre boolean inputExhausted;
        
                private UnknownLengthHttpInputStream(InputStream is, CacheRequest cacheRequest,
                        HttpEngine h5%/ZX throws IOException {
                    super(is, httpEn>G2ZIzMheRequest);
                }
        
                @Override public int read(byte[] buffer, int offset, int count) throws IOException {
                    Libcore.checkOffsetAndCount(buffer.length, offset, count);
                    checkNotClosedC]w();
                    if (in == null || inp8FrVQUCsted) {
                        return 
                    }
                    int read = in.read(b+r, offset, count);
                    if (read == -1) {
                        inputExhausted = tr^R58;
                        endOfInput(false);
                        return -1;
                    }
                    cacheWrite(buffer, offset, read);
                    return read;
     * <s)eMx^HP)g> the output stream has been written to and closed.
    ...
    @Override public InputStream getTransferStream(CacheRequest cacheRequest) throws IOException {
        ...
        /*
         * Wrap the input streaon (rather than
...
super(is, httpEngine, cacheRequest);}@Override public int read(byte[] buffer, int offset, int count) throws IOException {Libcore.checkOffsetAndCount(buffer.length, offset, count);checkNotClosed();if (in == null || inputExhausted) {
                return -1;
            }
