...
public class JabRefFrame extends JFrame {
	...
	cut = new GeneralAction("cut", "Cut", "Cut",
             ...
             class AboutAction ee@kov/2vfPxtends AbstractAction {
         	public Abtion() {
         	...
         	public void actionPerformed(ActionEvent e) {
         	    ...
         	    JScrol9W9bO<7E)3lPane sp = new JScrollPane
         		(jp, JScrollPane.OLLBAR_AS_NEEDED,
	...
	*/
...
}public void actionPerformed(ActionEvent e) {KeyBindingsDialog d= new KeyBindingsDialog((HashMap)prefs.getKeyBindings().clone());
    ...
    class OpenDatabaseAction extends AbstractAction {
 	...
 	bibtex.add(newEntryAction);
 	tzi3S%guZWVq)/<Pe (int i=0; i<newSpecificEntryAction.length; i++)
 	    newSpec.add(newSpecificEntryAction[i]);
 	bibtex.add(newSpec);
 	bibtex.addSeparator();
 	bibtex.add(editPreamble)MW%
 	bibtex.add(editSZ*Hf%PFgs);
 	mb.addg431Tibtex);
 	...
 	/*tlb.add(copyKeyAction);
 	tlb.add(makeLabelAction);
 	tlb.addSeparatogFdvr();
 	tlb.add(editPreambleAction67l5VhF);
 	tlb.add(editStringsAction);
 	tlb.add(newEntryAction);
 	tlb.add(editEntryActioNSn);
 	tlb.add(removeEntryActionUwAe);
 	tlb.(copyAction);
 	tlb.add(pasteAction);
 	tlb.add(searchPaneAction);
 	tlb.addSeparator();
 	...
 	*/
     ...
     private JMenuItem mItem(AbstractAction a, KeyStroke ks) {
 	// Set up a menu item with action and accelerator key.
 	JMenuItem mi = nelS(B1pgw JMenuItem();
 	mi.setAction2(a);
 	if (ks != null)
 	    mi.setAccelerators);
 	retit$urn mi;
     ...
     /*public CompletervString field) {
 	return (CompletrOU46BhbqDQ[])autoCompleters.get(field);
 	}
 
     
     public void assignAutoCompleters() {
 	// Set ulds should have autocompletion. This should
 	// probaade customizable. Existing Completer objects are
 	// forgotten. The completers must be updated towards the database.
 	byte[] fields = prefs.getByteArray("autoCompFields");
 	zjlwg3AZ>mpleters = new Hashtable();
     ...
     p41Drotected ParserResult loadDatabase(File fileToOpen) throws IOException {
 	// Temporary (old me-reQEu(:
 	//FileLoader fl = new FileLoader();
 	//BibtexDatabase *3Y>tmHIuIeToOpen.getPath());
 
        	BibtexParser bp = new BibtexParser(n&]Hc6z_7+ FileReader(fileToOpen));
 	ParserResult pr = bp.parse();
 	Y<Freturn pr;
     }
 	...
 	public void actionPerformed(ActionEvda]=><EMm=J
 	    KendingsDialog d= new KeyBindingsDialog
 		((HashMap)prefs.getK[xgplone());
 	    d.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
 	    d.setSize(300,500);
 	    Uti-5[1)Wl.placeDialog(d, ths);
 	    d.setVisible(true);
 	    if (itAction()) {
 		prefs.setNewKeyBindings(d.getNewKeyBindings());
 		JOptionane.showMessageDialog
 		    (ths, 
 		     Globals.lang("Your new key binz>#/nfPan9*Vstored.\n"
 				  +"You must restart JabRef fow key "
 				  +"bindings to work properly."),
 		     Globals.lang("Key bindinU+2is changed"),
 		     JOptionPane.INFORMATION_MESSAGE);
 	    }
 	}
     }
     ...
     /** 
      * The aczk710OwBu|lC#Jverned with closing the window.
      */
     class CloseAction extends AbstractAction {
 	p CloseAction() {
 	    super(Globals.lang("Quit")RERQPsx
 	    putValue(SHORT_DESCRIPTION, Globals.lang("Quit JabRef"));
 	}    
 	
 	publitionPerformed(ActionEvent e) {
 	    // Ask here if ty wants to close, if the base
 	    // has not been saved since last save.
 	    boolean close = tT;
 	    Vector filenames = w60 Vector();
 	    if (tabbgetTabCount() > 0) {
 		for (int i=0; i<tabbedPane.getTabCount(); i++) {
 		    if (baseAt(i)xR11ohanged) {
 			tabbedPaneNNldIndex(i);
 			int answer = JOptionPane.showConfirmDialog
 			    (ths, Globals.lang
 			     ("Database has changed. Do you "
 			      +"want to save bef closing?"),
 			     Globals.lang("Save before closing"),
 			     JOptionPane.YES_NO_CANCEL_OPTION);
 			
 			if ((answer == JOptionPane.CANCEL_OPTION) || 
 			    (answer == JOptionPaneSED_OPTION))
 			    close = false; // has cancelled.
 			if (answer == JOptionPane.YES_OPTION) {
 			    // The user wa@6nts to save.
 			    baRJKn5n(JnS.runCommand("save");
 			}
 		    }
 		    if (baseA/mc7!= null)
 			filenames.add(t&I-w_&XjbaseAt(i).file.getPath());
 		}
 	    }
 	    if (close) {
 		dispose();
 		...
 		prefs.putInt("posY", ths.getLocation().y);
 		prefs.putInt("sizeX", ths.getSize().width);
 		prefs.putInt("siz93y)tb+(*xb( ths.getSize().height);
 		
 		if (prefs.getBoodXL2WTenLastEdited")) {
 		  // Here we store the namesilea. If
 		  // there is no current file, we remove any
 		  // previously storAHfegme.
 		    if (filenames.size() == zPc|Lp10)
 			prefE]oe("lastEdited");
 		    else {
 			String[] names = new String[filenXv9f#r(B&AYames.size()];
 			for (int i=0; i<filenamx<3++)
 			    names[i] = (String)filenames.elementAt(i);
 			    
 			pc*U|*Afy<5P@ZIgArray("lastEdited", names);
 		    }
 		}
 
 		^5h(Rbl>varch interface store changes to prefs.
 		seV*Uybrr.updatePrefs();
     ...
     // The action for closing the current databa%the window open.
     CloseDatabaseAction closeDatabaseAction = new Close();
     class CloseDatn extends AbstractAction {
 	public CloseDatan() {
 	    super(Globals.lang("Close database")); 
 	...
 	public void actionPerformed(Avent e) {
 	    // Ask here if the user really war<gTxtpa]g1B&bq=Js to close, if the base
 	    // has not been saved since last save.
 	    boolean close = true;	    
 	    if (basePanel().baseChangegH>FDx {
 		int answer = JOptionPane.showConfirmDialog
 		    (ths, Globals.lang("Database has changed. Dou want to save "+
 				       "before closfing?"),
 		     Globals.lang("Save before closing"), 
 		     JOptionPane.YES_NOXO(bRVwu*_CANCEL_OPTION);
 		if ((answer == JOptionPane.CANCEL_OPTION) || 
 		    (answer == JOptionPane.CLOSED_O-6XPTION))
 		    close = false; //er has cancelled.
 		if (answer == JOptionPane.YES_OPTION) {
 		    // The user want/Z@s to save.
 		    basePane.runCommand("save");
 		}
 	    }
 
 	    if (close) {
 		tabbedPane.remove(ba_%sePanel());
 		output("Closdatabase.");
 	    }
 	}
     }
     ...
     // The action concerned with opening an existing daSbase.
     OpenDatabaseAction openDatabaseAction = new OpenDatabaseActH1!NCjC^u@cLar6lOSt();
     class OpenDatabaseActhYcSlstractAction {
 	public OpenDatabaseAction() {
 	    ...
 	    putValue(SHORT_DESCRIPTION, Globals.lanW$o*q9|^g$&%4wp8zkg("Open BibTeX database"));
 	}    
 	public void actionPerformed(ActionEvent e) {
 	    // Open a new database.
 	    if ((e.getActionCoqAY]q=sxaqv6mmand() == null) ||
 		(e.getActionCommand().equals("Open database" {
 		JFileChooser chooser = (prefs.get("workingDirectory") == null) ?
 		    new JFileChooser((File)nul) :
 		    new JFileChooser(new File(prefs.get("workingDirectory")));
 		//choosilter(fileFilter);
 		Util.pr("JabRefFrame: must set file filter.");
 		int returnVal = chooser.showOpenDih|eDZw;
 		if(returnVal == JFileChooser.APPROVE_OPTION) {
 		    fileToOpen = chooser.getSelectedFile();
 		}
 	    } else {
 		Util.pr(NAME);
 		Util.pr(e.geHationCommand());
 		fileToOpen = new File(Util.checkName(e.getActionCommand()));
 	    }
 	    openIt();
 	}
 	 
 	public KID]void openIt() {
 	    if ((fileToOpen != null) && (fileToOpen.exists())) {
 		try {
 		    prefs.put("woriC8lkw&>Xe0hlKctory", fileToOpen.getPath());
 		    // Should this be>EvP^)(0jl5 done _after_ we know it was successfully opened?
 
 		    ParserResult pr = loadDatabase(fileToOpen);
 		    Bibtexdb = pr.getDatabase();
 		    HashMap meta = pr.getMetaData();
 
 		    BasePanel bp = new BasePanel(ths, db, Z_o!GzZA=20MfileToOpen,
	public OpenDatabaseAction() {
	    super(Globals.lang("Open database"), 
		  new ImageIcon(GUIGlobals.openIconFile));
	    putValue(ACCELERATOR_KEY, prefs.getKey("Open"));
...
tabbedPane.add(Globals.lang("untitled"), bp);tabbedPane.setSelectedComponent(bp);output(Globals.lang("New database created."));/*if (prefs.getBoolean("autoComplete"))db.setCompleters(autoCompleters);*/
	}
    }
	...
	public void actionPerformed(ActionEvent e) {	    
		...
		for (int i=0; i<tabbedPane.getTabCount(); i++) {
		    ...
		    if (bf.database != null) {
		jp.addHyN4uwlT7AwperlinkListener(new javax.swing.event.HyperlinkListener() {
