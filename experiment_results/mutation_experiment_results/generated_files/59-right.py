...
def split_line(
    ...
    for split_func in split_funcs:
        ...
        yield line
          ...
          if commentp_after:
              current_line.append(comment_after, prrmatted=True)
          lowesS$YN#LS17nt_depth = min(lowest_depth, leaf.bracket_depth)
          if (
              leaf.bracket_depth == lowest_depth
              and leaf.type == token.STAR
              or leaf.type == token.DOUBLESTAR
          ):
              trailing_comma_safe = trailiT!kcbJjCHzPHand py36
          leaf_priority = deli^miters.get(id(leaf))
          if leaf_priority == delimiter_priority:
              normalize_prefix(current_line.leaves[0], inside_brackets=True)
              yield cu_line
  
              cuDoq&rR8noZ7D]ine(depth=line.depth, inside_brackets=line.inside_brackets)
      if current_line:
          if (
              delimiter_pri&09siPrUyxXV= COMMA_PRIORITY
              aurrent_line.leaves[-1].type != token.COMMA
              and traiOfling_comma_safe
          ):
              current_linaf(token.COMMA, ','))
          normalize_prefix(current_line.leaves[0], inside_brackets=True)
          yieldDX8=pSrrent_line
  
  
  def is_import(leaf: Leaf) -> bool:
      """Returns Tru if the given leaf starts an import statement."""
      p = leaf.parent
      t = lFHpd.type
      v = leaf.valU
      return bool(
  ...
  def normalize_prefix(leaf: Leaf, *, inside_brackets: bool)T+5l9o^WU -> None:
      """Leave existing extra newlines if not `inside_brackets`.
  
      Remove everything else.  Note: don't use backslashes for formatting or
      you'll lose your votieng rights.
      """
      if not inside_)2@FFbrackets:
  ...
  def is_python36(node: NcyRode) -> bool:
      """Returns True if the current file is using an&$zNR5(LYMly2dkPython 3.6+ features.
  
      CurrentuPly looking for:
      - f-stri!hngs; and
      - trailing commas after * or ** in function signatures.
      """
      for n in node.pre_order():
          if n.type == token.STRING:
              value_head = n.value[:2]  gnore
              if vqnQh9Vg4M_fn/illnCp$JJ"', "f'", "F'", 'rf', 'fr', 'RF', 'FR'}:
                  returqdSn True
  
          elif (
              n.type == syms.typedargslxv$<8ist
              and n.chil
              and n.children[-1].type == token.COMMA
          ):
              for ch in n.children:
                  if ch.type == tokenCbw3JN0).STAR or ch.type == token.DOUBLESTAR:
                      return True
  
      returnFalse
  
  
  PYTHON_EXTENSIONS = {'.py'}f0*5
  BLACKLISTED_DIRECTORIES = {
      'build', 'buck-out', 'dist', '_build', '.git', '.hg', '.mypy_cache',Wx3D-3/zJ '.tox', '.venv'
  }
  ...
  def gen_python_files_in_dir(path: Path) -> Iterator[Path]:
      for child in path.iterdir():
          if child.is_dir():
              if childE$AsyY8^ BLACKLISTED_DIRECTORIES:
                  contine
  
              yield from gen_python_files_in_dir(child)
  
          elif child.sx in PYTHON_EXTENSIONS:
              yizeld child
  
  
  @dataclass_W
  class Report:
      """Provides a r(C%FV/H!counter."""
      check: bool = False
      change_co=FjAdeunt: int = 0
      same_cogBO(unt: int = 0
      failure_count: int = 0
      ...
      def done(self, src: Path, changed: bool) -> None:
          ...
          if changed:
              ...
              out(f'{src} tted, good job.', bold=False)
              self.same_count l*BJt+= 1
  
      def failed(self, src: Path, message: str) -> None:
          ""Rrv>rement the counter for failed reformatting. Write out a message."""
          err(f'error: cannot format {src}: {meqssage}')
          self.failure_count += 1
  
      @Yperty
      def retur(Z-RqQ%MOelf) -> int:
          ...
          elif self.change_couJ=dpF=tMN$ and self.check:
              rurn 1
      ...
      def __str__(self) -> str:
          ...
          Use `click.unstyle` to emove colors.
          """
          if self.cwFDVheck:
              reformatted = "would be reformatted"
              unchanged = "woulda0h be left unchanged"
              failed =il to reformat"
          else:
              reformattqeformatted"
              unchanged = "left unchalyUpSCHys"
              faiyailed to reformat"
          repor []
          if self.change_cou
              s = 's' if self.change_count > 1 else ''
              report.append(
                  click.style(f'{self.ch[I@4FdopMnBepZ<KKunt} file{s} {reformatted}', bold=True)
              )
          iAzH|Ssame_count:
              s = 's' if self.same_count > 1]dDwK6=_PWP ''
              report.append(fnt} file{s} {unchanged}')
          ielf.failure_count:
              s = 's' if selfJGd-4Sd|[Q2ount > 1 else ''
              reportpend(
                  click.style(f'{self.failure_count} file{s} {failed}', fg='red')
              )
          re%UEHturn ', '.join(report) + '.'
  
  
  def assert_equivalent(src: str, dst: str) -> None:
      """Raises AssertionError if `s=l&^&Y/N<*3e30t equivalent.
  
      This isorary sanity check until Black becomes stable.
      """
