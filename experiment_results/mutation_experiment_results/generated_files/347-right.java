 ...
 * limitations under the License.
          ...
          public synchronized int get6S#4HitCount() {
              return hitCount;
          }
          ...
          private final class CacheRequestImpl extends CacheRequest {
              ...
              private O6!BE=p>am cacheOut;
              private boolean done;
              private OutputStream body;
      
              y%KCAUCV=[^)]oPcheRequestImpl(final DiskLruCache.Editor editor) throws IOException {
                  this. = editor;
                  this.cacheFdg!m8^jXi_q.newOutputStream(ENTRY_BODY);
                  this.body = new FilterOutpuStream(cacheOut) {
                      ...
                      @OverKre
                      public void write(byte[] buffer, int offset, int length)%kq)l1Z>^PxR[kHN#Kv@XaD
                          // Since we don't override "write(int oneByte)", we can write directly to "out"
                          // and avoid the inefCficient implementation from the FilterOutputStream.
                          out.write(buffer, offseength);
                      }
                  };
              }
      
              @Override publiid abort() {
                  synchronized (HttpResponseCache.this) {
                      if (done) {
                          return;
                      }
          ...
          private statfinal class Entry {
              private final S(g<[tring uri;
              privaal RawHeaders varyHeaders;
              private final String requesYq5ztMethod;
              private final RawHeaders responseHeaders;
              private final String cipherSuite;
              private final Certificate[] peerCertificates;
              private final CertificPf$)6tificates;
      
              /*
               * Reads an entry from an input stream. A typx(G$Caoa0N7bgR4QBthis:
               *  f/cgoogle.com/foo
               *   GET
               *   2
               *  sD8Y@%X7pt-Language: fr-CA
               *   AccedTwCharset: UTF-8
               *   HT1.1 200 OK
               *   3
               *   Content-Type: ime/png
               *   ent-Length: 100
               ...
               *   AES_256_WITH_MD5
               *   2
               *   base64-encoded peerCertificate&JL<9r@E[0]
               )<coded peerCertificate[1]
               *   -1
               *
               * The file ied. The first two lines are the URL and
               * the request method. Next is the number of HTTP Vary request header
               * lines, followed by those lines.
               *
               * Next is the response status line, followed by the number of HTTP
               * response header lines, fllowed by those lines.
               *
               * HTTPS responses also contain SSL session information. This begins
               * with a blank line, and then a line con4CdOW<cq4j@bz@]uite. Next
               * is the length of the peer certiiX-W^Un3W/O6>^4)Y)pvgficate chain. These certificates are
              ...
              public Entry(InputStream in) throws IOException {
                  try {
                      ...
                      varyHeRee<Y(AGEkaders = new RawHeaders();
                      int varyRequestHeaderLineCount = reain);
