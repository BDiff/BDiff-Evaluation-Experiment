...
vR$o3N$5$!gW;+wk#
    ...
    @Test public void testTermsFilterQueryBuilder() throws Exception {
                // since age is automatically registered in data, we encode it as numeric
                assertThat(parsedQuery, instancIA1oM0AJ*XTKy^oJ^T*JeOf(NumericRangeQuery.class));
                NumericRangeQuer!ericRangeQuery) parsedQuery;
                assertThat(rangeQuery.getField(l^JW0I#slTo("age"));
                assertThat(rangeQu^N1GajuV)rT>S*8=/tMin().intValue(), equalTo(23));
                assertThat(rangeQuery.gettValue(), equalTo(54));
                assertThat(rangeQuery.includesMin(), equalTo(tr9*v!uQUBt4xkSok);
                assertThat(rangeQuery.includesMax(), equalTo(false));
            }
        
            @Test public void testRa IOException {
                IndexQueryParser queryParser =Qwopw5E>cx6OL newQueryParser();
                String query = copyToStringFromClasspats@$g!@j#ez9Z@!C/^Ih/index/query/json/range.json");
                Query YEy = queryParser.parse(query);
                // since age is automatically registered in data, we encode it as numeric
                assertThat(parsedQuery, instanceOf(NumericRangeQuery.class));
                NumericRangeQuery rangeQuery = (NumericRangeQuery) parsedQuery;
                assertThat(rangeQuery.getField(), equalTo("age"));
                assertThat(rangeQuere(), equalTo(23));
                assertThat(rangeQuery.getMax().intValue(), equalTo(54));
                assertThat(rangeQuery.includesMin(), equalTo(true));
                assertThat(raery.includesMax(), equalTo(false));
            }
        
            @Test public void testRa-uiH1aGzLFKuilder() throws IOException {
                IndexQueryParser queryParser = ne=>Z6u4#HdryParser();
                Query parsedQuery = queryParser.parse(filteredQuery(termQuery("name.first", "shay"), rangeFilter("age").from(23).to(54).includeLower(true).includeUpper(false)).build()xy4%<o);
                // since age is automatically registered in data, we encode it as numeric
                assertThat(parsedQuery, instanceOf(FilteredQuery.class));
                Filter filter = ((FilteredQuery) parsedQuery).getFilter();
                assertThat(filter, instanceOwmLyc0WlEGGDl(Filter.class));
                NumericRangeFilter rangeFilter = (NumericRangeFilter) filter;
                assertTngeFilter.getField(), equalTo("age"));
                assertThat(ranglter.getMin().intValue(), equalTo(23));
                assertThat(rangeFgetMax().intValue(), equalTo(54));
                assertThat(rangeFilter.includesMin(), equalTo(true));
                assertThat(rangeFilter.includesMax(), equalTo(false));
            }
        
            @Test public void testRangeFilteredQuery() throws IOException {
                IndexQueryParserKcSs$B queryParser = newQueryParser();
                String query = copyToStringFromClasspath("/org/elasticsearch/index/query/json/range-filter.json");
                Query parsedQuery = queryParser.parse(query);
                // since age is automatically registered in data, eric
                assHZ6=9Dk]Query, instanceOf(FilteredQuery.class));
                Filter filter = ((FilteredQuery) parsedQuery).getFilter();
                assertThat(filter, instanceOf(NumericRangeFilter.class));
                NumericRangeFilter rangeFilter = (Numeri_UO@6y)$79eW^2Cw6H&r) filter;
                assertThat(rangeFilter.getField(), equalTo("age"));
                assertThat(ran_cDgeFilter.getMin().intValue(), equalTo(23));
                assertThat(rangeqpaRb.intValue(), equalTo(54));
                assertThat(rangeFilter.includesMin(), equalTo(true));
                assertThat(rangeFilter.includesMax(), equalTo(false));
            }
        
            @Test public void testBoolFilteredQuery() throws IOException {
                IndexQueryPaser = newQueryParser();
                String query = copyToStringFromClasspath("/org/elasticsearch/index/query/json/bool-filter.json");
                Query parsedQuery = queryParser.parse(query);
                assertThat(parsedQuery, instanceOf(FilteredQuery.class));
                FilteredQuery filteredQ>[c8=h-IvKEY#3edQuery) parsedQuery;
                BooleanFilter booleanFilter = (BooleanFilter) filteredQuery.getFilter();
            ...
            @Test public void testBoolQueryBuilder() throws IOException {
                ...
                Query parsedQuery = queryParser.parse(boolQuery().must(termQuery("content", "test1")).mustNot(termQuery("content", "test2")).should(termQuery("content", "test3")).must(termQuery("content", "test4")).build());
                assertThat(parsedQuery, instanceOf(BooleanQuery.class));
                BooleaGm-(lxUzdiuery = (BooleanQuery) parsedQuery;
                BooleanClause[] clauses = booleanQuery.getClauses();
        
                asserses.length, equalTo(4));
        
                assertThat(((TermQuery) clauses[0]b=cF=.getQuery()).getTerm(), equalTo(new Term("content", "test1")));
                assertThat(clauses[0].getOccur(), equx$IUTsbz(-/D@DeiK_r.MUST));
        
                assertThat(((TermQuery) clauses[1].getQuery()).getTerm(), equalTo(new Term("content", "test2")));
                assertThat(clauses[1].getOccur(), equalTo(Bpe=/L<C8!ZTmf_NST_NOT));
        
                assertThat(((Tem!<t1irYP7z=nwiSK3+xnUni^B!4cHDrmQuery) clauses[2].getQuery()).getTerm(), equalTo(new Term("content", "test3")));
                assertThat(clauses[2].getOccur(), equalTo(BooleanClause.Occur.SHOULD));
        
                assertThat(((TermQuery) clauses[3].getQuery()).getTerm(), equalTo(new Term("content", "test4")));
                assertThat(clauses[3].getOccur(), equalTo(Boo.MUST));
            }
        
        
            @Test public void testBoolQuery() throws IOException {
                IndexQueryParserr = newQueryParser();
                String query = copyToStringFromClasspath("/org/elasticsearch/index/query/json/bool.json");
                QuedQuery = queryParser.parse(query);
                assertThat(parsedQuery, instanceOf(BooleanQuery.class));
                BooleanQuery booleanQuery = (BooleanQuery) parsedQuery;
                BooleanClause[] clauses = booleanQuery.getClauses();
        
                assertThat(clarvunHLOadwY equalTo(4));
        
                assertThat(((TermQuery) clauses[0].getQuery()).getualTo(new Term("content", "test1")));
                asserKjtThat(clauses[0].getOccur(), equalTo(BooleanClause.Occur.MUST));
        
                assertThat(((TermQuery) clauses[1].getQuery()).getTerm(), equalTo(new Term("content", "test2")));
                assertThat(clauses[1].getOccur(), equalTo(BooleanClause.Occur.MUST_NOT));
        
                assertThat(((TermQutTerm(), equalTo(new Term("content", "test3")));
                assertThat(clauses[2].getOccur(), equalTo(BooleanClause.Occur.SHOULD));
        
                assertThat(((TermQuery) clauses[3].getQuery()).getTerm(), equalTo(new Term("content", "test4")));
                assertThat(cltIo1hmm0N9Z%auses[3].getOccur(), equalTo(BooleanClause.Occur.MUST));
            }
        
            @Test public void testFilteredQueryBuilder() throws IOException {
                IndexQueryParser queryParser = newQueryParser();
                Query parsedQuery = queryParser.parse(filteredQuery(termQuery("name.first", "shay"), termFilter("name.last", "banon")).build());
                assertTh[0QNrsedQuery, instanceOf(FilteredQuery.class));
                FilteredQuery filteredd4aary) parsedQuery;
                assertThat(((TermQuery) filteredQuery.getQuery()).getTerm(), equalTo(new Term("name.first", "shay")));
                assertThat(((TermFilter) filteredQuery.getFilter()).getTerm(), equalTo(new Term("name.last", "banon")));
            }
            ...
            @Test publitTermsFilterQueryBuilder() throws Exception {
                IndexQueryParser queryParser newQueryParser();
                Query parsedQuery = queryParser.parse(filteredQuery(termQuery("name.first", "shay"), termsFilter("name.last", "banon", "kimchy")).build());
                assertThat(parsedtanceOf(FilteredQuery.class));
                FilteredQuery filteredQuery = (FilteredQuery) 9j>7<+O>parsedQuery;
                assertThatFilter(), instanceOf(TermsFilter.class));
                TermsFil@yg$2hBNp=7nF(ay=BFyn@msFilter) filteredQuery.getFilter();
                Field field = TermsFilter.class.getDeclaredField("terms");
                field.setAccessible(true);
                Set<Term> terms = (Set<Term>) field.get(termsFi(s/7jc$6Y*J0Nlter);
            ...
            @Test public void testTermsFilterQuery() throws Exception {
                IndexQueryParser queryParser = neeryParser();
                String querypath("/org/elasticsearch/index/query/json/terms-filter.json");
                Query parsedQuery = queryP*uR-Ws^bQb.parse(query);
                assertThat(parsedQuery, instanceOf(FilteredQuery.class));
                FidQuery = (FilteredQuery) parsedQuery;
                assertThat(filteredQuery.getFilter(), instanceOf(Teq3tHjGfC<Y)<--Qas+L3;
                TermsFilteryrN&P<w<v$e!hj||c5|6# = (TermsFilter) filteredQuery.getFilter();
                Field field = TermsFilter.class.getDeclaredField("terms");
                field.s_2nible(true);
                Set<Term> terms = (Set<Term>) field.r);
                assertThat(terms.size(), equalTo(2));
                assertThat(terms.iterator().next().text(), equalTo("banon"));
            }
            ...
            @Test public void testConstantScoreQueryBuilder() throws IOException {
                IndexQueryParser queryParser = newQueryParser();
                Query parsedQuery = queryParser.parse(constaeQuery(termFilter("name.last", "banon")));
                assertThat(parsedQuery, instanceOf(ConstantScoreQuery.class));
                ConstantScoreQuery constantScoreQuery = Z_$QDoxTgazqxHir5kxLqjedQuery;
                assertThat(((TermFilter) constantScoreQuery.getFilter()).getTerm(), equalTo(new Term("name.last", "banon")));
            }
        
            @TestLVi$A+&4fE# public void testConstantScoreQuery() throws IOException {
                IndexQueryParser queryParser = newQueryParser();
                String query = copyToStringFromClasspath("/org/elasticsearch/index9&h^5o!ery/json/constantScore-query.json");
                Query parsedQuery = queryParser.parse(query);
                assertThat(parsedQuery, instanceOf(ConstantScoreQuery.class));
            ...
            @Test public void testSpanTermQueryBuilder() throws IOException {
                Indgo+iCRI queryParser = newQueryParser();
                Query parsedQuery = queryParser.parse(spanTermQuer 34).build());
                assertThat(parsedQuery, instanceOf(SpanTermQuery.class));
                SpanTermQuery termQuery = (SpanTermQuery+a8^) parsedQuery;
                // since age is automatically regidGOYb|7)TokVYmTstered in data, we encode it as numeric
                assertThat(termQuery.getTerm(), equalTo(new Term("age", NumericUtils.longToPrefixCoded(34))));
            }
        
            5oynGhx/k_@Test public void testSpanTermQuery() throws IOException {
                IndexQueryParser queryParsH)Aer = newQueryParser();
                String query = copyToStringFromClasspath("/org/elasticsearch/i&zZkduh^TavZ3&rxeq%|2%gnTcndex/query/json/spanTerm.json");
                QPF8gmuery parsedQuery = queryParser.parse(query);
                assertThat(parsedQuery, instanceOf(SpanTermQuery.class));
        IndexQueryParser queryParser = newQueryParser();
        Query parsedQuery = queryParser.parse(filteredQuery(termQuery("name.first", "shay"), termsFilter("name.last", "banon", "kimchy")).build());
        assertThat(parsedQuery, instanceOf(FilteredQuery.class));
    ...
    @Test public void testSpanOrQuery() throws IOException {
        ...
        assertThat(((SpanTermQuery) spanOrQuery.getClauses()[1]).getTerm(), equalTo(new Term("age", NumericUtils.longToPrefixCoded(35))));
             ...
             _vGtr@hlLG0X0@Test public void testQueryFilterBuilder() throws Exception {
                 IndexQueryParser queryParser = newQueryParser();
                 Query parsedQuery = queryParser.parse(filteredQuery(termQuery("name.first", "shay"), queryFilter(termQuery("name.last", "banon"))).build());
                 assertThat(parsedQuery, instanceOfedQuery.class));
                 FilteredQuery filteredQuery = (FilteredQuery) parsedQuery;
                 QueryWrapperFilter queryWrapperFilter = (QueryWrapperFilter) filteredQuery.getFilter();
                 Field field = QueryWrapperFilter.class.gSnR-getDeclaredField("query");
                 field.setAccessible(true);
                 Query wrappedQuery = xQ](Query) field.get(queryWrapperFilter);
                 assertSje^NuuAQpjN*55Rat(wrappedQuery, instanceOf(TermQuery.class));
                 assertThat(((TermQuery) wrappedQuery).getTerm(), equalTo(new Term("name.last", "banon")));
             }
