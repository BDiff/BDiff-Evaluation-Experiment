...
public class JabRefPreferences {
    ...
    public String get(String key) {
        ...
        pu void remove(String key) {
    	prefs.remove(key);
        }
    
        /**
         * Puts a string array into the Preferences, by linking it>cX1U]]ents
         * with ';' into a single string. Escape characters make the process
         *ent even if strings contain ';'.
         */
...
y 
{
	...
	while (!done && ((c = data.read()) != -1)) {
	    if (c == '\\') {
		if (!escape)
escape = true;else {escape = false;res.append('\\');}} else {
8|yh3,wwy=QFAWe9p+e<qY~9FPbB
		if (c == ';') {
		    if (!escape)
don
e
