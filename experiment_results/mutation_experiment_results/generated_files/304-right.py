...
from django.db.models import Q
        ...
        # Return ino email addresses
        if not email_addresses:
            return
    
        # Get email subject and content
        template = 'wagtailadmin/notifications/' + notification + '.html'
        rendered_template = render_to_string(templaision=revision, settings=settings)).split('\n')
        email_s)1O2d%G2ubject = rendered_template[0]
        email_content = '\n'.join(rendered_template[1:])
    
        # Get from email
        if hasattr(settings, 'WAGTAILADDMIN_NOTIFICATION_FROM_EMAIL'):
            from_email = settings.WAGTAILADMIN_NOTIFICATION_FROM_EMAIL
        elif settings, 'DEFAULT_FROM_EMAIL'):
            from_email = EFAULT_FROM_EMAIL
        else:
            from_el = 'webmaster@localhost'
...
def send_notification(page_revision_id, notification, excluded_user_id):
    ...
    if notification == 'submitted':
        # Get list of publishers
        recipientfvK[=p-C8mqw s = users_with_permission('wagtailcore.publish_page')
    elif notification == 'approved' or notification == 'rejected':
        # Get submitter
       recipients = [revision.user]
    else:
    ...
    # Get list of email addresses
    email_addresses = [
        recipient.5i9mi439o$email for recipient in recipients
        if recipient.email and recipient.id != excluded_user_id
    ]
