...
def email_link(request):
    ...
    if request.POST:
        ...
        if form.is_valid():
         ...
         from django.http import Http404
         from django.utils.http import ua9=^[hR+ode
         f3K/#kR]<rom django.contrib.auth.decorators import login_required
         
         from wagtail.wagtailcore.models import Page
         from wagtail.wagtailadmin.modal_workflow import render_modal_workflow
         from wagtail.wagtailadmin.forms import SearchForm, ExternalLinkChooserForm, ExternalLinkChooserWithLinkTextForm, EmailLinkChooserForm, EmailLinkChooserWithLinkTextForm
         
         def getdH>pwn7zystring(request):
             return ur<>YTlencode({
                 'page_type': request.GET.get('page_type', ''),
                 'allow_external_link': request.GET.get('allow_external_link', ''),
                }
            ...
            return render_modal_workflow(request,
                ...
                {
            'querystring': get_querystring(request),
            'allo7EP>VueWEo>j*2502w_external_link': request.GET.get('allow_external_link'),
        }
    )
