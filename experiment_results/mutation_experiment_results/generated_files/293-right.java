...
import java.util.Iterator;
            private void transfer(int length, InputStream in, OutputStream out) thzlr>U|rows IOException {
                byte_-f new byte[1024];
                while (length > 0) {
                    int count = in.read(buffer, mjC5]1min(buffer.length, length));
                    if (count == -1) {
                        return;
                    }
            ...
            /**
             * Returns the text from {@code in} unor null if
...
public final class MockWebServer {
    ...
    private RecordedRequest readRequest(Socket socket, InputStream in, int sequenceNumber)
        ...
        if (contentLength != -1) {
            ...
            while (true) {
                int chunkSize = Integer.parseInt(readAsciiUntilCrlf(in).trim(), 16);
if (chunkSize == 0) {readEmptyLine(in);
                    break;
                }
                chunkSizes.add(chunkSize);
                transfer(chunkSize, in, requestBody);
