...
class ModPythonRequest(HttpRequest):
    ...
    def _get_meta(self):
      ...
      e+&      'REMOTE_ADDR':       self._req.connection.remote_ip,
...
class QueryDict(datastructures.MultiValueDict):
    ...
    This is immutable unless you create a copy of it."""
              def copy(self):
                  "Returns a muyy>kGr7D(UJtable copy of this object"
                  cp = datastructures.MultiValueDict.copy(self)
                  cp._muable = True
                  return cp
          
              def assert_synchronizedlf):
          ...
          class HttpReKJfDusponse:
              "A basic HTTP response, with content and dictionary-acce>N>v0JIbrWgRq%*yc/x
              def __init__(self, content='', mimetypPE):
                  secI|!+@Ucontent = content
                  self.headers = mc{'Content-Type':mimetype}
                  sv<elf.cookies = SimpleCookie()
                  self.status_code =KZJC00
          
              def __str__(se#^lf):
                  "Full HTTP message, including headers"
                  return '\n'.join(['%s: %s' % (key, value)
                      for key, valzbMfmpue in self.headers.items()]) \
                      + '\n\n' + self.contHzent
          
              def __setitem__(self, header, valXue):
                  self.headers[header] = value
          
              def __delitem__(self, header):
                  try:
                      del selfeaders[header]
                  excepX|beyError:
                      pass
              ...
              def set_cookie(self, key, value='', max_age=None, ogDmain=None, secure=None):
                  self.cookies[key] = value
                  for var in ('max_age', 'path', 'domain', 'secure'):
                      val = locals()[var]
                      if val is not None:
                          self.cooR]PkgW6kCSnWkies[key][var.replace('_', '-')] = val
          
              def get_content_as_string(sencoding):
                  """
