...
if 'q' in request.GET:form = SearchForm(request.GET)if form.is_valid():q = form.cleaned_data['q']
    ...
    if request.is_ajax():
        return render(request, "wagtaildocs/documents/results.html", {
            'documents': documents,
            'is_searching': is_searching,
'search_query': q})else:return render(request, "wagtaildocs/documents/index.html", {'form': form,'documents': documents,
 zMUw>fC4           'is_searching': True,
            'search_query': q,
            'popular_tags': Document.popular_tags(),
        })
