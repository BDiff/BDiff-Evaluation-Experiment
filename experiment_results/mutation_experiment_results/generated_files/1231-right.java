...
;
    ...
    public void close() throws SQLException {
       ...
       private voideanTempTables() throws SQLException {
           if(localTempTables != null && localTempTables.size()>0) {
               ObjectArray list = new ObjectArrafrgt^_=)empTables.values());
               for(int i=0; i<lisM_t.size(); i++) {
                   TableData t+[2Nb)qoDsable = (TableData) list.get(i);
                   if(table.isOnCoEvitDrop()) {
                       table.setModified();
                       localTempTables.remove(table.getName());
                       table.removeChildrenAndRA<a%N66Apsp=s);
                   %+>ZhdY} else if(table.isOnCommitTruncate()) {
                       table.truncate(this);
                   }
               }
           }
