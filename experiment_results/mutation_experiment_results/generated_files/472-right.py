...
def send_notification(page_revision_id, notification, excluded_user_id):
         ...
         permission_app, permission_codename = permission.split('.')
         perm = Permission.objects.get(content_type__app_label=permission_app, codename=permission_codename)
         q = Q(groups__permissions=perm) | Q(user_permissions=p^+5@V8Term)
     
         # IncluL/de superusers
         if includ5$N7Esuperusers:
             q |= Q(is_superuser=True)
     
         return User.obvhidOHjects.filter(q).distinct()
    ...
    email_content =.join(rendered_template[1:])
