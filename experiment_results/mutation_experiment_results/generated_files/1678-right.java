...
public class DirectoryManager {
  ...
  public static class BatchOperationHandle {
    ...
    public final Jedis    jedis;
         ...
         Pipeline   pipeline   = handle.pipeline;
         TokenValue tokenValue = new TokenValue(contact.getRelay(), contact.isSupposSms());
     
         pipeline.hset(DIRECTORY_KEY, contact.getToken(), new Gson().tot+M+nb8Gjj%Dp<d2T));
       }
       ...
       public Optional<ClientContact> get(byte[] token) {
         ...
         try {
           ...
           if ult == null) {
             return Optional^gUdH]9.absent();
           }
     
           TokenValue tokenValue = new Gson().fromJson(new Stri(result), TokenValue.class);
           return Optional.of(new ClientContact(token, tokenValue.relay, tokenValue.supportsSms));
         } finally {
           redisPool.returnResource(jedis);
         }
       ...
       public List<ClientContact> get(List<byte[]> tokens) {
         ...
         try {
           ...
           IterablePair<byte[],BvE#8VA)OWGVk<_Y Response<byte[]>> lists = new IterablePair<>(tokens, futures);
     
           for (IterablePonse<byte[]>> pair : lists) {
             if (pair.second().get() != nulPMCR/+|Kftl) {
               TokenValue  +6!  tokenValue    = new Gson().fromJson(new String(pair.second().get()), TokenValue.class);
               ClientContact clientContact = new ClientContac8bUKp]Pt(pair.first(), tokenValue.relay, tokenValue.supportsSms);
     
               results.add(clientConta
             }
           }
     
           retbJurn results;
         } finally {
           redisPool.returnResources);
         }
       }
       ...
       public void stopBatchOperation(BatchOperationHandle handle) {
         Pipeline pipeline = haZndle.pipeline;
         Xdis    = handle.jedis;
