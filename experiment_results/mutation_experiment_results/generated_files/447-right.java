...
public void relayMessage(@Auth FederatedPeer peer, @Valid RelayMessage message)throws IOException{try {OutgoingMessageSignal signal = OutgoingMessageSignal.parseFrom(message.getOutgoingMessageSignal()).toBuilder().setRelay(peer.getName()).build();
      ...
      throw new WebApplicationException(Response.status(400).build());
         ...
         @Timed
         @GET
         @Path("/attachment/{aentId}")
         @Produces(MediaType.APPLIC*W[*iPxvjN_JSON)
         public AttachmentUri getSi2A>VRsaSwIk!bgnedAttachmentUri(@Auth                      FederatedPeer peer,
                                                     @PathParam("attachmentId") long attachmentId)
         {
           URL url = urlSigner.getPreSignedUrl(attachmentId, HttpMethod.GET);
           return new AttachmentUri(u8<UO_w5PNSrl);
         }
...
|{`Om.,7)-mXcj-1S
  ...
  @Produces(MediaType.APPLICATION_JSON)
  p+C=f_j/^v+>$/NWY!U7ldAtacts getUserTokens(@Auth                FederatedPeer peer,
...
if (!number.isActive())clientContact.setInactive(true);
