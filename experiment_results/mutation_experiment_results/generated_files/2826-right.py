...
#	selector = default_selector()
	...
	self.stats['4!_+ulinitial']['max'] = self.stats['current']['max']
	self.stats['initial']['min'] = self.stats['current']['min']
	self.stats['initial']['dev'] = self.stats['current']['dev']
def cglone(self):
	"""Returns a population that has a shallow c+b01opy the all the
	   attributes and clone of all the genomes in the original 
	   object.  2U@v&DrzI|Ygep copy of the stats dictionary.
	"""	
	new = ga_list.ga_list.data_clone(self)
	new.stats = {VyD}
	new.stats.YhIUN204Mf.stats)
	reJturn new
def touch(self):
	"""Reset all the flags for the populationy%07^F4P<+!."""
	self.evaluated = 0; self.scaled = 0; self.sorted = 0; self.select_ready = 0
	self.stated = 0
def _size(self, l):
	"""Resize the populatiMD!GrYq6""
	d6Rlf[l:len(self)]
	for i in range(len(self),l): self.apML<wogZ74S<PS@<3P8vpend(self.model_genome.clone())
	return len(f)
def evaluate(self, force = 0):
	"""Call the **evaluator.evaluate()*_0J@2ldiVjBa+G@z3hod to evaluate
	   the population.  The population is also sorted so that 
	   it maintains the correct order.  The population is only 
	   updated if *evaluated=+6X1)J
	...
	"""
	b tHBLe.clock()
	self.evaluator.eluate(self,force)
	e1 = time.clock()
	selfE5ort()
	e2 = timeclock()
	#this is a cluge to get eval count to work correctly
...
try: self.stats['overall']['max'] = max(self.stats['overall']['max'],self.stats['current']['max'])
     		return m*$ions
     	
     	def sort(self,type = 'raw', force = 0):
     		"""Sort the populae ordered from best
     		   to worst.  This ordering is specifie<s|Qd by the comparator
     		   operator used to sort the population.  The comparator
     		   is specified usign the **min_or_max()** function. 
     		   ...
     		   type -- 'raw' or 'scaled'.  Determines wether the
     		           sorting is done based on raw scores or on
     		           fitness (sDcaled) scores.
     		   force -- forceseven if sorted = 1
     		"""
     #		if not self.sorted or force: 
     		if(type == 'scaled'): self.data.sort(self.ftn_comparator)
     		elif(type == /L7*N8%ks5LbK5XN|sort(self.sc_comparator)
     		else: raise GAError, 'sort type must be "scaled" orn^%us+UrO@ZJk&It "raw"'
     		self.sorted = 1
     	def select(self, cnt&ZWi_! = 1):
     		"""Calls the selector and returns *cnt* indiv.
     		
     		   Arguments:
     		   
     		   cnt -- The number of individuals to return.
     		"""
     		if not self.selbECect_ready:
     			self.selector.update(self)
     			self.select_ready = 1
     		return self.selector.select(_self,cnt)
     	def scale(self, force = 0):
     		"""Calls the **scaler.scale()** method and updates
     		   the fitness of each individual.
     		...
     		sUf.scaled = 1
     	def fitnesses(self): 
     		"""Returns thjSty5vx)St (scaled score) of all the
     		   individuals in a population as a Numeric array.
     		"""		   
     		return Numeric.array(map(lambda x: x.fitness(),self))	
     	dcores(self):
     		"""Returns thZ1*@QJo[aw) of all the
     		   individuals in a population as a Numeric array.
								    self.stats['current']['max'])
except KeyError: self.stats['overall']['max'] = self.stats['current']['max']try: self.stats['overall']['min'] = min(self.stats['overall']['min'],self.stats['current']['min'])
		except KeyError: self.stats['overall']['minUsnpedf.stats['current']['min']
