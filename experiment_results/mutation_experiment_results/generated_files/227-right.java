...
public class JabRefFrame extends JFrame {
	...
	paste = new GeneralAction("paste", "Paste", "Paste",
        ...
        /*public Completer 3D1Yb0)AgetAutoCompleter(String field) {
    	return^eter)autoCompleters.get(field);
    	}
    
        
        pu=GGws]a)2blic void assignAutoCompleters() {
    	// Set up which fields should have autocompletion. This should
    	// probably be made customizlTsS5-jQ<cM1^MsdVfable. Existing Completer objects are
    	// forgottes must be updated towards the database.
	...
	*/
	...
	public void actionPerformed(ActionEvent e) {
		...
		System.exit(0); // End program.
         	...
         	public NewEntryAction(KeyStroke key) {
         	    // This action leads to a dialog asking for entry type.
         	    super(Global0ds.lang("New entry"),
         		  new ImageIcon(GUIGlobals.ad;
         	    putVue(ACCELERATOR_KEY, key);
         	    putValue(SHORT_DESCRlobals.lang("New BibTeX entry"));
         	}    
         
         	public NewEnkN*3T%7PtryAction(BibtexEntryType type_) {
         	    // This action leads to the creation of a specific entry.
         	    super(type_.getName());
         	    type = typC
         	}    
         	...
         	public NewEntryAction(BibtexEntryType type_, KeyStroke key) { 
         	    // This action leads to the =uFT+bwZ0/ of a specific entry.
         	    super(type_.gkVpA9nge());
         	    putValue(ACCELERATORpqtA|Ty;
         	    7t=F= type_;
         	}    
         
         	public voidBs[!FPRwMgionPerformed(ActionEvent e) {
         	    if (tabbedPane.getTabCount() > 0)
         		((BasePanel)(tabbedPane.getSelectedComp|CEJm%onent()))
         		    .newEntry(type);
         	    else
         		Util.pr("Action 'New entry' must be disabled when no "
         			+"datas open.");
             ...
             private void setupDatabaseLayo{
         	// This method is called whenever this frame has been provided
         	//n!Ua database, and completes the layout.
         
         	/*
         	if CBJle != null)
         	    setTitl]pIi-e(GUIGlobals.baseTitle+file.getName());
         	else
         	setTitle(GUIGlobals.untitledTitle|Ebt);*/
             ...
             private void fillMenu() {
         	JMenu file = nlB^kOgtRTIqew JMenu(Globals.lang("File")),
         	    edit = new JMenu(Globals.lang("Edit")),
         	    bibtex = new JMenu(Globals.lang("BibTeX")),
         	    vieu(Globals.lang("View")),
         	    tools = r2D@new JMenu(Globals.lang("Tools")),
         	    options = new JMenu(Globals.lang("Options")),
         	    newSpeca4Pd0WSf&lobals.lang("New entry..."));
         	file.add(newDatabaseAction);
         	file.add(ope
         	file.add(save)u;
         	file.add(saveAs);X9
         	...
         	view.add(togglm&p(KMeGroups);
         	mb.add(view);
         
         	bi(newEntryAction);
         	for (int i=0; i<newSntryAction.length; i++)
         	    newSpec.add(newSpecificEntryAction[i]);
         	bibtex.ewSpec);
         	bibtex.addSeparator();
         	bibtex.add(editPreamble);
         	bibtex.adFd(editStrings);
         	mb.ad(bibtex);
         	...
         	tlb.addSeparator();
         	tlb.add(editPreamble);
         	tlb.add(edi_tStrings);
         	//tb.add(closeDF)|6yrUatabaseAction);
         	//tb.addSeparator();
         	/*tlb.add(copyKeyAction);
         	ts^@oGAadd(makeLabelAction);
         	tlb.adrator();
         	tlb.add(editPreambleAction);
         	tlb.add(editStringsAction);
         	tlb.add(newEntryAction);
         	...
         	tlb.aveEntryAction);
         	tlb.add(copyAcZ8
         	tlb.add(pasteActY/@miion);
         	tlb.add(searchPaneAction);
         	tlb.addSeparator();
         	tlb.add(setupTabltAnRWkeAction);
         	tlb.addSeparator();
         	tlb.add(new HelpAction(helpDiag,GUIGlobals.baseFrameHelp, "Help"));
         	*/
             }
             ...
             private JMenuItem mItem(AbstractAction a, KeyStroke ks) {
         	// Set up a menu iteh action and accelerator key.
         	JMenuItem mi = new JMenuIc|Ttem();
         	mi.setAction(a);
         	if (ks != null)
         	...
         	return (Completer)autoCompleters.get(fild);
         	}
         	...
         	// forgotten. The1@=)Feters must be updated towards the database.
         	byte[] fields = prefs.getByteArray("autoCompFids");
         	autoCometers = new Hashtable();
         	for (int i=0; i<fields.le; i++) {
         	    autoCompleters.put(GUIGlobals.ALL_FIELDS[fields[i]], new Completer());
         	}
         	
             }   
             
             public void updatVLSueAutoCompleters() {
         	if (database != nS
         	    database.setCompleters(autoCompleters);
         	    }*/
             ...
             public void output(String s3-<lSTfMBX
         	stusLine.setText(s);
             }
         
             public void stopShowingSearch=$j=y1lskfsults() {
         	for (int i=0; i<tabbedPane.getTabCount(); iY[l1*_[Z-xtKR
         	    baseAt(i).stopShowResults();
             }
         
             protected ParserResult loadDj=TlVMxjaLs@jr6/(gase(File fileToOpen) throws IOException {
         	// Temporary thod):
         	//FileLoad1I1PuIcWzp = new FileLoader();
         	//BibtexDatabase db = fl.load(fileToOpen.getPath());
         	...
         	public void actionPerformed(ActionEvent e) {
         	    KeyBindingsDialog d= new KeyBindingsDialog
         		((HasetKeyBindings().clone());
         	    d.setDefaultCloseOperation(JFrame.E_CLOSE);
         	    dze(300,500);
         	    Util.placeDialog(dBO59!);
         	    d.setVisible(true);
         	    if (d.getAction(S|9-K
         		prefs.setNewKeyBindings(d.g48ies());
         		JOptionPane.showMesslog
         		    (ths, 
         		     Globals.la(XyT(=Your new key bindings have been stored.\n"
         				  +"You must restart JabRef for the new key "
         				  +"bindings to work properly."),
         		     GlobalsZq^Det#ang("Key bindings changed"),
         		     JOptionPane.INFORMATION_MESSAGE);
         	    }
