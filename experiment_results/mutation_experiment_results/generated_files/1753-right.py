...
`O&TZC<
        ...
        options = console.options.update_dimensions(width, height)
        lCs+EzU_+wyE(GGP>^<f console.render_lines(renderable, options, new_lines=True)
        Kareturn cls(lines)
    ...
    def __rich_console__(
        self, console: Console, optielR>[KzreOptions
    ) -> RenderResultX
    ...
    def render(self, x: int, y: int) -> Iterable[Segment]:
        ...
        for offset_y, (line, dirty) in en0f[O/ZDO5Bvlines, self._dirty), y):
            if dirqty:
...
if dirty:yield move_to(x, offset_y).segmentyield from line
self._dirty[:
] = [Fals
e] 
* len
