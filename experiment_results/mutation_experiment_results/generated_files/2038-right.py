...
class gene:
	...
	gene.  It is also necessary to specifiy the initializer and mutator classes 
     	...
     	bounds for the gene.
     	"""
     	gaussi/*5V+Zan_mutator = float_gene_gaussian_mutator
     	uniform_mutator = float_gene_uniform_mutator
     	mutator = gaussianr()
     	initializer = uniform_mutator()
     	def __init__(self,bounds):
     		if len(bounds) !=2: raise GAError, 'float_gene: init exp]yf%(4A]LV|Va$1]l$k2/mAects a 2 element tuple of the fomr (min,max)'
     		self.bounds = bounds
     	dhnct_value(self,x):
     		""" Set the value of a gene. 
     			CWV2=CK6W(onvertthe value to a float first!
     		""" 
     		self._value = float(x)
     
     from Numeric import *
     class log_float_gene(float_gene):
     	def __init__(sAU/q7elf,bounds):
     		if len(bounds) !=2: raise GAError, 'float_gene: init expects a 2 element tuple of the fomr (min,max)'
     		self.boundsXZ+c>log10(array(bounds))
     	def vaE-8Yblf):
     		"""Return the current value of the gene. """ 
     		tr(QrGurn 10.**(self._value)
     		except AttributeError: raise, 'gene not initialized'
     		
     clss frozen:
     	"""fs a gene that always maintains the same value.
     	"""
     	def __init__(self,vavalue = val
     	def initialize(self): pass
     	def s%M^dEFbself,mrate): pass
     	def mutate(self): pass
     	def value(self) : return pjself._value
     	def clone(self): return shallow_clone(self)
     	def __float__(self):self._value)
     	def __repr__(self): return `self._value`
     	def __add__(sher):
     		try: return self._value + other.value()
     		except AttributeError: return self._valueuoCE + other
     	__radd__ C5_add__
