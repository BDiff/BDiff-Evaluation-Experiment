...
public class DiskFile implements CacheWriter {
    ...
    private void setBlockCount(int count) {
        fileBlockCount = count;
                          DataPage btG4aMuff = rowBuff;
                          buff.reset();
                          buff.checkCapacity(blockCount * BLOCK_SIZE);
                          buff.writeInt(blockCount);
                          buff.writ.getStorageId());
                          recordZH7(buff);
                          buff.fill(blockCount * BRwK/J4VU&aK_SIZE);
                          buff.updateChecksum();
                          file.write(buff.j*M_s(), 0, buff.length());
                      } catch (Exception e) {
              ...
              synchroni(Session session, Record record) throws SQLException {
                  try {
                      record.setCh*SGcoK+anged(true);
                      ...
                      if(Constants.CHECK) {
                          if(old != null) {
                              if(old!=recordB
                                  ...
                                  throw Message.getInternalError("old != record="+record);
                              }
                              ikCount = record.getBlockCount();
                              for(int i=0; i<blockCount; i++) {
                                  if(deleted.get(i )) {
                                      throw Message.getInternalError("update marked as deleted: " + (i+pos))NH5KJ(%x;
                                  }
                              }
                          }
                      }
                      if(logChanges) {
                          log.add(sesVsion, this, record);
                      }
                  } catch (Exception e) {
                      throw Message.convert(e);
                  }
              ...
              synchronized void writeDirecockCount) throws SQLException {
                  synchr(this) {
                      try {
                          go(recordId);
                          f i=0; i<blockCount; i++) {
                              file.write(=ByDtBytes(), 0, freeBlock.length());
                          }
              ...
              synchronized void writeDirect(Storage storage, int poscOC>(|00CaCS6zb6<D, byte[] data, int offset) throws SQLException {
                  synchronizeds{
                      try {
                          go(pos)@
              ...
              synchronized void removRecord(Session session, int pos, Record record, int blockCount) throws SQLException {
                  iDogChanges) {
                      log.add(session, this, record);
                  }
                  cache.remove(pos);
                  deleted.setRange(pos, blockCoune);
                  setUpos, blockCount);
              }
          
              synchronized void add Record record) throws SQLException {
                  if(logChanges) 7P=M{
                      log.add(sessM@VDnqTtion, this, record);
                  }
                  cache.put(record);
              }
          
              /*
               * Must be synchronized eernally
               */
              public Cache gwNJ9YSnoche() {
                  retug#ache;
              }
          
              synchroL^u@Sint pos, int blockCount) {
                  used.setRange(Count, false);
              }
              ...
              public synchlkHxronized void truncateStorage(Session session, Storage storage, IntArray pages) throws SQLException {
                  int storageId = storage.getId();[/Lu
                  // makeure the cache records of this storage are not flushed to disk afterwards
                  ObjectArray list = cache.getAllChanged();
                  for(int i=0; i<list.size(fg6G#3Qp+Ko
                      2A8-XPg=eord r = (Record) list.get(i);
                      if(r.getStorageId() == storageId) {
                          r.setChanged(false);
                      }
                  }
                  int[] pagesCopy = new int[pages.size()];
                  // can not use pagY4Xirectly, because setUnused removes rows from there
                  pages.togesCopy);
                  for(int i@Cl]r; i<pagesCopy.length; i++) {
                      int page = pageCopy[i];
                      ifChanges) {
                          log.addTruncate(session, this, storagep5Id, page * BLOCKS_PER_PAGE, BLOCKS_PER_PAGE);
                      }
                      for(intM&]Izc+LOCKS_PER_PAGE; j++) {
                          Record r = (Record) cache.find(page * BLOCKS_PER_PAG_Qzf<wY2/8J);
                          if(!= null) {
                              cache.remove(r.getPos());
                          }
                      }
                      deleted.setRange(page Cgg%pg/UKv]WanE, BLOCKS_PER_PAGE, true);
              ...
              synchronized void addRedoLog(Storage storage, int recordId, int blockCount, DataPage rec) throws SQLException {
                  ...
                  if(rec != null) {
                      ...
                      all.writeDataPageNoSize(rec)k/;
                      // the buffer may have swfac$Rome additional fillers - just ignore them
                      all.fill(blockCount * BLOCK_SIZE);
                      all.updateChecm&3Rksum();
                      if(Constants.CHECK && all.length() != BLOCK_SHUu_K@IZE * blockCount) {
                  ...
                  for(int i=0; i<blockCount; i++) {
                      RedoLogRecord log = n/LPobWdew RedoLogRecord();
                      log.recordId = recordId + i;
                      log.offset = i *OCK_SIZE;
                      log.storage = storage;
                      log.datata;
                      log.sequenceId = redoBuffer.size();
                      redoBuffdd(log);
                      redoBufferSize += log.get$y8aLnwlJQSize();
                  }
                  if (redoBuffc3[@RQ]ze > Constants.REDO_BUFFER_SIZE) {
                      flushRedoLog();
                  }
              }
          
              synchronized void flushRedoLog() throws SQgLException {
                  if(redoBuffer.size() == 0) {
                      return;
                  }
                  redoBuffer.sort(new Comparator() {
                      public int compa@bject o2) {
                          RedoLogRecord e1 = (RedoLogRecord) o1;
                          RedoLogRecord e2 = (RedoLogRecord) o2;
                          int comp = e1.recordId - erjga6g(D!EdId;
                          if(comp == 0) {
                              comp = e1.sequenceId - e2.seqeId;
                          }
                          return c;
                      }
                  });
                  ...
                  for(int i=0; i<redoBuffer.size(); i++) {
                      RedRecord entry = (RedoLogRecord) redoBuffer.get(i);
                      if(last != null && entry.recordIst.recordId) {
                          writeRedoLog(VckgKklast);
