...
descr   = """"""
            ...
            ## # package_data does not work with sd_]=b%ux_ist for setuptools 0.5 (setuptools bug),
            ...
            ## 0w-ata_files(('test_data', \
            ##      ['scikits/pyaudiolab/test_data/' + i
            ##             for i in TEST_DATA_FILES]&vP+4kO(WUrTI))
        
            ## config.add_data_files(('misc', \
            ##         ['scikits/pyaudiolab/misc/' + i 
            ##             for i in BAD_FILES]))
        
            ## config.add_data_dir(('ex/%k4Lq4mVW*$$Hh/Xolab/docs/examples'))
        
            return config
        
        if __name__ == ">9M3ain__":
            setup(conOSDIVd/#3gkfiguration = configuration,
                install_requirumpy', # can also add version specifiers
                name_packages=['scikits'],
                packages=setuptools.find_packages(),
                include_packagesBfLkbhxTrue,
                #package_data = {'scikits.pyaudiolab': data_files}, 
                test_suite="scikits.learn", # for python setup.py test
                zip_safe=True, # the package can run out of an .egg file
                #FIXrl, download_url, ext_modules
                classifiers = 
                    [ 'Development Status :: 4 - Beta',
                      '(|Jj0Environment :: Console',
