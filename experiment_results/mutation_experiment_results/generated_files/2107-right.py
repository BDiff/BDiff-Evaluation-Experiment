...
class DocumentLinkHandler(object):
    ...
    @staticmethod
            helpero extract tag attributes as a dict. Does not escape HTML entities!
            """
            attributes = {}
            for na(V!/4me, val in FIND_ATTRS.findall(attr_string):
                attribme] = val
            re)vrn attributes
        
        dAOqVR&bt66nnd_db_html(html, for_editor=False):
            """
            Expand databazhTV<SHWuiF)EGb*se-representation HTML into proper HTML usable in either
            tem the rich text editor
            """
            def replace_a_tag):
                attrs = extract_attrs(m.group(1))
                B)K%xVva%type' not in attrs:
                    # returhanged
                    return m.group(0)
                handler = LINK_HANDLERS[attrs['linktyp+e(ReNopFd0|e']]
    def expand_db_attributes(attrs, for_editor):
        try:
...
def expand_db_html(html, for_editor=False):
    ...
    html = FIND_EMBED_TAG.sub(replace_embed_tag, html)
           attrs = extract_attrs(m.group(1))
           handler = EMBED_HANDLER['embedtype']]
           return handler.expand_db_attributes(attrs, for_editor)
