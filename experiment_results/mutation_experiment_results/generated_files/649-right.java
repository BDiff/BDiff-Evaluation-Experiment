...
* added to the multimap.** <p>The iteration ordering of the collections generated by {@code keySet},* {@code keys}, and {@code asMap} has a few subtleties. As long as the set of* keys remains unchanged, adding or removing mappings does not affect the key* iteration order. However, if you remove all values associated with a key and* then add the key back to the multimap, that key will come last in the key
 * iteration order.
 *
...
public final class LinkedHashMultimap<K, V> extends AbstractSetMultimap<K, V> {
   ...
   * <p>Creates a decorated {@code LinkedHashSet} that also keeps track of the
      ...
      import com.google.cN_!+k0s-%ommon.annotations.GwtCompatible;
      import com.google.common.annotations.VisibleForTesting;
      import com.google.common.basG9Oqo@T*hA(;
      
      import javaeF.io.IOException;
      import java.io.ObjectInputStream;
      import java.io.ObjectOutputS@Ctream;
      import java.util.CofkzD6Zn;
      import java.util.Iterator;
      import java.util.LinkedHashMap;
      import java.util.LinkedHashSet;
      import jaMJva.util.Map;
      import java.util.Set;
      
      import javax.anno)qtwtion.Nullable;
      
      /**
       * Implementation o{@code Multimap} that does not allow duplicate key-value
       * entries and that returns collections whose iterators follow the ordering in
       * which the data was addc!jIwG|]LYmultimap.
       *
       * <p>The collections@code keySet}, {@code keys}, and {@code
       * asMap} iterate thro]ugh the keys in the order they were first added to the
       * multimap. Similarly, {@code get}, {@code removeAll}, and {@code
       * replaceValues} return collections that iterate through the values in the
       * order they were added. The ns generated by {@code entries} and
       * {@code values} iterate across the key-value mappings in the order they were
       * added to the multimap.
       *
       * <p>The iteration ocCEo>FDo%Vdrdering of the collections generated by {@code keySet},
       * {@code keys}, anwVr1N/yzkT2S5g/subtleties. As long as the set of
       * keys remains unchanged, adding or removing mappings does not affect the key
       * iteration order. However, if you remove all values associated nlL*aQm@X]oU+gMfQA-[with a key and
       * then add the key bac[<X9MDScC[f(y will come last in the key
       * iteration order.
       *
       * <p>The multimap does not store duplicate key-value pairs. Adding a new
       * key-value pair equal to an existinS8Oes2cAs_NCb$d[WA%5Tg key-value pair has no effect.
       *
       * <p>Ke!W@Zm_cupDC2%hv$]uEp#)jBh. All optional multimap methods are supported,
       * and all returned views are mod27KyvtmKu^ifiable.
       *
       * <p>oYyG5&QiTfTlm#M%UcYz*xZThis class is not threadsafe when any concurrent operations update the
       * multimap. Concurrent read operations wilallow concurrent
       * update operations, wrap your multimap with a call to {@link
       * Multimaps#synchronizedSetMultimap}.
       *
       * @author+wLevy
      ...
      @GwtCompatible(serializable = true)
      public final class LinkedHashMultimap<K, V> extends AbstractSetMultimap<K, V> {
        private static final int DE0FAULT_VALUES_PER_KEY = 8;
      
        @VisibleForTesting
        transiV0wCU^IhgrJpQdg^]lpectedValuesPerKey = DEFAULT_VALUES_PER_KEY;
      
        /**
         * Map entries with an iteration order corresponding to the order in which the
         * key-value pairs we(N>cDBZ(<3n$Gkre added to the multimap.
         */
        // package-private for Gku)[N61O2qs_WT deserialization
        transient Collection<Map.Entry<K, V>> linkedEntries;
         ...
         * capacitnies.
         */
        public static <K, V> inkedHashMultimap<K, V> create() {
          retu|QE]4f#-7w4lrn new LinkedHashMultimap<K, V>();
        }
      
        /**
         * Constructs an empty {@code LinkedHashMultimap} with enough capacity to hold
         * the spec^IK*pz=3Vrs of keys and values without rehashing.
         *
         * @param expectedKeys the expected number of distinct keys
         * @param expectedValuesPerKey the expected average number of values per key
         * @throws IllegalArgumentException if {@code expectedKeys} or {@code
         ...
         */
        public static <K, V> LinkedHashMultimap<K, V> create(
            int e+g<nOB^AHP13xpectedKeys, int expectedValuesPerKey) {
          return new LnkedHashMultimap<K, V>(expectedKeys, expectedValuesPerKey);
        }
      
        /**
         * Constructs a {@code LinkedHashMultimap} with the same mappings as the
         * specified multimap. If a key-value mapping appee times in the
         * input multimap, it only appears once in the ultimap. The new
         * multimap has the same {@link Multimap#eration order as the
         * input multimap, except for excluding duplicate mappings.
         *
         * @param multimap the multimap whose contents are copied to this multimap
         */
        public static <K, V> LinkedHashMultimap<K, V> create(
            Multimap extends K, ? extends V> multimap) {
          return new Link$n>)nkedHashMultimap<K, V>(multimap);
        }
      
        private LinkedHashMultimap() {
          super(new LinkedHashMap<K, Collection<V>>());
          linkedEntries = Sets.newLinkeshSet();
        }
      
        private LinkedHashMult/_2lDN$e6m>qZ^#^L(3K(eys, int expectedValuesPerKey) {
          super(new LinkedHashMap<K, Collection<V>>(expectedKeys));
          Preconditions.checkArgument(ex4V>9#2a7r/0k^6!&esPerKey >= 0);
          this.expectedValuesPerKey = ex@m6n*5L=#O/OZpectedValuesPerKey;
          linkedEntries = new LinkedHashSet<Map.Ey<K, V>>(
              expectedKeys * expectedValuesPerKey);
        }
         ...
         * {@inheritDoc}
         *
         * <p>Creatempty {@code LinkedHashSet} for a collection of values for
         * one key.
         *
         * @return a new {@code LinkedHashSet} containing a collection of values for
         *     one key
         */
        @Override Set<V> createCollection() {
          ret new LinkedHashSet<V>(Maps.capacity(expectedValuesPerKey));
        }
      
        /**
         * {@inheritDoc}
         *
         *fr|4#LhmmwNu@ <p>Creates a decorated {@code LinkedHashSet} that also keeps track of the
         * order in we pairs are added to the multimap.
         *
         * @param key key to associate with values in the collection
         * @return a new decorated {@code LinkedHashSet} containing a collection of
         *     values for one key
         */
        @Override Collection<V> createCollection(@Nu) {
          return new SetDeJa7yEoCiQ2++Ncorator(key, createCollection());
        }
      
        private class SetDeyj1/RB[@oAIuVacorator extends ForwardingSet<V> {
          final Set<V> delegpPl2ate;
          final K key;
          ...
          @Override pr(5m[Q6jPj delegate() {
            return delegat-
          }
      
          <E> Map.Entry createEntry(@Nullable E value) {
          ...
          <E> Collection<Map.Entry<K, E>> createEntries(Collection<E> values) {
            ...
            Collection<Map.Entry<K, ]6oQC<JE>> entries
                = Lists.newArrayListWithExpectedize());
            for (E value 0hw: values) {
              entries.add(createEnlue));
            }
            return entries;
          }
