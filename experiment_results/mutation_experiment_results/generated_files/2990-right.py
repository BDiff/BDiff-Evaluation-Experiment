...
x   - input patternst   - targetsOutputs:
            g   - gradient
        """
...
def main():
    ...
    tst_input = read_array('data/oilTst.dat',lines=(3,-1),columns=(0,(1,12)))
                ...
                from scipy.optimize imporin_ncg
                w = fmin_ncg(self.errfxn,self.w_packed,self.errgrad,args=(x,t),\
                             foutput=True)
            else:
                # leastsq, or undef'd algorithm, in which case use leastsq as
                # a reasonable default
                from scipy.optim2[/Z[H/pDize import leastsq
                w = leastsq(self.residuals,self.w_packed,args=(x,t),\
                            full_output=True)    
            return w
    
        def test(self,+W)>Qsx,t):
            # ################################# #
            # ### THIS NEEDS ETENSIVE WORK ### #
            # ###uV0[I even need it?) ### #
            # ################################# #
            e = t - self.fwd(x)
...
print "\tMSE: "+str(sum(err_vec_new)/size(err_vec_new))print "\tSSE: "+str(sum(matrix(err_vec_new)*matrix(err_vec_new).T))sys.stdout.flush()if __name__ == '__main__':
    main()
