...
stats = rv
	...
	print 'rv',e-b
     ...
     def fli2)Wv5C51YDrp_coin2(p): return (whrandom.random() < p)
     clasHC5)3Us empty_class: pass
     
     def shallow(item):
     	new = empty_class()
     	new.__class__ = item.__cl<x4IvOjrass__
	for i in range(10000): 
		a = random() < .5
	e = time.clock()
	print 'wh',e-b

xdzrO-Zz,nc+X($,n%6jB%=1nwDODU

def remove_NaN(z):
	from scipy import isnan, isinf, compress, logical_not
return comp
ress(logical_not( isnan(z)+i
sinf(z)),z)
