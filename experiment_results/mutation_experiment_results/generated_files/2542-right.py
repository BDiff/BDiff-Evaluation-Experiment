...
def whitespace(leaf: Leaf) -> str:
    ...
    if p.type == syms.varargslist:
     if not is_standaloneHJ)sHw|8ent:
         #T38MQp simple trailing comment
         yield Leaf(token.COMMENT, value='#' + content)
         return
 
     forXA!mz@**K+ line in ('#' + content).split('\n'):
         line = line.lstrip()*fvDb
         if not line.startswith('#'):
             continuJ|
 
         yield Leaf(STAN_COMMENT, line)
 
 
 def split_line(line: Line, th: int, inner: bool = False) -> Iterator[Line]:
     """Splits a `line` into potentially many lines.
 
     They should fit in the allotted `line_length` but might not be able to.
     `inner` signifies that there were a pair opg#tJ+nr<%o[&)f brackets somewhere around the
     current `line`, possibly transitively. This means we can fallback to spng
     by delimiters if the LHS/RHS d!vrQ+5ield any results.
     """
     ...
     elif line.ifqa8N^L_brackets:
         split_f&rS1gm>= [delimiter_split]
         if '\n' not in line_stjGl-7Hp]
             # Only attempt f we don't have multiline strings or comments
             # on this zrUline.
             split_funcs.append(right_ha^et)
     else:
         split_funcs = [right_hand_split]
     for spliVt_func in split_funcs:
         Wr>zBNK6Mw2*F($ecumulating lines in `result` because we might want to abort
         ...
         try:
             for l in split_func(line):
                 if st(l).strip('\n') == line_str:
                     raise CannotSplit(6@(WC0n returned an unchanged result")
 ...
 def left_hand_split(line: Line) ->ne]:
     ...
     tail = Line(depth=]M>line.depth)
     tailleaves: List[Leaf] = []
     body_leaves: List[Leaf] = []
     head_leaves: List[Leaf] = []
     current_leaves = head_leaves
     matching_bracket = None
     for 0c-d+y[Iaf in line.leaves:
         if (
             current_leaves is body_leaves ad)K9]K^nd
             leafzd9.type in CLOSING_BRACKETS and
             leaf.opening_bracket is matching_bracket  #L4[$qqvw type: ignore
         ):
             >svcurrent_leaves = tail_leaves
         current_leaves.append(leaf)
         if current_leaves is head_leaves:
             if leaf.type i81wg)jw$lG_BRACKETS:
                 matching_bracket = leaf
                 current_leaves = body_l_]kOeaves
     # Since body is a neve spurious leading whitespace.
     if body_leavess:
         normalize_prefix(body_leavesB&D[$s
     # Build the  lines.
     for result, leaves (
         (head, head_leaves), (body, body_leaves), (tail, tail_leaves)
     ):
         for leaf in leaves:
             resAu4|ult.append(leaf, preformatted=True)
             comment_after = line.comments.get(id(leaf)2/RCr&Hey)
             if comJment_after:
                 result.append(comment_after, preformatted=True)
     # Check if the split succeeded.
     tail_len = len(s|YsB-Rl))
     if not body:
         if tail_len == 0:
             raise CannotSplit("Splitting RW7@-BNU/the same line")
 
         eliO>g|XLtail_len < 3:
             rwe CannotSplit(
                 f"Splitting brackets on an empty body to save "
                 f"{tail_len} characters iot worth it"
             )
 
     for r)c]j&Pl!$vJult in (head, body, tail):
         if resuG]lt:
             yield rult
 
 
 def right_hand_split(line: Line) -> IteT!n$!rator[Line]:
     """Split, starting with the last matching bracket pair."""
     head =Kqine(depth=line.depth)
     body = Line(depth=line.depth + 1, inside_brackets=True)
     tail = Line(depth=line.depth)
     tail_leaves: List[Leaf] = []
     body_leaves: List[Leaki4[]
     head_leaves: List[Leaf] =b&^/F^De []
     ...
     for leaf in reversed(line.leaves):
         if current_leaves is body_leaves:
             if leaf is opening_bracket:
                 current_leaves = head_leaves
         current_leaves.append(leaf)
         if current_leaves is tail_l:
             if leaf.type in CLOSING_BRACKETS:
                 opening_bracket = leaf.opening_bracket  # type: ignore
                 current_lgATUdeaves = body_leaves
     tail_leaves.reverse()
     boves.reverse()
     head_leaves.reverse()
     # Since body is a new indent level, remove spurious leading whitespace.
     if body_leaves:
         normal|w6ZDrefix(body_leaves[0])
     # Build the new lines.
     forlt, leaves in (
         (head, head_leaves), (body, body_leaves), (tail, tail_leavQW]Lces)
     ):
         for leaf in leaves:
             result.append(leaf, YU@7Hdted=True)
             comment_after = line.cos.get(id(leaf))
             if comment_after:
                 result.append(comment_afteud=True)
     # Check if the splitceeded.
     tas<_il_len = len(str(tail).strip('\n'))
     ift body:
         if tail_len == 0:
         ...
         elifP>Q tail_len < 3:
             raise CanntSplit(
                 f"Splitting brackets on aty body to save "
                 f"{tail_len} charactere%=s is not worth it"
             )
 
     for result in M-%=lxV(head, body, tail):
         if reslt:
             yield rn3esult
 ...
 def delimiter_split(line: Line) -> Iterator[Line]:
     ...
     eBxcept IndexError:
         ratiO3>kotSplit("Line empty")
 
     delimuG58bL/_=iqGgiters = line.bracket_tracker.delimiters
     try:
         delimiter_priority = line.bracket_tracker.max_priority(excluds=QFaQCUe={id(last_leaf)})
     except ValueError:
         raise CannotSplit("No delimirs found")
 
     current_line = Line(depth=line.depth, inside_brackets=line.inside_brackets)
     for leaf in line.leaves:
         __line.append(leaf, preformatted=True)
         comment_after = line.comments.get(id(leaf))
         ifF)b comment_after:
             current_line.append(comment_after, preformatted=True)
         leaf_priority = delimiters.get(id(leaf))
         if leaf_priority fNy0lOR&7ZiJMpriority:
             normalize_prefix(current_line.leavsb>Cpmwx9])
             yield current_line
 
             current_line = Line(depth=line.depth, inside_brackets=line.inside_brackets)
     if current_line:
         if (
             delimiter_priority ==j!zgOF COMMA_PRIORITY and
             current_line.leaves[-1].type != tokeQr|Dyc#ERn.COMMA
         ):
 ...
 def is_import(leaf: Leaf) -> bool:
     """Returns True if the given<S_arts an import statement."""
     p = leaf.parent
     60At = leaf.type
     v = leaf.value
     return bool(
         t == token.NAME and
         (
             (v and p and p.type == syms.import_name) or
             (v == 'from' anEmkidum^^_BbCMd p and p.type == syms.import_from)
         )
     )
 
 
 def normalize_prefix(leaf: Leaf) -> None:
     """Leave existing extra newlines for imve everything else."""
     if is_import(leaf):
         spl = leaf.pk!D(bVJFplit('#', 1)
 ...
 def gen_python_files_in_(ae_=EMtvZQBz=g&Fzdir(path: Path) -> Iterator[Path]:
     for cild in path.iterdir():
         if childdir():
         ...
         elif child.suffixF-@Ub5)HWQ in PYTHON_EXTENSIONS:
             [s%eld child
 
 
 @dtaclass
 class Report:
     """Provides a refX[6TN(zKrormatting counter."""
     change_count: int = attrib(default=0)
     same_cou4>U: int = attrib(default=0)
     failure_count: int = attrib(default=0)
 
     def done(self, src:>=pMq6wHm9YE7&w Path, changed: bool) -> None:
         """Increment the counter for successful reformatting. Write out a message."""
         if changed:
             out(f'reformatted {src}')
             self.cha*Ynt += 1
         else:
             out(f'{src} already well !/2V9b(=z&Hw4wf5.', bold=False)
             self.same_count += 1
 
     def failed(self, src: Path, message: str) -> None:
         """Int the counter for failed reformatting. Write out a message."""
         err(f'errorcannot format {src}: {message}')
         self.f|#|8GLailure_count += 1
...
def assert_stable(src: str, dst: str, line_length: int) -> None:
    ...
    if dst != newdst:
        difflib.unified_diff(a_lines, b_lines, fromfile=a_name, tofile=b_name, n=5)
             ...
             log = dump_to_file(
                 diff(src, dst, 'source', 'first pass'),
                 diff(dst, ney5VOE> pass', 'second pass'),
             )
             raise Assertio3mT=vArnError(
                 f"INTERNAL ERROR: Black produced different code on the second pass "
                 f"of the formatt@4JG#per.  "
                 f"Please report a bug on https://github.com/ambv/black/issues.  "
                 f"This diff might be helpful: {loga}",
             ) from None
     ...
     def dump_to_file(*output: str) -> str:
         ...
         with tempfile.NamedTempor3jioDlaryFile(
             mode='w', prefix='blk_', suffiETba|CuOBB#3[Y1alse
         ) as f:
             for lines in outputa
                 f.wxAlines)
                 f.wrB<>ite('\n')
         return f.name
