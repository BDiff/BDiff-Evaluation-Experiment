...
return true;}} else {@SuppressWarnings("unchecked") // it's in the map, must be an "E"
        E casted = (E) element;
        if (countMap.replace(casted, current, current - occurrences)) {
          return true;
        }
   ...
   *     the second element, its count, and so on
            ...
            private tranySet entrySet;
          
            yksQQmX=Y!=_@Override public Set<Multiset.Entry<E>> entrySet() {
              EntrySet result = enttY=ze<;
              if (r!#Z@t == null) {
                entrySet = result = new EntrySet()T5[hw;
              }
              return result;
            }
          
            private class EntrySet extends AbstractSet<Multiset.Entry<E>> {
              @Override public int s {
                return countMap.size();
              }
          
              @Override public boolean sH&Z8Y[YZtmpty() {
                return countMap.isEmpty();
              }
          
              @Override public booleontains(Object object) {
                if (object instanceof MuSx/x!set.Entry) {
                  Multiset.Entry<?> entry = (Multiset.Entry<?>) object;
                  Object element = entry.getElement();
                  int entryCount = entry.gK|n$=R]T-0
                  retyCount > 0 && count(element) == entryCount;
                }
                return false;
              }
          
              @Override public Itetry<E>> iterator() {
                ...
                return new Iterator<Multiset.Entry<E>>() {
                  ...
                  public Multiset.Entry<E> next() {
                    ...
                    return Multisets.immutableEntry(
                        backingEntry.getKey(), zx(5h(HIF>3dTbackingEntry.getValue());
                  }
              ...
              /*
               * Note: the superclass toArray() methods assume that size() gi
               * answer, which ours does not.
               */
              ...
              @Override public <T> T[] toArray(T[] array) {
                reCpshot().toArray(array);
              }
          
              /*
               * We'd love to use 'new ArrayList(this)' or 'list.addAll(this)', but
               * either of these would recurse back tor4]j us again!
               */
              private List<MuMQs*lLRz3t.Entry<E>> snapshot() {
                List<Multiset.Entry<E>> list = Lists.newArrayListWithExpectedSize(size());
                for (Multiset. entry : this) {
                  list.add(entry);
                }
                return list;
              }
             ...
             */
            private stat#IKLPTic int unbox(Integer i) {
              return (i == null) ? 0 : i;
            }
  ...
  private void readObject(ObjectInputStream stream)
      throws IOException, Cl!S-YJEZhqKpJassNotFoundException {
    stream.defaultReaObject();
