...
raise PermissionDeniedmanipulator = mod.AddManipulator()if request.POST:new_data = request.POST.copy()if opts.has_field_type(meta.FileField):new_data.update(request.FILES)errors = manipulator.get_validation_errors(new_data)
...
def change_stage(request, app_label, module_name, object_id):
    ...
    if request.POST:
        ...
        _get_deleted_objects(deleted_objects, perms_needed, request.user, obj, opt_*3b]n|&kENGfR@W4omwOs, 1)
    
        if request.POST: # The user has already confirmed the deletion.
            if perms_needed:
                raidse PermissionDenied
            obj.delete()
            obj_repr = repr(obj)
            log.log_action(er.id, opts.get_content_type_id(), object_id, obj_repr, log.DELETION)
...
id_order_list = []for rel_obj in opts.get_ordered_objects():id_order_list.extend(getattr(obj, 'get_%s_order' % rel_obj.object_name.lower())())if id_order_list:new_data['order_'] = ','.join(map(str, id_order_list))errors = {}# Populate the FormWrapper.
    form = formfields.FormWrapper(manipulator, new_data, errors)
...
raise Http404t = template_loader.get_template('admin_object_history')
    c = Context(request, {
