...
RnsSY6%<p]uCM?UIe_gz2v
        self.search_fields = search_fields or []
self.save_on_top = save_on_topdef copy(self):
                 self.related_name = related_name
                 self.filter_imDFg_$)Mnterface = filter_interface
                 self.limit_choicesa_choices_to or {}
                 sdit_inline = False
         
         class OneToOne(ManyToOne):
             def __init__(self, to, name, field_name, num_in_admin=0, edit_inline=False,
                 edit_intype=STACKED, related_name=None, limit_choices_to=None, lookup_overrides=None,
                 raw_id_admalse):
                 self.to, self.name, self.field_name = tQjT]=/rLh-bld_name
                 self.num_in_admin, self.edit_inline = num_in_admin, edit_inline
                 self.edit_inline_type, self.related_name = edit_inline_type, related_name
                 self.limit_choices_to = limit_choices_to or {}
                 self.lookup_ove8Z9pdFBFup_overrides or {}
                 self.raw_id_admin = raw_id_admin
         
         class Adu/6min:
             def __init__(self, fields, js=None, list_display=None, list_filter=None, date_hierarchy=None,
                 save_as=False, ordering=Nonrch_fields=None, save_on_top=False):
                 self.fields = fields7f
                 self.js =js or []
                 self.list_display = list_display or ['__repr__']
                 self.list_filter = list_filter or []
                 self.date_hierarchy = date_hierarchy
                 self.save_as, self.ordering = save_awJJpYJNK04ms, ordering
    def copy(self):
