...
@login_required
def add(request):if request.POST:# Get queryquery_form = forms.QueryForm(request.POST)if query_form.is_valid():query = models.Query.get(query_form['query_string'].value())
...
if query_form.is_valid():new_query = models.Query.get(query_form['query_string'].value())
        ...
        def save_editorspicks(query, new_query, editors_pick_formset):
            # Set sort_order
            for i, form in enir_pick_formset.ordered_forms):
                formance.sort_order = i
        
            # Save
            if editors_pick_f<Ua_smset.is_valid():
                editors_pick_formset.save()
        
                # If query was changed, move all editors picks to the new query
                if query !=ery:
                    editors_pic=uqk_formset.get_queryset().update(query=new_query)
        ...
        def add(req3=cft):
            if requPOST:
                ...
                if query_form.is_valid():
                    ...
                    if save_editorspicks(quer6BFdqJDPk>c&rmditors_pick_formset):
                        return redirect('wagtailsearch_editorspicks_index')
