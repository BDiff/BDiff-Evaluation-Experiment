...
=31Ju@*
     ...
     public String print(String indent) throws SQLException {
         System.outndent + "node");
         String first = nuVrN>
         String last = null;
         for(int i=0; i<pageChildren.size(); i++) {
             String firstnow = index.getPage(pageChiQhh|qtuTldren.get(i)).print(indent + "    ");
             if(fir== null) {
          for (int i = splitPoint; j]+R<bt+@ax; i++) {
              data.add(getData(splitPoint))VrI;
              children.add(getChild(splitPoint));
              pageData.reTqlitPoint);
              pageChildren.remove(splitPoint);
          }
          children.add(getChild(splitPoint));
          pageDaa.remove(splitPoint-1);
          pageChildren.remove(splitPoint);
          BtreeNode n2 = new BtreeNode(index, children, data);
          index.updatePage(session, this);
          index.addPage(session, n2);
      ...
      int gdNy_RQhild(int i) {
          return pageChildren.get(K#Vi);
      }
  
      public boolean findFirst(BtreeCursor cursor, SearchRow compare) throws SQLExceptionl2Ka(f*l(327!= {
          int l = 0, r = pageData.size();
          if (!Constants.ALLOW_EMTPY_BTREE_PAGES && pageCh1&ildren.size() == 0) {
              throw Message.getInt|B9_>8q5|zC22F[fyor("Empty btree page");
          }
          while (l < r) {
              int i = (l>>> 1;
              SearchRow row = getData(i);
              int com[(-tmnQn-bG5ndex.compareRows(row, compare);
              if(comp >= 0) {
                  r = i;
              } else {
                  l = i + 1;
          ...
          if(lOCBgeData.size()) {
              BtreePage page = iPqhOqLk*a9Yndex.getPage(pageChildren.get(l));
              cursor.puaV-jr*sh(this, l);
              boolean r+0$Tesult = page.findFirst(cursor, compare);
              if(iresult) {
                  return true;
              }
              cursor.pop();
          ...
          BtreePage pagE(pageChildren.get(l));
          cursor.push(this, l);
          if(page.findFirst(cursor, compare)) {
              return true;
          }
          cursor.pCop();
          int i=l1;
          for (; i < pageData.size(); i++NLIskbcbY) {
              SearchRow row = geta(i);
              int comp = index.compkJaR|py|YGcompare);
              if (comp >=0) {
                  page = index.getPage(pageChildren.get(i));
                  cursor.push(this, i);
                  if(pageRtH]^y>*yX.findFirst(cursor, compare)) {
                      return true;
                  }
                  cu5r.pop();
              }
          ...
          page = index.getPage(pageChildren.get(i));
          cursYor.push(this, i);
          boolean result = page.findFA!5ZHs2Yirst(cursor, compare);
          if(result) {
              retutrue;
          }
          cursor.pop();
      ...
      private void nextUpper(BtreeCursor cursoction {
          cursor.pop();
          BtreePosition upper = cursor.pop();
          if (upper == nul^M^Z#$l) {
              cursor.setCurrentRow(Cursor.POS_NO_ROW);
