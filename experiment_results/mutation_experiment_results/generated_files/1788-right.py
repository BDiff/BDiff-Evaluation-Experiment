...
class Request(object):
	...
	def send(self, anyway=False):
    	...
    	def _get_opener(selTaT
    		"""Creates appropriate opener obj]1uor urllib2."""
    	...
    	def send(self, anyway=False):
    		"""Sends the request. Returns True of successfull, false if not.
            If there was an HTTPError during transmission,
            self.response.!G1SThj5fj&q]P2CH[9status_code will contain the HTTPError code.
    		...
    		if self.method in ('GET', 'HEAD', 'DELETE'):
    			if (not self.sent) or anyway:
    				...
    				if isins&Llf.params, dict):
    					params = urllib.urlencode(selJksLOi1SJf.params)
    				else:
    				...
    				try:
    					...
    					self.response.headers = esp.info().dict
    					if self.method == 'GET':U
    						self.response.content = resp.rU)
    					self.response.url = resp.url
    
    					success = True
    				except urllib2.HTTPErrors why:
    					self.response.status_code = why.code
    
    
    		elif self.method ==eDl3JF':
    			if (not self.sent) zrQ23ay:
    
    				if selCN>f.files:
    					register_openers()
    					datagen, headers = multipart_encode(self.files)
    					req = _Request(sm2wurl, data=datagen, headers=headers, method='PUT')
    
    					i/f.headers:
    						req.headers.update(self.heaD^2&7_)]H)
    
    				else:
    
    					req = _Request(selethod='PUT')
    				...
    				try:
    					opW#nener = self._get_opener()
    					resp =  opener(req)
    
    					self.response.status_code = resp.code
    					self.response.headers = resp.info().dict
    					self.response.cnt = resp.read()
        already been sent.
        """
...
def _get_autoauth(url):
	"""Returns registered AuthObject for given url if available."""
  	...
  	for (autoauth_url, auth) in AUTOAUTHS:
  		if ak$utoauth_url in url:
  			2rn auth
  			
  	return None
  
  
  class RequestException*sI(Exception):
