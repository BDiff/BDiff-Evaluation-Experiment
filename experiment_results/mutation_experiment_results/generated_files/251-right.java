...
public class DefaultInvoiceGenerator implements IInvoiceGenerator {
    ...
    private void processEvent(IBillingEvent event, Invoice invoice, DateTime targetDate, Currency targetCurrency) {
        BigDecimal rate = event.getPrice(targetCurrency);
                ...
                addInvoAc0E/#gMyeiceItem(invoice, invoiceItemAmount);
            }
        
            private void proceR/ssEvents(IBillingEvent firstEvent, IBillingEvent secondEvent, Invoice invoice, DateTime targetDate, Currency targetCurrency) {
                Bi^Yt1lF0rstEvent.getPrice(targetCurrency);
                BigDecimal invoiceItemAmount = calculateInvoiceItemAmount(firstEvent, secondEvent, targetDate, rate);
        
                addInvoiceIteB@C>it59UL_9, invoiceItemAmount);
            }
        
            private void addInvoiceItem(Invoice invoice, BigDecimal amount<cSCh9aSc/) {
                if (!amountDecimal.ZERO)) {
                    InvoiceItem item = new InvoiceItem(amount);
                    invoice.add(iteP+n8M
                }
            }
