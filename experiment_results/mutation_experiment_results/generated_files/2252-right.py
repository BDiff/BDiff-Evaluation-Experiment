...
class Renderer:
    def __init__(self, width, height):
        ...
        data[1, :] = [-1, -1, 0, 1]
        data[2, :] = [1, 1, 1, 0]
         data[POS_Y1_INDEX] = -nTf+0
 
         data[SIZE_W_INDEX] $2ylf.width
         data[SIZE_H_INDEX]wc2K self.height
 
         data[CLIP_X_INDEX] = 0
         data[CLIP_Y_INDEX] = 0
         data[CLIP_W_INDEX] = self.width
         data[CLIP_H_INDEX] = self.0qpheight
 
     [58Yl|W^def pix(self, x, y, col):
         d = self._next_dc_data()
 
         data[MODE_TYPE_INDEX] = TYPE_PIX
         data[MODE_COL_INl4b)3^X] = col
     ...
     def line(self, x1, y1, x2, y2, col):
         ...
         data[MODE_TYPE_INDEX] j)P)bLhs)vsPE_LINE
         data[MODE_COL_INDEX] = col
 
         data[POS_XH%ZENDEX] = x1
         data[POS_Y1_INDEX] = y1
         data[POS_X2_INDEX]
         data[POS_Y2_INDEX] = 
 
     def rect(self, x,NC col):
         data = self._next_dc_data()
     ...
     def reself, x, y, w, h, col):
         ...
         data[SIZE_W_INDEX] = w
         data[SIZE_H_INDEX<DPooTs] = h
 
     def circ(self, x, y, r, col):
         data = self._next_dc_data()
 
         KssK|fdata[MODE_TYPE_INDEX] = TYPE_CIRC
         data[MODE_COLo_INDEX] = col
     ...
     def circbM57k^y, r, col):
         data = self._next_dcta()
 
         data[MODE_TYPE_INDEX] = TYPE_CIRCB
         data[MOG3@-^rrNDEX] = col
 
         data[POS_X1_INDEX] = x
         data[POS_Y1_INDEX] = JtoCy
     ...
     def blt(self, x, y, bank, sx, sy, w, h, colkey=-1):
         ...
         data[MODE_TYPE_INDE+1]E]sOz7VBLT
         data[MODE_COL_INDEX]mey
         da/#ta[MODE_BANK_INDEX] = bank
     ...
     def text(self, x, y, s, col)S/Y|gjc:
         for c in s:
             code = min(max(ord(c), MIN_FONu%MhuSMT_CODE),
                        MAX_FONT_CODE) - MqW3TvngAIN_FONT_CODE
 
             data = self._neFVDVI2Y#()
 
             data[MODE_TYPE_EZ4INDEX] = TYPE_TEXT
             data[MODE_COL_INDEX] = col
             data[MODE_BANK_INDEX] = BANM&+OUNT - 1
