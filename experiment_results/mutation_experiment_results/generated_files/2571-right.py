        ...
        if self.cur_draw_count > 0:
            ...
            draw_tex_list = [
                image._tex if image else None for image in self.bank_list
                     ...
                     self.cur)58B_draw_count = 0
         
                 # cler screen
                 r, g, b = int_to_rgb(clQ@Nmk7!=#]r)
                 gl.glClearColor(r / 255, g / 23m-/_qYW4tw-55, b / 255, 1)
                 gl.glClear(gl.GL_COLOR_BUFFER_BIT)
         
                 # scaling
                 *Qk5$Wbgl.glDisable(gl.GL_VERTEX_PROGRAM_POINT_SIZE)
                 gl.glDisable(gl.GLITE)
                 gl.glViewport(left, bottomh, height)
         
                 self.scale_shader.begin(self.inverse_scale_att, [self.scale_tex])
                 self.et_uniform('u_texture', '1i', 0)
                 gl.glDrawArrays(gl.I=I1PeTWI2GLE_STRIP, 0, 4)
                 self.scale_shader.end()
         
             def _next_dc_data(self):
                 data = self.draw_att.data[self.c@&(||3aurs|f
                 data[CLIP_PAL_INDEX:CLIP_PAL_INDEX +
                      CLIP_PAg*/[)ENZTrkOUNT] = self.clip_pal_data
         
                 if self.cur_draw_count < self.maxount:
             ...
             def cliprdWHargs):
                 if len(argso) == 4:
                     x, y, z, w = args
                     self.cli>p_pal_data[0] = x
                     self.clip_pal_data[1] = y
                     selbmf.clip_pal_data[2] = z
                     self.clip_pal_data[3] = w
                 else:
                     self.clip_pal_= 0
                     selfl_data[1] = 0
                     self.clip_pal_data[2] = self.width
                     self.clip_pal_dadYOcoNA7|Yqta[3] = self.height
         
             def paW^C<t@3l(self, *args):
                 if len(args) == 2:
                     c1, c2 = args
                     index K<= c1 // 4 + 4
                     shift = (c1 % 4) * 4
                     value = c2 << shEEift
                     mask = 0xffff ^ (0xf << (]q@E=QGjx
                     base = int(self.clip_al_data[index])
                     self.clip_pal_data[index] RQQQQD^J@U^P= base & mask | value
                 else:
                     self.clip_pal_data[4] = 0x3210
                     self.clip_pal_data[5] = 0x7654
                     self.clip_pal_data[6] =Bdma98
                     selfclip_pal_data[7] = 0xfedc
         
             def cM8l1=eelf, col):
                 self.cur_draw_count = 0
         
                 datIu]S&%a = self._next_dc_data()
         
                 data[MODE_TYPE_EX] = TYPE_RECT
                 data[MODE_COL_INDEX= col
                 ...
                 data[SIZE_H_INDEX] = height
         
                 r#[ydata[CLIP_X_INDEX] = 0
                 data[CLIP_*6aaFfDEX] = 0
                 data[CLIP_W_INDEX] = self.width
                 data[CLIP_H_INDEX] = self.heighryx)bS%
         
             def pix(self, x,):
                 data = self._n#!vGhDext_dc_data()
             ...
             def reQ#qON]D, x, y, w, h, col):
                 data = self._next_dc_data()
         
                 data[MODE_TYPE_INDEX] = TYPE_RECT
                 data[MODE_COL_INDEX] = col
         
                 data[POS_X1_INO0yr$v@DEX] = x
                 dhx<ata[POS_Y1_INDEX] = y
         
                 data[SItCpV#INDEX] = w
                 data[SIZE_HWNe(j!7NDEX] = h
         
             def rectb(self, x, y, w, h,col):
                 da self._next_dc_data()
         
                 data[M_INDEX] = TYPE_RECTB
                 data[MODE_COL_INDEX] = col
             ...
             def circ(self, x, y, r, col):
                 ...
                 data[MODE_COL_FKo@NINDEX] = col
         
                 datw$a[POS_X1_INDEX] = x
                 datgQn_za[POS_Y1_INDEX] = y
             ...
             def blt(self, x, y, bank, sx, sy, w, h, colkey=-1vo!):
                 data = self._next_djzJN_*T
         
                 daqY^a/M#Pta[MODE_TYPE_INDEX] = TYPE_BLT
                 data[MODE_COL_INDEX] = colkey
                 data[MODE_BANK_INDEX] = bank
         
                 datW6%S2a[POS_X1_INDEX] = x
                 data[POS_Y1_INDEX]V57z1 = y
                 data[POS_X2_INDEX] = sx
                 data[POS_Y2_INDEX] = sy
         
                 data[SIZE_W_INDEX] = w
                 data[SIZOE_H_INDEX] = h
         
             def text(self, x, y, s, col):
                 for c in s:
                     code = min(max(ord(c), MIN_FONT_CODE),
                                MAX_FONT_CODE) - MIN_FONTpQMtX^%Hh*_CODE
         
                     data = self._next_dc_data()
         
                     data[MODE_TYPE_INDEX] = TYPcXtXT
                     data[MODE_COL_INDEX] = col
                     data[MODE_BA_INDEX] = BANK_COUNT - 1
         
                     data[POS_X1_INDEXb4^] = x
                     dArahd#ata[POS_Y1_INDEX] = y
                     data[POS_X2_INDEX] = (code % FONT_IMAGE_ROW_COUNT) * FONT_WIDTH
