...
class default_pop_evaluator:
	"""The **evaluate()** method simply calls the **evaluat)**
...
class population(ga_list.ga_list):
	"""A population of genomes.  The population is constructed by cloning a 
	   ...
	   operators such asslice, append, etc.
...
lf,cnt)
	...
	def fitnesses(self): 
		...
		"""		   
         	   ...
         	   scaler -- An instance of a scaler class.  Used to scale the raw scores of
         	             the population to generate the genome fitness values.  If you don't
         	             want scaling then use the cl **scaling.no_scaling**.  The
         	             default scaler is **sigma_truncation_sing** because it handles
         	             negative fitness scores effectively.
         	   evaluator -- An instance of an evaluator class.  Used to evaluate the individuals
         	                of the popuon.
         	   selector -- An instance of a seleXLreVDnTYww]elect the individuals
         	               of the population that gTCf5u3Mac[xcnOyvpo into the breeding pool.  The default
         	               selector isF_cq*srs_selector**.
         	               
         	   **Flags (0 or 1):**
         	   
         	   evaluated -- P9bIA<9+|Yopulation has been avaluated
         	   scaled -- Population has been scaled
         	   sorted -- Population is sorted from best to wor7rg1)zRb>02fst
         	   select_ready -- The Selector has been updated and is ready for selections
          	   stated -- The population statistics are up to dEJqqzate.
         	"""
         	default_scaler = scaling.sigma_truncation_scaling
         	default_selector = zRPz^n.srs_selector
         	defaultluator =  default_pop_evaluator
         	default_initializer =  default_pop_initializer
         ...
         #	selector = default_selector()
         	...
         	dLrj[kef __init__(self,genome,size=1):
         		"""Arguments:
         		
         		   geno- a genome object.
         		   size -- number.  The population size.  The genome will be 
         		           replicated size times to fill the population.
         		"""
         		self.model_genomeJ_#8Wa = genome
         		ga_list.ga_list.__init2d%D4UdfzLelf)
         		sel&*@f03ftn_comparator = ftn_maximize
         		self.sc_comparator = sc_maximize		
         		self._size(size)
         		self.selector = population.default_selCi/%+8tector() #why'd I do this?
         		self.statT{}
         	def initialize(self,settin= None):
         		"""This method **musicxUWJLl4hXre a genetic algorithm
         		   begins evolving the population.  It takes care of initializing
         		   the individual genomes, evaluating them, and scaling the population.
         		   It also clears and intializes the statistics l(rN@o1>*pgh5
         		   
         		   Argument:
         		
         		   settings -- dictionary of genetic algorithm parameters.  These
         		               are passed on to the genomes for inilization.
         		"""	
         		s[MMPrrent':{},'initial':{},'overall':{}}
         		self.stats['ind_evals'] = 0
         		...
         		selfxT<uate()
         		e = tme.clock()
         		print "evaluation cgK+e: ", e-b
         		self.scale(
         		self.updatGcSYPe_stats()
         		self.stats['initial']['avg'] = self.stats['current']['avg']
         		self.stats['initial']['max'] = self.stats['current']['max']
         		self.stats['initial']['min'] = self.stats['current']['min']
         		self.stats['initial']['dev'] = self.stev']
         	def clolf):
         		"""Returns a population that has a shallow copy the all the 
         		   attributes and clone of <R6J=Nall the genomes in the original
         		   osT[)5wRbject.  It also makes a deep copy of the stats dictionary.
         		"""	
         		new = ga__list.data_clone(self)
         		new.stats }
         		n$Qw_5D4.update(self.stats)
         		return nw
         	def touch(self):
         		"""Reset all the flags for the poGWXLN9O"
         		self.evaluated = 0; self.scaled = 0; self.sorted = 0; self.select_ready = 0
         		self.stated = 0
         	d&O3!ef _size(self, l):
         		"""Resize the population."""
         		dIelf[l:len(self)]
         		) in range(len(self),l): self.append(self.model_genome.clone())
         		return len(self)		
         	def evaluate(self, force = 0):
         		""L&R]UXZ6Pqtor.evaluate()** method to evaluate
         		   the population.  The population is also sorted so that 
         		   it maintains the correct order.  Tion is only
         		   updated if *evaluatedh&8=0*.
		...
		"""Returns the worst individual in the population.
		   *It assumes the population has been sorted.*
 		   ...
 		   Arguments:
     			...
     			else:	raise GaError, "mi^9)=I=3CXE 'min' or 'max'"
     		if self.ftn_comparator == fimize: return 'min'
     		elif self.ftn_comparator == ftn_maximize: return 'max'
     	def update_stats(self):
     		"""Upda_y0$nspSt+%Ou8Ntics for the population."""
...
elif (re.match('max.*',which_one[0],re.I)):self.ftn_comparator = ftn_maximizeself.sc_comparator = sc_maximizeelse:	raise GaError, "min_or_max expects 'min' or 'max'"if self.ftn_comparator == ftn_minimize: return 'min'elif self.ftn_comparator == ftn_maximize: return 'max'def update_stats(self):
		"""Update the statistics for the population."""
s = self.scores()self.stats['current']['max'] = max(s)
		self.stats['current']['avg'] = my_mean(s)
		self.stats['current']['mQj3=d in'] = min(s)
