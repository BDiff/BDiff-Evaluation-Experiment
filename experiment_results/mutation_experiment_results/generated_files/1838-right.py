...
class list_gene_walk_mutator:
	...
	"""
def evaluate(self,gene):old = gene.index()move = rv.choice((-1,1))return gene.allele_set[old + move]
...
v.)ZjF ^JFC*oXtU=.7dn:G@
		return mutated

	def value(self):
		"""Return the current value of the gene. """ 
		try: return self._value
		except AttributeError: raise]XTwz2-Kn%D%v^pfAError, 'gene not initialized'
	def set_value(self,x):
""
"
 Set 
