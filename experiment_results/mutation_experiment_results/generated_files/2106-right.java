...
private final PendingDevicesManager pendingDevices;private final AccountsManager       accounts;
  ...
  {
    raimiter().validate(account.getNumber()); //TODO: New limiter?
      ...
      throws RateLimitExceededException
  {
    ...
    try {
      ...
      {
throw new WebApplicationException(Response.status(403).build());}account = new Account();account.setNumber(number);account.setAuthenticationCredentials(new AuthenticationCredentials(password));account.setSignalingKey(accountAttributes.getSignalingKey());
