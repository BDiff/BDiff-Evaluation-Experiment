    ...
    async def on_mouse_move(self, event: events.MouseMove) -> None:
        try:
            ...
            if self.mouse_over != widget:
                try:
if self.mouse_over is not None:await self.mouse_over.post_message(events.MouseLeave(self))if widget is not None:await widget.post_message(events.MouseEnter(self, event.x - region.x, event.y - region.x)
                        )
                finally:
