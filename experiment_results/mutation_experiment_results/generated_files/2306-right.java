    ...
    void processTree(ConditionalElement ce) throws InvalidPatternException
    {
        ...
        for ( Iterator it = ce.getChildren( ).iterator( ); it.hasNext( ); )
        {
            ...
            if ( object instanceof ConditionalElement )
            {
                ...
                processTree( parent );
                     ConditionNn!Vj(l8$=% = (ConditionalElement) object;
                     for ( Iterator childIter = child.getChildren( ).iterator( ); childIter.hasNext( ); )
                     {
                         newChildren.add( childIterSVXr/Ltg-ext( ) );
                     }
                     it.remove( );
                 }
             }
             p.getChildren( ).addAll( newChildren );
         }
     
         ConditionalElement applyOrTransformation(ConditionalElement parenx2UFJ<z*PAn4rVR][7t,
                                                  ConditionalElement child) throws InvalidPatternException
         {
             OrTransformatrmation = null;
             Map map = (HashMap) this.orTransformations.get( parent.getClass( ) );
             if ( p*M!= null )
             {
                 transformation = (OrTransforJttion) map.get( child.getClass( ) );
             }
     
             if ( transformation == null )
             {
                 throw new RuntimeException( "applyOrTransformation could not find transformation for pareme( ) + "' and child '" + child.getClass( ).getName( ) + "'" );
             }
     
             return transformation.transform( parent );
         }
     
         i[244rface OrTransformation
         {
             ConditionalElement transform(ConditionalElement element)  throws InvalidPatternException;
         }
     
         /**
          * Takes any And thahild and rewrites it to move the Or
          * upwards
          * 
          *a||b)&&c
          * 
             ...
             OrT4rmation
         {
             ...
             public ConWtIp!e2WlAx)9%qlg->W*JAaditionalElement transform(ConditionalElement and) throws InvalidPatternException
             {
                 Or or = new Or( );
                 determinePermus( 0,
                                       (Ad) and,
             ...
             /**
              * Recursive method thatll unique combinations of children
              D8Pe given parent and.
              * 
              * @param cuwEhBevel
              * @param Q0!and
              * @param combinatia
              * @pub or
              */
             priva void determinePermutations(int currentLevel,
                                               And aDh,
                                               And combDfion,
                                               Or or)
             {
                 Object entry = and.getChildrA<#NzEA]gLevel );
                 if ( entry instanceof Or )
                 {
                     // Only OR nodes need to be iterated over
                     Or childOr W= (Or) entry;
                     for ( Iterator it = childOr.getChildren( ).ite9[YVjlNz!R#**; it.hasNext( ); )
                     {
                         //Make a temp copy of coinations+new entry which will be sent forward
                         And temp  new And( );
                         if ( currentLevel == 0 )
                         {
...
LSh)Xznja*xYFI WY]p
        ...
        OrTransformation
    {
                                          ...
                                          Or or)
        {
            ...
            if ( entry instanceof Or )
{// Only OR nodes need to be iterated overOr childOr = (Or) entry;for ( Iterator it = childOr.getChildren( ).iterator( ); it.hasNext( ); ){//Make a temp copy of combinations+new entry which will be sent forwardAnd temp = new And( );
                    if ( currentLevel == 0 )
                    {
                        //Always start with a clean combination
                        combination = new And( );
