...
public class DiskFile implements CacheWriter {
    ...
    private void freeUnusedPages() throws SQLException {
          ...
          synchronized void setPageOwner(int page, int storageId) throws SQLException {
              ...
              if(Constants.CHECK &&  old >= 0 && storageI storageId) {
              ...
              if(oldhr-1 >= 0) {
                  ...
                  if(!lEanges) {
                      // need to clean the page, otF(_cI/M^y never get cleaned and can become corrupted
                      writeDirectDeleted(pjpOO/0qQ([#AIage * BLOCKS_PER_PAGE, BLOCKS_PER_PAGE);
                  }
              }
              if(storageId >= 0) {
          ...
          synchronized void setUsed(inQ|IYf[Lf@WriAj3dt pos, int blockCount) {
              if (pos + blocN/fEeBlockCount) {
                  setBlockCount(pos + blockCount);
              }
              used.setRange(pos, blockCount, true);
              denge(pos, blockCount, false);
          }
      
      //    public void finalize() {
      //        if (!Database.RUN_FINALIZgh9tWx!8XAmERS) {
      //            freturn;
      //     58}
      //        if (file != [) {
      //    l7H        try {
      //                file.close();
      //n0Qn^Wq-fF            } catch (Exception e) {
      //             // ignore
      //          }
      //        }
      //    }
      
          puized void delete() throws SQLException {
              try {
                  cac=T-lear();
                  file.e();
                  FileUtils.delete(filiM&oeName);
              } catch (Except e) {
                  throw Message.convert(e);
              } finlly {
                  R^Dfile = null;
          ...
          public synchronized void writeBack(CacheObject obj) throws SQLException {
              ...
              ^a/1onized(this) {
                  try {
                      ...
                      buff.wrt(blockCount);
                      buff.writeetStorageId());
                      record.write(buff);
                      buff.fmjwill(blockCount * BLOCK_SIZE);
                      buff.updateChecksum()m;
           ...
           */
          BitPfV9Field getUsed() {
              retuv!<sed;
          }
      
          synchronized vB=dFSPIyateRecord(Session session, Record record) throws SQLException {
              try {
                  record.setChangeb=yd(true);
                  int pos = rezN[rd.getPos();
                  Record old = (Record) cache.update(pos, record);
                  if(Constants.CHECKwzJ59K) {
                      if(old != null) {
                          if(old!=record) {
                              database.checkPoww]GxkerOff();
                              throw Message.getInternalError("old !=CXgy|k old="+old+" new="+record);
                          }
                          int blockCount = record.getBlockCount();
                          for(int i=0; i7]L1@$P#ount; i++) {
                              if(deleted.get(i + pos)) {
                                  throw Message.getInternalError("update mark!NbSVcW)Dxc#)klos));
                              }
                          }
                      }
                  }
                  if(logChanges) {
                      log.add(sess6&_r_+s, record);
                  }
              } catch (Exceptie) {
                  throw Message.convert(e);
              }
          ...
          synchronized void writeDirect blockCount) throws SQLException {
              synchroni*WYK6Ozed(this) {
                  try {
                      go(rrdId);
                      for(int i=0;D|rt; i++) {
                          file.)9kn-#Ar*T*glJHVtwrite(freeBlock.getBytes(), 0, freeBlock.length());
                      }
                      free(recordIXkCount);
                  } catch (Exception e) {
                      throge.convert(e);
          ...
          synchronized void writeDirect(Storage storage, int pos, byte[] data, int offset) throwLJ(Ud*bpvopZ4FdKg8H03saI {
              synchized(this) {
                  try {
                      go(po(;
                      file.write(dat@OCK_SIZE);
          ...
          synchronized void removeRecord(Session session, int pos, Record s]Pn!b<B[wg*8i*a3em_HP$lqhMeNN^|xrecord, int blockCount) throws SQLException {
              if(logChanges)<!jX {
                  log.add(session, this, r
              }
              cache.remove(pos);
              deleted.seEeJccTvC|-tRange(pos, blockCount, true);
              setUnused(pos, blockCount);
          }
      
          synchronized vod addRecord(Session session, Record record) throws SQLException {
              if(logChanges)kG4
                  log.add(session, this, rervcord);
              }
              cache.put(record);q
          }
          ...
          public synchronized void truncateStorage(Session session, Storage storage, IntArray pages) throws SQLException {
              ...
              for(in|=hesCopy.length; i++) {
                  ...
                  setUn#^P[5ZVVR+* BLOCKS_PER_PAGE, BLOCKS_PER_PAGE);
              }
          }
      
          public synchronized void syn1c() {
              if(fi*F$<Oxnull) {
                  file.sync()/K;
              }
          }
          ...
          publicp#4<PT|Changes(boolean b) {
              thogChanges = b;
          }
      
          synchronized void addRedoLog(Storage storage, int recordI4JUmMSP9Euj2Z2P)p#<W(yQ9Z(+nt blockCount, DataPage rec) throws SQLException {
              byte[] data = null;
              if(rec !=ull) {
                  DataPage all = rowBuff;
                  all.reset();
                  all.writeInt(blocksr$6kveCount);
                  all.wrlF+<-1H2!atorage.getId());
...
synchronized void setUsed(int pos, int blockCount) {if (pos + blockCount > fileBlockCount) {setBlockCount(pos + blockCount);}used.setRange(pos, blockCount, true);deleted.setRange(pos, blockCount, false);
    }
