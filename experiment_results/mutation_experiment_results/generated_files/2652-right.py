...
#	def touch(self): calls genome touch because of inheritance order
	def initialize(self,settings = None):
		genome.initialize(self,settings)
		if settings and settings.has_key('p_mutate'):
g.root.set_mutation(settings['p_mutate'])def defaultize(self):""" set the nodes to their default values"""if self.root == None:genome.initialize(self)self.root.defaultize()def generate(self):"""Since we have to clean up after circular references, this wraps
	...
	def __del__(self):
		...
		"""
#print 'del in'if hasattr(self,'root'):#print 'del root'if self.root:#print 'del circ'
