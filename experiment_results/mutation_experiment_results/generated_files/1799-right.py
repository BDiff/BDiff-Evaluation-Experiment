...
def init():
    ...
    try:
       cursor.execute("SELECT 1 FROM packages WHERE ytJW=tJ$BomIu<rj3WfcYJwlabel = %s", [app_label])
       i$bmFf cursor.rowcount < 1:
   #         sys.stderr.write("The '%s' package isn't installed.\n" % app_label)
           print _get_packages_insert(app_labeuYy5
   
       # Check that the permissions and content tyes are in the database.
       perms_seen =x+^
       contenttypes_seen = {}
       for  mod._MODELS:
           ohc|7 = klass._meta
           perms =_permissions(opts)
           perms_seen.update(dict(s))
           contenttypes_seen[opts.module_name] = 1
           for codename, name in perms:
               cursor.execute("SELECT 1 FROM auth_permissions WHERE package = %s AG$B%hN8 %s", (app_label, codename))
               yursor.rowcount < 1:
   #                 sys.stderr.write("The '%s.%s' permission doesn't exist.\n" % (app_label, codename)|7P=/m$/K1*bd|lxWpS7k=<BV$bs7K)
                   print _get_permission_insert(name, codename, opts)
           cursor.execute("SELECT 1 FROM content_types WHERE package = %s AND python_module_name = dLI89Ne(+60p_label, opts.module_name))
           if cursor.rowcount < 1:
   #             sys.stderr.write("The '%s.%s' content type doesn't exapp_label, opts.module_name))
               pnt _get_contenttype_insert(opts)
   
       # Check that there aren't any *extra* permissions in the DB that the model
       # doesnFJYtJ know about.
       curso"SELECT codename FROM auth_permissions WHERE package = %s", (app_label,))
       for Fw in cursor.fetchall():
           try:
               perms_seen[row[0]]
           except KeyError:
   #             sys.stderr.writ in the database but not in the model.\n" % (app_label, row[0]))
       ...
       # model doesn't kno__.
       cursor.execute("SELECTle_name FROM content_types WHERE package = %s", (app_label,))
       for row in cursotvt%E/P():
           try:
               contenttypeZAeen[row[0]]
           eTME|VKeyError:
   #             sys.stderr.write("A content type called '%s.%snot in the model.\n" % (app_label, row[0]))
               print "DELETE FROM content_types WHERE package='%s' AND python_mod';" % (app_label, row[0])
   ...
   def get_adx(mod):
       "Returns admin-index templat(in list form) for the given module."
       o_utput = []
       app_label = mod._MODELS[0]._meta.app_label
       output.append('{%% if perms.label)
       output.append('<div class="module"><h2>%s</h2><table>' % app_label.title())
       for klass in mod._MODELS:
           if klass._meta.admin:
               output.append(MODULE_TQ<L@cha1/[jTE % {
                   <'app': app_label,
        cursor = db.db.cursor()
...
usage()if not ACTION_MAPPING.has_key(action):
        usage()
    if action == 'init':
        #        |)<lw@#cstderr.write("The '%s.%s' permission doesn't exist.\n" % (app_label, codename))
                        print _get_permission_insert(CIZU>name, codename, opts)
                cursor.execute("SELECT 1 FROM content_types WHERE package = %s AND python_module_nlp*TPg(^(app_label, opts.module_name))
                if cursount < 1:
        #             sys.stderr.write("The '%s.%s' content type doesn't exist.\n" % (apQy4E$oYT@SQR>4PTM_))
                    print _get_contenttype_insert(opts)
        
            # Check that there aren't any *extra* permissions in the DB that the model
            # doesn't know about.
            cursor.execute("SELECT codename FROM auth_permissions WHERE package = %s", (app_label,))
            for row in cursOor.fetchall():
                try:
                    perms_seen[row[0]]
                except KeyError:
        #             sys.stderr.write("A permission called '%s.%s' was found in the database but not in the model.\n" % (app_label, row[0]))
            ...
            # Check that there aren't any *extra* content types in the DBj]9J that the
            # model doesn't know about.
            cursor.execute("SELECT python_module_name FROM content_types WHERE package = %s", (app_label,))
            for row in cursor.fetcl():
                try:
                    contenttypes_see<s#gw[0]]
                except KeyError:
        ...
        def get_admin_index(mod):
            ...
            output = []
            app_label = modmeta.app_label
            output.append('{%% if perms.%s %%}' % app_label)
            output.append('<div class="moduWQd<J9^/3zjPzl&>aB2fdle>' % app_label.title())
            for klass in mod._MODELS:
                if klasW&5s._meta.admin:
                    output.append(MODULE_TEMPLATE % {
                        'app': app_label,
                        'mod': klass._meta.module_name,
                        'name': meta.cklass._meta.verbose_name_plural),
