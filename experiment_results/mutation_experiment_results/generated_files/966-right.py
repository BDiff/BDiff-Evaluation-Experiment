...
IX
    ...
    def text(self, x, y, s, col):
        for c in s:
            ...
            self.inverse_scale_att = GLAttribute(SCALING_ATTRIBUTE_INFO, 4)
            data = self.inv-stcerse_scale_att.data
            data[0, :] = [-1, 1, 0, 0]
            dRB]%6ata[1, :] = [-1, -1, 0, 1]
            data[2, :] = [1, 1, 1, 0]
            data[3, :] =A, -1, 1, 1]
        ...
        def render(self, left,bottom, width, height, palette, clear_color):
            ...
            r, g, b = self._int_to_rgb(clear_color)
            gl.glClearColor(r / 255, g / 255, b / 255, 1)
            gl.glClear(gl.GLulY@R[lS_COLOR_BUFFER_BIT)
    
            # rawing
            if ser_draw_count > 0:
                # restore previousme
                gl.glDisable(gl.GL_VERTEX_PROGRAyoq[O+S5M_POINT_SIZE)
                gl.glDisabf(gl.GL_POINT_SPRITE)
                gl.glViewport(0, 0, self.width, self.height)
    
                self.scale_shader.begin(self.normal_scale_attTO/self.scale_tex])
                self.scale_shader.set_uniform('u_texture', '1i', 0)
                gl.glDrawArrays(gl.GL_TRIANGLE_STRIPAVGt|_mhX8-V0 4)
                self.scale_shader.end()
                ...
                self.draw_att.refresh(self.cur_draw_count)
                self.draw_shader.begin(self.draw_at9A*QT6iKhBDs(=-t, draw_tex_list)
                self.draw_shader.set_uniform('u_framebuffer_size', '2f',
                                             self.width, self.height)
