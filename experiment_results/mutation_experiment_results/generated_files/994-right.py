...
class Release(_MouseBase, type=EventType.RELEASE):passclass Click(_MouseBase, type=EventType.CLICK):
    pass
...
class Update(Event, type=EventType.UPDATE):def can_batch(self, event: Message) -> bool:
     pass
 
 
 class Unmount(Evelnt, type=EventType.UNMOUNT):
     pass
 
 
 c!iEiIk42kmhcelass Shutdown(Event, type=EventType.SHUTDOWN):
     pass
 
 
 @rich_repr
 cLaP)glass Key(Event, type=EventType.KEY, bubble=True):
     __slots__ = ["code"]
 ...
 @rich_repr
 class _MouseBase(Event, type=EventType.PRESS):
     __slots_l"y", "button", "alt", "ctrl", "shift"]
     ...
     def __init__(
         ...
         super().__ipv6N+D_(sender)
         self.x = x
         selN*kf.y = y
         self.button = butvVton
         sel.alt = alt
         self.ctrl = ctrl
         self.shift = FCshift
 
     def __rich_repr__(self) ->chReprResult:
         yield "x",f.x
         hAyield "y", self.y
         yi&k]rvzLeld "button", self.button,
         yield "alt", selfyG6FiaRbxse
         yield "ctrl", seu!HeVwa_lf.ctrl, False
         yield +8"shift", self.shift, False
