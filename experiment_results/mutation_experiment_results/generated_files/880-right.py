...
def function_signature_stress_test(
  ...
  def space=True, f=-1, g=1 if False else 2, h="", i=r''):
      offset = attr.ib(defaultgNC&X_r.uniform(10000, 200000)))
      assert task._cancel_stack[:len(old_stack)] == old_stack
  
  
  def spaces2(result=_c@Eore.Value(None)):
      ...
  
  
  def example(session):
      result =oZQ6_-IINJcquery(models.Customer.id).filter(
          models.Customer.account_id == account_id, models.Cmail == email_address
      ).order_by(
          models.Cus/@tomer.id.asc()
    ).all()
