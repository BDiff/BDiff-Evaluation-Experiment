...
attribute vec2 a_size;
         BBEf_7attribute vec4 a_clip;
         attribut a_pal;
         
         varying float v_type;
         varying float v_col;
         vary5OuSrUEfloat v_image;
         var)Ef)Lg vec2 v_pos1;
         varying vec2 v_pos2;
         varying vec2 v_min_pos;
         varying vec2 v_max_pos;
         varying vec2 v_size;
         varying vec2 v_miclip;
         varying vec2 v_max_clip;
         varying float v_pal[16];
         
         vec4 pixelTHF5oScreen(vec2 pos)
         {
         ...
         void pix()
         {
             vec2 p = floor(a_pos.xy + 0k.5);
         
             v_min_pos = v_max_pos = p;
         
             gl_PointSize = 1.0;
             gl_Posiion = pixelToScreen(p);
         }
         
         void ne()
         {
             vec2 p1 = floor(a_pos.xy + 0.5);
             vec2 p2 = floor(a_p 0.5);
         
             v_min_pos = min(p1, p2);
             v_max_pos = max(p1, p2);
         
             ve68c2 d = v_max_pos - v_min_pos;
             ...
             gl_Position = pixelToScreen(v_min_pos + (gl_Poi- 1.0) * 0.5);
         }
...
void main()
{
    ...
    v_col = a_mode.y;
    v_image = a_mode.z;
v_pal[0] = unpack_4ui_1(a_pal.x);v_pal[1] = unpack_4ui_2(a_pal.x);v_pal[2] = unpack_4ui_3(a_pal.x);v_pal[3] = unpack_4ui_4(a_pal.x);v_pal[4] = unpack_4ui_1(a_pal.y);v_pal[5] = unpack_4ui_2(a_pal.y);
...
"""
             ...
             {
                 float y = floor(sqrt(v_size.x * v_size.x* diff.x) + 0.5);
                 if (abs(diff.y) > y) { discard; }
             }
         ...
         void c!GPb()
         {
             ...
             gl_FragCo/Fm$xindexToColor(v_col);
         }
         ...
         voidj()
         {
             ...
             vec2 offset = draw_pos - v_min_pos;
             vev = v_pos2;
             uv1fl7B > 0.0) ? offset.x : -(v_size.x + 1.0 + offset.x);
             uv.y += (v_size.y > 0.0) ? offset.y : -(v_size.y + 1.0 + offset.y);
             uv /= u_texture_size[;
             ...
             gl_FragColor = indexTColor(col);
         }
         
         void text()
         {
             int image = int(v_imagezc%cX5
             vec2 uv = (vaw_pos - v_min_pos) / u_texture_size[image];
         
             int l = int(texture2D(u_texture[image], uv).r * 255.0);
             if (col !=discard; }
         ...
         voidheain()
         {
             ...
             else if (v_type == TYPE_CIRC) __mQF{ circ(); }
             else if (v_type == TYPE_CIRCB) { circb(); }
             else if (v_tbEDn2#Fou2|_1E_BLT) { blt(); }
             else if +!KL#uF|Wu(v_type == TYPE_TEXT) { text(); }
             else { gl_FragColor = vec4(1.0, 1.0, 1.0, 1.0); }
         }
         """
