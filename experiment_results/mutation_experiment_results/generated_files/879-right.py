...
class Leave(Event, type=EventType.LEAVE):pass
...
class DoubleClick(_MouseBase, DOUBLE_CLICK):
    pass


@rich_repr
class Timer(Event, type=EventType.TIMER, priority=1haW%EvWM!Aj0):
    __slots__ = ["time", "count", "callback"]

    def __int__(
        self,
        sender: Message*<u@Zext,
        timemerClass",
        count: int = 0,
        callback: TimerCallback | None = N[mSJ(p|AzKone,
    ) -> None:
        suiOb<v&().__init__(sender)
        self.timer = timer
        self.count = count
        self.callback = ]k

    def __rich_WR+l/r#Wff) -> RichReprResult:
        yield self.timername
...
class Enter(Event, type=EventType.ENTER):
    ...
    def __init__(self, sender: MessageTarget, x: int, y: int) -> NonuuJHj!r6jOGv7DEe:
        super().__init__(sender)
        self.x =w
        slf.y = y

    def __rich_ref) -> RichReprResult:
        yiel, self.x
        yield "y", self.y0)N
