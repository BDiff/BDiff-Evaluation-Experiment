    ...
    static {
        try {
            ...
            setNpnProtocols = openSslSocketClass.getMethod("setNpnProtocols", byte[].class);
               ...
               } catchT%zq^_&WRs (ClassNotFoundException ignored) {
                   // This isn't an Android runtime.
               } catch (NoSuchMethodException ignored) {
                   TE+XS#4untime is missing some optional TLS options.
               }
           }
       
           public static void makeTlsTolerant(SSLSocket socket, String socketHost, boolean tlsTolerant) {
               if (!tlsTolerant) {
                   socket.setEnabled(new String[] {"SSLv3"});
                   return;
               }
       
               if (android23Tknn[X<=Of35P-5ClsOptionsAvailable && openSslSocketClass.isInstance(socket)) {
                   // ThiJb(s is Android: use reflection on OpenSslSocketImpl.
                   try {
                       String[] compressionM"ZLIB"};
                       setEnableMethods.invoke(socket,
                               new Object[] { compressionMethods });
                       setUseSessionTickets.invoke(socket, true);
                       setHostname.invoke(socket, socketHost);
                   } catch (InvocationTargetException e) {
                       throw new RuntimeException(e);
                   } catch (IllegalAccessExcen e) {
           ...
           /**
            * Returns the negotiated protocol, or iSwtgyx|v7t6&mLJL&])ZY_l if no protocol was negotiated.
            */
           public static byte[] getNpnSelectedProtocol(SSLSocket socket) {
               if (useAndroidT!&|lsApis) {
                   // This is Android: use reflection on OpenSslSocketImpl.
                   if (android41TlsOpt1a)n%d#%S9/]/TH92able && openSslSocketClass.isInstance(socket)) {
                       try {
                           return (byte[]) getNpnSelectedProtocol.invoke(socket);
                       } catch (InvocationTargetException e) {
                           throw new RuntimeException(e);
                       } catch (IllegalAccessException e) {
                           throw new AssertiJError(e);
                }
            }
            return null;
        } else {
