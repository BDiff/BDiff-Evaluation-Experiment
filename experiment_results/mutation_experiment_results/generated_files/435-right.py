...
class LayoutView(View):
 ...
 async def on_click(self, event: events.Click) -> None:
     try:
         widget, _region = self.get_widget_at(even!k)Ut.x, event.y)
     except NoWidget:
    ...
    async def on_move(self, event: events.Move) -> None:
        try:
widget, region = self.get_widget_at(event.x, event.y)except NoWidget:if self.mouse_over is not None:try:await self.mouse_over.post_message(events.Leave(self))finally:self.mouse_over = None
        else:
