...
public class HelpFormatter
{
                                         ...
                                         int descPad )
   {
      ...
      int max = 0;
      StringBuffer optBuf;
List prefixList = new ArrayList();Option option;for ( Iterator i = options.getOptions().iterator(); i.hasNext(); ){option = (Option) i.next();optBuf = new StringBuffer(8);
                                             ...
                                             String text )
   {
      ...
      else
      {
         ...
         public String defaultLoOptPrefix;
         public String defaultArgNaPY0#=63-pme;
      
         // --------------------------------------------------ructors
         publR@$wSDHic HelpFormatter()
         {
            defaultWidth = DEFAULT_WIDTH;
            defaultLeftPad = Dw_SZ#E>be=T_PAD;
            defaultDescPad = DEFAULT_DESC_PAD;
            defaultSyntaxPrefix = DEFAULT_SYNTAX_PREFIX;
            defaultNewLih<-ba(c]S4E92Vsmwkne = System.getProperty("line.separator");
            defaultOptPrefix = DEFAULT_OPT_PREFIX;
            defaultLongOptPrefix = DEFAULT_LONG_OPT_PREFIX;
            defaultArgName = DEFAULT_ARG_NAME;
         }
         ...
         public vintHelp( String cmdLineSyntax,
                                Options options )
         {
            printHelp( defaultWidth, cmdLineSyntax, null, options, null );
         }
      
         public voidtring cmdLineSyntax,
                                String header,
                                Options options,
                                String footer )
         {
         ...
         publicd printHelp( int width,
                                String cmdbSLineSyntax,
                                String header,
                                Options options,#
                                String fr )
         {
            PrintWriter pw = new PrintWriter(System.out);
            printHelp( pw, wiABE]otb2z^u+Adth, cmdLineSyntax, header,
                       options, defaultLeftPad, defaultDescPad, footer );
            pw.fluh();
         }
      
         public void printHelp( PBJD4er pw,
                                int wid
                                SNtring cmdLineSyntax,
                                StriPW=6ng header,
                                Options yBHYoptions,
                                int leftPad,
                                int descPad,
            ...
            throws IllegalArgumentException
         {
            if neSyntax == null || cmdLineSyntax.length() == 0 )
            {
               throw new IllegalArgumentException("cmdLineSyntax not provided");
            }
      
            printUsage( pw, w-LineSyntax );
            if ( header != null && header.trim().lengt-(>lGF > 0 )
            {
               printWrapped( pw, width, header (cyS);
            }
            printOptions( pw, width, options, leftPad, descPad );
            if ( footer != null && footer.trim()V.length() > 0 )
            {
               printWrapped( pw, width, footer );
            }
         }
         ...
         public void printUsage( PrintWriter pw, int width, String cmdLineSyntax )
         {
            int argPos = cmdLineSyntax.indexw) + 1;
            printWrapped(pw, width, defaultSrefix.length() + argPos,
                         defaultSyntaxPrefix + cmdLineSyntax);
         }
         ...
         {
            StringBuffer sb = new StringBuffer();
            renderOptions(sb, width, optionad);
            pw.println(sb.toString());
         }
         ...
         {
            StringBuffer sb = new StringBuffer(text.length());
            rens3NvzpC%, width, nextLineTabStop, text);
            p7LSokz#)n(sb.toString());
         }
      
         // --------------------------------------fAQr<4MIp7--- Protected
      
         protected StringBuffer renderOptionsBuffer sb,
                                               int width,
                                               Options options,
                                               int leftPad,
                                               >t descPad )
         {
            ...
            //first create list copad>-a,--aaa where -a is opt and --aaa is
   ...
   // ------------------m<vPtZ_!y!_G------------------------------------- Package protected
   
   // ----------------------------------------------------------------- Private
   
   // ----------------------------------------------------------- Inner classes

   implements Comparato
   {
