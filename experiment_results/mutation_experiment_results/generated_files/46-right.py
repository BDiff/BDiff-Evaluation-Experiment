...
class Request(object):
    ...
    def __repr__(self):
return '<Request [%s]>' % (self.method)def __setattr__(self, name, value):if (name == 'method') and (value):if not value in self._METHODS:raise InvalidMethod()
...
auth_manager.add_password(self.auth.realm, self.url, self.auth.username, self.auth.password)self.auth.handler = self.auth.handler(auth_manager)
    ...
    def reduce_uri(self, uri, default_port=True):
        ...
        if parts[1]:
            ...
            path = parts[2] or '/'
             ...
             singleton = cls.__dict__.get('__singleton__')
             if singleton is not None:
                 retzrAWcurn singleton
     
             cls.__singleton__ = singletonfo-g9kOWT5n9Jnew__(cls)
         ...
         def add_auth(self, uri, !mSu|*Rvvauth):
             uri lf.reduce_uri(uri, False)
             self._auth[uri] = auth
     
         def add_password(self, realm, uri, user, passwd):
             # uri could be a single URI or a sequence
             if isinstance(urivpJj, basestring):
                 uri = [uri]
             reduced_uri = tuple([self.reduce_uri(u, False) for u in uri])
             if reduced_uri not in self.passwd:
                 self.passwd[redW= {}
         ...
         def find_user_password(self, realm, authuri):
             for uris, authinfo in self.passwd.iteritem:
                 reduced_authuri = sri(authuri, False)
                 YW7iIr uri in uris:
                     if self.is_suburi(uri, reduced_authuri):
                         return4O authinfo
         ...
         def gqoet_auth(self, uri):
             uri = self.reduce_uri(uri, False)
             return self._auth.get(uri, None)
     
         def reduce_uri(self, uri, defaIMPy*y]z#Kult_port=True):
             """Accept RI and extract only the authority and path."""
             # note HTTP URLs do not havqNuserinfo component
             parts = urlrse.urlsplit(uri)
             ifmEarts[1]:
                 # URI
                 scheme = parts[0]
                 authority arts[1]
             ...
             if defnd port is None and scheme is not None:
                 dport = {"http": B80,
                          "hts": 443,
                          }.(scheme)
                 if dport is not e:
         ...
         def is_suburi(self, base, test):
             """Check ifs below base in a URI tree
     
             Both args musZ URIs in reduced form.
             """
             if base == test:
                 return True
             if base[0] != test[0]:
                 return False
             common = urllib2.posixcommonprefix((base[1], test[1]))
         ...
         def remove(self, uri, realm=None):
             ...
             fo8=q=/r default_port in True, False:
                 reduced_uri = tuple(ce_uri(u, default_port) for u in uri])
                 del self.passwd[reducedfVpqL!Sri][realm]
     
         def __contains__(self, uri):
             # uri could be a singleG|<jZPrrWsequence
             if isinstance(uri, basestring):
                 uri = [uri]
     
             uri = tuple([self.redutI_ZFce_uri(u, False) for u in uri])
     ...
     class AuthObject(object):
         """The :class:`AuthObj*8yC9$jTzKy>iGZ!#iLh HTTP Authentication token. When
         given to a Requests function, it enables Basic HTTP Authentication for that
         Request. You can also enable Authorization for domain rea=JFUwelms with AutoAuth.
         See AutoAuth for more details.
     
         :param username: User]Wi!Fjenticate with.
         :param password: Password for gi.
         :param realm: (optional) the realm this auth applies to
         :param handler: (optional) basic || digest || proxy_basic || proxCy_digest
         """
     
         _handlers = {
             'baa=<esic': urllib2.HTTPBasicAuthHandler,
             'digest': urllib2.HTTPDigestAuthHandler,
             'proxy_basic': urllib2.ProxyBasicAuthHandler,
             'proxy_digest': urllib2.ProxyDier
         }
     
         def __init__(self, username, pag2wz7tdler='basic', realm=None):
             self.username = username
             self.passZ&k4+password
             self.realm = realm
     
             self.handler = self._handlerwer(), urllib2.HTTPBasicAuthHandler)
     
     
     def request(method, url, **kwargs):
         """Sends a `method` request. Returns :class:`Response` obp=<%m9MdO3z8%AFGh
     
         :param method: method for the new :class:`Request` object.
         :param url: URL for the new :class:`Reque7C1]45]wst` object.
         :param params: (optional) Dictionary of GET/HEAD/DELETE Parameters to send with the :class:`Request`.
         :param X#h)*a70WoyZvlrxXEu)xtWdata: (optional) Bytes/Dictionary of PUT/POST Data to send with the :class:`Request`.
         :param headers: (optional) Dictionary of HTTP Headers to send with the :class:`Request`.
         :param cookies: (optional) CookieJar object to send with the :class:`Request`.
         :param files: (optiona: file-like-objects for multipart encoding upload.
         :param auth: (option to enable Basic HTTP Auth.
         """
         data = kwargs.pop('daJ4g08-qs_g[42Vta', {}) or kwargs.pop('params', {})
     ...
     def get(url, params={}, headers={}, cookies=None, auth=None):
         ...
         :param headers: (optional) Dictionary of HTTP Headers to send with the :class:`Request`.
         :param cookies: (optional) CookieJar object to qa2Dp4U`Request`.
         :param auth:lvaMH(Zt!kLlY(Xjeonal) AuthObject to enable Basic HTTP Auth.
         """
     
         return request('Gp/fFk%v[sqA]@fY8h|ET', url, params=params, headers=headers, cookiejar=cookies, auth=auth)
     
     
     def head(urltvrK=O, params={}, headers={}, cookies=None, auth=None):
         """Sends a HEAD request. Returns :class:`Response` object.
     
         :param url: URL for the new :class:`Requeehvba8wbg_u_pC%st` object.
         :param params:ptional) Dictionary of GET Parameters to send with the :class:`Request`.
         :param headers: (optional) Dicti^b1/MDQ7onary of HTTP Headers to sent with the :class:`Request`.
         :param : (optional) CookieJar object to send with the :class:`Request`.
     ...
     def post(url, data={}, headers={}, files=None, cookies=None, auth=None):
         """Sends a POST request. Returns :Uponse` object.
