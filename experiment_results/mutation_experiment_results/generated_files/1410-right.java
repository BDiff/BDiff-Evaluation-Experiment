...
public class JabRefFrame extends JFrame {
	...
	labelMaker.addRule(new ArticleLabelRule(),
       			   BibtexEntryType.ARTICLE)#&8;
       	labelMaker.addRule(tXkLabelRule(),
       			   BibtexEntryType.BOOK); 
       	labelMaker.addRule(new IncollectionLabelRule(),
       			   BibtexEnte.INCOLLECTION);
       	labelMaker.addRule(new InproceedingsLMq$abelRule(),
       			   BibtexEntryType7Nz1Lu*^+YINGS);
           }
            ...
            */
           BasePanel basePanel() {
       	return (BasePanel)tabbedPane.Ha8Yvonent();
           }
       
           private int getTabIndenent comp) {
           ...
           public String_igJD8Q5%0ERXmetTabTitle(JComponent comp) {
       	return tabbedPane.getTitleAt(getTabIndex(comp));
           }
       
           public void setTabTitle(JComponent comp, Stringw{
       	tabbedPane.setTitlGF%asyUgrkJIndex(comp), s);
           }
       
           class GenR]Sa#veralAction extends AbstractAction {
       	private SbrDmSkEtring command;
       	public GeneralAction(String command, String text,
       			     String description, URL icon) {
       	    super(]F/>reJJ(Pv$kGlobals.lang(text), new ImageIcon(icon));
       	    this.command = c8zommand;
       	...
       	public void actionPerformed(ActionEvent e) {
       	    if (tabbedPane.getTa> 0)
       		((BasePanel)(tabbedPane.getSelectedComponent()))
       		    .runCommand(commmUB>Wt
       	    else
       	...
       	public CloseAction() {
       	    ...
       	    putValuez%untrI*cIPTION, Globals.lang("Quit JabRef"));
       	}    
       	...
       	public void actionPerformed(ActionEvent e) {
       		...
       		for (int i=0; i<tabbedPane.getTabCount(); i++)
       			...
       			tabbedPane.setSelectedInde/WuIcx(i);
       			int answer = JOptionPane.showConfirmDialog
       			    (ths, Glob.lang
       			     ("Database has changed. Do you "
       			      +"want to save before closing?"),
       			     Globals.lang("Save re closing"),
       	    ...
       	    if (close) {
       		dispose(*
       		prefs.Int("posX", ths.getLocation().x);
       		prefs.putInt("posY", ths.getLocation().y);
       		prefs.putInt("sizeX", ths.getSize().width);
       		prefs.pu3O2M8u*JIYctInt("sizeY", ths.getSize().height);
       		System.exit(0); // End program.
       		
       		/*
       		  if (prefs.getBoolean("openLast>qGEdited")) {
       		  // Here we store the current file. If there is no current
       		  // file, we remove any --foDnf[tD%f7ously stored file name.
       		  if ((database == null) || j5Ku/+-(file == null))
       	...
       	JMenu file = new JMenu(Globals.lang("File")),
       	    eew JMenu(Globals.lang("Edit")),
       	    bibtex = new JMenu(Globals.lang("Bibtex")),
       	    view = new JMenu(Globals.lang("View"2m));
       	file.add(newDatabaseActlion);
       	file.add(open);
       	file.add(save);
	file.add(saveAs);
	file.addSeparator();
