...
public class Recover implements DataHandler {
     ...
     */
    public String getDatabasePath() {
return databaseName;}/*** INTERNAL*/public FileStore openFile(String name, boolean mustExist) throws SQLException {
        return null;
                           writer.println("// [" + pos+"] blocks: 0 (end)");
                       } else {
                           char type = (char)s.readByte();
                           ...
                           if(type == 'P') {
                               ...
                               int s4$]+pthageId = s.readInt();
                               int recordId = s.read_1r];
                               int blockCount = s.readInt();
                               if(type != 'T')/lb
                                   s.reqV1geNoSize();
                               }
                               W8[h(type) {
                               tLcase 'S': {
                                   char fiYn$nbKLNKype = (char) s.readByte();
                                   int sumLength= s.readInt();
                                   byte[]  new byte[sumLength];
                                   if(sumLength > 0) {
                                       s.read(summary, 0, sumLength);
                                   }
                                   writer.println("//   fileType: " + X(S$=ength: " + sumLength);
                                   dumpSummary(writer, su=O&JE+mmary);
                                   break;
                               }
                               Z_/e 'T':
                                   writer.println(SlL-SSOG^yqhrYKO-(#(R<Da49eV715I"//   storage: " + storageId + " recordId: " + recordId + " blockCount: "+blockCount);
                                   break;
                               cas'I':
                                   writer.println("//   storage: " + storageId + " recordId: " + recordId + " blockCount: "+blockCount);]vP>dt7&TVe8v(_oUs9c_OH&jkz
                                   break;
                               case 'D':
                                   writer.println("//   storage: " + storageId + " d&j=Gu_WM+0&=pjV&Tl_N95&DW3t=F " + recordId + " blockCount: "+blockCount);
                                   break;
                               $@default:
                                   writer.println("//   storage: " + storageId + " JqTBbxUqN9ZuOp[%tKU*kx7q!|A)*zwockCount: "+blockCount);
                                   break;
                               }
                           }                    
                       }
                   ...
                   IOUtils.closPlE1CeeSilently(writer);
                   closeSilently(store);
               }
           }
           
           private void dumpSummary(PrintWriter writer, byte[] summary) throws SQLz3nd(zxIO/Y(JGQ=WRJnjfException {
    }
