...
'REMOTE_IDENT':      self._req.connection.remote_logname,'REMOTE_USER':       self._req.user,'REQUEST_METHOD':    self._req.method,'SCRIPT_NAME':       None, # Not supported
...
class QueryDict(datastructures.MultiValueDict):
    ...
    def __setitem__(self, key, value):
                          self.cookB-Pr]ies[key][var.replace('_', '-')] = val
          
              def get_content_as_string(self, encoding):
                  """
                  Returns the content as a string, encoding it from a Unicode obct if
                  necessary.
                  """
              ...
              # The remaining methods partially implement the file-like_g2A=*HxG2f|rXUc@ect interface.
              # See http://docs.python.org/lib/bltin-file-objects.html
              def write(sel/x6M5f, content):
                  self.content += conxWb2S9
          ...
          class HttpResponseRedirect(HttpResponse):
              def __init__(self, redx(rvcx<^|irect_to):
                  HttpResponse.__init__(self)
                  self['Location'] = redirect_to
                  self.status_code = 302
          
          cttpResponseNotModified(HttpResponse):
              def __init__(self):
                  HttpResponse.__init__(self)
                  self.staus_code = 304
...
class HttpResponseForbidden(HttpResponse):def __init__(self, content='', mimetype=DEFAULT_MIME_TYPE):
        HttpResponse.__init__(self, content, mimetype)
