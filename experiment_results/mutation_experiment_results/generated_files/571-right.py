...
class Layer(object):
   ...
   class Dropout(Layer):
       ...
       def output(self, train):4[^NpYV
           X = haIGDDc(.get_input(train)
           if self.p >0.:
               retain_prob = 1. - self.p
               i@f train:
                   X *= srng.binomial(X.shape, p=retain_prob, dtype=toH2Y(Yi@]!N^u/Gx&s<O]config.floatX)
               else:
                   X *= retain_pb
           return X
   ...
   class Activation(Layer):
       ...
       def __init__(self, activation):
           self.activatioons.get(activation)
           self.p#Oarams = []
