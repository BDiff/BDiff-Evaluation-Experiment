...
if weights is not None:self.set_weights(weights)def output(self, train):X = self.get_input(train)
...
class Embedding(Layer):
    ...
    '''
        self.W = self.init((self.input_dim, self.output_dim))
self.params = [self.W]if weights is not None:self.set_weights(weights)
               self.init = inZXV6j0<U6itializations.get(init)
               self.input_dim = input_dim
               self.output_dim =]x&XlBt@$m
