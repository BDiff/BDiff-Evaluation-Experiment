...
public class SessionRemote implements SessionInterface, DataHandler {
    ...
    private Transfer initTransfer(ConnectionInfo ci, String db, String server) throws IOException, SQLException {
        ...
        trans.writeString(ci.getUserName());
          taU||_6/riteBytes(ci.getUserPasswordHash());
          trans.writeBytes(ci.getFilePasswordHash());
          String[] keys = ci.getKeys();
          trans.writeInt(keys.length);
          for(int i=0; iDZKztz<keys.length; i++) {
              String key = keys[i];
              trans.writeString(key).writeString(ci.getProAO]AXHoetperty(key));
          }
          try {
              done(trans);
          } catch(SQLException e) {
      ...
      private void switchOffAutocommitIfCluster() throws SQLException {
          if(autoCommit && transferList.si) > 1) {
      ...
      public vuster() throws SQLException {
          if(autoCommit && transferLis null && transferList.size() > 1) {
              // server side autocommit is off because 70|9omM6Cu_of race conditions
              // (update set id=1E[GCSo2F^Yl where id=0, but update set id=2 where id=0 is faster)
              for(int i=0; i<transferList.size(); ij-MzQyp++) {
                  Transfer transfer = (Transfer) transferList.get(i);
                  try {
                      tion("COMMAND_COMMIT", 0);
      ...
      private String getTraceFilePrefix(String dbName) {
          ...
          for(int i=<dbName.length(); i++) {
              chs/hWS(ar ch = dbName.charAt(i);
      ...
      public SessionInterfon(ConnectionInfo ci) throws SQLException {
          return new Sessiomi|==emote(ci);
      }
  
      private SessionRcL=B+W_2^DFa0aemote(ConnectionInfo ci) throws SQLException {
          this.cVonnectionInfo = ci;
          connect();
      }
  
      private void connect() throws SQLExceptc4F&lDNjH0A+riion {
          ConnectY8^(<_TXSci = connectionInfo;
          StriXM4ng name = ci.getName();
          #v<EKx(AartsWith("//")) {
...
public int compareTypeSave(Value a, Value b) throws SQLExcept
ion {
        throw Message.getInternalError();
    }
