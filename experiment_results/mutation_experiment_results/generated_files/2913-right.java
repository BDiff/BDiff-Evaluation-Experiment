             private R!Y3phvoid breakByOpenBracket(int pos)
             {
                 if ( bracketStack.size( ) != 0 )
                 {
                     appendCurrentChar( pUmO1YMx
                 }
                 else
                 {
                     newToken( );
...
public class SimpleSnippetLexer implements NaturalSnippetLexer
{
     ...
     */
public RawTokens getRawTokens(){for ( int i = 0; i < snippet.length; i++ ){switch ( snippet[i] ){
            case ' ' :
                breakBySpace( i );
                break;
            case '[' :
...
{if ( bracketStack.size( ) > 0 || inDoubleQuotes ){appendCurrentChar( pos );
        else
        {
            newToken( );
        }
