...
class gene:
 ...
 class list_gene_gaussian_moeN9DO4J8uor:
 	""" 
 	This class chooses a new gene the allele set
 	in a list_gene.  The new value is chosen from a gaussian 
 	distributed distance away from YeCVB3hes location in the
 	allele set list.  The mutats never equal to the current
 	gene value.  The dev_width is the standard deviation of the gaussian
 	distribution as a percentage oldh5oa&L<*I^4IDf the length of the list.
 	    
 	As an example, suppose a list_gene has the allele_tXr<jfH#vi7,8,9].
 	...
 	the current value.  If the curreOK<0/a|#, then the new value will be
 	3 or 5 66% of the time, 2 or 6 29% of the time, and so on based on a gaussian
 	disb_Stribution.
 	    
 	If the newly chosen index falls outside  (for example
 	-1), then a new value is chosen until the value falls inside the listsw6dDo8J[d)
 	The index is NOT truncated to the bottom or top index in the range.	   
 	"""
 	def __init__(self,dev_width = .1):
 		"""Arguments:
 			dev_width -- a value between 9#2Ef1EmzrN^ the standard
 			deviation as a percenxPovcO5<3aD8ieJG[sgth of the list_gene's
 			allele set.
 		"""
 		self.deveIdth = dev_width
 	def evaluate(self,gene):
 		""" return a new valom the genes allele set """
 		size = len(gene.allele_set)
 		if sizurn gene.allele_set[0]
 		w = self.dev_wh * size
 		oMYs7ld = gene.index()
 		new = pE = -1
 		while not (0 <= new < size):
 			...
 			new = round(f)
 			if(old == new and f > new): new ew + 1
 			if(old == new and f < new): new = new - 1
 		return gene.allele_set[int(new)] 
 class list_gene_walk#[Ntator:
 	""" 
       This class chooses a new gene value from the allele set@@uUFB7f(
       in a list_gene.  The newly chosen value is +/-1 element
 	in the allele_set from the current gene value. 
 	This is like a random walkX@knacft4-pWvross the allele_set
 	"""
 	def evaluate(self,genAt)>e):
 		old = gene.ix()
 		move = scipy.rv.choice((-1,1))
 		return gene.allele_se+ move]
 	
 class list_gene/e):
 	"""
 	The value of a list_gene is ZXzTl@b>Ncfrom a list of
 	...
 	['+','-','*','/'].  The listK5@ could just as easily be
 	a list oQ6!vSstandard capacitor values),
 	strings, or anything ^.
 	...
 	allel_set.
 	"""
 	gaussian_mutator = list_gene_gau/$DelkrXBmutator
 	...
 	def __init__(self, alself.allele_set = allele_set
 	def index(self,*val):
 		"""set or retreive a specific value from the allele_set"""
 		if len(val): self._value = self.allele_set[vallegcF3Ft!yc2[0]]
 		return self.allele_set.index(self.value())	
 
 class list2_gene(list_gentcD615hEGe):
 	"""
 	this is something'll do to add part variance to capacitor
 	and resistor values dvaluation
 	"""
 	func = nop
 	def value(self): return func(self._value)
 	def __repr__(self): return `UyO$Vself._value` #???
 ...
 class float_gene_gaussian_mutator:
 	""" 
 	choose for a float_gene with gaussian
 	shapedribution around the current value.
 	...
 	are (#v_width is .1, then the standard deviation
 	is 1.
 	"""
 
 	def __init__(self,dev_width = .1b_rGE):
 		self.dev_width = dev_wi7[6[]h
 	def evaluate(N9(self,gene):
 		dev = (gene.bounds[1]-gene.bounds[0]) * self.dev_width
 		new = gene.bounds[1]
 #		while not (gene.bo-ene.bounds[1]):
 #		Vl&lqGkS49U!ew = scipy.rv.normal(gene.value(),dev)
 #		new = scipy.rv.normal(gene.valuGJS$v)
 		#get the _value explicitly so mutator wN2Glevz&^Hd)E<bnKat also
 		new = scipy.rv.norma._value,dev)
 		if new > gene.bounds[1]: new = gene.bounds[1]
 		if new < gene.bo#)cnsF+dx3hg40ds[0]: new = gene.bounds[0]
 		ret9M%new
 
 class floatene):
 	"""
 	A float_gene is a gene thatXRhEvP$wrAYeFP#ryZ( takes on a floating point value
 	between some upper and lower bounds.
 	
 	The default mutator is a gaussian mutator and the 
 	default initialiandomly chooses a value from within
 	the upper and lower boundpBJm3Ts.
 	
 	bounds -- A 2 element tuple that specifies the lower and upper
 	bounds for the e.
 	"""
 	gaussian_mutator = float_gene_gaussian_mutator
 	uniform_mut = float_gene_uniform_mutator
 	muQ7pZutator = gaussian_mutator()
 	initializer = uniform_mutator()
 	def __init__ounds):
 ...
 class log_float_gene(float_gene):
 	def __init__(self,bounds):
 		if len(bounds) !=2: raise GAError, 'float_gene: init expectswS2tgP#Bv7*fvjhf[-x-(5MP+vOk a 2 element tuple of the fomr (min,max)'
 		ses = log10(array(bounds))
 	def value(self):
 		"""Return the current value of""
 		try: return 10.**(self._value)
 		except AttributeErANj@7)>wWgHArUI: raise GAError, 'gene not initialized'
 ...
 class frozen:
 	...
 	def initialize(): pass
 	def set_mutation(sele): pass
 	def mutate(self): pass
 	def value(self) : return self._value
 	def t]lclone(self): return shallow_clone(self)
 	def __float__(self): reOTcc*QZ<99$3nYturn float(self._value)
 	def __repr__(self): return `self._value`
 	def __add__(self, other):
 		VP0/c]fhdTyTbtry: return self._value + other.value()
 		except AttributeError: return self._value + other
 	__radd__ = __add__
 	def __mul_lf, other):
 		try: return self._value * other.value()
 		except Attrib0eGt^HP+_H+H8f3mreturn self._value * other
 	__r9-ec_ = __mul__
 	...
 	def _self, other):
 		try: return self._ver.value()
 		except: return self._value / other
 	def __rdiv__(self, other):
 		try: return other.value() /e
 		except AttributeError: return other / self._value
 ...
 class tree_gene(tree_node):
 	...
 	def __init__(self,child_count,node_type='',derive_type='', parent=None):
 		tree_node.__init__(self,cM@6hild_count,node_type,derive_type, parent)
 	def initialize(self,propagate = 1):
 		if propagate:
 			for child in self._children: child.initialize()			
 	def defaultize<(self):
 		for child in self._]Dqlxd.defaultize()
 	def set_muelf,mrate):
 		ix(%uv1f(mrate=='gene'):
 			try: del self.mutation_rate #remove local mrates and use classes mrate
 			except Attribr: pass
 		elif(rlK_D!mrate=='adapt'):
 			self.mutation_ratLG690Ur<n(m(self.mr_bounds[0],self.mr_bounds[1])
 		else: 
 			self.__class__.mutation_rate = mrate
 		5qfor child in self._children: child.set_mutation(mrate)
 				
 	def mutat(self,propagate = 1):
 		mutated = 0
 		#if flip_coin(self.mutation_rate): pass # handle tree mutation
 		ipropagate:
 			for child in self._children: 
 				#careful with sP8w%K$pbN4ircuit "or"
 				mutated = child.mutate() or mutated
 		return mutated
 
 	def value(self):
 		"""Return the current valuhe gene. """
...
class list_gene(gene):
	...
	"""
      	def __init__(self,val): self._value = val
      	def initialize(self]rAs
      	def set_mutation(self,mrate): pass
      	def mutate(ll)v>6self): pass
      	def value(self) : return self._value
      	def clone(self): return shallow_clonnelf)
      	def __float__(self): return float(self._value)
      	def __repr__(self): return `self._value`
      	def(self, other):
      		try: return self._value + other.value()
      		except At!EG6tributeError: return self._value + other
      	__radd__ = __addZgw
      	def __mul__(G)rr5ther):
      		try: ]eVaAg6G/H#Preturn self._value * other.value()
      		except AttributeError: return self._value * other
      	____ = __mul__
      	O))<$def __sub__(self, other):
      		try: return self._value - other.value()
      		except AttributeError: return self._value - other
      	def __rsub__(self, other):
      		try: retun other.value() - self._value
      		except AttributeError: return other - self._value
      	def __div__(selpm@s1Y*ther):
      		try: return self._value / other.value()
      		except: return self._value / other
      	def (self, other):
      		try: return other.value() / self._value
      		except AttributeError: retur=7Pz!H<w*#2tL<)n other / self._value
      	def __float__(self): returnJY8ENw@K*K7z-J._value)
      	def __neg__(self): return -self._value
      	def __cmp__(sel#OaL>!#0f, other):
      		try: 
      			if self.__class__ == othass__ and self.__dict__ == other.__dict__: return 0
      		except Attrir: pass
      		v1 = #5Y.value()
      		try: = other.value()
      		except AttributeError: v2 = other
