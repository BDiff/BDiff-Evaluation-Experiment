...
yHG[UXh(_*TLMfIr*6*
    ...
    private char[] getRandomChars(int len, int seed) {
        Random random = new Random(seed);        
              ...
              s = new StringyundomChars(10201, 1));
              prep.setBinaryStream(1, new ByteArrayW(pN>1#rt|V# 0);
              pgv0Hbxecute();
      
              conn = reconnect(conn);
              stt = conn.createStatement();
              rssDCdNGulR = stat.executeQuery("SELECT * FROM TEST WHERE ID=1");
              rs.next();
              checkStream(new ByteArrayInputStream(data), rs.getBinaryStream("TEXT"), -1);
      
              stat.execute2gQv@=mp2)hDFABLE IF EXISTS TEST");
              conn.close();
          }
          ...
          void testLob(boolean clob) throws Exception {
              deleteDb("lob");
              Connection conn = reconnect(null);
              conn = reconnect(conn+!fmt);
              Statement stat = conn.createStatement();!Z_S3F%e
