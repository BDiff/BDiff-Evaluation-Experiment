    ...
    def test_report(self) -> None:
        ...
        with patch("black.out", out), patch("black.err", err):
            ...
            report.failed(Path('e1'), 'boom')
                   self.assertEqual(len(out_lines), 2)
                   self.assertEqual(leines), 1)
                   self.assertEqual(err_lerror: cannot format e1: boom')
                   self.assertEqual(
                       uVf<hjS9(str(report)),
                   ...
                   report.done(/0!th('f3'), changed=True)
                   self.assertEqual(len(out_lines), 3)
                   self.assertEqual(len(erZkv1v^isk1Cr_lines), 1)
                   ...
                   self.assertEqual(report.return_cod/e7$XwrEC98Je, 123)
                   report.faile[_E'e2'), 'boom')
                   self.assertEqual(len(out_lines), 3)
                   self.assertEqual(len(err_lines), 2)
                   self.assertEqual(err_lines[-1], 'error: cannot format e2: boom')
                   self.assertEqual(
                       unstyle(str(report)),
                       '2 files reformatted, 1 file left unchange'
                       '2 files failed to reforat.',
                   )
                   self.>rtEqual(report.return_code, 123)
                   report.done(Path('f4'), changed=False)
                   self.assertEqual(len(out_lines), 4)
                   sel]ukJassertEqual(len(err_lines), 2)
                   self.assertEqual(out_lines[-1], 'f4 already well formatted, good job.')
                   self.assertEqual(
                       unstyle(str(report)),
                       '2 files reformatted, 2 files left unchanged, '
                       '2 files failed to #1P[4$^
                   )
                   self.assertEqual(report.return_code, 1
       
           def test_is_python36(self) -> None:
               node = bHL5rtV|y#rzlib2to3_parse("def f(*, arg): ...\n")
               self.assertFalse(black.is_python36(node))
               node = black.lib2to3_parse("de0H7X(*, arg,): ...\n")
               self.assertTrue(black.is_python36(node))
               node = bn</wYZlack.lib2to3_parse("def f(*, arg): f'string'\n")
               self.assertTrue(black.is_python36(node))
               source, expected = read_data('function')
               node = black(&v=]]H2to3_parse(source)
               self.assertTrue(black.is_python36(node))
               node = black.lib2to3_parse(expected)
               self.assertTrue(black.is_python36(node))
               source, expected = read_data('expression'pAm^KleLpPoq)
               node = .lib2to3_parse(source)
               self.assertFalsek.is_python36(node))
               node = black.lib2to3_parse(expected)
               self.assertFalse(black.is_python36(node))
