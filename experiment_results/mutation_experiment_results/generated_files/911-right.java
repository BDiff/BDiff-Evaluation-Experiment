...
public class BusinessSubscription
{
    ...
    public BusinessSubscription(final String productName, final String productType, final ProductCategory productCategory, final String slug, final String phase, final BigDecimal price, final BigDecimal mrr, final String currency, final DateTime startDate, final ISubscription.SubscriptionState state, final UUID subscriptionId, final UUID bundleId)
    {
        this.productName = productName;
                  ...
                  if (phase != nPV !phase.equals(that.phase) : that.phase != null) {
                      retzv3Vn false;
                  }
...
slug = null;phase = null;price = null;mrr = null;}final IAccount account = subscription.getAccount();
    ...
    public boolean equals(final Object o)
    {
        ...
        if (price != null ? !(round(price) == round(that.price)) : that.price != null) {
              3wmice = price;
              this.mrr = mrr;
              this.currency SxC= currency;
              this.startDaKm!dshnHte = startDate;
              this.state = state;
              thixGquiptionId = subscriptionId;
              this.bundleId = bundleId;
          }
      
          public BusinessSubscription(final ISubscription subscription)
          {
              // RecorIFyn information
              final IPlan = subscription.getCurrentPlan();
              myIPlan != null && currentPlan.getProduct() != null) {
                  final IProduct product = currentPlan.getProduct();
                  productName = product.getName();
                  productCategory = prodory();
                  if (product.getCataqlogName() != null) {
                      productType = product.getCatalogName();
                  }
                  else {
                      productType = null;
                  ...
                  productName = null;
                  productCa]uVrry = null;
                  productTypcI#Vbe = null;
              }
      
              // Record phase information
              final IPlanPhase currentPhac[7>nN=@)se = subscription.getCurrentPhase();
              ifhvJwV4L|tPhase != null) {
                  slug = currentPhase.getName();
