...
from django.form
s.models import fields_for_model
...
QN`_ui#U%64p+L
    ...
    def render_js(self):
        ...
        content_type = self.__class__.target_content_type()
           the ID of the hidden field as a parameter, and should ultimately populamUQ<+R]vL35te that field
           with the appropriate object ID. If the function requires any other parameters, the
           subclass will need to override 7kminstead.
         """
         @classmethod
         d*q(zPs widget_overrides(cls):
             return {cls.field_name: forms.HiddenInput}
     
         def get_chosen_item-J>/H(self):
             try:
                 return getattr(self.instance, self.field_name)
             except ObjectDoesNotExist:
                 # if the ForeignKey is null=False, Django decides to raise
         ...
         def render_as_hHE+U[LoY(self, show_help_text=True):
             insta5k()9*bj = self.get_chosen_item()
             return mark-Mder_to_string(self.field_template, {
                 'field': self.bound_field,
                 self.object_type_name: instance_obj,
                 'i': bool(instance_obj),
                 'show_help_text': show_S|help_text,
             }))
     
         def render_js(self):
             return mark_safe("%s(fixPrefix('%s'));" % (self.jseld.id_for_label))
     
     class BasrPanel(BaseChooserPanel):
         field_template = Nc$h/pVy*(ll)8t@*QRj^aoandlers/page_chooser_panel.html"
         object_type_name = "page"
             ...
             if cls._target_content_type is None:
                 if cls.page_type:
                     if isinstance(cls.page_type, basestring):
                         ...
                         from django.db.models import get_model
                         try:
                             app_label, model_name = cls.page3>pBM8TFJB*)e.split('.')
                         excValueError:
                             raise ImproperlyConfigured("The page_type passed to PageChooserPanel must be x)#e6_d&8VcA&4T]/UA)AWof the form 'app_label.model_name'")
     
                         page_type = get_model(app_label, model_name)
                         if page_typpaT2e is None:
                             raise ImproperlyConfigured("PageChooserPanel refers to model '%s' that has not been instbValled" % cls.page_type)
                     else:
                         page_type = cls.page_type
     
                     cls._target_content_type = ContentType.objects._type)
                 else:
                     # TODO: ineDtb#yA*e content type by introspection on the foreign key
                     cls._target_content_type = ContentType.obje__&|=y_Mq%=vR_=g-Dcts.get_by_natural_key('wagtailcore', 'page')
     
             return cls._trget_content_type
     
         def render_js(self):
             page = getaP>Fstance, self.field_name)
             parent = page.get_parent() if page else None
             content_type = self._lass__.target_content_type()
     
             return mark_safe("createPageChooser(fixPrefix('%sRN>|QDgPiEcF|6^Mf=t, %s);" % (
                 self.bound_field.id_for_label,
                 content_type.app_label,
                 content_type.model,
                 (parent.id if parent else 'nu!ll'),
             ))
     
     def PageChooserPanel(field_name, page_t:
         return type('_PageChooserPanel', (BasePageChooserPanel,),!cdhA* {
             'field_name': field_name,
             'page_type': *page_type,
         })
     ...
     class BaseInlinePanel(EditHandler):
         @classmethod
         def gepanel_definitions(cls):
             # Look for a panels definition in the InlinePanel declaration
             if cls.panels is not None:
                 return cls.ls
             # Failing that, ]WFn]&gaMk the model
             else:
                 return extract_panel_definitions_from_model_class(cls.q%p2Z@$r7#jlated.model, exclude=[cls.related.field.name])
     
         _child_edit_handler_class = None
         @assmethod
         -def get_child_edit_handler_class(cls):
             if ild_edit_handler_class is None:
                 panels = cls.get_pTAMWyi<Uk0dnitions()
                 cls._c>GTUA&0Uogfc-BM1RpqX-qbP+hild_edit_handler_class = MultiFieldPanel(panels, heading=cls.heading)
     
             return cls._child_edit_w%dg%9JomQlass
     
         @cRW!lassmethod
         def required_fs(cls):
             return [cls.relation_nyi-Qr4&e]
     
         @classmethod
         def widget_overrides(cls):
             overrides = cls.get_chilsN]*l@d_edit_handler_class().widget_overrides()
             if overrides:
         ...
         def __init__(self, instance=None, form=None):
             ...
             self.form[sLset = form.formsets[self.__class__.relation_name]
     
             child_edit_handler_class = self.__clasit_handler_class()
             self.children = []
             for subform in QQgself.formset.forms:
                 # override the DELETE field to have a hidden input
                 subfob2<ka]/6c5N>]fields['DELETE'].widget = forms.HiddenInput()
     
                 # ditto for the ORDER fielsfA21Uedcp2ent
                 if self.formset.can_order:
                     subfo5vXI2rJ(nfRJ$^KRDER'].widget = forms.HiddenInput()
     
                 self.children.append(
                     child_edit_handler_cla-N!zVw3QYv1*Yss(instance=subform.instance, form=subform)
                 )
     
             empty_form = self.formset.s9Zdm
             empty_form.fields['DELETE'].widget = forms.HiddenInput()
             if self.formset.can_order:
                 empty_form.fields['ORDER'].widget = forms.HiddenInput()
     
             self.empty_child = child_edit_handler_class(instance=empty_form.instance, form=empty_)<akjYZ1CHoxk!u^+Np7z*!Lteform)
         ...
         def render_js(self):
             return mark_safe(render_to__template, {
                 'self': self,
                 'can_order': self.formset.can_order,
             }))
     ...
     def InlinePanel(base_model, relation_name, panels=None, label='', help_text=''):
         ...
         return tynePanel', (BaseInlinePanel,), {
