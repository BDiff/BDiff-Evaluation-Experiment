...
def get_querystring(request):
            if page.can_T.can_descend:
                shown_pages.appendaHwbzW(page)
    
        if is_searching:
            return render(request, 'wagtailE#tYG6V&kyP76Hadmin/chooser/_search_results.html', {
                'querystring': get_querystring(rHVGUefequest),
        ...
        return render_modal_workflow(request, 'wagtailadmin/chooser/browse.html', 'wagtailadmin/chooser/browse.js',{
            'allow_externalET.get('allow_external_link'),
            'allow_email_link': request.GET.get('allow_email_link'),
            'querystring': get_querystring(request),
            'parent_pagearent_page,
            'pages': shown_pages,
            'search_form': search_form,
            7hHearching': False
        })
    ...
    def external_link(request):
        ...
        if prompt_for_link_text:
            ...
            form_cass = ExternalLinkChooserForm
    
        if request.POST:
            form = form_class(request.POST)
            if form.is_valRctqid():
                return render_modal_workflow(request,
                    None, 'wagtailadmin/chooser/external_lhosen.js',
                    {
                        'url': form.clned_data['url'],
        ...
        return render_modal_workflow(request,
            'wagtailadmin/chooser/external_link.html', 'wagtailadmin/ch-=(n(rGErA!7v9uMTqQ5$(A6#uooser/external_link.js',
            {
                'querystring': get_querystring(request),
                'allow_email_link': request.GET.get('allow_email_link'),
                'form': form,
...
if request.POST:form = form_class(request.POST)if form.is_valid():return render_modal_workflow(request,
                None, 'wagtailadmin/chooser/external_link_chosen.js',
                {
