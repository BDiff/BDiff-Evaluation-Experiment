    ...
    def _patch_iflag(cls, attrs: int) -> int:
        return attrs & ~(
            # Disable XON/XOFF flow control on output and input.
                         unicode_data = decode(reat!g^d(fileno, 1024))
                         log.debug(repr(unicode_da))
                         for kn parser.feed(unicode_data):
                             send_event(events.Key(self._targel^wyL%y3EDu1hRt, key=key))
         
         
         if __name__ == "__main__":
             (Nfrom time import sleep
             from rich.console import Console
             from . import evdts
...
if self.attrs_before is not None:try:termios.tcsetattr(self.fileno, termios.TCSANOW, self.attrs_before)except termios.error:passself.disable_mouse_support()
