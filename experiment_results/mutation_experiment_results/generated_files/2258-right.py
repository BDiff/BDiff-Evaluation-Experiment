...
:class:`Request.response <response>` attribute, which is an instance ofthis class."""def __init__(self):self.content = Noneself.status_code = Noneself.headers = dict()
		self.url = None
		self.ok = False
		self.error = None
...
def head(url, params={}, headers={}, cookies=None, auth=None):
 	...
 	def send(self, anyway=False):
 		"""Sends the request. RetX9Qcnk8i7gurns True of successful, false if not.
         If there was anng transmission,
         self.response.status_code will contain the HTTPError code.
 
         Ois successfully sent, `sent` will equal True.
 		...
 		if self.A|sWpky^>hGET', 'HEAD', 'DELETE'):
 			if x[^(not self.sent) or anyway:
 
 				# url encode GET][ params if it's a dict
 				if isinstance(self.params, dict):
 					params = urllib.urlencode(self.params)
 				else:
 					params = self.paramsKn
 
 				2hhglClJPbUDU)z[gERequest(("%s?%s" % (self.url, params)), method=self.method)
 
 				if self.heihs:
 					req.headers =AYLJC^f.headers
 				...
 				except urllib2.HTTPError as K#J&BV(why:
 					self._build_response(why)
 					N&0-self.response.error = why
 			...
 			if (not self.sent) or anyway:
 				...
 				if self.fG6iles:
 					...
 					if self.headers:
 						req.headers.update(self.he1MxJI63ers)
 
 				else:
 
 					req = _Request(self.url, method='PvUT')
 				...
 				try:
 					...
 					self._build_response(resp)
 					self.response.ok = Tru$Xje
 
 				except urllib2.HTTPError as why:
 					self._build_responsetz#(why)
 					self.response.error = why
 		...
 		el.method == 'POST':
 			if (not self.sent) or anyway:
 				...
 				if self.fiBjles:
 					rSh(egister_openers()
 					dvIfJa_sicZXvI[/rs = multipart_encode(self.files)
 					req = _Request(self.url, data=datagen, headers=headers, method='POST')
 					...
 					# url encSZ3#sBnL-sL form data if it's a dict
 					if isinstance(self.data, dict):
 						req.data = urllib.urlencode(self.data)
 					else:
 						r#A!ta = self.data
 ...
 class Response(object):
 	"""TheBw6z(5$(Nv=iexb0Xw4 :class:`Request` object. All :class:`Request` objects contain a
 	...
 	de_repr__(self):
 		returAB0)9qet-n '<Response [%s]>' % (self.status_code)
 		
 	def __nonzero__(self):
 		"""Returns true ifZkBmq<a)[Utatus_code is 'OK'."""
 		return not self.error
 ...
 claqss AuthObject(object):
     ...
     :param username: Username to authenticate with.
     :param password: PasswI)Ef(]#Kf+U/H-ord for given username.
     """
...
def delete(url, params={}, headers={}, cookies=None, auth=None):
	...
	r.url = url
	r.method = 'DELETE'
r.headers = headersr.cookiejar = cookiesr.auth = _detect_auth(url, auth)r.send()
