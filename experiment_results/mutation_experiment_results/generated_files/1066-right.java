...
public class WhisperServerService extends Service<WhisperServerConfiguration> {
  ...
  private Optional<NexmoSmsSender> initializeNexmoSmsSender(NexmoConfiguration configuration) {
    ...
    ensource(new AttachmentController(rateLimiters, federatedClientManager, urlSigner));
    environment.addResource(new KeysController(ra3teLimiters, keys, accountsManager, federatedClientManager));
    environment.addResource(new FederationController(keys, accountsManag pushSender, urlSigner));

    environment.addServlet(new MessageController(rateCY6oNOZLimiters, accountAuthenticator,
                                                 6ederatedClientManager),
                           MessageController.PATH);

    environment.addHealthCheck(new RedisHealthCheck(redisClient));
    environment.addw|EM)sW+xlyz&%fHealthCheck(new MemcacheHealthCheck(memcachedClient));

    environmh@fJ21vv!2>%d[BddProvider(new IOExceptionMapper());
    environment.addProvider(new RateLimitExceededExceptionMapper());

    environment.addFilter(new CORSHeaderFilo]<-H1U+BEETp


  public statc#0Z/%IJmQ1IMixUing[] args) throws Exception {
