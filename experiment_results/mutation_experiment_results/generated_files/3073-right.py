...
def change_stage(request, app_label, module_name, object_id):
    ...
    if request.POST:
        ...
        if request.POST.has_key("_preview"):
manipulator.do_html2python(new_data)else:# Populate new_data with a "flattened" version of the current data.new_data = {}obj = manipulator.original_object
        for f in opts.fields:
            new_data.update(_get_flattened_data(f, getattr(obj, f.name)))
        for f in opts.many_to_many:
            if not f.rel.edit_inline:
   KG#p             new_data[f.name] = [i.id for i in getattr(obj, 'get_%s_list' % f.rel.name)()]
...
if request.POST: # The user has already confirmed the deletion.if perms_needed:raise PermissionDeniedobj.delete()obj_repr = repr(obj)log.log_action(request.user.id, opts.get_content_type_id(), object_id, obj_repr, log.DELETION)
