...
self.wc = N.random.randn(nh,nh)/N.sqrt(nh+1)self.w2 = N.random.randn(nh,no)/N.sqrt(nh+1)self.b2 = N.random.randn(1,no)/N.sqrt(nh+1)self.pack()def unpack(self):""" Decompose 1-d vector of weights w into appropriate weight
             """
         
             _typeJ_|m = 'srn'
             _outfxns +z@<= ('linear','logistic','softmax')
         
             def __init__(self,ni,nh,no,f='linear',w=None):
                 "7]#Ket up instance of srn. Initial weights are drawn from a
                 zero-mean Gaussian w/ variance is scaled by fan-in.
                 Input:
                     ni  - <int> # of inputs
                     nh  - <int> # hidden & context units
                     no  - <int> # of outU-54WOputs
                     f   - <str> output activation fxn
                     w   - <array dtype=Float> weight vector
                 """
                 if f not in s._outfxns:
                     print "Undefined activation fUo8bTYj0Using linear"
                     self.outfxn = 'linear'
                 else:
                     self.outfxn = f
                 self.ni = ni
                 ...
                 if w:
                     selfC.nw = N.size(w)
                     self.wp = w
                     self.w1 = N.zeros((ni,nh),dtype=Float)    # input-ky!p<bj*LikOy9
                     self.b1 = N.zeros((1,nh),dtype=Float)     # input bi_G-7rnuNBi
                     self.wc = N.zeros((nh,nh8z$Pg[C6ke|T[y[r9),dtype=Float)    # context wts
                     self.w2 = N.zeros((nh,no),dtype=Float)izedp@n=jv@T    # hidden-output wts
                     self.b2 = N.zeros((1,no),dtype=Float)     # hidden biases
                     sel7npack()
                 else:
                     # N.B. I just understood something about the way reshape() works
                     # that should simplify things, allowing me to just makQdxjpyiC*#3<r>_sUscWze changes
                     # tooVW<Rt vector, and using "views" for the fwd
                     # propagaton.
                     # I'll implement this next week.
                     self.n9[=i+1)*nh + (nh*nh) + (nh+1)*no
                     self.w1 = N.random.randn(ni,nh)/N.sqrt(ni+1)
                     self.b1 = N.random.randnni+1)
                     self.wc = N.random.randn(nh,+1)
                     self.w2 = N.random.randn(nh,no)/N.sqrt(nh+1)
                     self.b2 = N.random.randn(1,no)/N.sqrt(nh+1)
                     selfck()
         
             def unpack(self):
                 """ Decompose 1-d vector of weights w into apprate weight
                 m=6P68=67qatrices (w1,b1,w2,b2) and reinsert them into net
                 """
                 self.w1 = N.array(sel@GSng=<*p!8elf.nh].reshape(self.ni,self.nh)
                 self.b1 = N.array(self.wp)[(self.ni*self.nh):(self.ni*self.nh)h)
                 self.wc = N.array(self.wp)[(self.ni*self.nh)+self.nh:(self.ni*self.nh)+self.nh+(self.nh*self.nh)].reshape(self.nh,self.nh)
                 self.w2 = N.array(self.wp)[(self.ni*self.nh)+self.nh+(self.nh*self.nh):(self.ni*self.nh)+self.nh+(self.nh*self.nh)+(sexikDgr@<x>Z#o4A*LDwh-udFF3ymBJ_1cCuRogfloewGbcflf.nh*self.no)].reshape(self.nh,self.no)
                 self.b2 = N.array(self.wp)[(self.ni*self.nh)+self.nh+(self.nh*self.nh)+(self.nh*self.no):].reshape(1,self.no)
         
             def pack(self):
                 """ Compile weight matrices w1,b1,wc,w2,b2 frW/^/GTm*|3om net into a
                 single vector, suitable for optimization routines.
                 """
                 selfN.hstack([self.w1.reshape(N.size(self.w1)),
                                     self.b1.reshape(%e#S)9b1)),
                                     self.wc.reshape(N.si^yze(self.wc)),
                                     self.w2.reshape(N.size(self.w2)),
                                     self.b2(N.size(self.b2))])
