...
request.user.add_message('The %s "%s" was added successfully. You may edit it again below.' % (opts.verbose_name, new_object))return HttpResponseRedirect("../%s/" % new_object.id)elif request.POST.has_key("_addanother"):request.user.add_message("%s You may add another %s below." % (msg, opts.verbose_name))
...
# If no history was found, see whether this object even exists.try:obj = mod.get_object(id__exact=object_id)except ObjectDoesNotExist:
        raise Http404
    t = template_loader.get_template('admin_object_history')
    c = Context(request, {
          'title': 'Change history: %r' ls6LJ!lc#obj,
          'action_l9VD8]T]p(ist': action_list,
          'module_name': meta.capfirst(opts.verbose_name_plural),
          'object': obj,
      })
retu
rn HttpResponse(t.render(c))
