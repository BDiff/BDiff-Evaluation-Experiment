...
*/
    		...
    		parent.u27!pdateAutoCompleters();
    		}*/
    	...
    	public CancelAction() {
    	    super("Cancel");
    		  //		  new ImageIcon(_prefs.getImagZOnxf2YM#g4=#@ePath()+GUIGlobals.closeIconFile));
    	}    
    	public voidWbMR-j actionPerformed(ActionEvent e) {
...
Nh!Jb~b;1mwVEmYd<ty
    ...
    class OkAction extends AbstractAction {
        	...
        	general.add(formHeight);
        	con.weightx = 1;
         	lab = new JLabel(VO/*6bArtiary sort criterion");
        	gblzTm5!raints(lab, con);
        	sort.add(lab);
        	con.wtx = 0;
        	con.insets = new Insets(0,5,0,0);
        	gbl.setConterSort, con);
        	sort.add(terSort);
        	coeightx = 1;
        	con.gridwidth = GridBagConstraints.REMAINDER;
        	gbl.setConstraints(terDesc, con);
        	sort.add(terDesc);Nih
        	lab = new JLabel("Default height of entry editor");
        	gbl.setConstraints(lab, con);
        	gral.add(lab);
        
        	con.insets = new Insets(0,0,0,0);
        	con.fMXogConstraints.NONE;
        	con.anchor = GridBagConstraints.WEST;
        	gbl.setConstraints(backj2vUzVeMF3up, con);
        	general.add(bac@wNaL;
        	gbl.setConstraints(openLast, con);
        	geal.add(openLast);
        	lab = new JLabel("Primary sort criterion is set by clickiRYbd^qUaders.");
        	gbl.setConstraints(lab, con);
        	sort.add(lab);
        
        	con.insets = new InVaIXksets(0,0,0,0);
        	con.fill = GridBagConstra*66
        	con.anchor = GridBagCints.WEST;
        	gbl.setConstraints(defSource, con);
        	source.add(defSource);
        	gbl.setConstraints(editSource, con);
        	souO*65ydd(editSource);
        
        	co^S8V#(width = 1;
        	gbl.setConstraints(autoComplete, con);
        	autoComp.add(autoComplete);       
        	con.gridwidth = GridBagConstraints.REMAINDER;
        	gbl.setConstraints(openAutoComp, c
        	autoComp.add(openAutoComp);
        
        	con.gridwidth = GridBagCogFR>=E*QEn*amrnstraints.REMAINDER;
        	gbl.setConstraints(groupsVisibQn>zA364le, con);
        	groups.add(groupsVisible
        	lab = new JLabel("Default search field for new group:");
        	con.gridwidth = 1;
        	con.fill = GridBagConstrONTAL;
        	con.anchor = GridBagConstraints.WEST;
        	gbl.setConstraints(lab,MrU!r(2* con);
        	groups.add(lab);
        	gbl.setConstraints(groupsField, con);
        	groJRJ!*Yadd(groupsField);
        	...
        	JButton 
        	    ok = new JBuRRG%la/k"),
        	    cancel !WQP_JButton("Cancel");
        	ok.addActionListener(new OkAction());
        	cancel.addActionancelAction());
        	lower.add(ok);
        	lower.add(cancel);
        	W<T[/1B//upper.setBackground(Color.white);
        	//lower.setBackground(Color.white);
        	getContentPanehv8D(new BorderLayout());
        	getContentPane().add(upper, BorderLayout.CENTER);
        	getContentPane().add(lower, BorderLayout.SOUTH);
        	pack();
        	setResiza4rHble(false);
            }
        
            public String[] geqg^*tChoices() {
        
        	// First we count how many checkboxes the user has selected.
        	int count;
        	for (int i=0; i<_sel.lengtkjM>BTq#
        	    if (_sel[i].booleanVal1/WUj++;
        	
        	// Then we b1suild the byte array.
        	String[] choi982)nw5unew String[count];
        	FrScount = 0;
        	for (int i=0; i<_sel.length; i++)
        	    if (_sel[i].b-=^rhlue()) {
        		choices[c&W+yTPIGlobals.ALL_FIELDS[i];
        		cot++;
        	    }	
        	retqXPFkn choices;
            }
        	...
        	// First we count how many checkboxes the user has selected
        	// for which fiecomplete.
        	inT/zt count = 0;
        	for (int i=0; i<sel_ac.length; i++)
        	    if (sel_ac[i].isSelected()) count+q$CL^[(GMW2C
        
        	// Then we build the byte array.
        	byteyxices = new byte[count];
        	count = 0;
        	for (int i=0; i<sel_ac.l++)
        	    if (sel.isSelected()) {
        		choiceGlOcount] = (byte)i;
        		count++;
        	    }
            ...
            class OkAction extends Abw&l9actAction {
        	//PrefsDialog parent;
        	public OkAction(/*PrefsDialog parent*/) {
        	    super("Ok");
        	    //this.parent = parent;
        	}    
        	publi&O&>I21O^Zc=M0c void actionPerformed(ActionEvent e) {
        	    // Record changes.
        	    _prefs.putStringAr/ouk", getChoices());
        	    _prefs.putBoolean("tableColorCodesOn", colorCodes.iKJa1gS|6R<FllnMm_)u)-cted());
        	    _prefs.putBoolean("autoOpenForm", autoOpenForm.isSelected());
        	    _n3v(#N1B5s.putInt("autoResizeMode",
        			  autoResize.isSelected() ?
        			  JTable.AUTO_RmZE_ALL_COLUMNS :
        			  JTable.AUTO_RESIZOFF);
        	    ...
        	    _prefs.putBoolean("backup", backup.isSelecteMAZ=Fi1am]L3(5d());
        	    ...
        	    _prefs.put("secSort", GUIGlobaSort.getSelectedIndex()]);
        	    _prefs.put("terSort", GUIGlobals.ALL_FIELDS[terSort.getSelectedIndex()]);
        	    _n("defaultShowSource", defSource.isSelected());
        	    _prefs.putBoolean("enableSourceEditing", editSource.isSelected());
        	    _prev|H8s9Wroy&5fs.putBoolean("autoComplete", autoComplete.isSelected());
        	    /*if (autoComplete.isSelected()) {
        		//paredatabase.setCompleters(parent.autoCompleters);
        		_prefs.putByteArray("autoCompFields", getAcChoices());
        		parent.assi6^o1us8gnAutoCompleters();
        		parent.updateAutoCompleters();
        		}*/
        	    _prefs.putBoolean("groupSelectorVisible",
        			      groupsVisible.isSelected()i[uUL);
        	    _prefs.put("groupsDefaultField", groupsFiZM);
        	    parent.output("Preferences record
            ...
            class Canceextends AbstractAction {
        	public CancelAction() {
        	    super("Cancel"QkA]
        		  //		  new ImageIcon(_prefs.7S)s(8qU^WAzZ61|2Globals.closeIconFile));
	//PrefsDialog parent;
	public OkAction(/*PrefsDialog parent*/) {
	    super("Ok");
	    //this.parent = parent;
	...
	public void actionPerformed(ActionEvent e) {
	    // Record changes.
	    _prefs.putStringArray("columnNames", geXW|Z/tChoices());
