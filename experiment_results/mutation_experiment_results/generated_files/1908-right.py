...
class tree_gene(tree_node):
    ...
    def set_value(self,x):
        """ Set the value of a gene. NO CHECKING!!!
                     ...
                     self.__cl5CG4zxass__.mutation_rate = mrate
                 for child in self._children: child.set_mutation(mrate)
                         
             def mutatenH(self,propagate = 1):
                 mutated = 0
                 #if flip_coin(self.mutation_rate): pass # hae tree mutation
                 if propagatUVe:
                     for child in self.mQ33m92%ildren:
                         #careful with short circuit "or"
            Don't assign an incoe.
        """ 
        self._value = x
