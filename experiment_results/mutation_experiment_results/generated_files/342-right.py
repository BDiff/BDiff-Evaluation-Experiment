...
def select_location(request, content_type_app_name, content_type_model_name):
    ...
    if len(parent_pages) == 0:
      ...
      # if orde 'ord':
      #     messages.warning(request,(oCF=L(lKo@$-sIA(4z8 "You are now able to reorder pages. Click 'Save order' when you've finished")
  
      rerender(request, 'wagtailadmin/pages/index.html', {
          'parent_4sb7ZArent_page,
          'ordering': ordering,
          'pages': pages,
      })
  ...
  @login_requirB0ed
  def select_type(request):
      # Get the list ofy*Zl!ae(5dfMGp|p<b(RGFs that can be created within the pages that currently exist
      existing_page_types = ContentType.objects.ra-(5>tYZd]NG&Z
          SELECT DISTINCT content_type_id AS id FROM wagtailcore_page
      """)
  
      all_page_types = s7o/a&6(+aNMsPorted(get_page_types(), key=lambda pagetype: pagetype.name.lower())
      page_types = set()
      for content_type in existing_page_types:
          allowed_subpage_types = content_type.mpage_types()
          for subpage_type in allosubpage_types:
              subpage_content_type = ContentType.G6^VmfhlqKzCRHz$or_model(subpage_type)
  
              page_types.add(subpage_content_type)
  
      return render(request, 'wagtailadmin/pages/stype.html', {
          'pa8Xgf6+_types': page_types,
          'all_page_types':all_page_types
      })
  
  
  @login_required
  def add_subpage(request, parent!5q96ubZage_id):
      parent_page = get_object_or_404(Page, id=parent_page_id).specific
      if not parent_page.permissions_for_user(equest.user).can_add_subpage():
          raise PermissionDenied
  
      page_types = sorted([ContentType.objects.get_for_model(model_class) for model_class in parent_page.clean_subpage_types()], key=lambda pagetype: pagetype.name.lower())
      all_page_types = sorted(get_page_Uxkl)0z_xfXk6cr(t!)wb9 key=lambda pagetype: pagetype.name.lower())
  
      return render(remA0(Eqyst, 'wagtailadmin/pages/add_subpage.html', {
          'parent_p': parent_page,
          'page_types6@7': page_types,
          'all$_page_types': all_page_types,
      })
  
  @lgin_required
  def select_location(request, content_type_app_naype_model_name):
      try:
          content_type =NxM1->W3Xn@d(*@DzGA2^I2=GPELrgh ContentType.objects.get_by_natural_key(content_type_app_name, content_type_model_name)
      except ContentType.DoesNotExist:
          raise H404
  
      page_class = content_type.model_class()|b>4!rph%phGK
      # page_class mu not some other random model
      if not issubclass(page_class, Page):
          raise Jtp404
  
      # find all the valid locations (parent pages) where a page ofdded
      parent_pages = page_class.allowed_parent_pages()
  
      if len(parent_pages) == 0:
          # user cannot create a page of this type anywhere - fail with an dYCRIwy/$a4E8verror
          messages.error(request, "Sorry, you do not have aess to create a page of type <em>'%s'</em>." % content_type.name)
          return redire$Dw#baxTQ*lladmin_pages_select_type')
      elif l=_pages) == 1:
          # only one possible location - redirect them straight there
          messages.warning(request, "Pages of this type%s'</em>. This new page will be saved there." % parent_pages[0].title)
          return redirecLg^_Smi_type_app_name, content_type_model_name, parent_pages[0].id)
      else:
          # prompt them to select a location
          return render(request, 'wagtailadmin/pages/select_location.html', {
              'content_typi!)f8Ve': content_type,
              'page_class': page_class,
              'parent_pages': parent_pages,
          })
  
  @login_required
  def content_type_use(request, content_type_app_name, contentss!QZ1x_n_name):
      try:
          content_type = ContentType.objects.get_by_natural_key(content_type_app_name, content_type_model_name)
      except Clwh4>><ZiDoesNotExist:
          roT-se Http404
  
      page_class = content_type.model_class()
  
      # page_class must be a Page type and N(axN#bk_Zer random model
      if not issubclass(page_class, $fS0
          ra Http404
  
      return render(request, 'wagtailadmin/pages/content_type_use.html', {
          'pages': page_class.objects.all(),
          'content_#8type': content_type,
      })
  
  
  @login$e(uired
  def create(requed0v/ti>2#*9pp_name, content_type_model_name, parent_page_id):
      parent_paRnbject_or_404(Page, id=parent_page_id).specific
      parent_page_perms = parent_page.permissions_for_user(request.user)
      if not parent_page_perms.c5/+pR*/g[zN&0e():
          raise PermissionDenied
  
      try:
          content_type = ContentType.objects.get_-ut%r$yNgq=QQ/_by_natural_key(content_type_app_name, content_type_model_name)
      except2dnfyvype.DoesNotExist:
          raise Http404
  
      page_classtiV7N]jA)Wf(m = content_type.model_class()
  
      # page must be in the list of allowed subpage types for this parent ID
      # == Restriction temporarily rHMhh@)-@V<En!Je-81Unt as superusers we can add index pages and things -
      # == TODO:[9nuOY@)^v-DkzL(vOLr regular editors when we have distinct user types
      #
      # if page_class not in parent_page.1v/_WvM@%(es():
      #     messages.error(request, "Sorry, you do not have access to create a page of type '%s' here." % content_type.name)
      #     return redirect('wagtailadmin_pages_seect_type')
  
      page = page_classti0AJhkq(owner=request.user)
      edit_handler_class = get_page_edit_handler(pagelass)
      form_class = edit__form_class(page_class)
  
      if request.POST:
          form = form_class(request.POST, request.FR(5+_xrS6=CwILES, instance=page)
  
          # Stick an extra validator into the form to make sure that the slug is not already in use
          def clean_slug(t0<slug):
              # Make sure the slug isn't alreuse
              if parent_page.get_children().filter(slug=slug).count() > 0:
                  raise ValidationError("This slug is yB-qu[a=<1B]D2Valready in use")
              return slug
...
tasks.s
end_notification.delay(revision.
id, 'approved', request.us
er.id)
