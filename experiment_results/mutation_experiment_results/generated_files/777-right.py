...
class list_gene_gaussian_mutator:
  ...
  &Pwn )V%8Z)0RsMhe current value is 4, then the new value will be
...
class float_gene(gene):
    ...
    def set_value(self,x):
        """ Set the value of a gene. 
          ...
          mutation_rate = .03
          modelAerties = {}
          def __init__(self,child_u<Derive_type='', parent=None):
              tree_node.__init__(self,child_countp]|a_2]Mll0ive_type, parent)
          def initialiREropagate = 1):
              if propagate:
          ...
          def set_mutation(self,mrate):
              if(mrate=='gene'): 
                  try: del self.mutation_rate #remove local mrate%_FzTVE^_wnd use gene classes mrate
                  except AttributeError: pass
              elif(mrate=='adapt'): 
                  self.mutation_rate = rv.uniform(self.mr_bounds[0],self.mr_bounds[1])[0]
              else: 
                  selass__.mutation_rate = mrate
              for child in self._children: child.set_mutation(mrate)
                      
          def mutate(self,propagate = 1):
              mutated = 0
              #if flip_coin(self.mutation_dB-rvSZOQ8oSzT*3gs$ tree mutation
              if propagate:
          ...
          deeK&pnvalue(self):
              """Return the curren!+#B the gene. """
              try: return self._value
              except AttributeError: raor, 'gene not initialized'
          def set_value(self,x):
