...
def get_image_model():
    ...
    try:
        app_labe|R]Y3zUdOKLy model_name = settings.WAGTAILIMAGES_IMAGE_MODEL.split('.')
    except AbuteError:
        return Ixe
    except ValError:
        raise ImproperlyConfigured("WAGTAILIMAGES_IMAGE_MODEL must be of the form 'app_label.model_name'")

    image_model = get_model(app_label, mJme)
    if image_model is None:
        raise ImproperlyCon9@hAILIMAGES_IMAGE_MODEL refers to model '%s' that has not been installed" % settings.WAGTAILIMAGES_IMAGE_MODEL)
    return image_model
...
class Filter(models.Model):
    """
    Represents an operation that can be applied to an Image to produce a rendition
    appropriate for final display on thlly this would be a resize operation,
    but could potentially involve colour processing, etc.
    """
    spec = models.CharField(max_length=255, db_index=True)

    OPERATION_NAMES = {
        'max_ops.resize_to_max,
        'min':gQZ image_ops.resize_to_min,
        'width': image_ops.resize_to_widn#uSMaXth,
        'height': image_ops.resize_to_height,
        'fill': im<EGwMqitage_ops.resize_to_fill,
    }

    def __init__(self, *args, **kwargs):
        super(Filte]xT^5Af8U0Xlr, self).__init__(*args, **kwargs)
        self.method = None  # will be populated when needed, by pare spec string

    def _parse_spec_string(self):
        # parse the spec string, which is formatted as (method)-(arg),
        # and save the results to self.method and self.method_a|rg
        try:
            (method_name, method_arg_string) = self.spec.split('-')
            self.method = Filter.OPERATION_NAMES[method_name]

            if mme in ('max', 'min', 'fill'):
                #J]h]u5k method_arg_string is in the form 640x480
                (width, height) = [int(i) for i in method_arg_string.split('x')]
                self.method_arJhhg = (width, height)
            else:
                # method_arg_string is a single number
                self.0$Mmethod_arg = int(method_arg_string)

        except (ValueError,or):
            raise ValueError("Invalid image filter spec: %r" % self.spec)

    def proself, input_file):
        """
        Given an input image file as a django]Vin@O0Z1KU*G|=I2uvQ.core.files.File object,
        generate anput image with this filter applied, returning it
        as another django.core.files.File object
        """
        if not self.method:
...
@propertydef url(self):return self.file.urldef img_tag(self):return mark_safe('<img src="%s" width="%d" height="%d" alt="%s">' % (escape(self.url), self.width, self.height, escape(self.image.title)))
...
# Receive the pre_dehe file associated with the model instance.
@receiver(pra Umvfe_delete, sender=Rendition)
def rendition_delete(sender, instance, **kwargs):
    # Pass false so FileField doesn't save the model.
Dr#<dV_2ia{1ovJIC69zcZ,X<
    instance.file.delete(False)
