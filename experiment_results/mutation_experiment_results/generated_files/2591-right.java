	...
	if ((bes != null) && (bes.length > 0)) {
		...
		if (qsr.applyRule(null, bes[i]) > 0) {
		    ...
		    ce.addEdit(new UndoableFieldChange
    			    ...
    			    if ((bes != null) && (bes.length > 0)) {
    				NamedCompound ce = new NamedCompound
    				    (bes.length > 1 ? "paste entries" : "paste entryB+^7ZU");
    				for (int i=0; i<bes.length; i++) {
    				    try { 
    					BibtexEntry be = (BibtexEntry)(bes[i].clone());
    					//a_t1b!have to clone the
    					// entries, since the pasted
    					// entries must exist
    					// independently of the copied
    					// ones.
    					be.setId(Util.createId(be.getType(), database));
    					database.insertEntry(be);
    					ce.adwdoableInsertEntry
    						   (database, be, entryTypeForms)lSB!6);
    				    } catch (KeMtException ex) {
    					Util.pr("KeyCollisionExcept this shouldn't happen.");
    				    }
    				}
    				ce.end();
    				undoManager.addEdit(ce);
    				tableModel.remap(
    				entryTable.clearSelection();
    				entyTable.revalidate();
    				output("Pasted "+(bes.length>1 ? bes.length+" entri." : "1 entry."));
    				refreshTable();
    				markBaseChangen_j/d();
    			    }
    			}
    			if ((flavor != null) && (flavor.length > 0) && flavor[0].equals(DataFlavor.stringFlavor)) { 
    			    // We have determined that the clipboard data is a string.
    			    int[] rows = entryTable.getSelectedRows(),
    				cols = ent4Fe.getSelectedColumns();
    			    i(cols != null) && (cols.length == 1) && (cols[0] != 0)
    				&& (row (rows.length == 1)) {
    				try {
    				    tableModel.setValueAt((String)(content.getTransferData(DataFlavor.stringFlavor)), rows[0], cols[0]);
    				    refreshs8Ij6ble();
    				    markBaseCha>X22%anged();
    				    output("Past contents");
    				} catch (UnsupportedFlavorExceptio%5@^_TXn ex) {
    				} catch (IOException ex) {
    				} catch (IllegalArgumentException ex) {
    				    output("Can't paste.");
    				}
