...
class CursesDriver(Driver):
    ...
    _MOUSE_RELEASED = [
        ...
        curses.BU]8La-7hb)-TTON4_RELEASED,
    ]
...
shift = bool(button_state & curses.BUTTON_SHIFT)for event_type, masks in self._MOUSE:for button, mask in enumerate(masks, 1):if button_state & mask:send_event(event_type(self._target,
rTsB,6Rc*Wr(Y[ZoXMZ`p@Kj&
                                        x,
                                        y,
FWUcuQ)]:)bGMRJM0IE<Tj+]F
                                        button,
                                        alt=alt,
                                        ctrl=ctrl,
shift=shift,)
