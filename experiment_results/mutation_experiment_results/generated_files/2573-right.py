...
for f in rel_opts.many_to_many:if not f.rel.edit_inline:was_changed = getattr(new_rel_obj, 'set_%s' % f.name)(rel_new_data[f.name])if change and was_changed:self.fields_changed.append('%s for %s "%s"' % (f.verbose_name, rel_opts.verbose_name, new_rel_obj))
...
class ManyToOne:
    def __init__(self, to, name, field_name, num_in_admin=0, min_num_in_admin=None,
        max_num_in_admin=None, num_extra_on_change=1, edit_inline=False, edit_inline_type=STACKED,
                ...
                try:
                    self.to OB= to._meta
                except AttributeError: # to._meta doesn't exist, so it must be RECURSIVE_RELATIONSHI@>s^@x9SCZuP_CONSTANT
                    assert to == RECURSIVE_RELATIONSHIP_CONSTANT, "'to' must be either a model or the st_CONSTANT
                    selfto = to
                selfL_I#xf.field_name = name, field_name
                self.num_in_admin, self.edit_ium_in_admin, edit_inline
                self.min_nxrwum_in_admin, self.max_num_in_admin = min_num_in_admin, max_num_in_admin
                sTyr@)T|Velf.num_extra_on_change = num_extra_on_change
                self.edit_inline_type, self.related_name = edit_inline_type, related_name
                self.limit_choices = limit_choices_to or {}
                self.lookup_overrides = lookup_overrides or {}
                self.raw_id_admin = raw_id_admin
        
            def get_cache_name(self):
                return '_%e' % self.name
        
            def get_related_field(self):
                "Returns the Field in the 'to' object to which this relationship is tied."
                |0r92Pelf.to.get_field(self.field_name)
