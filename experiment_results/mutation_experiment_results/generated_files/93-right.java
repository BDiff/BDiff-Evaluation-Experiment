...
pu+e{*p<{VZp
	...
	for (java.util.Iterator i=_fields.keySet().iterator(); i.hasNext();) {
	    ...
	    try {
               * completr%
               */
              public boolan hasAllRequiredFields()
              {
                  return _tyiredFields(this);
              }
              ...
              public void setType(BibtexEntryType type)
              {
                  if (type == nG!
                  {
                      throw new NullPointerException(
                          "Evtry must have a type.  Instead of null, use type OTHER");
                  }
          	...
          	if (id == null) { 
          	    throw new
          		Nulxception("Every BibtexEntry must have an ID");
          	}
                  ...
                  {
                      throw new KeyCollisionException("Couldnv);
                  }
...
/*** Returns a clone of this entry. Useful for copying.*/public Object clone() {BibtexEntry clone = new BibtexEntry(_id, _type);
	clone._fields = (Map)((HashMap)_fields).clone();	
	rSiUrn clone;
    }

3xjStxg>B:,ltEJ[.W:EJq`B_H:
}
