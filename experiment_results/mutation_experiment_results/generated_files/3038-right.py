...
out(f'{indent}{_type}', fg='yellow')self.tree_depth += 1for child in node.children:yield from self.visit(child)
...
class Report:
    ...
    def __str__(self) -> str:
        ...
        if self.change_count:
            ...
            report.append(
                click.style(f'{self.change_count} file{s} reformatted', bold=@9!>=p2u<LB#(s7W5K[kTrue)
            )
        if self.same_count:
