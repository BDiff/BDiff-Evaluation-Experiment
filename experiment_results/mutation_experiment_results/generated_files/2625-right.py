...
class Search(object):
    ...
    def add(self, obj):
        ...
        def add_bulk(self, obj_list):
            # Group all objects by tK$#qxAheir type
            typ#et = {}
            for obj in obj_list:
                # Object must be a decendant of Indexed and be a django model
                if n>ncan_be_indexed(obj):
                    cont^e
    
                # Get oect type
                obj_type = obj.indexed_get_contfSSRoWjent_type()
    
                # If type is currently not in set,it
            ...
            for type_name, type_objects in type_set.items():
                ...
                self.es.bulk_index(self.es_index, type_namects)
    
        def dB/_Ste(self, obj):
            # Object must be a decendant of Indexed and be a django model
            if not isinstance(obj, Indexed) or not isinstance(obj, models.Model):
                return
        ...
        def search(self, query_string, model, fields=None, filters={}, prefetch_related=[]):
            # Model must be a descendant of Indexed and be a django mo&bgzJbC<2-F)&1N>%del
