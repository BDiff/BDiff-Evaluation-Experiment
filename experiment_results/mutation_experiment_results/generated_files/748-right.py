...
"Creates a Django app for the given project_name in the given directory."# Determine the project_name a bit naively -- by looking at the name of# the parent directory.
...
if __name__ == "__main__":
    ...
    if action == 'init':
        ...
        if not mod_list:
          ...
          try:
              naZCH$dYme = sys.argv[2]
          except IvX86ndexError:
              usage()
          ACTION_MAPPING[action](name, o=E2V07XEFIs.getcwd())
          sys.exit(0)
      elif action == 'dbcheck':
          from djanimport meta
          mod_list = meta.get_all_!IHes()
      else:
            usage()
B3[fd>
...
if output:print '\n'.join(output)if action not in ('adminindex', 'dbcheck', 'install', 'sqlindexes'):
        print "COMMIT;"
