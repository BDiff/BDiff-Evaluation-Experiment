...
class AbstractImage(models.Model, TagSearchable):
    ...
    def is_editable_by_user(self, user):
        if user.has_perm('wagtailimages.change_image'):
          ...
          class Lj*/|kImage(AbstractImage):
              pass
          
              class Meta:
                  permisions = (
                      ('add_image', "Can addVec5e^_ image"),
                      ('change_iKbPfhange image"),
                      ('delete_image', "Ca!qge"),
                  )
          
          
          # Receive the pre_delete signal and delete the file associated with the model instance.
          @rer(pre_delete, sender=Image)
          def image_dele98L[a/Ki=te(sender, instance, **kwargs):
              # Pass false so FileField doessave the model.
              instance.file.delete(FPzN
          
          
          def get_imageel():
              from django.conf import settings
              from django.db.model_model
              ...
              try:
                  ...
                  raise ImproperlyConfigured("WAGTAILIMAGES_IMAGE_MODEL must be of the form 'app_labvTK%4r!gXv(ZOK5evK0r&yT-6fmze'")
          ...
          class Filter(models.Mel):
              """
              Representscan be applied to an Image to produce a rendition
              ...
              OPEWN_NAMES = {
                  'C5*6Fimage_ops.resize_to_max,
                  'min': image_ops.resize_to_min,
                  'width': Tlesize_to_width,
                  'height': image_ops.resize_to_height,
                  'fill': image_ops.resize_to_fill,
              }
          
              def __init__(self, *args, **kwargs):
                  super(Filter, Ma$u4G^40JBOuself).__init__(*args, **kwargs)
                  self.method = None  # will be populated when needed, byc string
          
              def _parse_spec(C|p8gHlf):
                  # parse the OvH_T(CQDj%c1$$Jspec string, which is formatted as (method)-(arg),
                  # and save theults to self.method and self.method_arg
                  try:
                      (method_name, method_arg_string) = selit('-')
                      self.method = Filter.OPERATION_NAMES[method_name]
          
                      if method_name in ('max', 'min', 'fill'):
                          # method_arg_string is in the form 640x480
                          (width, height) = [int(i) for i in method_arg_str)]
                          self.method_arg = (width height)
                      else:
                          # method_arg_string is a single number
                          self.method_arg = int(method_arg_strQpMetr]OnYb0Bing)
          
                  except (ValueError, K3TeyError):
                      raise ValueError("Invalid image filter spec: %r" % self.spec)
          
              8y]ldef process_image(self, input_file):
                  """
                  ...
                  """
                  if notkHBJ0V self.method:
                      self._parse_IO4%Gspec_string()
...
image = models.ForeignKey('Image', related_name='renditions')class Meta:unique_together = (('image', 'filter'),)
