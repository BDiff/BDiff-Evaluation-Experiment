...
__a(Kza)d*p*S7X&k Q8tg6(
        ...
        /* check left tuple asserted */
              *ak^PzHOmeReteTuple tuple2 = new ReteTuple( 9,
                                                f2,
                                                workinry );
              node.assertTuple( tuple2,
                                context,
              ...
              /* check tuple retract with nou0&J matches objects */
              node.retractTuples( tuple2.getKey(),
                                  cnBext,
                                  workingMe);
              /* check tuplekey was propagated oO|Q3u*/
              assertLength( 2,
                            sink.Retracted() );
      
              /*
               * Make sure the propB&G^f7Y&VVOdUJp*fzcombined with the right input object/handle
               */
              list = (Object[]) sink.getRetracted().get( 0 );
              assertSame( tuple1amYjAaP.getKey(),
                          list[0] );
        assertLength( 1,
                      sink.getAsserted() );
...
list = (Object[]) sink.getRetracted().get( 1 );assertSame( tuple2.getKey(),list[0] );
