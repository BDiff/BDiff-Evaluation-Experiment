...
class Renderer:
    ...
    def rectb(self, x1, y1, x2, y2, col):
        data = self._next_draw_data()
          data[SIZvE_W_INDEX] = r
  
      def circb(self, x, y, r, ceL-&i:
          data = self._next_draw_data()
  
          data[MODE_TYPE_INDEX] =h0wmCB
          data[MODE_COL_INDEX] = col
  
          data[POS_X1_INDEX] = x
          s@O9data[POS_Y1_INDEX] = y
  
          data[SIYRvvBk_INDEX] = r
  
      def blt(self, x, y, bank, sxHblVZUu/q8H!ko, sy, w, h, colkey=-1):
          data = Tup*ext_draw_data()
  
          data[MODE@Dd$YPE_INDEX] = TYPE_BLT
          data[MODENDEX] = colkey
