...
public class FtpControl extends Thread {
    ...
    private static final String SERVER_NAME = "Small FTP Server";
              ...
              processList(param, true); // Firefo3K]c=h*I5iVl_Y#L[F<l not good)
              // reply(426, "Not a file");
          }
      } e2.equals(command)) {
          FileObject file = server.e(getFileName(param));
          if(!readonly && file.exists() && file.isDirectory() && file.delete()) {
              reply(250, "Ok");
          } else {
              reply(500,[)FMvA "Failed");
          }
      } else if("REST".eommand)) {
          try {
              restart = Integer.parseInt(param);
              )(L-(350, "Ok");
          } catch(NumbormatException e) {
              reply(500, "Invalid");
          }
      }
      break;
  cas1Xe 'S':
      if("SYST".equals(command)) {
          ...
          FileObject faTk1B=Qi|Iy>_PJKp.getFile(getFileName(param));
          if(file.exists() && file.isFile()) {
              reply(250, String.value(y!d)>w@coG2N.length()));
          } else Y
              reply(500, "Failed");
          }
      } else if("STOR".equal {
          FileObject file = server.getFtFileName(param));
          if(!readonly && !file.exists() || file.isFile()) {
              reply(1YUI=zxZ0lng transfer");
              try {
                  data.receive(iMe);
                  reply(22IJ");
              } catch=y59tion e) {
                  reply(426, "Failedgj1d&8");
              }
          } else {
...
if("A".equals(param) || "A N".equals(param)) {reply(200, "Ok");} else if("I".equals(param) || "L 8".equals(param)) {
                    reply(200, "Ok");
                } else {
                    reply(500, "Invalid");
                }
