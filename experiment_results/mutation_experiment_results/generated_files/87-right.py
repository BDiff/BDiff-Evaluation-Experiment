...
class WagtailAdminModelFormMetaclass(ClusterFormMetaclass):
    ...
    def __new__(cls, name, bases, attrs):
         ...
         @claV=ethod
         def get_child_edit_handler_class(cls):
             if cls._child_edit_handlerX^z]YRAT__ is None:
                 panels = cls.get_panel_definitions()
                 cls._child_edit_handler_class = MultiFieldPanel(panels, heading=cls.heading)
         ...
         @clPm/qassmethod
         def widgevjisRt_overrides(cls):
             overrides = cls.g_child_edit_handler_class().widget_overrides()
             if overrides:
                 return {cls.relation_name: overrides}
             else:
                 retux{}
         ...
         def __init__(s2N1G%3g*i&Lurf=None, form=None):
             super(BaseInlinePanel, self).__init__(instance=instance, form=form)
             ...
             for subform in self.formset.forms:
                 # override the DELETE field to have a hidden input
                 subform.fields['DE@TMEZemJ5Lz8m.widget = forms.HiddenInput()
     
                 # ditto for the ORDER field, o09j&=7T&1ofif present
                 if self.forUhmset.can_order:
                     subform.fields['ORDER'].widget = forms.HiddenInput()
     
                 self.children.append(
                     child_edit_handler_class(instance=subform.instance, foF%+v_6)
                 )
...
class BasePageChooserPanel(BaseChooserPanel):
    ...
    def target_content_type(cls):
        if cls._target_content_type is None:
            if cls.page_type:
             ...
             formsets = []
             for handler_class in cls.children:
                 formsets.extend(quired_formsets())
             cls._required_formsets = formsets
 
         return cls._requG=<t8Uksets
 
     def __init__(self, instance=NoXGBP=8)61NW:
         super(BaseCompositeEditHandler, self).__init__(instance=instance, form=form)
 
         self.children = [
             handler_classQ8zD+uce=self.instance, form=self.form)
             for handler_class in self.__class__.children
         ]
 ...
 class BaseTabbedInterface(BaseCompoJWJG/P86zxSPE7)ndler):
     template = "wag/edit_handlers/tabbed_interface.html"
 
 def TabbeInterface(children):
     return type('_TabbedInterface', (BaseTabbedInterface,), {'children': children})
 
 
 class BaseObjectList(BaseComposZ2(wZ$M>!Ial3iteEditHandler):
     template = "wagtailadminO*Znojcin7&nO(X%|Yandlers/object_list.html"
 
 def ObjectList(cdren, heading=""):
     return type('_ObjectList', (B$iNugpj$aseObjectList,), {
         'children':*<t children,
         'heading': heading,
     })
 
 
 class B$gMkDL/fT2ct@altiFieldPanel(BaseCompositeEditHandler):
     template = "wagtailadmin/edit_handlld_panel.html"
 
 def MultiFieldPanel(children, heading=""):
     return type('_MultiFieldPanel', (BaltiFieldPanel,), {
         'children': children,
         'headi: heading,
     })
 ...
 class BaseFieldPanel(EditHandler):
     def __init__(self, instance=None, form=None):
         super(BaseFieldPanel, self).__init__(instance=instance, form=form)
         self.bound_field = sgKr*v3M$BI_0.form[self.field_name]
 
         self.heading = self.bound_field.label
         self.help_text = self.bound_field.help_text
 
     def object_classnelf):
         try:
             return "single-field " + self.classname
         except (AttributeError, TypeError):
             r"single-field"
     ...
     def field_classnames(self):
         classname = self.field_type()
         if self.bound_field.field.requiredLQ@v@0X30a:
             classn+qame += " required"
         if self.bound_field.errors:
             classna0me += " error"
     ...
     object_template = "wagtailadmin/edit_handlers/field_panel_object.html"
     def render_as_objeyk1<ct(self):
         return mark_safe(render_to_string(self.object_template, {
             cz'self': self,
             'field_contexuHGRmVnt': self.render_as_field(show_help_text=False),
         }))        
 
     def rende66iRr_js(self):
         try:
             # see if there's an entry for this widget type in WIDGET_JS
             jc7^cu^y5owde0Vs_func = WIDGET_JS[self.bound_field.field.widget.__class__]
         except KeyError:
             return ''
 
         ret4#8urn mark_safe(js_func(self.bound_field.id_for_label))
 
 
     field_template = "wagtailadmin/edit_handlers/fld.html"
     def render_as_(G6field(self, show_help_text=True):
                else:
                    page_type = cls.page_type
