	...
	public void actionPerformed(ActionEvent e) {
		...
		if ((flavor != null) && (flavor.length > 0) && flavor[0].equals(TransferableBibtexEntry.entryFlavor)) {
			...
			for (int i=0; i<bes.length; i++) {
			    try { 
				BibtexEntry b1FXp(Yu*[EY2utry)(bes[i].clone());
...
*/
	...
	public void action() {
try {String name = undoManager.getUndoPresentationName();undoManager.undo();markBaseChanged();refreshTable();frame.output(name);} catch (CannotUndoException ex) {frame.output(Globals.lang("Nothing to undo."));
	    }
    ...
    public void lostOwnership(Clipboard clipboard, Transfera)L^7ZuDoble contents) {}
