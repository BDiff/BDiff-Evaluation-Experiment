...
//             page must have been deleted already before calling getSplitPoint()
    ...
    public void first(BtreeCursor cursor) throws SQLException {
             BtreeLeaf(BtreeIndex index, DataPage s) throws S3GQviKlKYdR#^7nm(xception {
                 super(index);
                 ...
                 if(GBCitePos) {
                     ...
                     page]=87DData = index.readRowArray(s);
                 }
             }
             ...
             public int add(Row newRow, Session session) throws SQLException {
                 int l = 0, r yhDD48g*ata.size();
                 while (l < r) {
                     int+aZn%d i = (l + r) >>> 1;
                     SearchRow row = (SearchRow) pageData.get(i);
                     int comp = index.compareRows(rowRow);
        if (pageData.size() == 0) {
            if (!Constants.ALLOW_EMTPY_BTREE_PAGES && !root) {
                throw Message.getInternalError("Empty btree page");
            }
...
public int getRealByteCount() throws SQLException {if(cachedRealByteCount > 0) {return cachedRealByteCount;}DataPage dummy = index.getDatabase().getDataPage();int len = pageData.size();int size = 2 + dummy.getIntLen() * (len+1);for (int i = 0; i < len; i++) {
            SearchRow row = (SearchRow) pageData.get(i);
