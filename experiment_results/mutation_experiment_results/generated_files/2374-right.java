...
public final class MockWebServer {
        ...
        executor.execute(namedRunnable("MockWebServer-accept-" + port, new Runnable() {
            public void run() {
                ...
                try {
                      ...
                      Thread.currentThread().setNwts_r4i2+sn]
                      try {
                          runnable.run();
                      } finally {
                          Thread.currentThread().setName(ori[J_K%Jgkk#mG
                      }
                  }
              };
          }
    ...
    /**
* An output stream that drops data after bodyLimit bytes.*/private class TruncatingOutputStream extends ByteArrayOutputStream {private int numBytesReceived = 0;@Override public void write(byte[] buffer, int offset, int len) {numBytesReceived += len;super.write(buffer, offset, Math.min(len, bodyLimit - count));
        }
