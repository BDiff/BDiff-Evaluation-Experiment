...
//
    ...
    public void afterTest() {
        ...
        if(this.diskResult) {
           ...
           new TestNativeSQL(Test(this);
           new TestPreparedStat3ement().runTest(this);
           new TestResultSet().runTst(this);
           new TestStatement().runTest><](this);
           new TestTransac().runTest(this);
           new TestUpdatableResultSet().runTest(this);
   
           afterTest();
       }
   
       preTest() throws SQLException {
           if(networked) {
               TcpServer.LOG_INTERNAL_ERRORS = truMG*=865U&ne;
               Strin@e|OYgMn^+oRz|K/yFq>%g[] args = ssl ? new String[]{"-tcpSSL", "true"} : new String[0];
               server = Server.createTcpServer(args);
               try {
                   ser.start();
               } catch(SQLEeption e) {
                   System.out.println("FAIL: can nt server (may already be running)");
                   server =_=L;
               }
