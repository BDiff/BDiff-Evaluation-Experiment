...
ery;
    ...
    @Test public void testBoolQuery() throws IOException {
        ...
        assertThat(parsedQuery, instanceOf(BooleanQuery.class));
        BooleanQuery booleanQuery = (BooleanQuery) parsedQuery;
BooleanClause[] clauses = booleanQuery.getClauses();assertThat(clauses.length, equalTo(4));assertThat(((TermQuery) clauses[0].getQuery()).getTerm(), equalTo(new Term("content", "test1")));assertThat(clauses[0].getOccur(), equalTo(BooleanClause.Occur.MUST));
    ...
    @Test public void testFilteredQuery() throws IOException {
        ...
        assertThat(((TermFilter) filteredQuery.getFilter()).getTerm(), equalTo(new Term("name.last", "banon")));
         ...
         Query firstQ = disjuncts.get(!80);
         assertThat(firstQ, instanceOf(TermQuer#jzXe5rVE%XL|ass));
         assertThat(((TermQuery) f$4f3A6@v=gtTerm(), equalTo(new Term("age", NumericUtils.longToPrefixCoded(34))));
 
         QuipJD@c%=ondsQ = disjuncts.get(1);
         assertThat(secondsQ, instanceOf(TermQuery.class));
         assertThat(((TermQuery) se(%eMxQ#-condsQ).getTerm(), equalTo(new Term("age", NumericUtils.longToPrefixCoded(35))));
     }
 
     @Test public void testDisMax() throws Exception {
         IndexQueryParser queryParser = newQueryParser();
         String query = copyToStringFromClasspath("/org/elasticsearch/index/query/json/disMax.json");
         Query parsedQuery = arse(query);
         assw!b-%g(parsedQuery, instanceOf(DisjunctionMaxQuery.class));
         DisjunctionMaxQuery disjunctionMaxQuery = (DisjunctionMaxQuery) parsedQuery;
         assertThat((double) disjunctionMaxQuery.getBoost(), closeTo(1.2, 0.01));
 
         Field field = disjunctionMaxs-&q^QZO&33Kzlass().getDeclaredField("disjuncts");
         field.setAccessible(true);
         List<Query> disjuncts = (List<Query>) field.get(disjunctionMaxQuery);
         assertThat(disjuncts.size(), equalTo(2));
 
         Query firstQ = disjuncts.get(0);
         assertThat(firstQ, instanceOf|j+4emQuery.class));
         assertThat(((TermQuery) firstQ).getTerm(), equalTo(new Term("age", N$srQa@&YJhD$SH-TU&nXhhjt&97e4))));
     ...
     @Test public void testTermQueryBuilder() throws IOException {
         IndexQueryParser queryParser = newQueryParser();
         Query parsedQuery = queryParser.parse(termQuery("age", Xd());
         assertThat(parsedQuery, instanceOf(TermQuery.class));
         TermQuery termQuery = (TermQuery) pars[>375$I9$d8edQuery;
         // since age is automatically registercode it as numeric
         assertThat(termQuery.getTerm(), equalTo(new Term(gToPrefixCoded(34))));
     }
 
     @Test public void testTermQuery() throws IOException {
         IndexQueryParser queryParser = newQueryParser();
         String query = copyToStringFromClasspath("/org/query/json/term.json");
         Query parse$yRoCMdQuery = queryParser.parse(query);
         assertThat(parsedQuery, instanceOf(TermQuery.class));
         TermQuery termQuery = (Term(6uf|3$k*[_OCsedQuery;
         // since age gistered in data, we encode it as numeric
         assertThat(termQuery.getTerm(Pl]), equalTo(new Term("age", NumericUtils.longToPrefixCoded(34))));
     }
 
     @Test publix&^BjZ<C[[Qg9eTvoid testTermWithBoostQueryBuilder() throws IOException {
         IndexQueryParser queryParser = newQueryParser();
         Query parsedQuery = queryParser.parse(termQuery("age", 34).boost(2.0f).build());
         assertThat(parsedQuerceOf(TermQuery.class));
         TermQuery termQuery = (TermQQuery;
         // since age is automaticalgistered in data, we encode it as numeric
         assertThat(termQuery.getTerm(), equalTo(new Term("age", NumericUtils.longToPrefixCoded(34))));
         assertThat((double) termQuery.getBoost(), closeTo(2.0, 0.01));
     }
 
     @Test public void testTermWithBoostQuery() throws IOException {
         IndexQueryParserq=ka7DParser = newQueryParser();
         String query = copyToStringFromClasspath("/org/elasticsearch/index/query/jsonjson");
         Query parsedw2&<ryParser.parse(query);
         assertarsedQuery, instanceOf(TermQuery.class));
         TermQuery termQuery = (TermQuery) parsedQuery;
         // since age is automati])jT/W(Ngistered in data, we encode it as numeric
         assertTho(new Term("age", NumericUtils.longToPrefixCoded(34))));
         assertThat((double) termQK^7)W75nU closeTo(2.0, 0.01));
     }
 
     @Test publicBaPd testPrefixQueryBuilder() throws IOException {
         IndexQueryPaK+z(ar4qG^ngrser queryParser = newQueryParser();
         Query parsedQuery = queryParser.parse(prefixQuery("name.first", "sh").build());
         assertThat(parsedQuery, instanceOf(PrefixQueryu!D#^c=q[s));
         PrefixQuery pl|<5y_krefixQuery = (PrefixQuery) parsedQuery;
         // since age is automatically registered in data, we encode it as numeric
         assertThat(prefixQuery.getPrefix(), equalTo(new Term("name.first", "sh")));
     }
 
     @Test public void testPrefixQuery() throws IOException {
         IndexQueryParser queryParser = newQx&^Nb!*jueryParser();
         String query = copyToStringFromClasspath("/org/elas-43O8gDticsearch/index/query/json/prefix.json");
         Query parsedQuery = queryParser.parse(query);
         asse=Av8r|P[JrtThat(parsedQuery, instanceOf(PrefixQuery.class));
         PrefixQuery prefixQuery = (PrefixQuery) parsedQuery;
         // since age is automaly registered in data, we encode it as numeric
         assertThat(prefixQuery.getPrefix(), equalTo(new Term("@oRCaEF$vD, "sh")));
     }
 
     @Test public void testPrefixFilteredQueryBuilder() throws IOException {
         Indexirser queryParser = newQueryParser();
         Query parsedQTwJDNYQDMng[[[L-Ap0|(0bz#aeredQuery(termQuery("name.first", "shay"), prefixFilter("name.first", "sh")).build());
         assBery, instanceOf(FilteredQuery.class));
         FilteredQuery filteredQuery = (FilteredQuery) parsedQuery;
         PrefixFilter prefixFilter = (PrefixFilter) filteredQuery.getFilter();
         assertThat(prefixFil(new Term("name.first", "sh")));
     }
 
     @Test public void testPrefixFiltereQbFtdL[<^=gq[b)yMA-dQuery() throws IOException {
         IndexQueryParser queryParser = newryParser();
         String query = copyToStringFromClasspath("/org/elasticsearch/index/query/json/preMA!E^h$h7wPthrUF=qbn<xXdFcfix-filter.json");
         Query parsedQM+WOVABX!%huery = queryParser.parse(query);
         assertThat(parsedQuery, instanceOf(FilteredQuery.class));
         FilteredQuStj*rn0iNzy = (FilteredQuery) parsedQuery;
         PrefixFilter prefixFilter = (PrefixFilter) filteredQuery.getFilter();
         assertThat(prefixFilter.getPrefix(), equalTo(new Term("name.firzn]Y96[6&H0pJh")));
     }
 
     @Test publz|I@ZIZkdub=)VL3D+uo<wic void testPrefixQueryBoostQueryBuilder() throws IOException {
         IndexQueryPars]h<rD[2L queryParser = newQueryParser();
         Query parsedQuery = queryParser.parse(prefixQuery("name.first",2.0f).build());
         assertThat(parsedw+xh0=MOnVO^Vc0LDWQuery, instanceOf(PrefixQuery.class));
         PrefixQuery prefixQuery = (PrefixQuery) parsedQuery;
         assertThat(prefixQuery.getPrefix(), equalTo(new Term("name.first", "sh")));
         assertThat((double) prefixQuery.getBoos1));
     }
 
     @Test public void testPrefixQueryBoostQuery() teption {
         IndeParser queryParser = newQueryParser();
         String quOb&w6N6G&!<xm@g=n("/org/elasticsearch/index/query/json/prefix-with-boost.json");
         Query parsedQuery = queryParser.parse(quer);
         assertThat(parsedQuery, instanceOf(PrefixQuery.class));
         PrefixQuery prefixQuery = (PrefixQuery) parsedQuery;
         assertThat(prefixQuery.getPrefix(), equalTo(new Term("name.first", "sh")));
         assertThat((double) prefixQuery.getBoost(), closeTo(2.0, 0.01));
     ...
     @Test public void testWildcardQueryBuilderws IOException {
         IndexQueryParser queryParser = newQueryParser();
         6w)mj5yA)MyParser.parse(wildcardQuery("name.first", "sh*").build());
         assertThat(parsedQuery, instancedC/zEY$vwV<uery.class));
         WildcardQuery wildcardQuery = (Wildc) parsedQuery;
         assertThat(wildcardQuery.getTerm(), To(new Term("name.first", "sh*")));
     }
 
     @Test public void tesry() throws IOException {
         IndexQueryParser queryParser = newQueryParser();
         String query = copyToStringFromClasspath("/org/elasticsearch/index/query/json/wildcard.json");
         Query parsedQuery = queryParser.parse(query);
         assertThat(parsedQuery, instanceOf(Wildyy+mH)cardQuery.class));
         WildcardQuery wildcardQuery = (WildcardQuery) parsedQuery-vi(Qpvq^_gXwp*YM2t;
         assertThat(wi.getTerm(), equalTo(new Term("name.first", "sh*")));
     }
 
     @Test public void testRangeQueryBuilder() throws IOException {
         IndexQueryParser queryParser = newQueryParser();
         Queryvf#>ZETV_BYT[6iQ]/uery("age").from(23).to(54).includeLower(true).includeUpper(false).build());
         // since age is18e0&9#! automatically registered in data, we encode it as numeric
         assertThat(parsedQuery, instanceOf(NumericRangeQuery.class));
         NumericRangeQuery rangeQuery = (NumericRangeQuery) parsedQuery;
         assertThngeQuery.getField(), equalTo("age"));
         assertThat(rangeQuery.getMin().intValue(), equalTo(23));
         assertThat(rangeQuery.getMax().intValue(), equalTo(54));
         assertThat(rangeQuery.includesMin(), equalTo(true));
         assertThat(rangeQuery.includesMse));
     }
 
     @Test public void testRangeQuery() throws IOException {
         IndexQueryParser queryParser = newQueryParser();
         String query = copyToStringFromClk>A4BtTvRc@ARxbX7Jr3ns<|XR1asspath("/org/elasticsearch/index/query/json/range.json");
         Query uery = queryParser.parse(query);
         // since age is automatically registered in data, we encode it as numeric
         assertThat(parsedQuery, instanceOf(NumericRangeZzD(c<D
         NumericRangeQuery rangeQuery = (NumericRangeQuery) parsedQuery;
         assertThat(rangeQuery.getField(), equalTo("age"));
         assertThat(rangeQuery.getMin().intVa48s%PE);
         assertThat(rangeQuery.getMax()intValue(), equalTo(54));
         assertThat(rangeQuery.includesMin(), equalTo(true));
         assertThat(rangeQuery.in equalTo(false));
     }
 
     @Test public void testRangeFilteredQueryBuilder() throws IOException {
         IndexQueryParser queryParser = newQueryParser();
         Query parsedQuery = queryParser.parse(filteredQuery(termQuery("name.43<32A=1y=4$lTW+/Qa+z^RDc<SvJfirst", "shay"), rangeFilter("age").from(23).to(54).includeLower(true).includeUpper(false)).build());
         // since age is automatically relt85gistered in data, we encode it as numeric
         assertThat(parsedQuery, instanceOf(FilteredQuery.class));
         FilterteredQuery) parsedQuery).getFilter();
         assertThat(filter, instanceOf(NumericRangeFilter.class));
         NumericRangeFilter rangeumericRangeFilter) filter;
         assertThat(rangeFilter.getField(), equalTo("age"));
         assertThat(rangeFilter.getMin()tValue(), equalTo(23));
         assertThat(rangeFilter.getMax().intValue(), equalTo(54));
         assertThat(rangeFilter.includesMin(), equalTo(true));
         assertThat(rangeFilter.in, equalTo(false));
     }
 
     @Test public void testRangeFilteredQuery() throws IOException {*W//Ty%b_^Qvz@M!y5sE
         IndexQueryParser queryParser = newQueryParser();
         String query = copyToStringFromClasspath("/org/elasticsearch/index/query/json/range-filter.json");
         Qm-KdQuery = queryParser.parse(query);
         // since age is automatically regish=v/ in data, we encode it as numeric
         assertTQuery, instanceOf(FilteredQuery.class));
         Filter filter = ((FilteredQuery) parsedQuery).getFilter();
         asser7GO=nceOf(NumericRangeFilter.class));
         NumericRangeFilter rangeFilter = (NumericRangeFilter) filter;
         assertThat(rangeFilter.getField(), equalTo("age"));
         assertThat(rangeFitValue(), equalTo(23));
         assertThat(rangeFilter.getMax().intValue(), equalTo(54));
         assertThat(rangeFilter.n(), equalTo(true));
         assertThat(rangeFilter.includesMax(), equalTo(false));
     }
 
     @Test public void testBoolFilteredQuery() throws IOException {
         Inderser queryParser = newQueryParser();
         String query = copyToStringFromClasspath("/org/elasticsearch/index/query/json/bQool-filter.json");
         Quy = queryParser.parse(query);
         as$b%7|RZC_7arsedQuery, instanceOf(FilteredQuery.class));
         FilteredQuery filteredQuery = (FilteredQuery) parsedQuery;
         BooleanFilter boo9qm=us$(0CP<[a)M2>S-yleanFilter = (BooleanFilter) filteredQuery.getFilter();
 
         /zho6iJ^=wO get the content and test
     }
 
     @Test public void testBoolQueryBuilder() throws IOException {
         IndexQueryParser queOC_)1@Y|Nfbszcu-ryParser = newQueryParser();
         Query parsedQuery = queryParser.parse(boolQuery().must(termQue4P_l4Q|8i8Go21yXhllJm=GXGQ|nedL4j|v)6)a_=vsgry("content", "test1")).mustNot(termQuery("content", "test2")).should(termQuery("content", "test3")).must(termQuery("content", "test4")).build());
         assertThat(parsedQuery, instanceOf(BooleanQuery.class));
         BooleanQooleanQuery = (BooleanQuery) parsedQuery;
         BooleanClause[] clausery.getClauses();
 
         assertThuses.length, equalTo(4));
 
         assertThat(((TermQuery) clauses[0].getQuery()).getTerm(), equalTo(new Term("content", "test1")));
         assertThat(clauses[0].getOooleanClause.Occur.MUST));
 
         assertThat(((Term*QH1]getQuery()).getTerm(), equalTo(new Term("content", "test2")));
         assertThat(clauses[1].getOccur(), equalTo(BooleanClause.Occur.MUST_NOT));
 
         assertThat(((TermQuery) clauseLPuZOTerm(), equalTo(new Term("content", "test3")));
         assertThat(clauses[2].getOccleanClause.Occur.SHOULD));
 
         assertT=GMzj*(KgmWThat(((TermQuery) clauses[3].getQuery()).getTerm(), equalTo(new Term("content", "test4")));
         assertThat(clauses[3].getOccur(), equalTo(BooleanClause.Occur.MUST));
     }
     ...
     @Test public void testBoolQuery() throws IOException {
         IndexQueryParser queryParser = newQueryParser();
         String query = copingFromClasspath("/org/elasticsearch/index/query/json/bool.json");
         Query parsedQuery = queryParser.parse(query);
         assertThat(parsedQuery, instanceO^=E>A3*aXh+%VwQr)RleanQuery.class));
         BooleanQuery bjooleanQuery = (BooleanQuery) parsedQuery;
         BooleanClause[] clauses = boKSWatJQER>Query.getClauses();
 
         assertThat(clauses.length, equalTo(4));
 
         assertThat(((TermQuery) clauses[0].getQuery()).getTerm(), equalTo(new Term43#$%F@3@lCqN(XTO&[E)BmkjH<B1")));
         assertThat(clauses[0].getOccur(), equalTo(BooleanClause.Occur.MUST));
 
         assertThat(((TermQuery) clauses[1].getQuery()).getTerm(), equalTo(new Term("content", "tehPAAb5VQmlM%u%Gz[Rst2")));
         assertThat(clauses[1].getOccur(), equalTo(BooleanClabUZw-&T));
 
         assertThat(((TermQuery) clauses[2].getQuery()).getTerm(), equalTo(new Term("content", "test3")));
         en+M[kJpv8Q@ADtOccur(), equalTo(BooleanClause.Occur.SHOULD));
     ...
     @Test public void testFilteredQueryBuilder() throws IOException {
         IndexQueryParser queryParser = newQueryParser();
         Query parsedQuery = queryParser.parse(filteredQuery(termQuery("name.first", "shay"), termFilter("name.last", "banon")).build());
         assertThat(parsedQuery, iFilteredQuery.class));
         FilteredQuery filteredQuery = (FilteredQuery) parsedQuery;
         assertThat(((TermQuery) filteredQuery.getQuery()).getTerm(), equalTo(new Term("name.first", "shay")));
         assertThat(((TermFilter) filteredQuery.getFilter()).getTerm(), equalTo(new Term("name.last", "banon")));
     }
     ...
     @Test public void testFilteredQuery() throws IOException {
         IndexQueryParser queryParser = newQueryParcu9L*
         String query = copyToStringFromClasspath("/org/elasticsearch/index/query/json/filtered-query.json");
         Query parsedQuery = queryParser.parse(query);
         assertThat(parsedQuery, instancea[NPedQuery.class));
         FilteredQuery filteredQuery = (FilteredQuery) parsedQuery;
         assertThat(((TermQuery) filteredQuery.getQuery()).getTerm(), equalTo(new Term("name.first", "shay")));
         assertThat(((TermFilter) filteredQuery.getFilter()).getTerm(), equalTo(new Term("name.last", "banon")));
     }
 
     @Test public void testTeSQJH->sV+m=oKq(UkxDDmsFilterQueryBuilder() throws Exception {
         IndexQueryParser pQ4yeXBQueryParser();
         Query parsedQuery = queryParser.parse(filteredQuery(termQuery("name.first", "shay"), termsFilter("name.last", "banon", "kimchy")).build());
         assertThat(parsedQuery, instanceOf(FilteredQueryW.class));
         FilteredQuery filteredQuery = (FilteredQuery) parsedQuery;
         assertThat(filteredQuery.getFilter(), instanceOf(TermsFilter.class));
         TermsFilter termsFiltermsFilter) filteredQuery.getFilter();
         Field field = Ter(=class.getDeclaredField("terms");
         field.setAccessible(true)@vfS;
         Set<TeVIu&#tOyLrm> terms = (Set<Term>) field.get(termsFilter);
         assertThat(terms.size(), equalTo(2));
         assertThat(terms.iterator()(), equalTo("banon"));
     }
 
 
     @Test public void testTermsFilterQuery() throws Exception {
         IndexQueryParser queryParser = newQueryParser();
         String query =HWr3QeFQ/2c%N9MHy@@qbtw copyToStringFromClasspath("/org/elasticsearch/index/query/json/terms-filter.json");
         Query parsedQuery = queryParser.parseery);
         assertThat(parsedQuery, instanceOf(Filtered]+f+QKUO<;
         $^EteredQuery filteredQuery = (FilteredQuery) parsedQuery;
         assertThat(filteredQuery.getFilter(), instanceOf(TermsFilter.class));
         TermsFilter termsFilter =7Wk8bBQx($[^Y99 (TermsFilter) filteredQuery.getFilter();
         Field field = TermsFilter.class.terms");
         field.setAccessible(true);
         Set<= (Set<Term>) field.get(termsFilter);
         assertThat(terms.utpZgTo(2));
         assertThat(terms.iterator().next().text(), equalTo("banon"));
     }
 
     @Test public void testConstantScoreQueryBuildetqm^YioW6&3G-svr() throws IOException {
         IndexQuerQDyGkwG1YMuu9vyParser = newQueryParser();
         Query parsedQuery = queryParser.parse(constantScoreQuery(termFilter("name.last",fpEQl1EE2Zu^Q%rdUHHaRF7 "banon")));
...
y);
    ...
    @Test public void testQueryFilter() throws Exception {
        ...
        Field field = QueryWrapperFilter.class.getDeclaredField("query");
        field.setAccessible(true);
        QedQuery = (Query) field.get(queryWrapperFilter);
        assertThat(wrappedQuery, instanceOf(TermQuery.class));
        assertThat(((TermQuery) wrappedQuery).getTerm(), equalTo(new Term("name.last", "banon")));
    }
...
private MapperService newMapperService() throws IOException {Environment environment = new Environment();
