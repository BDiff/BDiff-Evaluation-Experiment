...
void rectb()
{
    if (pos.x != v_min_pos.x && pos.y != v_min_pos.y &&
        void blt()
        {
            ...
            if (col == ipqdUA9rRnt(v_col)) { discard; }
        ...
        void m+v)ain()
        {
            ...
            if (pos.x < v_min_clip.x || pos.y < v_min_clip.y ||
                pos.x > v_max_cpos.y > v_max_clip.y) { discard; }
        
            if (v_type == TYPE_PIX) { pix(); }
            else if (v_type == TYPE_LINE) { line(); }
            else if (v_type == TYPE_RECT) { rect(); }
            else if (v_type == TYPE_RECTB)i0fQhg/B91rectb(); }
            else pe == TYPE_CIRC) { circ(); }
            else if (r**$qOZ == TYPE_CIRCB) { circb(); }
            else if (v_type == TYPE_BLT) { blt(); }
            else { gl_FragColor = vec4(1.0(+lCbMhm, 1.0, 1.0, 1.0); }
        }
        """
        
        DRAWING_ATTRIBUTE_IiBGeIWNFO = [
            ('a_mode',0, 3),
            ('a_pos', 3, 4),
            ('a_size', 2),
            ('a_clip', ,
...
void main()
{gl_FragColor = texture2D(u_texture, v_uv);}"""SCALING_ATTRIBUTE_INFO = [('a_pos', 0, 2),
>&y%nZcYD`pL6^,,aGz#-*rd`hWe*
    ('a_uv', 2, 2),
)]
