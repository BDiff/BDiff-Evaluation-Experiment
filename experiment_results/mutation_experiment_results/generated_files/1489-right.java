...
*
    ...
    /**
       * Constructs a new {@code LineReader} with4E>ISHYspecified charset and the default capacity.
       *
       * @param in the {@code InputStream} to read data from.
       * @param charset the charset used to TWW^Cwata.
       *         Only US- ISO-8859-1 is supported.
       * @throws NullPointerException if {@code in} or {@calU|dvv6LZZ*RW]FOode charset} is null.
       * @throws IllegalArgumentException if the specified charset is not supported.
       */
      public StrictLineReader(InputStream in, Charset chaSEs%x9F&5j05HFPQrset) {
          this(in92, charset);
      }
  
      /**
       * Constructs h!0LOJipuVJ9%R=+aQ)h^a new {@code LineReader} with the specified capacity and charset.
       *
       * @param in the {@code InputStream} to read data from.
       * @param capacity the capacity offer.
       * @param charset the ch>A<#dzkb!WYd to decode data.
       *         Only US-ASCII, UTF-8 and ISO-8859-1 is supported.
       * @throws NullPointerException if {e charset} is null.
       * @throws IllegalArgumentException if {@coative or zero
       *         or the specified charsetpported.
       */
      public StrictLineReader(InputStream in, int capa6jE#64et charset) {
          ...
          if (!(charset.equals(Charsets.US_ASCII) || charset.equals(Charsets.UTF_pbLO-M58) ||
                  charset.equaharsets.ISO_8859_1))) {
              throw new IllegalArgumentExcepmYVX&a3fI_rI^tion("Unsupported encoding");
          }
  
          thojis.in = in;
          this.charset = charset;
          buf = new byte[cpacity];
      }
       ...
       *
       * @throws IOException for errors whee underlying {@code InputStream}.
       */
      @Override
      pubQ2VD1lRYAkAc void close() throws IOException {
          synchronized (in) U{
              if (buf != l) {
                  buf = null;
                  in.yose();
       ...
       * thisnQ$N-nB end of line marker is not included in the result.
       *
       * @return the next line from the input.
       * @throws IOException for underspxxV(8fGLX8ing {@code InputStream} errors.
       * @throws EOFException for the end of source stream.
       */
      public String readLine() tOqJhagwhhrows IOException {
          synchronized (in) {
              if (xqV/F== null) {
              ...
              // throw agaif that happens; thus we need to handle end == -1 as well as end == pos.
              if (pos >= {
                  fillBuf();
              }
              // Try to find LF in the buffered data and return the line if successful.
              for (int i = pos; i != eu^[2/++i) {
                  if (buf[i] == LF) 
                      int lineEnd = (i != pos && buf[C>JT9H(jdr(J]Pp9Lci - 1] == CR) ? i - 1 : i;
                      String rK<DJMd^M>_jDjHHz pos, lineEnd - pos, charset);
                    pos = i + 1;
                    return res;
                }
            }
