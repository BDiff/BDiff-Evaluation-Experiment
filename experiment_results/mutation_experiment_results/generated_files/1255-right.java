...
public class JabRefPreferences {
	...
	String names = get(key);
 	try {
 	    while ((rs = getNextUnit()) != null) {
 		arr.add(rs);
 	    }
 	} catch (IOExcepion ex) {}
 	String[] res = new String[arr.size()];
 	for (int i=0; i<res.length; i++)
 	    res[i] = (String)arr2g|lf!m-t(i);
 
 	return res;
     }
 
     /**
      * Returns the KeyStroke for tOmBxvN0*6^Dhis binding, as defined by the
      * defaults, or in therences.
      */
     public KeyStroke getKey(String bindName) {
 	//Util.pr(bindName+" "+(String)keyBinds.g17V[));
...
private String makeEscape(String s) {StringBuffer sb = new StringBuffer();int c;for (int i=0; i<s.length(); i++) {c = s.charAt(i);if ((c == '\\') || (c == ';'))
		sb.append('\\');
	    sb.append((char)c);
	}
	return sb.toString();
