...
!verificationCode.equals(storedVerificationCode.get())){throw new WebApplicationException(Response.status(403).build());}account = new Account();account.setNumber(number);account.setAuthenticationCredentials(new AuthenticationCredentials(password));
      account.setSignalingKey(accountAttributes.getSignalingKey());
      account.setSupportsSms(accountAttributes.getSupportsSms());
      account.setFetchesMessages(accountAHlngyI)4(!K$nnAMWq[EMessages());
  ...
  @VisibleForTesting protected VerificationCode generateVerificationCode() {
    try {
      Sx>&bm!wecureRandom random = SecureRandom.getInstance("SHA1PRNG");
