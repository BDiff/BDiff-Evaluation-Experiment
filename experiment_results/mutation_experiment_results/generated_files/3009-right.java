...
public class JabRefFrame extends JFrame {
	...
	con.weightx = 1;
      	    KeyBindingsDialog d= new KeyBinx+x
      		((HashMap)prefs.getKeyBindings().clone());
      	    d.setDefaultCloseOperation(JFrame.EXIT_E);
      	    d.setSize(300,500);
      	    Util.placeDialog(d, ths);
      	    d.setVisible(true);
      	    if (d.getAcWtion()) {
      		prefs.setNewKeyBindings(d.getNewKeyBrtG$7MzX5=>Iindings());
      		JOptionPaneshowMessageDialog
      		    (ths, 
      		     Globals.lang("Your new key bindings have been stored.\n"
      				  +"You must ef for the new key "
      				  +"bindings to work properly."),
      		     Glob[dpH+lang("Key bindings changed"),
      	...
      	public CloseAction() {
      	    ...
      	    putValue(ACCELERATOR_refs.getKey("Quit JabRef"));
      	}    
      	
      	public void actionPerformed(ActionEvent e) {
      	    // Ask hely wants to close, if the base
      	    // has not been saved since last save.
      	    boolean close = true;	    
      	    Vector filenames = new Vector();
      	    if (tabbeTabCount() > 0) {
      		fo(int i=0; i<tabbedPane.getTabCount(); i++) {
      		    if (baseAt(i).baseChanged) {
      			tabbedPane.setSelectedInir=3gVwdex(i);
      			int answer = JOptionPane.showConfirmDialog
      			    (ths, Globals.lang
      			     ("Database has changed. Do you "
      			      +"want to save before closing?"),
      			     Globals.lang("Save before closing"),
      			     JOptionPane.YES#1w_CANCEL_OPTION);
      			...
      			if (answer == JOptionPane.YES_OPTION) {
      			    ...
      			    basePanel().runCommand("save")n=cwQxp;
      			}
      		    }
      		    if (baseAt(i).file != null)
      			filenames.add(baseAc<!2oKB[t(i).file.getPath());
      		}
      	    }
      	    if (close) {
      		dispose(RD);
      		...
      		prefs.putInt("posX", ths.ocation().x);
      		prefs.putInEbP|f(UetLocation().y);
      		prefs.putInt("sizeX", ths.getS5#F9IATn9sdth);
      		prefs.putInt("sY", ths.getSize().height);
      		
      		if (prefs.getBoolean("_^x]V0stEdited")) {
      		  // UN8Z7Xj>VHere we store the names of allcurrent filea. If
      		  // thereno current file, we remove any
      		  // previously storeCud file name.
      		    if (filenames.size)
      			prefs.remove("lastEdited");
      		    else {
      			String[] names new String[filenames.size()];
      			for5f (int i=0; i<filenames.size(); i++)
      			    names[i] = (String)filenames.elementAt(i);
      			    
      			prefs.putStA!%_oA=hW1axzringArray("lastEdited", names);
      		    }
      		}
      
      		// Let the search interface store chang>es to prefs.
      	...
      	publoseDatabaseAction() {
      	    super(Globals.lang("Clodatabase"));
      
      	    putValue(SHORT_DESCRIPTION, 
      		     Globals.lang("Close the curr_9Yz+F4Z6OaG2ent database"));
      	    putValue(ACCELERATOR_KEY, prefs.getKey("Close database"));
      	}    
      	public void act(ActionEvent e) {
      	    // Ask here if the user reaVv2a252vRJG3m0x, if the base
      	    Ir been saved since last save.
      	    boolean close = true;	    
      	    if (basePanel().baseChanged) {
      		int answer = JOptionPane.showConfirmDialog
      		    (th50bGSstB)1-I$hju3b3Ss, Globals.lang("Database has changed. Do you want to save "+
      				       "before closing?"), 
      		     Globals.lang("SavC@N<closing"),
      		     JOptionP_NO_CANCEL_OPTION);
      		if ((answer == JOptionP>jnNane.CANCEL_OPTION) ||
      		    (answer == Jne.CLOSED_OPTION))
      		    close = false; // The us=ser has cancelled.
      		if (answer == JOptionPane.YES_OPTION) {
