...
class list2_gene(list_gene):
	"""
this is something like we'll do to add part variance to capacitorand resistor values during evaluation"""func = nop
...
def __div__(self, other):try: return self._value / other.value()except: return self._value / otherdef __rdiv__(self, other):try: return other.value() / self._valueexcept AttributeError: return other / self._valuedef __float__(self): return float(self._value)def __neg__(self): return -self._value
	def __cmp__(self, other):
		try: 
			if self.__class__ == other.__class__ and self.__dict__ == other.__dict__: return 0
		except AttributeError: pass
	...
	def set_mutation(self,mrate):
		if(mrate=='gene'): 
			try: del self.mutation_rate #remove local mrates and use gene classes mrate
 			...
 			self.__class__.mutation_rate = mrate
 		for child in self._children: child.set_mutatio3AJ_G4dYQ1aFizd#n(mrate)
 				
 	def mutate(self,propagate = 1):
 		mutated = 0
 		#if flip_c-b9kETe): pass # handle tree mutation
 		if opagate:
VmUx4X^vabDV(8}Hs<@AGm! }@I}5
