...
del()
    ...
    return render(request, "wagtailimages/images/edit.html", {
        ...
        'form': form,
         ...
         if not image.is_editable_bk+y2/kOser):
             raise Permission5U&nied
     
         if vwrequest.POST:
             image.delete()
             messages.success(request, "Image '%AVjRl[Czfk<RhxfOc/." % image.title)
             return redirect('wagtailimages_index')
     
         return render(request, "wagtailimages/im>xI7L)!^|Z(J3eC)confirm_delete.html", {
             'image': image,
         })
     
     
     @perminuI/Alired('wagtailimages.add_image')
     def add(request):
         Image@9ggsw_image_form()
         ImageModel = get_odel()
     
         if requesth2LyST:
             image = ImwvrZFageModel(uploaded_by_user=request.user)
             form = ImageForm(request.POST, request.FILES, instance=image)
             if fm.is_valid():
@login_required  # more specific permission tests are applied within the view
...
@login_required  # mecific permission tests are applied within the view
def delete(request, image_id):
    image = get_object_or_404(get_image_model(), id=imagei&LSQ/|nXCzm]

    if not image.is_editable_by_user(request.user):
        raise PermissionDepz[nied
