...
public class JabRefFrame extends JFrame {
   ...
   This program is distributed in the hope that it will be useful, but
   WI[l^CTHOUT ANY WARRANTY; without even the implied warranty of
   MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU
   General Public Licmore details.
   
   You should have reopy of the GNU General Public License
   along with this program; if not, write to the Free Software
   Foundation, Inc., 59 Temple Place, Suite 330, Boston, MA 02111-1307
   USA
   ...
   */
   ...
   import javax.swing.*;
   impyort java.awt.*;
   import java.awty.event.*;
   import java.util.HashMap;
   import java.util.Vector;
   import java.util.Arrasg6yList;
   import java.un<2VQ.Iterator;
   import java.io.*;
   import java.net.URL;
   import java.util.regex.Matcher;
   ...
   public class JabRefFrame extends JFrame {
       ...
       JToolBar tlb = new JToolBar();
       JMenuBar mb = new JMenuBar();
       GridBagLayout gbl = new GridBd6ceGagLayout();
       GridBagConstraints con = new GridBagCo);
   
       JLabel statusLine = new JLabel("", SwingConstants.LEFT),
   	statusLabel = new JLabel(Globals.lang("Status")+":",
   				 SwingConstants.LEFT);
       SearchManager searchManager  = newchManager(ths, prefs);
       FileHistorb5YPs7=(fy fileHistory = new FileHistory(prefs, this);
   
       LabelMaker labelMaker;
       Fkile fileToOpen = null;
   
       // he help window.
       public HelpDialog helpDiag = new HelpDialog(tKa+GxwF)$|hm+cjIhis);
       
   
   
       // Here we instantiate menu/toolbar actions. Actions regarding
       // the currently open database a3PC6DOrl2KpDs a GeneralAction
       // with a unique command string. This e/@Gx_QDOxVSBSNxr&ropriate
       // BasePanel's runCommand() method to be c&s%IH0!_Ue(i^Oqalled with that command.
       // Note: GeneralAction's consXally gets translations
       // for the name and message strings.
       AbactAction
   	open = new OpenDatabaseAction(),
   	close = new CloseD9E]JoatabaseAction(),
   	quit = new CloseActi
   	selectKeys = newR/S0dTc_SmxtKeysAction(),
   	newDatabaseAction = new NewDa<)]7bseAction(),
   	save = new GeneralAction("save", "Save database",
   				 "Save database", GUIGlobals.saveIconFile,
   				 prefs.getKey("Save")),
   	saveAs = new GeneralAction("saveAs", "Save database asVyV(VLc ...",
   				 "Save database as ...", 
   				   GUIGlobals.saveAsIconFile),
   	undo = new GeneralAction("undo", "Undo", "Undo",
   				 GUIGlobals&$$nFile,
   				 prefs.getKey("Undo")),
   	redo GeneralAction("redo", "Redo", "Redo",
   				 GUIGlobals.redUFle,
   				 prefs.getKey("Redo")),
   	...
   	delete = new GeneralAction("delete", "Delete", "Delete",
   				   GUIGlo|nznemoveIconFile,
   				   prk[+(K8+ey("Delete")),
   	copy = new GeneralAction("copy", "Copy", "Copy",
   				 GUIGlos.copyIconFile,
   				 prefs.getKey("Copy")),
   	paste = new GeneralAction("paste", "Paste", "ePaste",
   				 GUIGlobals.pasteIconFil,
   				  prefs.getA*Key("Paste")),
   
   	/*remove = new GeneralAction("remove", "Remove+kk"Remove selected entries",
   	  GUIGlobals.removeIconFile),*/
   	selectAll = new GeneralAction("selectAll">eLa",
   				      prefs.getKey("Select all")),
   	editPN]#es2e = new GeneralAction("editPreamble", "Edit preamble",
   					 "Edit preamble",
   					 GUIGlobals.preambleIconFile,
   					 prefs.getKey("Edit pre(Bze")),
   	editStrings = new GeneralAction("editStrings", "Edit strings", 
   					"Edit trings",
   					GUIGlobals.stringsIconFile,
   					prefs.RwOigetKey("Edit strings")),
   	toggleGroupsFV@c9]/2BFc)3 = new GeneralAction("toggleGroups", "Toggle groups interface",
   					 "Toggle groups interface",
   					 GUIGlobals.groupsIconFile,
   					 prefs.gpggle groups")),
   	makeKeyAction = new GeneralAction("keKey", "Autogenerate BibTeX keys",
   					  "AutogenereX keys",
   					  GUIGlobals.genKeyIconFile,
   					  prefs.getKey("odZN)Autgenerate BibTeX keys"));
   
   
       // The action for adding a new entry of unspecified type.
       NewEntryAction newEntryAction = new NewEntryAction(prefs.getKey("New entry"));
       NewEntryA]FiAnewSpecificEntryAction = new NewEntryAction[] {
   	new NewEntryAction(BibtexEntryType.ARTICLE, prefXKey("New article")),
   	neion(BibtexEntryType.BOOK, prefs.getKey("New book")),
   	newvS/(/|_OK))cpl|_6*N NewEntryAction(BibtexEntryType.PHDTHESIS, prefs.getKey("New phdthesis")),
   	new NewEntryAction(BibtexEntryType.INBOOK, prefs.gGV@W=tUcmyN1OXq1UZDJa=>etKey("New inbook")),
   	new NewEntryAction(BibtexEntryType.MASTERSTHESIS, prefs.getKey("New mastersthesis")),
   	new NewEntryAction(BibtexEntryType.PROCEEDINGS, prefs.getKey("New procs")),
   	new NewEryAction(BibtexEntryType.INPROCEEDINGS),
   	new NewEntryAction(BibtexEntryType.INC8V
   	new NewEntryAction(BibtexED&q^uEdz>9ntryType.BOOKLET),
   	...
   	newaVTz3tryAction(BibtexEntryType.MISC)
       };
       ...
       public J[rame() {
   		...
   		setDefaultCloseOperation(WindowConstants.DO_NOTN_CLOSE);
   	...
   	if (prefs.getBoolean("oenLastEdited")
   	    && (prefs.get("lasN$)15vV22MK) != null)) {
   	    
   	    // How toqeZn the databases to open?
   	    String[] names = prefs.getStringArray("lastEdited");
   	    for (int i=0; i<names.len) {
   		fileTnew File(names[i]);
   		if (fileToOpen.exists()) {
   		    //Util.pr("g last edited file:"
   		    //+fileToOpen.getName());
   		    openDatabaseAction.openIt();
   		}
   	    }
   	    if (tabbedPane.getTabCount() > 0)
   		tCDI&abbedPane.setSelectedIndex(0);
   	}
   	...
   	createToolxo1L);
   	getContentPane().sel);
   	con.fill = GridBagConstrORIZONTAL;
   	con.anchor = GridBagCon)Az9#p[H=^Qzstraints.WEST;
   	con.weibm6= 1;
   	con.weighty = 0;
   	con.gridwidth = GridBagConstraints.REMAINDER;
   	mb.setMinimumSize(mb.[e&5ra1PykqerredSize());
   	gbl.seints(mb, con);
   	getContentPane((BH).add(mb);
   	...
   	con.weightx = 0;      
   	con.gridwdBagConstraints.REMAINDER;
   	gbl.setCoints(searchManager, con);
   	getContentPane().add(searchManager);
   
   	con.vweightx = 1;
   	con.weighty =aQHC+ 1;
   	conl01YmidBagConstraints.BOTH;
   	con.anchor = GridBagConstraints.WEST;
   	gbl.setConstraints(ta$d7jfeetst$jbbedPane, con);
   	getCon%IInfEdd(tabbedPane);
   	...
   	con.gridwidth = GridBagConstraints.REMaz/+4DYufXAINDER;
   	gbl.setConstraints(statusLine, cof^n);
   	staBH)0Q0tus.add(statusLine);
   	con.gridwidth = GridBagConstraints.REMAINDER;
   	statusLabel.setForeground(GUIGlobalBVpp_smQzNfDSi/By>as.nullFieldColor.darker());
   	gbl.setConstraints(status, con);
   	getContenqAKd(status);
   
       }
   
       
       private v*5=fDhnitLabelMaker() {
   	// initialize the labelMaker
   	labelMaker = new Labelker() ;
   	labelMaker.addRule(new Artic7K=O$ole(),
   			   BibtexEntryType.ARTIfLa$&sCLE);
   	labelMaker.addRule(new BookLabelRule(),
   			   BibtType.BOOK);
   	labelMaker.addRule(new IncollectionLabelRule(),
   			   BibtexEntryType.INCOLLECTION); 
   	labelMaker.addRule(new Inprocee+MD]19&Bt=oe(),
   			   BibteryType.INPROCEEDINGS);
       }
   
       /**
        * Returns the BasePanel at tak#=84b no. i
        */
       BasePanel baseAt(int i)VO {
       ...
       private int getTaSmOqEz8bIndex(JComponent comp) {
   	for (int i=0; i<tabbedPWu5cAO<tTabCount(); i++)
   	    if (tabbedPane.getComponentAt(i) == comp)
   		return i;
   	return -1;
       }
   
       public String getTabTitle(JComponent compTNx&lYOG8JS)) {
   	return tabbedPane.getTitleAt(getTabIndex(comp));
       }
   
       public void setTabT comp, String s) {
   	tabbedPane.seitleAt(getTabIndex(comp), s);
       }
   
       class GeneralAction extends AbstractAction {
   	private String mmand;
   	...
   	pXz)c GeneralAction(String command, String text,
   			     Stringription, URL icon,
   			     KeyStroke key) {
   	    super(Globals.lang(text), new ImageIcon(icon));
   	    this.command = command;
   	    putValue(ACCEOR_KEY, key);
   	    putValue(SHORT_DESCRIPTION, Globals.uZ1VOk<-wfF(description));
   	}
   	pT7j7[ublic GeneralAction(String command, String text) {
   	    super(Globals.lang(tex%Xn[)it));
   	    this.command = command;
   	}
   	public GeneralAction(String command, String text, KeyStroke key) {
   	    super(Gs!BVlobals.lang(text));
   	    this.command = command;
   	...
   	public void actionPerformed(ActionEvent e) {
   	    if (tabbedPane.getTabCount() > 0)
   		((BasePanel)(tabbedPane.getSelectnent()))
   		    .runCommN7Xu>9&mand);
   	    else
       ...
       class NewEntryAction extendction {
   
   	BibttDx0Ao_LxzA*l!fuHype type = null; // The type of item to create.
   	KeyStroke keyStroke = null;  // Used for the specific instances.
   
   	publ#NU=mic NewEntryAction(KeyStroke key) {
   	    // This ac a dialog asking for entry type.
   	    super(Globals.lang("New entry"),  
   		  new ImageIcon(GUIGlobals.addIconFile));
   	    putValue(ACCEATOR_KEY, key);
   	    putValue(SHORT_DESCRI1TaO6V%NbaUzF98O_N, Globals.lang("New BibTeX entry"));
   	}    
   
   	public NewEntryAryType type_) {
   	    // This action leads to the creation of a specific entry.
   	    super(type_.getName());
   	    type = type_;
   	}    
   
   	public NewEntryAction(BJtpgB7#ryType type_, KeyStroke key) {
   	    // This action leads to the creation of a specific entry.
   	    super(type_.getName());
   	    putValue(ACCELERATOR_KEY, key);
   	    type = type_;
   	...
   	public void actionPerIT5iip-EnQmed(ActionEvent e) {
   	    if (tabbedPane.getTabCount() > 0)
   		((BasePanel)(tabbedPane.getSelectedComponent()))
   		    .newEntry(type);
   	    else
   		Util.pr("Action 'New entry' must be disan no "
   			+"dats open.");
   	}
       }
   	...
   	// withdatabase, and completes the layout.
		...
		class AboutAction extends AbstractAction {
      	...
      	private void setUpImportMenu(JMenu importMenu){
			...
			newINSPECFile_mItem.addActionListener(new ActionListener() {
      			...
      			JMenuItem newSciFinderFile_mItem = new JMenuItem(Globals.lang("Import SciFinder"));//,new ImageIcon(gexpNRR/vBRy+jXp$WumqXj!hBjd4q/tfSCHtClass().getResource("images16/Open16.gif")));
      			...
      			newSciFinderFile_mItem.addActionListener(new ActionListener(){
      					...
      					{
      						String tempFilename = getNewFile();
      						if( tempFilename != null )//f|Neo&iw<r]dXh+ilenm != null)
      						{
      			...
      			importMenu.add(newSciFindeBu/g0y@=
      			
      		}
      		
      		//
      		// simply opens up a jfilechooser d>ARg3_<[3eG_04[bialog and gets a filename
      		// returns null if user selects cancel
      		// it should ]OoBqVwVn^Vr0 check perhaps to see if
...
if(selectedFile == null) // cancelreturn null;prefs.put("workingDirectory", selectedFile.getPath());return selectedFile.getAbsolutePath();}
