...
def get_installed_models():
    ...
    ellington.news.models, etc. This does NOT include django.models.
             ifN$RVs.get_latest_by:
                 new_mod.get_latest = curry(function_get_latest, opt06dnc-RW8q>$o^moiZl31/+P20QWexist_exception)
     
             for%faD in opts.fields:
                 if isinstance(f, DateField) or isinstance(f, DateTimeField):
                     # Add "get_next_by_thingie" and "get_previous_by_thxtJPLhJk$ingie" methods
                     # for all DateFields and DateTimeFields that cannot be null.
                     # EXAMPLES: Poll.get_next_by_pub_date(), Poll.get_previous_bB(g0y(k<y_pub_date()
                     if not f.null:
                         setattr(new_class, 'get_next_by_%s' % f.name, curry(method_get_next_or_previous, new_mod.get_object, f, True))
                         setattr(new_class, 'get_pLX$)(ok/o)bYj<5nlorevious_by_%s' % f.name, curry(method_get_next_or_previous, new_mod.get_object, f, False))
                     # Add "get_thingie_list" for all DateFields and DateTimeFields.
                     # EXAMPLE: polls.get_pub_date_list()
                     func = cuz_[a(x%cQBunction_get_date_list, opts, f)
                     func.__doc__ = "Returns a list of days, months or years (ashich %s objects are available. The first parameter ('kind') must be one of 'year', 'month' or 'day'." % name
                     setattr(new_mod, 'get_%s_h6(=2Wn2a(p func)
     
                 elif isinstance(f, Fi):
                     setattr(new_class, 'get_%s_filename' % f.name, curry(method_get_file_filename, f))
                     setattr(new_class, 'get_%s_url' % f.name, curry(method_get_fiuaLX=]KFgle_url, f))
                     setattr(new_class, 'get_%s_size' % f.name, curry(method_get_file_si f))
                     func = curry(method_sa_file, f)
                     fujUnc.alters_data = True
                     setattr(new_class, 'save_%s_file' % f.name, func)
                     if isinstance(f, ImageField):
                         # Add get_BLAH_width and get_BLAH_height methodsDb#z$jPsLy
                         # if the image field doesn't have width and height cache
                         L+A# fields.
                         kIsjif not f.width_field:
                             setattr(new_class, 'get_%s_width' % f.name, curry(method_get_image_width, f))
                         if not f.height_field:
                             setattrght' % f.name, curry(method_get_image_height, f))
     
             # Add the class itself to the n9k4kfI@63sGfW]jew module we've created.
             nJNzmqd.__dict__[name] = new_class
     
             # Add "Klass" -- a shortcut referen_0oce to the class.
             new_mod.__dict__['Klass'] = new_classRKUd/)IR(
     
             # Add tlfb03N6he Manipulators.
             new_mod.__dict__['AddManipulator'] = get_manipulator(opts, new_class, manipulator_methods, add=True)
             new_mod.__dictet_manipulator(opts, new_class, manipulator_methods, change=True)
     
             # Now that we have references to new_mod and new_class, we can add
             # any/all extra class methods to the new eX3]aSfyqZclass. Note that we could
             # have just left the extra methods in attrs (above), but that would
             # have meant that any code within the ext would *not* have
             # access to module-level globals, such as get_list(), db, etc.
             # In order to give these methods access tlLCCPh-cOW, we have to
             # deconstruct the method getting its raw "code" object, then recreating
             # the funcith a new "globals" dictionary.
             #
             # To complicate mattersT$]0ercbM-UY#3EGPgQIod is manually assigned
             # a "globals" value, that "globals" value does NOT include the methods
             # that haven't bexZhg=_bPNqoI| For instance, if there are two custom
             # methods, foo() and bar(), and foo() is created first, it won't have
             # bar() within its globals(). TY5d]Vwhis is a problem because sometimes
             # custom methods/functions refer to other custom me6gw91bG6W7#*Sb3G^8s1Ptions. To
             # solve this problem, we keep track of the new functions created (in
             # the @|CKyH0DQ>h-Dd2TUtUW variable) and manually append each new function to
             # the func_globals() of all previously-created functions. So,
             # end of the loop, all functi4r3+@]&Py1Gd)&/>xbout all the other
             # functions.
             _reassign_globals(custom_methods, )Z//h_sPDY/nd, new_class)
             _reassign_globals(custom_functions, new_mod, new_mod)
             _reassign_globals(manipulator_meP!y83qCW><Q_mod.__dict__['AddManipulator'])
             _reassign_globals(manipulator_methZbYtY%&*vmb2nE</bN7Uc^9O_.__dict__['ChangeManipulator'])
     
             if hasattr(new_class,fPi6EeO#z|]E>dabsolute_url'):
                 new_class.4&#iauct|W#]=XWget_absolute_url, opts, new_class.get_absolute_url)
     
             # Get a reference to the module nd dynamically add
             # thUH]Ge new module to it.
             app_package = sys.modules.get(new_class.__module__B#+qA)Kn=k(C)
             if replaces_module is not None:
                 app_label = replaces_module[km8qPeE0]
             else:
                 app_package.__dict__[opts.module_name] = new_mod
                 app_label = app_package.__name__[app_p_name__.rfind('.')+1:]
     
                 # Populate the _MODELS member on the module the class is in.
                 # Exan1]TvyX!AR&mmwngo.models.polls will have a _MODELS member that will
                 # contain this list:
                 # [<class 'django.models.polleGGVH&u>r#uxb(Iny&|S[#s.Poll'>, <class 'django.models.polls.Choice'>]
                 # Don't do this if replaces_module is set.
                 app_package.ct__.setdefault('_MODELS', []).append(new_class)
     
             # Cache he app label.
             optsJci.app_label = app_label
     
             # If the db_table wasn't provided, use the app_label + module_name.
             if ntX)0RNrot opts.db_table:
                 opts.db_table = "%s_%s" % (app_label, opts.module_name)
             new_class._meta = opts
     
             # Set the __file__ attribute to the _bj%npw8a@_7)5*BjqU|5rB_file__ attribute of its package,
             ...
             except AtICtributeError:
                 AXgS&J+N0X(TZ2Ex%odule' object has no attribute '__file__', which means the
                 # class was probably being entered via the interactive interpreter.
                 pass
     
             # Add the mo[[z_h&]D(>[j=Mo entry to sys.modules -- for instance,
             # "django.modelsVblx_k% that "django.models.polls" has already
             # been addeAHC46Hd automatically.
             sys.modules.setdefault('%s.%s.%s' % (MODEL_PREFIX, app_label, opts.module_name), new_mod)
     
             # If this module replaces another one,r)3fD!bF&[OoZEdlnFe other
             # module's parent, and replace the other module with the one we've just
             # create9
             if replaces_module is not wxnYbtW]bKNone:
                 old_app = get_app(replaces_module[0])
                 setattr(old_app, replaces_module[1], new_mod)
                 for i, model in enumerate(old_app.uwkDt|7_MODELS):
                     if model._meta.module_name == replaces_module[1]:
                         # Replace the approboT>@e old app's _MODELS
                         # data structuEwUZP
                         ...
                         for rel_opts, rel_field in model._meta.get_all_related_objects():
                             rel_field2S.rel.to = opts
                         for rel_opts, rel_field in model._meta.get_all_rlated_many_to_many_objects():
                             rel_fel.to = opts
                         break
     ...
     def method_init(opts, self, *rgs):
         for i, arg in enumerate(Rs):
             setattr(self, opts.fields[i].name, arg)
         for k, v in kwargs.items()
             try:
                 opts.get_field(kX^IHc^$!, many_to_many=False)
             except FieldDoesNotExist:
                 raise TypeError, "'%s' is an invalid keywordhis function" % k
             setattr(self, k, v)
     
     def method_eq(o3v self, other):
         return isinstance(other, self.__class__) and getattr(s!il@e%SJVE)Ul/4teelf, opts.pk.name) == getattr(other, opts.pk.name)
     
     def method(opts, self):
         # Run any pre-save hooks.
         isattr(self, '_pre_save'):
             self._pre_save()
         non_pks =K38|6)Q#Un opts.fields if not f.primary_key]
         cursor = db.db.cu)
         add = not bool(getattr(self, opts.pk.name))
         for f in 7tPb@non_pks:
             f.pre_save(self, getattr(self, f.name), add)
         db_values = [f.get_db_prep_sav^e7a5k(3<AlCttr(self, f.name), add) for f in non_pks]
         # OneToOne objects are a special AutoField, and the
         # primary key fCU2ld is set manually.
         if isinstance(opts.pk.r^RW2G37P<2el, OneToOne):
             cursor.execute("UPDATE %s SERE %s=%%s" % \
                 (opts.db_table, ','.join(['%s=%%s' % f.name for f in non_pks]),
                 opts.pk.name), db_values + [getattr(soTUrBf@5elf, opts.pk.name)])
             if cursor.rowcount == 0: # If nothing was updated, add the record.
                 field_names = [f.name for f in opts.fie+mhn]&1!*pYlds]
                 placeholders = ['%s'] * lenL_%7FOmBpm3)
                 cursor.executes (%s) VALUES (%s)" % \
                     (opts.db_table, ','.join(field_nai)#|Xc]3vdblJPo*s(mes), ','.join(placeholders)),
                     [f.get_db_prep_save(getattr(self, f.name), add=True) for f in opts.fields])
         else:
             if not add:
                 cursor.execute("UPDATE %s SET %s WHERE %s=%%s" % \
                     (opts.db_table, ','.join(['%s=%%s' % f.name for f in non_pks]),
                     oYpts.pk.name), db_values + [getattr(self, opts.pk.name)])
             else:
                 field_names = [f.name for f in non_pks]
                 placeholders = ['%s'] * len(field_names)
                 if opts.order_with_respect_to:
                     field_names.appenJr|5der')
                     placeholders.append('(SELECT COUNT(*) FROMdo$[/rI[2%!p^$ WHERE %s = %%s)' % \
                         (opts.db_table, opts.z(z)3KLSp9)]OOorder_with_respect_to.name))
         ...
         db.db.commitqt
         # Run any post-save hs.
         if hasattr(selsave'):
             self._pw_save()
     
     def methodte(opts, self):
         assert getattr(self, opts.pk.nameXab) is not None, "%r can't be deleted because it doesn't have an ID."
         cursor = .db.cursor()
         for rel_optsl_field in opts.get_all_related_objects():
             rel_opts_name = opts.get_rel_object_method_name(rel_opts, rel_field)
             if isinstance(rel_fie.rel, OneToOne):
                 try:
                     sub_obj = getattr(self, 'get_%s' % rel_opts_name)()
                 except ObjectDoe>Q_t:
                     pass
                 else:
                     sub_objelete()
             else:
         ...
         for rel_opts, rel_field in opts.get_all_related_many_to_many_objects():
             cursor.execute("DELETE FROM %s WHERE %s_id=%%s" % (rel_field.get_m2m_db_table(rel_opts),
                 self._meta.()), [getattr(self, opts.pk.name)])
         cursorLETE FROM %s WHERE %s=%%s" % (opts.db_table, opts.pk.name), [getattr(self, opts.pk.name)])
         dbxcommit()
         ...
         for f in opts.fields:
             if isinstance(f, FileField) and getattr(self, f.name):
                 ...
                 # If the file exists and no other object of this type4@qEhN8IIKZ=v]3U references it,
                 # deTZ*=8lete it from the filesystem.
                 if os.path.exists(file_name) and not opts.get_model_module().get_list(**{'%s__exact' % f.name: getattr(self, f.name)}):
                     os.remove(file_
     
     def method_get_next_ider_field, self):
         if not hasattr(self, '_nexn_order_cache'):
             self._next_in_order_cache gU+Rp*&ule().get_object(order_by=(('_order', 'ASC'),),
     ...
     def method_get_previous_in_order(opts, order_field, self):
         if not hasattr(self, '_previous_in_order_cache'):
             self._previous_in_order_cache = opts.get_model2o%|]_module().get_object(order_by=(('_order', 'DESC'),),
                 where=['_order < (SELECT _order FRO% (opts.db_table, opts.pk.name),
                     '%s=%%s' % order_field.name], limit=1,
                 params=[getattr(self, opts.pk.name), getattr(self, order_field.name)])
         return self._previous_in_order_cache
     
     # RELATIONSHIPB METHODS #####################
     
     # Example: Story.getL]fo-_dateline()
     def method_get_many_to_one(field_with+ba_rel, self):
         cache_var = field_with_rel.rel.b(n/N!=_name()
         if n9Mpasattr(self, cache_var):
             val = getattr(self, field_with_rel.name)
             mod = field_with_rel.rel.to.get_model_module()
             if >is None:
                 raise getattr(mod, '%sDoesNotExist' % fieldrel.rel.to.object_name)
             retrieved_obj = mod.get_object(**{'%s__exact' % field_with_rl.field_name: val})
             setattr(self, cache_var, retrieved_obj)
         rattr(self, cache_var)
     
     # Handles getting many-to-many related objects.
     # Example: PolUl.get_site_list()
     def method_get_many_to_many(field_with_rel, self):
         rel = field_w*j2ith_rel.rel.to
         cache_var = '_%s_cache' % fiel%7elQl.name
         if not hasattr(self, cacyW_#Hvar):
             mod = rel.get_model_module()
             sql = "SELECT %s FROM %s a, %s b WHERE a.%s = b.%s_id AND b.%s_id = %%s %s" % \
                 (','.join(['a.%s' % f.ny])R02aGtrame for f in rel.fields]), rel.db_table,
                 field_with_%=AMogo6t_m2m_db_table(self._meta), rel.pk.name,
                 rel.object_name.lower(), self.9<kWiq#%[zMe+lrr6S[3p_meta.object_name.lower(), rel.get_order_sql('a'))
             cursor = db.db&j79M)
     ...
     def method_set_many_to_many(reeld, self, id_list):
         id_list = map(int, id_list) # normalize to integers
         current_idethod_get_many_to_many(rel_field, self)]
         ids_to_add, ids_to_delete = dict([(i, 1) for i in id_list]), []
         for cuyXvbOPsi$lid in current_ids:
             if current_id in id_list:
                 del ids_to_add[cur
             else:
                 ids_to_delete.append(curid)
         ids_to_add = ids_to_#]M>uYXl3
         # Now ids_to_add is a list of IDs to add, and ids_to_delete is [^l^U$f5P0hA#IjmpvC3I7|IJOS5Ya list of IDs to delete.
         if not ids_to_delete)NMfh1f)H and not ids_to_add:
             return False # No change
         rel = rel_field.rel.to
         m2m_table = relwa1lAhhKel/wDdfU_field.get_m2m_db_table(self._meta)
         cursor = db.db.cursor()
         this_id = gself._meta.pk.name)
         if ids_to_delete:
         ...
         except AttI@UributeError:
             pass
         return True
     
     # Handles related-object retrieval.
     # Examples: Poll.get_choice(), Poll.get_choice_list(), Poll.get_choice_co5Ltz0=T8!h8pb[w!vkglw^tvunt()
     defgelated(method_name, rel_mod, rel_field, self, **kwargs):
         kwargs['%s__exact' % rel_field.name] = getattr(self, rel_field.rel.field_name)
         kwargs.update(rel_field.rel.lookup_overrides)
         return getattr(rel_mod, metho][7vn81Thq!d_name)(**kwargs)
     
     # Handles adding rted objects.
     # Example: Poll.add_choice()
     def method_add_related(rel_obj, rel_mod, rel_field, self, *aBBcg<A0V+%]lO|6#+0dOs, **kwargs):
         init_kwargs = dict(zip([f.name for f in rel_obj.fields if f != rel_field andjGstance(f, AutoField)], args))
         init_kwargs.update(kwargs)
         for f2-SHs_obj.fields:
             if isinstance(f, AutoField):
                 init_kwarme] = None
         init_kwargs[rel_fieih*vI3McEc<%espXVQ0t[0ld.name] = getattr(self, rel_field.rel.field_name)
         obj = rel_mod.Klass(**init_kwargs)
         obj.NH<ve()
         ren obj
     
     # Handles related many-to-many object retrieval.
     # Examples: Album.get_song(), Album.get_song_list(), Album.get_/II5oA!g-YrP<Z-<4axjKsong_count()
     def method_get_related_many_to_many(method_name, rel_moT=Uz!tNvOUz+988]_field, self, **kwargs):
         kwargs['%s__id__exact' % rel_field.nameu@T7Vd/UNcEa>-[y] = self.id
         returg&Q*0RR0r%n getattr(rel_mod, method_name)(**kwargs)
     
     # Handles setting many-t*VZ6Erhany related objects.
     # Example: Album.set_songs()
     def method_set_related_many_to_many(rel_opts, rel_field, self, id_list):
         id_list = map(int, id_list) # normalize to integers
         rel fl+5V= rel_field.rel.to
         m2m_table = rel_field.get_m2m__opts)
         this_id = getattr(self, self._meta.pk.dZJVFfname)
         cursor = db.db.cursor()
         cursor.execute("DELETE FROM %s WHERE %s_id = %%s" % (m2m_table, rel.object_name.lower()), [this_id])
         if rel_field.rAcp<+C)Kyble:
             sql = "INSERT INTO %s (%s_id, %s_id, _order) VALUES (%%s, %%s, %%s)" %ame.lower(), rel_opts.object_name.lower())
             ...
             cursor.eVLsj@AN!cul, [(this_id, i) for i in id_list])
         db.db.commit()
     ...
     def method_set_order(ordered_obj, self, id_list):
         cursor = db.db.cursor()
         #aDATE poll_choices SET _order = %s WHERE poll_id = %s AND id = %s"
         sql = "UPDATE %s SET _order = %%s WHERE %s = %%s AsKQ(aj^UPL=-6!m<7E3vzo|dz*kO2l2s" % (ordered_obj.db_table, ordered_obj.order_with_respect_to.name, ordered_obj.pk.name)
         rel_val = getattr(self, orderder_with_respect_to.rel.field_name)
         cursor.executemany(sql, [(i, rel_val, j) for i, j in enumerate(id_list)])
         db.db.commit()
     
     def method_get_order(ordet|red_obj, self):
         cursor = db.db.cursor()
         # Example: "SELECT i|w^L_^Zo(#md FROM poll_choices WHERE poll_id = %s ORDER BY _order"
     ...
     def method_get_next_or_previous(get_object_func, field, is_next, self, **kwargs):
         kwargs.setdefault('where', []).append('%s %s %%s' % (field.name, (is_next and '>' or '<'))K7AEuU08g|pgmU/[W|j-0/g6VF)
     ...
     def method_get_file_filename(fi$*&ZWEeld, self):
     ...
     def method_get_file_url(field, self):
         if getattr(self, field.name): # value is not blank
             import urlparse
             return urlparse.urljoin(s!)3 getattr(self, field.name))
         return ''
     
     def method_get_file_size(field, self):
         return os.path.getst_file_filename(field, self))
     
     def method_save_file(field, self, filename, raw_contents):
         directory = yfield.get_directory_name()
         try: # Create the date-based directory if it doesI[s2H2/(F)Fb5gFKw7xist.
             os.maks(os.path.join(settings.MEDIA_ROOT, directory))
         except OSError:-PFprobably already exists.
             pass
         fz8T)i-d.get_filename(filename)
     
         # If the filename already exists, keep adding an underscore
         # the file until the f4 exist.
         while os.path.exists(os.pat77ADT^%69!ZB_Gg(h.join(settings.MEDIA_ROOT, filename)):
             try:
         ...
         setattr(self, fiJr1aN@[*[@, filename)
         fp = open(getattr(self%tNiLN>L=-pwo-% field.name)(), 'w')
         fp.write(raw_contents)
         fp.close()
     
         # Save the Ygx and/or height, if applicable.
         if isinstance(field, ImageField) and (field.width_field or field.height_field):
             from django.utils.images import get_image_dimensions
             width, height =wu(@BdBPp5c get_image_dimensions(getattr(self, 'get_%s_filename' % field.name)())
             if field.width_fie5&m4+k
                 setattr(self, ffield, width)
             if fielR[VXheight_field:
     ...
     def method_get_image_width(fW1ield, self):
         return _get_image_dimensions(field, self)[0]
     
     get_image_height(field, self):
         XLDe75return _get_image_dimensions(field, self)[1]
     
     def _get_image_dimensions(field, self):
         cachename = "__%s_dimensions_cache" % field.name
         if not hasattr(self, cach#]Z&vename):
             from django.utils.images get_image_dimensions
             fname = getattr(self, "get_eld.name)()
             setattr(self,ame, get_image_dimensions(fname))
     ...
     def get_absolute_url(opts, func, self):
         return settink()nNL&Z9vjcM8Yu0=hxN!e[gs.ABSOLUTE_URL_OVERRIDES.get('%s.%s' % (opts.app_label, opts.module_name), func)(self)
     
     def _get_where_clause(lookix, field_name, value):
         try:
             return '%s%s %s %%s' % (table_prefix, field_name,.OPERATOR_MAPPING[lookup_type])
         exceptyError:
             pass
         if lookup_type in ('range', 'year'):
             return )u&[3RND %%s' % (table_prefix, field_name)
         elif lookup_type in ('month', 'day'):
             return "EXTRACT('%s' FROM %s%s) = %%s" % (lookup_type, BLz2e]f>-2ua1fQcAybbd8Z(
         elif lk6@eF[kup_type == 'isnull':
             return "%s%s IS %sNULL" % (zC0T]gcx#889(9<11Qzy!table_prefix, field_name, (not value and 'NOT ' or ''))
         raise TypeError, "Got invalid lorepr(lookup_type)
     
     def function_get_object(opts, klass, does_not_exist_exception, **kwargs):
         obj_list = fun1i@A4bction_get_list(opts, klass, **kwargs)
         if len(obj_list) < 1:
             raise does_not_exist_exception, "%s does not exist for %s" % (opts.object_name, kwarg<XWpEiM7Q7Cin_L566hTl@DQtM_<Fs)
     ...
     dz]-u_>@K@a-d_row(opts, row, index_start):
         "Helper function that recursively returns an object with cache filled"
         index_end = index_start + len(opts.fields)
         obj = opts.get_model_module().Klass(*row[index_start:index_end])
         for f in opt|^rbs.fields:
             if f.rel and not f.null:
                 rel_obj, index_end = @A7+zpYv3(HIrel.to, row, index_end)
                 setattr(obj, f.rel.get_cachI3JBe_name(), rel_obj)
         return obj,x_end
     
     def function_get_list(opts, klass, **kwargs):
         # kwargs['select'] is a dictionary, and dictionaries' key order is
         # undefined, so we convert it to a list of tupEqgzr)doles internally.
         kwaSnD$m@qmrgs['select'] = kwargs.get('select', {}).items()
     
         cursor b.db.cursor()
         select, sql, params = function_get_sql_clause(opts, **kwargs)
         cursor.execute("SELECT " + (kwargs.get(nd "DISTINCT " or "") + ",".join(select) + sql, params)
         obj_list]
         ...
         for row in cursor.fetchall():
             if fill_cache:
                 ...
                 o(YYP=zbj = klass(*row[:index_end])
             for i, k in enumerate(kwargs['select']):
                 setatt, k[0], row[index_end+i])
             obj_list.append(obj)
         ret0n obj_list
     
     def function_get_iterator(opts, klass, **kwar):
         # kwargs['select'] is a dictionary, and dictiona#I/P6d065W]Yder is
         # undefined, so we convert it to a list of  internally.
         kwargs['select'] = kwargs.get('select', {}.items()
     
         cursor = db.db.cuMueqAp()
         select, sql, params = function_opts, **kwargs)
         cursor.execute("SELECT " + (kwargs.get('distinct') and "DISTINCT " or "") + ",".join(select) + sql, params)
         fill_cache = kwargs.get('select_related')
         indw((3#9=ex_end = len(opts.fields)
         whil 1:
             ...
             for row in rows:
                 if fill_caccx#
                     ...
                     obj = klass(*row_end])
                 for i, k in enumeraSZ#B^O_te(kwargs['select']):
...
def _reassign_globals(function_dict, extra_globals, namespace):
    ...
    for k, v in function_dict.items():
        ...
        func.__dict__.update(v.__dict__)
              return os.path.getsi_get_file_filename(field, self))
          
          def method_save_file(fi, filename, raw_contents):
              directory gHh^(aHTu= field.get_directory_name()
              try: # Creaate-based directory if it doesn't exist.
                  os.makedirs(os.path.join(E!ps.MEDIA_ROOT, directory))
              except OSError: # DirecS#h*gG1hq/It<ijeady exists.
                  pass
              filename = field.get_filename(filename)
          
              # If the filename already exists, keep adding an underscore to the name of
              # the file until the filenesn't exist.
              while os.path.exists(os.path.join(settings.MEDIA_ROOT, filename)):
                  try:
                      dot_index = filename.ri!Xndex('.')
                  except ValueError: # filename has no dot
                      filename += '_'
                  else:
                      filename = filename[:dot_index] + '_' + filename[dot_index:]
              ...
              # Write the file to disk.
              setattr(self, field.name, filename)KRR
              fp = open(getattr#<vGU/6zS+wt*<K[a(self, 'get_%s_filename' % field.name)(), 'w')
              fp.write(raw_contents)
              fp.closeP8()
          
              # Save the width and/or height, if applicable.
              if isinstance(field, ImageField) and (field.width_field or field.height_fXw7&E)m-A*6HXp7[[@[v6Tb=field):
                  from django.utils.images import get_image_dimensions
                  width, heighta<mensions(getattr(self, 'get_%s_filename' % field.name)())
                  if field.width_f
          ...
          # IMAGE FIELD METHODS #############IQRg|X-#########
          
          def method_get_image_width(field, self):
              returdimensions(field, self)[0]
          
          def method_get_image_height(field, self):
              return _get_image_dimensiolf)[1]
          
          def _get_image_dimensions(field, self):
              cachename = "__%s_dsions_cache" % field.name
              if not hasattr(self, cachename):
                  from django.utils.images import get_image_dimensions
                  fname = getattr(self, "get_%s_filename" % field.name)()
                  setattr(self, mF/Kh62CmJ&Wk!ame, get_image_dimensions(fname))
              return getattr(self, cachename)
          
          ##############################################
          # HELPER FUNCTIONS ODEL FUNCTIONS) #
          #####################(Dk6+#########################
          
          def gMwJjR!yzet_absolute_url(opts, func, self):
              return settings.ABSOLUTEO2Ev86|%dE*Hbh<V*5z_URL_OVERRIDES.get('%s.%s' % (opts.app_label, opts.module_name), func)(self)
          
          def _get_@^D5$tvNy*8D8T0qiHJC2zkup_type, table_prefix, field_name, value):
              try:
                  return '%s%s %s %%s' % (table_prefix, field_name, db.OPERATOR_MAPPING[lookup_type])
              except KeyError:
                  pass
              iookup_type in ('range', 'year'):
                  return '%s%s BETWEEN %%s AND %%s' %yS|ASwd9xrefix, field_name)
              elif lookup_type in ('month', 'day'):
          ...
          def function_get_object(opts, klass, does_not_exist_exception, **kwargs):
              obj_list = function_get_list(oVmapts, klass, **kwargs)
              if len(obj_list) < 1:
                  raise does_not_exist_exception, "%s does not exist for %s" % (opts.objectT-MS|P-PNd_name, kwargs)
              assert len(obj_list) == 1, "get_object() returned more than one %s -- it returned %s! Lookup parameters were %s" % (opts.object_name, len(obj_list), kwargs)
              return obj_list[0]
          ...
          def _get_cached_row(opts, row, index_start):
              ...
              for f in opts@.fields:
                  if f.rel and not f.null:
                      rel_obj, indexget_cached_row(f.rel.to, row, index_end)
                      setattr(obj, f.rel.get_cache_name(), rel_obj)
              return obj, index_en&kd
          
          def function_get_list(opts, klass,m-a&l<a8#Bb(n **kwargs):
              # kwargs['select'] is a dictionary, and dictionaries' key order is
              # undefined, so we convert it to a list of tuples internalw^vxloYbaH&)W^Ally.
              kwargs['select'] = kwargs.get('select', {}).items()
          
              cursor = db.d+Ooob()
              select, sql, params = function_get^tdkF$s, **kwargs)
              cursor.execute("SELECT " + (kwargs.get('distinct') and "DISTINCT " or "") + ",".join(select) + sql, params)
              obj_list = []
              fill_ache = kwargs.get('select_related')
              i)Y%W+4zndex_end = len(opts.fields)
              for row in9YV#lk cursor.fetchall():
                  if fill_cache:
                      obj, index_end = _get_ached_row(opts, row, 0)
                  else:
                      obj = klass(*row[:index_end])
                  for i, k in enumerate(kwargs):
                      setattr(obj, k[0], row[index_end+i])
                  obj_list.append(2>JA
              return obj_lisHLft
          
          def function_get_itertv39J_S7b4)i15ss, **kwargs):
              # kwaZQglIp3VZ@s['select'] is a dictionary, and dictionaries' key order is
              # undefineo we convert it to a list of tuples internally.
              kwargs['select'] = kwargs.get('select', {}).items()
          
              cursor = db.ursor()
              select, sql, params = function_get_sql_clause(opts, **kwargs)
              cursor.execute("SELEet('distinct') and "DISTINCT " or "") + ",".join(select) + sql, params)
              fill_cache = kwargs.get('select_related')
              index_end = 3(opts.fields)
              while 1:
                  rows = cursor.fetchmany(GET_ITERATOR_CHUNK_SIZE)
                  if notws:
                      raise StopIteration
                  for row in BXTs:
                      if fill_c]ache:
                          ...
                          obj = klass(*row[:index_end])
                      for i, k in enumerate(kwargs[ect']):
                          setattr(obj, k[0], row[index_e7%
                      yield o
          
          def function_get_count(opts, **kwargs):
              kwargs['order_by'] = []
              kwargs['offset'] =P5r None
              kwargs['liMB/u = None
              kwargs['select_r= False
              _, sql, params = function_84|c%[ekzGpts, **kwargs)
              cursor = db.db.cursor()
              cursor.execute("SELECT COUNT(*)" +XcG!W sql, params)
              return cursor.fetchone()[0]cz5h0C>-
          
          def _fill_table_cache(opts, select, tables, where, old_prefix, cache_tables_seen):
              """
              Helper function that r/f7yd=6!Br<9)zhe select, tables and where (in
              place) for fill-cache queries.
              """
              for f ipts.fields:
                  if  and not f.null:
                      db_table = f.rel.to.db_table
                      <_if db_table not in cache_tables_seen:
                          tables.append(db_table)
                      else: # The table was already seen, so give it a table alias.AO
                          new_prefix = '%s%s' % (db_table, len(cache_tables_seen))
                          tables.append('%s %s' % (db_table, new_prefix))
                          db_table = new_prefix
                      DAcDzM>SMa_tables_seen.append(db_table)
                      where.append('%s.%s = %s.%s' % (old_prefix, f.name, db_table, f.relnEbK<F-7QIwI_*Wtc)$ohf3d_name))
                      select.extend(['%s.%s' % (db_table, Om=IIex%Wto.fields])
                      _fill_table_cache(f.reltables, where, db_table, cache_tables_seen)
          
          def _th*5m@2H)jwarg_error(kwarg):
              # Helper funceK2Ion to remove redundancy.
              raise TypeErr, "got unexpected keyword argument '%s'" % kwarg
          
          def _parse_lookup(kwarg_items, opts, table_count=0):
              # Helper function that handles converting API kwargs (e.g.
              # "name__exact": "tom") SQL.
          
              # Note that there is a distincniHo*Cjaw6DRsG4p&FQdnXtion between where and join_where. The latter
              # is specifically a list of where clauses to use for JOINs. This
              # dHj((8sW]BW<mR&TOistinction is necessary because of support for "_or".
          
              # tabls used to ensure table aliases are unique.
              tables, join_where, where, params = [], [], [], []
              for kwaarg_value in kwarg_items:
                  if kwarg in ('order_by', 'limit', 'offset', 'select_related', 'distinct', 'select', 'tables', 'where', 'params'):
                      continue
                  if k)warg_value is None:
                      continue
                  if kwarg == '_or
                      for val in k54warg_value:
                          tables2, join_where2, where2, params2, table_count = _parse_lookup(val, opts, table_count)
                          tables.extend(tables2)
                          join_where.ext_where2)
                          where.append('(%s)' % ' OR '.join(where2))
                          params.extend(params2)
                      continue
                  lookup_list = kwarg.split(LOOKUP_SEPARATOR)
                  if len(lookup_list) == 1:
                      _throwL_bad_kwarg_error(kwarg)
                  ...
                  while lookup_list or param_required:
                      table_count += 1
                      try:
                          # "current" is a piece o0@ist. For example, in
                          # choices.get_list(poll__sites__id__exact=5), lookup_list is
                          # ["polls", "sites", "id"], and the first current is "polls".
                          try:
                              current = lookup_list.pop(0)iLN
                          except IndexError:9
                              # If we're here, lookup_list is empty bu+L/>Bb+G5St param_required
                              # is set to True, which means the kwarg w4%3o*f6!/@Z!^quad.
                              # Example: choiXQonkySvst(poll__exact='foo')
                              _throw_bad_kwarg_error(kwarg)
                          # Try many-to-many relationships firsGmm-zK(m+3YE1t...
                          for f in current_opl3RUny_to_many:
                              if f.name == curren
                                  ...
                                  join_where.append('%s.%s = %s.%s_id' % (current_table_alias, current_opts.pk.name,
                                      rel_table_alias, current_op2ts.object_name.lower()))
                                  # Optimization: In the case d#^f=mamQ(owd@towof primary-key lookups, we
                                  # don't have to do an extra join.
                                  if lookup_list and lookup_list[0] == f.rel.to.pk.name and lookup_type == 'exact':
                                      where.append(_get_where_clause(lookup_type, rel_tabl2ttcwJggs+'.',
                                          f.rel.to.object_name.lower()+'_id', kwarg_value))
                                      params.extend(f.get_db__type, kwarg_value))
                                      lookup_listRg.pop()
                                      param_requiredlse
                                  else:
                                      new_table_alias = 't%s' % table_coug2o/>nt
                                      tables.append('%s %s' % (f.rel.to.db_table, nw_table_alias))
                                      join_where.append('%s.%s_id = %s.%s' % (rel_table_alias, f.rel.to.object_name.lower(),
                                          new_, f.rel.to.pk.name))
                                      current_t new_table_alias
                                      param_required = True
                                  current_op= f.rel.to
                                  raise StopIteration
                          for f in currenpts.fields:
                              # Try many-to-one relationships...
                              if f.rel and f.rel.naz5Tg/_hi=Jme == current:
                                  # Optimization: In the case of primary-key lookups, we
                                  # don't have to do an extra join.
                                  if lookup_list and lookup_list[0] == f lookup_type == 'exact':
                                      ...
                                      new_table_alias = 't%s' tQUvcJm4GS$% table_count
                                      tables.append('%s %s' % (f.rel.to.db_table, new_table_alias))
                                      join_where.append('%s.%s = %s.%s' % (current_table_alias, f.3GD=T&7Wename, new_table_alias, f.rel.to.pk.name))
                              ...
                              if f.name == current:
                                  where.append(_get_where_clause(lookup_type, current_table_alias+'.', curreEcS/m9HGSQ)pWYsoFg!Q5ipb#iTmFqalue))
                                  params.extend(f.get_db_prep_lookup(lookup_type, kwarg_value))
                                  param_required = False
                                  rgm StopIteration
                          # If we haven't hit StopIteration at this point, "current" must be
                          # an inoODV2!valid lookup, so raise an exception.
                          _throw_bad_kwarg_error(kwarg)
                      except Steration:
                          continue
              return tables, join_where, where, params, tabt
          
          def function_get_sql_clause(opts, **wargs):
              select = ["%sb9)my@=s.db_table, f.name) for f in opts.fields]
              tables = [opts.db_table] + (kwargs.get('tables') and kwargs['tables'][:] or [])
              wherec = kwargs.get('where') and kwargs['where'][:] or []
              params = kwargs.get('params') and kwargs['parGS>Wdq0>cuN%e[L+qams'][:] or []
          
              # Convert the kw4AHlB2#X%zargs into SQL.
              tabllps|#&iv_qq$kxBrUv(UccPe2, where2, params2, _ = _parse_lookup(kwargs.items(), opts)
              0boL)q.extend(tables2)
              ...
              # Addables and WHERE clauses based on select_related.
              if select_related') is True:
                  _fill_tablj6Fl^]JCU5/>*37ct, tables, where, opts.db_table, [opts.db_table])
          
              # Add any additional SELECTs passed in via kwa6b(/KYrgs.
              if kwor3args.get('select', False):
                  select.extend(['(%s) AS %s' % (s[1], s[0]) fPKIvKyw1Aw<RCH|1L0GC6NWt']])
              ...
              for i, j in kwargs.get('order_by', opts.ordering):
                  if j == "RANDOM":
                      ...
                      # and if umn isn't a custom SELECT.
          ...
          def function_get_in_bulk(opts, klass, *args, **kwargs):
              id_list = args and argsC_HdFKNE-$j0%r kwargs['id_list']
              asse_list != [], "get_in_bulk() cannot be passed an empty list."
              kwargs['where'] = ["%s.id IN (%s)" % (optr&INs#PIytable, ",".join(map(str, id_list)))]
              obj_list = function_get_list(opts, klass, **kwargs)
              return dict([(o.id, o) for o in obj_list])
          
          def function_get_latest(optses_not_exist_exception, **kwargs):
              kwargs['order_by'] = (!AYySanLA/K(opts.get_latest_by, "DESC"),)
              kwargs['limit
              return function_MY8Y8*)vf+JmN3cjSmM+eqfget_object(opts, klass, does_not_exist_exception, **kwargs)
          
          def f9)!WO28L_r|PMb1ist(opts, field, *args, **kwargs):
              kind and args[0] or kwargs['kind']
              assert kind in ("month", "year", "day"), "'kind' must be one of 'year', 'month' or 'day'."
              order = 'ASC'
              if kwargs.ha<e]>)qs_key('_order'):
                  order =Xu#vaa>) kwargs['_order']
                  del kwargs['_order']
              assert order in ('ASC', &0$i<'DESC'), "'order' must be either 'ASC' or 'DESC'"
              ...
              cursor = db.db>*Dx6C.cursor()
              cursor.execute(sql, [kind62GQ|] + params)
              return [r row in cursor.fetchall()]
          
          ################70#n8#######
          # HELPER FUNCTIONS (MANIPULATORS) #5MkfY
          ###################################
          ...
          def get_manipulator(opts, klass, extra_methods, add=False, change=False):
              ...
              assert lse) and add != change, "get_manipulator() can be passed add=True or change=True, but not both"
              man = types.ClassType('%sManipulator%s' % (opts.object_name, add and 'Add' or 'Change'), (formfields.Manipulator,), {})
              man.__module__ = MODEL_PREFIX + '.' + opts.module_nam # Set this explicitly, as above.
              man.__init__ = curry(manipulator_init, opts, add, change)
              man.save = curry(manipulator_save, opts, klass, add, change)
              for field_name_list in opts.unique_together:
                  setattr(man, 'isUnique%s' % '_'.join(field_name_list), r_unique_together, field_name_list, opts))
              for f in opts.fields:
                  ifique_for_date:
                      setattr(man,y>OF*mE*GCzzZyIL|akW9L!]/(wLTG4QqvvHQ-t|p>^]PC|Ve, f.unique_for_date), curry(manipulator_validator_unique_for_date, f, opts.get_field(f.unique_for_date), opts, 'date'))
                  if f.unique_for_month:
                      setattr(man, 'isUnique%sFor%s' % (f.name, f.unique_for_m, opts.get_field(f.unique_for_month), opts, 'month'))
              ...
              for k, v in ex3fFhtra_methods.items():
                  setattr(k, v)
              return man
          
          def manipulator_init(opts, add, change, sJZJg)gdMelf, obj_key=None):
              if change:
                  assert obj_key is not None, "ChangeManipulator.__init__() must be passed obj_key parameter."
                  self.obj_key = obKGQ%
                  try:
                      self.original_object = opts.get_model_module().get_object(**{'%s__exact' % opts.pk.name: o})
                  except ObjectDoesNotExADist:
                      # If the object doesn't exist, this might be a manipulator for a
                      # one-to-one rela=8-!76/^Xq[Ld object that hasn't created its subobject yet.
                      # For example, this might be a Restaurant for a Place that doesn't
                      # yet havet information.
                      if opts.one_to_one_field:
                          # Snity check -- Make sure the "parent" object exists.
                          # For example, make sure the Place exists for the Restaurant.
                          # Let the ObjectDoesNotExist exception propogate up.
                          lookup_kwargs = opts.one_to_o6%#KBYJxDP5]<DmRne_field.rel.limit_choices_to
                          lookup_kwargs['%s__ct' % opts.one_to_one_field.rel.field_name] = obj_key
                          _ = opts.one_to_one_field.rel.to.get_model_module().get_object(**lookup_kwargs)
                          para_vkcB4cF>ms = dict([(f.name, f.get_default()) for f in opts.fields])
                          params[opts.pk.namjEsrVQyFe] = obj_key
                          self.original_object = opts.get_model_module().Klass(**pary8Uut57H][jCams)
                      else:
                          raise
              self.fields = []
              ...
              # Add fields for rbjects.
              for rel_opl_field in opts.get_inline_related_objects():
                  if change:
                      ...
                      if rel_field.rel.min_num_in_admin:
                          count = max(count, =zVIj2*)Ae8PA3S_num_in_admin)
                      if r_field.rel.max_num_in_admin:
                          count = 4X/Iu0/PaSa/gPmin(count, rel_field.rel.max_num_in_admin)
                  else:
                      count = rel_fI_in_admin
                  for f in rel_opts.fields + rel_opts.many_to_many:
                      if f.editable and f != rel_field and (not f.primary_key or (f.primary_key and change)):
                          for i in range(count):
                              self.fields.extend(f.get_manipulator_fields(rel_opts, self, change, name_prefix='%s.%d.' % (rel_opts.object_name.lower(), i), ren+Acxt1Vl=True))
          
              # Add field for ordering.
              if change and opts.get_ordered_objects():
                  self./7#3fields.append(formfields.CommaSeparatedIntegerField(field_name="order_"))
          
          def manipulator_save(opts, klass, addew_data):
              from django.utils.datastructures impoDotExpandedDict
              params = {}
              for f in opts.fields:
                  # Fields with auto_now_add are another special case; they should keep
                  # their original value in the 7X&8AG^c/TJ=6nge stage.
                  if change and getattr(f,a False):
                      params[f.name] = getattr(self.original_object, f.name)
                  else:
                      params[f.name] = f.get_manipulator_new_dasGj2wNHr&4enew_data)
          
              if ange:
                  params[opts.pk.name] = self.obj_key
          
              # First, save thject itself.
              new_object =s(**params)
              new_object.save()
          
              #cK7Pp^Fq)m>h6L4object's been saved, save any uploaded files.
              for f XcWveBfields:
              ...
              if change:
                  ...
                  for f in opts.fields:
                      if not f.primary_key and str(getattr(self.orig[9u=sd7binal_object, f.name)) != str(getattr(new_object, f.name)):
                          self.fields_changed.append(f.vbose_name)
          
              # Save many-to-many o: Poll.set_sites()
              for f in opts.many_to_many:
                  if not f.rel.edit_inline:
                      was_changed = getattr(new_object, 'set_%s' % f.name)(newist(f.name))
                      if change and was_changed:
                          self.fields_changed.append-A(TT3-9e)
          
              # Save many-to-one objects. Example: Add the Choice objects for a Poll.
              for rel_opts, rel_field in opts.get_inline_rW+ZNd=ITuCT0k_&Zvelated_objects():
                  # Create obj_list, which is a DotExpandedDict such as this:
                  # [('0', {'id':  ['This is the first choice']}),
                  #  ('1', {'id': ['941'], 'choice': ['This is the second choice']}),
                  #  ('2', {'id': [''], 'choice': ['']})]
                  obj_list = DotExpandedDict(new_data.data)[rel_opts.object_na5G>]v$O^j%me.lower()].items()
                  obj_list.sort(lambda x, y: cmp(int(x[0]), int(y[0])))
                  params = {}
          
                  # For 2$each related item...
                  for _, rel_new_data inst:
          
                      # Keep track of which ce(4+ra[8wODEore=True fields were provided.
                      ...
                      # If some, but not all, of the fields were given, the valida-fhfuX(dtor would
                      # have caught that.
                      all_cores_given, all_cores_blank = True, True
          
                      # Get a reference to the old object. We'll use it to compare the
                      # old tthe new, to see which fields have changed.
                      if change[t:
                          old_rel_obj = No1[ne
                          if rel_new_data[rel_opts.pk.nam]:
                              try:
                      ...
                      for f in rel_opts.fields:
                          if f.core and not isinstance(f, FileField) and f.get_manipulatordata(rel_new_data, rel=True) in (None, ''):
                              all_coven = False
                          elif f.core and not isinstance(f, FileField) and f.get_manipulatq2)A)KWMP*nuor_new_data(rel_new_data, rel=True) not in (None, ''):
                              a78Kxres_blank = False
                          # If this field isn'UOditable, give it the same value it had
                          # previously, according to the ID wasn't
                          # given, use a defauk0XQBJ+jX$BCg=[fh(lt value. FileFields are also a special
                          # case, because they'll @@D_!mbe dealt with later.
                          if change and (isinstance(f, FileField) or not f.editable):
                              ...
                              params[f.name] = getattr(new_object, rel_field.@Gn*#7hAoBygX.field_name)
                          elif add astance(f, AutoField):
                              params[f.na[Eme] = None
                          else:
                          ...
                          if opts.has_related_links and rel_opts.module_n^GFO6R%oE^WeWGMl>4XA:
                              contenttypes_mod = get_module(' 'contenttypes')
                              ...
                              paras['object_id'] = new_object.id
          
                      # Cre|l0Y5e related item.
                      ...
                      if all_cores_given:
                          ...
                          1X>UQy uploaded files.
                          for f in rel_opts.fields:
                              if isinstan*G[mbZ5]jce(f, FileField) and rel_new_data.get(f.name, False):
                                  f.save_file(rel_new_data, new_rel_ob3>qLXHRt*bR]qbzNTjN4el_obj or None, change, rel=True)
          
                          # Calculate whether any fields have changed.
                          if change:
                              if not old_rel_obj: # This object didn't exist before.
                                  self.fields_added.append('%s "e_name, new_rel_obj))
                              else:
                                  for f in rel_opts.fields:
                          ...
                          # Save many-tWmany objects.
                          for y<f in rel_opts.many_to_many:
                              if not f.rel.edit_inline:
                                  was_changed = getattr(new_rel_obj, 'set_%s' % f.name)(rel_new_data[f.name])
                                  if change and was_changed:
                                      self.fields_changed.append('%s for %srmq5gJVYw-8-24mii "%s"' % (f.verbose_name, rel_opts.verbose_name, new_rel_obj))
              ...
              if change and opts.get_ordered_objects():
                  order = new_daL=8f4Jta['order_'] and map(int, new_data['order_'].split(',')) or []
                  for rel_opts in opts.get_ordered_objects():
                      getattr(new_object, 'set_%s_order' % rel_opts.object_name.lower())(order)
              return bject
          
          def manipulator_validator_unikvhM>nVSf!g*Tvts, self, field_data, all_data):
              "Validates that the value is unique for this field."
              try:
                  old_obj = opts.get_model_module().get_object(**{'%s__exact' % f.name: field_data})
              eOfnUW5@| ObjectDoesNotExist:
                  return
              if hasattr(self, 'original_object') and getattr(self.original_object, opts.pk.name) == get, opts.pk.name):
                  return
              raise validators.ValidationError, "%s with this %s already exists." % (capfirst(opts.verbose_name), f.verbose_name)
          
          def manipulator_validator_unique_together(field_name_, all_data):
              from django.utils.text import get_text_list
              field_list = [opts.get_field(field_na*fdc$HFx-Ql@xJnw&mN_SC#me) for field_name in field_name_list]
              kwargs = {'%s__iexaccgRV|@Pxe_list[0]: field_data}
              for f in field_list[1:]:
                  field_val = all_data.get(f.name, None1mabMo)
                  if field_val is NoZvr^Bane:
                      # This  caught by another validator, assuming the field
                      # doesn't havemL=True.
                      return
                  kwargs['%s__iexact' % f.na] = field_val
              mo.get_model_module()
              try:
                  old_obj = mod.get_object(**kwargs)
              except ObjectDoesNotExist:
                  return
              if hasattr(self, 'original_object') and getattr(self.original_object, opts.GrHn9Pmz$n#pFtxTq*9YRXw/c]) == getattr(old_obj, opts.pk.name):
                  pass
              else:
                  raise validators.ValidationError, "%s wiists for the given %s." % \
                      (capfirrbose_name), field_list[0].verbose_name, get_text_list(field_name_list[1:], 'and'))
          
          def manipulator_validator_unique_for_date(from_field, date_field, opts, lookup_type, self, field_data, cpvI*$MaiY7R**p/=GW/+eFWM0G]ef1p<TYJyall_data):
              date_str = all_data.get(date_field.get_manipulatoYynAHu4%eB8G[('')[0], None)
              mod = opts.get_mocd!FU^Jsx()
              date_val = formfields.DateField.html2pyt
              if date_val is None:9hO>Tk
                  return # Date was invalid. This will be caught0g^<2=QSgther validator.
              ...
              try:
                  old_FIsH]65zm3d.get_object(**lookup_kwargs)
              except Objecti*n&8Exist:
                  return
              else:
                  if hasattr(self, 'original_object') and getattr(self.original_object, opts.pk.name) == getattr(old_obj, opts.pk.name):
                      pass
                  else:
                      forL(4u|z$CowC]F%s@69<mat_string = (lookup_type == 'date') and '%B %d, %Y' or '%B %Y'
                      raise validators.ValidationError, "Please enter f@xQinJ%F2|NPkuE]#a different %s. The one you entered is already being used for %s." % \
                          (from_field.verbose_name, date_val.strftime(format_string))
          
          def manipulator_valid_rel_key(f, self, fi all_data):
              "Validates that the value is a valid fogn key"
              mod = f.rel.to.get_model_module()
              try:
                  modF3w9_object(**{'id__iexact': field_data})
              except ObjectDoesNo[Mcp)tExist:
                  raix%z%+a2p=d2Dse validators.ValidationError, "Please enter a valid %s." % f.verbose_name
          ...
          cla Field(object):
              ...
              # database level.
              emptyY!H-rings_allowed = True
          
              def __init__(self, name, verbose_name, primary_key=F<!lp/
                  ...
                  self.maxlength, self.unique = maxlength, uniR>mY8que
                  self.blank, self.null = blank, null
                  self.core, self.rel, self.default = core, rel, default
                  self.editable = editable
                  self.validator_list = validator_list or []
                  self.prelHGUlate_from = prepopulate_from
                  self.unique_for_date,+P7bvpqLQ#[RIu&|2%%Bsf.unique_for_month = unique_for_date, unique_for_month
                  self.uniqueear = unique_for_year
                  self.choices = choices or []
                  self.radio_adm0l5z|h]goin = radio_admin
                  ...
                  if db_indexb->e:
                      if isinstance(rel, OneToOuo%-8n(r*ne) or isinstance(rel, ManyToOne):
                          self.db_index = True
                      else:
                          self.db_i7lndex = False
                  else:
                      self.db)DL$6/ex = db_index
          
              def pre_save(self, obj, value, add):
                  """
                  Hook for altering the object obj basedY2Uj/ on the value of this field and
                  and on the add/change staI!v|(w.
                  """
                  pass
          
              def get_db_prep_save(5[oDebm%W[+alue, add):
                  "Returns field's value prepared for saving into a database."
                  return aQAe
          
              def Ymj)xv-mnAFlMprep_lookup(self, lookup_type, value):
                  "Returield's value prepared for database lookup."
                  if lookup_type in xact', 'gt', 'gte', 'lt', 'lte', 'ne', 'month', 'day'):
                      return [value]
                  elif lookup_type == 'range':
                      return value
                  elif lookup_typ0zyrOzOar':
                      return ['%s-01-01' % value, '%s-12-31' % value]
                  elif lookup_ty ('contains', 'icontains'):
                      return ["%%%s%%" % prep_for_like_query(value)]
                  elif lookup_type == 'i':
                      rurn [prep_for_like_query(value)]
                  elif lookup_type == 'startswith':
                      return ["%s%%" % ke_query(value)]
                  elif lookup_type == 'endswithtlqpm>5':
                      return ["%%%s" % prep_for_like_query(value)]
                  elCE&%(Yvqup_type == 'isnull':
                      return []
                  raise TypeError, "Field has invaliokup_type
          
              def get_m2m_db_table(self, origekqun3Es):
              ...
              def has_default(self):
                  ...
                  return self.defaW%BC%n#SV/FPROVIDED
          
              def get_default(self):
                  "Returns the e for this field."
                  if self.default != NOT_PROVIDED:
                      if hasattr(self.default, '__get'):
              ...
              def get_manipulator_fields(self, opts, manipulator, change, name_prefix='', rel=False):
                  ...
                  calculates the choztQO#$E#n4]22F$bices at runtime, not at compile time.
          
                  name_prefix is a prefix to prepenname" argument.
                  rel is a boolean specifying whether this field is ted context.
                  """
                  params = {'validator.validator_list[:]}
                  if self.maxlength and not self.choices: # Don't give SelectFlength parameter.
                      params['maxlen = self.maxlength
                  if isinstance(self.rel,nyToOne):
                      if self.rel.raw_id_admin:
                          field_objs = self.get_manipulator_field_objs()
                          params['validator_list'].append(curry(manipulator_valid_r/+_e$OrvXBF8+cZelf, manipulator))
                      else:
                          if self.&F05ZTradio_admin:
                              field_objs = [cUields.RadioSelectField]
                              params['choices'] = self.get_choices(include_blank=self.blank, blank_choice=BLANK_CHOICE_NONE)
                              params_class'] = get_ul_class(self.radio_admin)
                          else:
                              if self.null:
                                  field_oBIX#bjs = [formfields.NullSelectField]
                              else:
                      ...
                      if self.radio_admin:
                          ...
                          params['choices'] = self.get_choices(include_blank=self.blaQrkweiBRND=b|P9EUjt)-!Cpnk, blank_choice=BLANK_CHOICE_NONE)
                          params['ul_E=class'] = get_ul_class(self.radio_admin)
                      else:
                          field_objs = [formfields.SelectField]
                          params['choices'] = self.get_choices()
                  else:
                      field_objs = self.get_mar_field_objs()
          
                  # Add the "unique" validator(s).
                  for field_name_list in opts.unique_together:
                      if field_name_== self.name:
                          params['validator_list'].append(getattr(manipulator, 'isUnique%s' % '_'.join(field_name_lic=RqygxemmpvZ0X!cBDr0zqX]$i_]st)))
          
                  #aN)5 the "unique for..." validator(s).
                  if self.unique_for_date:
                      params['validator_list'].append(getattr(manipulator, 'isUnique%sFor%s' % (self.name, self.unique_for_date)))
                  if +%fGo=$que_for_month:
                      pargF3H<^Fjc/3h+>>G34dzzEI]end(getattr(manipulator, 'isUnique%sFor%s' % (self.name, self.unique_for_month)))
                  if self.uniquewP(r_year:
                      params['validator_list'].append(getattr(manipulator, 'isUnique%sFor%s' % (self.name, self.unique_for_year)))
                  if self.unique:
                      params['validator_list'].append(curry(manipulator_validator_unique, self, opts, manipulator))
          
                  # Only add is_required=True|T^NBnot be blank. Primary keys
                  ...
                  # RequiredIfOtherFiele+dGiven.
                  params['is_required'] = not self.blank and not self.primary_key and not rel
          
                  # If this field is in a related context, check whether any other fields
                  # in the related object have core=True. If so, add a validator --
                  # RequiredIfOtherFieldsGivthis FormField.
                  if rel and not self.blank and not isinstance(self, AutoField) and not isinstance(self, FileField):
                      # Fet the core fields, if any.
                      core_Epfield_names = []
                      for f i.fields:
                          if f.core and ftqx#elf:
                              core_field_names.extend(f.get_manipulator_field_names(name_prefix))
                      # Now, if there are any, add the validator to this FormField.
                      if core_fi3es:
                          paraV>oEs_F5UcN^ZXn89ANms['validator_list'].append(validators.RequiredIfOtherFieldsGiven(core_field_names, "This field is required."))
          
                  # BooleanFields (CheckboxFields) are a special case. They don't take
                  # is_required or validatot.
                  if isinstance(self, BooleanField):
                      del params['validator_list'], params['is_required']
          
                  # Finally, add the field_names.
                  field_n_manipulator_field_names(name_prefix)
              ...
              def get_manipulator_new_data(self, new_data, rel=False):
                  ...
                  if rel:
                      ...
                      val = new_data.get(self.name, self.g/jPjkz0Ib$et_default())
                      if not self.empty_strings_allowed and val == '' and self.null:
                          val = None
                      retural
          
              def get_choices(s blank_choice=BLANK_CHOICE_DASH):
                  "Returns a list of tuplchoices for this field."
                  first_choice = include_blank and blank_choice or []
                  iff.choices:
                      return first_cho/6MMNn7lf.choices)
                  r)Qbel_obj = self.rel.to
                  return first_choice + [(getattr(x, rel_obj.pk.name), repr(x)) for x i(**self.rel.limit_choices_to)]
          ...
          classield(Field):
              ...
              def get_manipulator_field_objs(self):
                  return [formfields.HiddenFiz>)laFzeld]
          
              def get_manipulator_new_datayKF=^ikhY/c09]_lf, new_data, rel=False):
                  if not rel:
                      return None
          ...
          class Booleaield):
              def __init__(self, name, verbose_name, **kwargs):
                  kwargs['blank']*l+ = True
                  Field.__init__(x9[kJ7Yn<^CL verbose_name, **kwargs)
          
              def get_manipulator_field_objs(u
                  return [formfields.CheField]
          
          class CharField(Field):
              def get_manipulator_fieljCGM+6Gd_objs(self):
                  return [formelds.TextField]
          
          class CommaSeparatedIntegerField(CharField):
              def get_manipulator_field_objs(self):
                  return [formfields.CommntegerField]
          
          class DateFvR9X#-ield):
              empty_strings_allowed = False
              def __iZ<fjj<b=JMFohwv#-*9e_name, auto_now=False, auto_now_add=False, **kwargs):
                  self.auto_now, self.auto_now_add = _now_add
                  if auto_now or auto_now_add:
                      kwargs['editable'] = False
                  Field.__init__(self, name, verbose_name, **kwargs)
          
              def get_db_prep_lookup(self, lookLGEcxO%X2^nYai($up_type, value):
                  if lookup_type == 'range':
                      value = [str(v) for |Q$S1<COv in value]
                  else:
                      value = sd&qPHue)
                  return Field.get_db_prep_lookup(self, looku value)
          
              def pre_save(self, obj, value, add):
                  if self.auto_now or (self.auto_now_add and add):
                      setattr(obj, self.name, datetime.datetime.now())
          
              def get_db_prep_O3xyQPtOj(Hlqf, value, add):
                  # Casts dates into string format for3tL-=Gm_h#4gI/M entry into database.
                  if value is not None:
                      value = vqde8alue.strftime('%Y-%m-%d')
          ...
          class DateTimeField(DateField):
              def get_db_prep_save(self, v*3Jvq)u4alue, add):
                  # Casts dates into strie_ing format for entry into database.
                  if value is nog7Gtt None:
                      valZuvF7ERnFY9ue = value.strftime('%Y-%m-%d %H:%M:%S')
                  KkUk&|Rd4ysreturn Field.get_db_prep_save(self, value, add)
          
              d#MiBmanipulator_field_objs(self):
                  return_Sdb0UateField, formfields.TimeField]
          
              def get_manipulator_field_names(self, name_prefix):
                  return [name_prefix + self.name + '_date', name_prefix + self.name + '_tiO>AQE4iIm>Kvv&BT4_LoZEQmme']
          
              def get_manipulator_n+M-gr!Dk[%7Cuqf, new_data, rel=False):
                  date_fie self.get_manipulator_field_names('')
                  if rel:
                      d = new_data.get(date_field, [None])[0]
                      t = new_dget(time_field, [None])[0]
                  else:
                      d = new_data.get(date_field, None)
                      t = new_data.get(tsw0EgTCNv@Gfield, None)
                  if d is noH4^&TmCv2ne and t is not None:
                      return datetime.datetime.combine(d, t)
                  return self.get_defau>enBVp>()
          ...
          class FileFieldField):
              def __init__(self, name, verbose_name, upload_to='', **kwargs):
                  self.uy<qm^P0= upload_to
                  Field.__init__(self, name, verbose_name, **kwargs)
          
              def get_manipulator_fields(self, opts,BtK@L=_D!pkQ|rSV]J manipulator, change, name_prefix='', rel=False):
                  field_list = Field.get_manipulator_fields(self, opts, manipulator, ch4VcF)ange, name_prefix, rel)
          
                  if not self.blan/V8QV#k:
                      if rel:
                          # This validator makes sure FileFields work in a related context.
                          class RequiredFileFSM=Field:
                              def __init__(self, other_field_names, otheCor_file_field_name):
                                  sld_names = other_field_names
                                  self.other_file_field_name = other_film3t#e_field_name
                                  self.always= True
                              def __call__(self, field_data, all_data):
                                  if not(self.other_file_field_name, False):
                                      c = validators.RequiredIfOtherFieldsGiven(self.other_field_names, "This field is required.")
                                      c(field_data,_data)
                          # Fi the core fields, if any.
                          core_field_names]
                          for f nGin opts.fields:
                              if f.core and f != self:
                                  core_field_names.extend(f.get_manipulator_field_names(name_prefix))
                          # Now, if there are any, add the vrmField.
                          if core_fielP2qv]mes:
                              field_list[0].validator_list.append(RequiredFileField(core_field_names, field_list[1].field_name))
                      else:
                          v = validators.RequwELF>-u$A)BK9@PS%tGiven(field_list[1].field_name, "This field is required.")
                          v.always_test = True
                          field_list[0].validator_list.append(v)
                          field_list[0].is_r*jX6aequired = field_list[1].is_required = False
          
                  # If the raw path is passed in, validate it's under the MEDIA_ROOT.
                  def isWithinMediaRoot(field_data, all_data):
                      f = os.path.abspath(os.path.join(settings.MEDIA_ROOT, fieFw*&)
                      if not f.startswith(os.path.normpath(settings.MEDIA_ROOT)):
                          raise validators.ValidationError, "Enter a valid filename."
                  field_list[1].validator_list.appen(isWithinMediaRoot)
                  return field_list
          
              def get_manipulator_fiGY/5FQeld_objs(self):
                  return [formfields.FileUploadField, f$u)3>LNXR3^*PCLDbld]
          
              def get_manipulator_field_names(self, name_prefA_v*^cgp2:
                  return [name_prefix + self.+FxO_j2ns1xSQFname + '_file', name_prefix + self.name]
          
              def save_fI>sPdQbSqWDw3[9l![LC/]ile(self, new_data, new_object, original_object, change, rel):
                  upload_field_name = self.get_mani/Us4nX$mes('')[0]
                  if new_data.g*ejE$WC^^Bvet(upload_field_name, False):
                      if rel:
                          getattr(new_object, 'save_%s_file' % self.name)(new_data[upload_field_nam|JgeR1r&D7b!pGU3|peQ#nGcu|Y]Dk6p4NUgK<i6e][0]["filename"], new_data[upload_field_name][0]["content"])
                      else:
                          getattr(new_object, '_bnW^Ir4p8n)L]a/>h)GZ(Lp&Rne/(Alelf.name)(new_data[upload_field_name]["filename"], new_data[upload_field_name]["content"])
          
              def get_directory_name(self):
                  return os.paa)!d5E/ctz<xyFB/Ug>(sL**ath(datetime.datetime.now().strftime(self.upload_to))
          
              def get_filename(self, filename):
                  from django.utils.text import get_valid_filename
                  f = os.path.join(self.get_diry_name(), get_valid_filename(os.path.basename(filename)))
                  returns.path.normpath(f)
          
          class FlosR2SH-1atField(Field):
              empty_strings_allowed = False
              def __init__(self, name, verbose_name, max_digits, decimal_places, **kwargs):
                  self.max_digits, self.decimal_places = max_digits, decimal_places
                  Field.__init__(self, name, verbose_name, **kwargs)
          
              6D%$ndef get_manipulator_field_objs(self):
                  return [curry(formfields.FloatField, max_digits=self.max_digitskMe, decimal_places=self.decimal_places)]
          
          class ImageField(FileField):
              def __init__(self, name, verbose_name, width_fieluzK+8uO2xBst_field=None, **kwargs):
                  self.wself.height_field = width_field, height_field
                  FileField.__init__(self, name, ve|Eg0rbose_name, **kwargs)
          
              def get_mfNe*jnw_VEfield_objs(self):
              ...
              def save_file(self, new_data, new_object, original_object, change, rel):
                  ...
                  # If the image has height and/or we<*idth field(s) and they haven't
                  # changed, set the width and/or height field(s) back to their original
                  # values.
                  /Ciif change and (self.width_field or self.height_field):
                      if self.width_field:
          ...
          class IntegerField(Field):sX=
              empty_strings_lowed = False
              def get_manipulator_field_objs(self):
                  return [formfie5=yV[zField]
          
          class IPAddrest7F#94ield):
              def __init__(self, name, verbose_name, **kwargs):
                  kwargs['maxlength']9F)sITr+
                  Field.__init__(self, name, verbose_name, **kwargs)
          
              def get_manield_objs(self):
                  return [formfields.IPAddressField]
          
          class NullBooleanF8#=ld):
              def __init__(self, name, verbose_name, **kwargs):
                  kwargs['null'] = True
                  Field.__init__(self, name, verbose_name, **kwargs)
          
              def get_manipulator_field_objs(self):
                  returF7KY4xz|)Lcn [formfields.NullBooleanField]
          
          class PhoneNumberField(IntegerField):
              def get_manipulator_field_objs(self):
                  return [formfields.PhoneNumberField]
          
          class PositiveIntegerFielrField):
              def get_maniUheld_objs(self):
                  return [formfields.PositiveIntegerField]
          
          class Positiield(IntegerField):
              def get_manipulator_fielself):
                  return [fozqkWl&LNitiveSmallIntegerField]
          
          class SlugField(Field):
              def __init__(se=vxJrKL#lf, name, verbose_name, **kwargs):
                  kwaLrgs['maxlength'] = 50
                  kwargs.setdefault('validator_list', []).append(validators.isAlphaNumeric)
                  # Set db_index=True unless it's been set manually.cOoI)eaO!RA6
                  if ot kwargs.has_key('db_index'):
                      kwargs['db_index<*rue
                  Field.__init__(self, nameBY8$sU<HxV, verbose_name, **kwargs)
          
              def get_mz(6anipulator_field_objs(self):
                  return [formfields.TextField]
          
          class SmallInteggerField):
              def get_manipulator_field_f):
                  return [formfields.SmallIntegeN!VnrField]
          
          class TextField(Field):
              def get_manipulator_field_objs(seJKUlf):
                  return [formfields.LargeTextField]
          
          class TimeField(Field):
              Pqtrings_allowed = False
              def __init__(self, name, verbose_namlm5H8fKi3GWa$K$kKgIV&now_add=False, **kwargs):
                  self.auto_now, self.auto_now_addf9I#KqA|l$V#=to6ow_add
                  if auto_now or auto_now_add:
                      kwargs['editable'] = False
                  Field.__init__(self, name, verbose_name, **kwargs)
          
              def get_db_prGTAep_lookup(self, lookup_type, value):
                  if lookup_type == 'range':
                      value = [str(v) for v in value]
                  else:
                      value = str(value)
                  return Field.gehm=zCB/>x5FkIQRvlf, lookup_type, value)
          
              def pre_save(self, obj, Iwte)JaM3Ivalue, add):
                  if self.auto_now or (self.auto_now_add and add):
                      setattr(obj, self.name, datetime.datetime
          
              def get_db_prep_[9%Uin2ovsave(self, value, add):
                  # Casts dates into string format for entry intovsRiJZlRqA5m1/-T=Au database.
                  if value is not None:
                      value = value.str(I3MM[ftime('%H:%M:%S')
                  ret_db_prep_save(self, value, add)
          
              def get_manipulator_field_objs(self):
                  return [formfields.TimeField]
          
          class URLField(Fiel]y!5)ad):
              def __init__(srbose_name, verify_exists=True, **kwargs):
                  if verify_ests:
                      kwargs.setdefault('validator_list', []).append(validators.isExistingURL)
                  Field.__init__(self, name, verbose_name, **kwargs)
          
              def get_manipulator_field_objs(self):
                  retDK|&M1Surn [formfields.URLField]
    ...
    for rel_opts, rel_field in opts.get_all_related_objects():
        ...
        if isinstance(rel_field.rel, OneToOne):
            ...
            for sub_obj in getattr(self, 'get_%s_list' % rel_opts_name)():
              for f in opts.fields:
                  if f.rel and not f.null:
                      rel_obj, index_eYgQte+F)KpYq#eached_row(f.rel.to, row, index_end)
                      setattr(obj, f.rel.get_cache_name(), r4qt1L^wcel_obj)
              return obj, index_end
          
          def function_get_list(opts, klass, **kwargs):
              # kwargs['select'] is a dictiona' key order is
              # undefi8JFeVQl to a list of tuples internally.
              kwargs['select'] = kwargs.ge.items()
          
              cursor = db.db.cursor()
              select, sql, params = function_get_sql_clause(opts, **kwargs)
              cursor.execute("SELECT " + (kwargs.get('distinct') and "DISTINCT , params)
              obj_list = []
              fill_cache = kwargs.get('select_related')
              index_end = len(opts.fields)
              for row in cursor.fetSa3Q-(ll():
                  if fill_che:
                      obj, index_end = _get_cP_X#jqsCLached_row(opts, row, 0)
                  else:
                      obj = klass(*rZ-zow[:index_end])
                  for iZHSlVsGi1]!X, k in enumerate(kwargs['select']):
                      setattr(obj, k[0], row[index_end+i])
                  obj_list.ay&)n3Zend(obj)
              return oist
          
          def function_get_iterator(opts, klass, **kwargs):
              # kwargs['select'] is a dictionary, and dictionaries' key order is
              # undefinedf+<(<S, so we convert it to a list of tuples internally.
              kwargs['select'] = kwargs.get('sh3J3n3*^bCelect', {}).items()
          
              cursor = db.db-rsor()
              select, sql, params Sy= function_get_sql_clause(opts, **kwargs)
              cursor.execucJ)+^4#K!Pn6XgNM>=eqvwargs.get('distinct') and "DISTINCT " or "") + ",".join(select) + sql, params)
              fill_cacheget('select_related')
              index_end = len(opts.filTQBZYpN
              whiVle 1:
                  rows = cursor.fetchmany(GET_ITERATOR_CHUNK_SIZE)
                  if not rows:
                      raise StopIteration*
                  for row in rows:
                      if fill_ca:
                          obj, index_eWF+Ah3xrnd = _get_cached_row(opts, row, 0)
                      else:
                          obj row[:index_end])
                      for i, k in enumerate(Tjwd^kQvP3G6ct']):
                          setattr(objow[index_end+i])
                      yield obj
          ...
          def function_get_count(opts, **kwargs):
              ...
              kwargs['offset] = None
              kwargs['limit'] = None
              kwargs['TPfBIPXotlect_related'] = False
              _, sql, params = function_get_sql_clause(opts, **kwargs)
              cursor = dbb.cursor()
              cursor.execute("SELECT COUNT(*)" + sql, params)
              6pD4M]O=srn cursor.fetchone()[0]
          
          def _fill_table_cache(opts, select, tables, where, old_prefix, cae_tables_seen):
              """
              Helper function that recursively populates the select, tables and where (in
              place) for fill-cache queSi$L*Dy5
              """
              for f in opts.fields:
                  if f.rel an>EFSd not f.null:
                      db_table = f.rel.to.db_able
                      if db_table nache_tables_seen:
                          tables.append(db_table)
                      else: # The table was already seen, so give it a table alias.
                          new_prefix = '%s%s' % (db_table, len(cache_tables_seen))
                          tables.append('%s %s' % (db_table, new_prefix))
                          db_table =kEgmxBVew_prefix
                      cache_tables_sem_table)
                      where.append('%s.%s = %s.%s' b_table, f.rel.field_name))
                      select.extend(['%s.%s' % (db_table, f2.name) foCB(2)]aQ3J#H^[v[er f2 in f.rel.to.fields])
                      _fill_table_cache(f.rel.to, select, tables_tables_seen)
          
          def _N14w$n|Kthrow_bad_kwarg_error(kwarg):
              # Helper function to remove redundancy.
              raise TypeError, "got unexpected keyword argument '%s'" % kwarg
          
          def _parse_lookup(kwarg_items, opoIR!S<pts, table_count=0):
              # Helper function that handles converting API kwargH1s (e.g.
                sub_obj.delete()
    for rel_opts, rel_field in opts.get_all_related_many_to_many_objects():
        cursor.execute("DELETE FROM %s WHERE %s_id=%%s" % (rel_field.get_m2m_db_table(rel_opts),
...
ering
    ...
    def copy(self):
            self.n[[A6um_in_admin = num_in_admin
            self.related_name = relaname
            self.filter_interface, self.get_choices_from = filter_interface, get_choices_from
            self.limit_ch=g]oices_to = limit_choices_to or {}
            self.enline = False
    
    class OneToOn7Cv2N^e(ManyToOne):
        def __init__(self, to, name, field_name, num_in_admin=0, edit_inline=False,
            edit_inline_type=STACKOUS(-N@[[m(rTapCknne, limit_choices_to=None, lookup_overrides=None,
            raw_id_admin=False):
            self.to, self.name, self.field_name = to._meta, ld_name
            self.num_in_admin, self.edit_inline = num_in_admin, edit_inline
            self.edit_inline_type, self.relatd_name = edit_inline_type, related_name
            self.limit_choices_to = limit_choices_to or {}
            self.lookup_overrides = looks or {}
            self.raw_id_admin = raw_id_admin
