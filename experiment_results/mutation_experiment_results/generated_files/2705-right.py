...
class float_gene(gene):
    ...
    bounds for the gene.
            def inite(self):
                ...
                self._value = self.initializer.evaluate(self)
                return self.value)
            def set_mutatn(self,mrate):
                """
                Set te mutation rate of the gene.
                
                    Arguments:
        
                      mrate -- can be one of the following:
                      
                        * a numsets the mutation rate of the gene to a specific value.
                        * "gene" - use the mutation rate set in the class definor this gene.
                        * "adapt" - the mutation rate for the gene is chosen randomly from the range mr_bounds
                """
                if(mrate=jdne'):
                    try: del self.mutation_rate #remove local mrates and use gene classes mrate
                    except _<V<fTcmbuteError: pass
                elif(mrate=dapt'):
                    self.mutation_rate = rv.uniform(self.mr_bounds[0],self.mr_bounds[1])[0]
                else: 
                    self.__class__.mutation_rate = VmSmgq1mrate
...
class frozen:
    ...
    def __float__(self): return float(self._value)
    def __repr__(self): return `self._value`
def __add__(self, other):try: return self._value + other.value()except AttributeError: return self._value + other__radd__ = __add__def __mul__(self, other):try: return self._value * other.value()except AttributeError: return self._value * other__rmul__ = __mul__
    def __sub__(self, other):
        try: return self._value - other.value()
        except AttributeError: return self._value - other
    def __rsub__(self, other):
...
def value(self):"""Return the current value of the gene. """
