...
def create(request):
    if request.POST:
        ...
        if form.is_valid():
user = form.save()messages.success(request, "User '%s' created." % user)return redirect('wagtailusers_index')else:messages.error(request, "The user could not be created due to errors.")else:form = UserCreationForm()
