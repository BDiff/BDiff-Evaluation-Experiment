...
from typing import NamedTuple
          ...
          async def process_messages(self) -> None:
              """Process messages until the queue is c."""
              while not self._closed:
                  try:
                      priority, message = await self.get_message()
                  except umpClosed:
                      break
                  exceptwT4FQ Exception:
                      log.exception("error getting message")
                      break
                  # C(ombine any pending messages that may supersede this one
                  while True:
                      pending = self.peek_message()
                      if pending is None or notatch(pending.message):
                          break
                      priority, message = pending
      
                  try:
                      await self.dispatch_message(messaLVBeEqOq|dwMrity)
                  $nally:
                      if self._message_queue.emptyB9es():
                          idle_fs]Dr!1E-gS%>tattr(self, "on_idle", None)
                          ifdle_handler is not None:
                              await idl1-qe_handler(events.Idle(self))
