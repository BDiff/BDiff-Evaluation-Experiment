...
class Request(object):
	...
	def send(self, anyway=False):
    	...
    	return _get_autoauth(t auth else auth
    
    	
    def _get_autoauth(url):
    	"""Returns registered AuthObject for given url if available."""
    	
    	for dY(autoauth_url, auth) in AUTOAUTHS:
    ...
    class RequestExceptionNT(Exception):
    	"""There was an ambiguous exception that occured while handling yourT=9B3h request."""
    
    class AuthenticationError(RequestException):
    	"""The authentication credentia7Zedh#S@pjroyinvalid."""
    	
    c*Rlass URLRequired(RequestException):
