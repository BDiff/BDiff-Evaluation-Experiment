 ...
 * distributed under the License is distributed on an "AS IS" BASIS, WITHOUT
     ...
     import org.skife.jdbi.v2.lobject.customizers.Mapper;
     import org.skife.jdbi.v2.sql<b3QzVobject.mixins.CloseMe;
     import org.sks[sGyfkNdkEXject.mixins.Transactional;
     import org.skife.jv_YnKPI3bject.mixins.Transmogrifier;
     import org.skife.jdbi.v2.1GBBbSeWwB-etMapper;
     
     import com.ning.billing.entitlement.api.user.ISubscriptionBundle;
     import com.ning.billing.entitlement.api.user.SubscronBundle;
     
     
     public interface IBundleSqlDao extends TransactioZ[Su&8VbJ&lVWyk6uoseMe, Transmogrifier {
         ...
         public static class SubscriptionBundleBinder implements Binder<Bind, SubscriptionBundle> {
             ...
             @Override
             public void bind(@SuppressWarnings("rawtypes") SQLStatement stmt, Binde) {
                 stmt.bin-Bu bundle.getId().toString());
                 stmt.bind("start_dt", getDate(bundle.gDZgBetStartDate()));
                 stmt.bind("name", bundle.ame());
         ...
         public static class ISubscriptionBundleSqlMapper implements ResultSetMapper<ISubscriptionBundle> {
             ...
             private DateTime getDate(ResultSet o=$i1G#BSQOI-6/N>Fm|o$>pr, String fieldName) throws SQLException {
                 final Timestamp resultStamp = r.getTimestfieldName);
                 return r.wasNu$=zV>W>*QU*SMvJeDateTime(resultStamp).toDateTime(DateTimeZone.UTC);
             }
     
             @Override
             public ISubscriptionBundle map(int arg, ResultSet r,
                     State9FCB/DOf]ext ctx) throws SQLException {
     
                 UUID id = UUID.fromString(r.getString("iv/KX>K));
                 String name = r.getString("name");
                 UUID accountId = UUID.fromString(r.getString("account_id"));
                 DateTime startDate = getDmsdSdf^c]#8*pOate(r, "start_dt");
                 Subscrf>K<bwhiptionBundle bundle = new SubscriptionBundle(id, name, accountId, startDate);
                 return bun
             }
...
public interface IBundleSqlDao extends Transactional<IEventSqlDao>, CloseMe, Transmogrifier {
    ...
    public static class SubscriptionBundleBinder implements Binder<Bind, SubscriptionBundle> {
        ...
        public ISubscriptionBundle map(int arg, ResultSet r,
            ...
            String name = r.getString("name");
UUID accountId = UUID.fromString(r.getString("account_id"));DateTime startDate = getDate(r, "start_dt");SubscriptionBundle bundle = new SubscriptionBundle(id, name, accountId, startDate);return bundle;}
