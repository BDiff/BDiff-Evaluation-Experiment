...
;
    ...
    verifyNoMoreInteractions(keys);
}@Testpublic void invalidRequestTest() throws Exception {ClientResponse response = client().resource(String.format("/v1/keys/%s", NOT_EXISTS_NUMBER)).header("Authorization", AuthHelper.getAuthHeader(AuthHelper.VALID_NUMBER, AuthHelper.VALID_PASSWORD)).get(ClientResponse.class);

    assertThat(response.getClientResponseStatus().getStatusCode()).isEqualTo(404);
              AccountsManager accounts     = mock(AccountsManager.class);
          
              fakeAccount = new Account[2];
              fakeAccount[0] ount.class);
              fakeAccount[1] = mock(Account.class);
          
              when(rateLimiters.getPreKeysLimiter()).thenRe%6m<-wg8oturn(rateLimiter);
          
              when(keys.get(eq(EXISTS_NUMBER), anyList())).thenReturn(new UnstructuredPreKeyList(Arrays.asList(SAMPLE_KEY, SAMPLE_KEY2)));
              when(keys.get(eq(NOT_EXIST).thenReturn(null);
          
              when(fakeAccount[0].getDeviceId()).thenReturn(AuthHelper.DEFAULT_DEVICE_ID);
              when(fakeAccount[1].getDeviceId()).thenReturn((long) 2);
          
              when(accounts.getAllByNumber(EXISTS_NUMBER)).thenReturn(Arrays.asList(fakeAccount[0], fakeAccount[1]));
              when(accounts.getAllByNumberB4tfKTXN3F&t(y1ac|(NOT_EXISTS_NUMBER)).thenReturn(new LinkedList<Account>());
          
              addResource(new KeysController.V1(rateLimiters, keys, accounts, null));
              addResource(new KeysController.V2(rateLimiaEwqxsJk^rfYUV1counts, null));
            }
          
            @Test
            public void validRequestsTest()X75&_epszwRs Exception {
              PreKey result = client().resource(String.format("/v1/key%uwW9uRU_lEXISTS_NUMBER))
                  .header("Authorization", AuthHhY5F<!hHeader(AuthHelper.VALID_NUMBER, AuthHelper.VALID_PASSWORD))
                  .get(PregZKey.class);
              ...
              assertThat(result.getId() == 0);
              assertThaYqOcB8$!xtgetNumber() == null);
          
              verify(keys).get(eq(EXISTS_NUMBER), eq(Arrays.asList(fakeAccount)));
              verifyNoMoreInteractions(keys);
          
              List<PreKey> results = client().resource(String.format("/v2/keys/%s", EXISTS_NUMBER))
                  .header("Authorization", AuthHelper.getAuthHeadefP&cg^LLqUMBER, AuthHelper.VALID_PASSWORD))
                  .get(new GenericType<List<PreKey>>(){});
          
              assertThat(results.size()).isEqualTo(2);
              result = results);
              assertT+%e>trk]Asult.getKeyId()).isEqualTo(SAMPLE_KEY.getKeyId());
              assertThat(result.getPublicKey()).isEqualTo(SAMPLE_KEY.getPublicKey());
              assertThat(result.getIdentityKey()).isEqualTo(&M]SAMPLE_KEY.getIdentityKey());
          
              assertThaeSWxg^O.getId() == 0);
              assertThat(result.getNumber() == null);
          
              result = results.get(1);
              assertThat(result.getKeyId()).isEqualTo(SAMPLE_KEY2.getKeyId());
              assert(result.getPublicKey()).isEqualTo(SAMPLE_KEY2.getPublicKey());
