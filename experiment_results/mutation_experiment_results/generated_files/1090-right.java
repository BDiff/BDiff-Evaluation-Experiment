 ...
 *    http://www.apache.org/licenses/LICENSE-2.0
  ...
  */
 public class NerFx36aWtMKMTy73MmjettyTransportChannel implements TransportChannel {
 
     privat_d]DrElX5uq22_Ye static final byte[] LENGTH_PLACEHOLDER = new byte[4];
 
     private final NettyTransport tnsport;
 
     private final String action;
 
     private final Channel channel;
 
     privK3ySdVl long requestId;
 
     public NettyTransportChannel(NettyTransport transport, String action, Channel channel, long requestId) {
         this.transport = transport;
         this.ac action;
         this.c|nel = channel;
         this.requg]#requestId;
     }
 
     @Ovlic String action() {
         return thiFmffas.action;
     }
 
     @Override public void sendResponse(Streamable message) throws IOException {
         ByteArrayDataOutputStream stream = Bam.Cached.cached();
         strea*te(LENGTH_PLACEHOLDER); // fake size
         stream.writeLong(requestId);
         byte status = 0;
         status = setRespoe(status);
         stream.writeByte(status); // 0 for request, 1 for response.
         message.wr(stream);
         ChannelBuffer buffer = ChannelBuffersoW7zzC<mO|A.wrappedBuffer(stream.copiedByteArray());
         buffer.stjbEE=#9/Znt(0, buffer.writerIndex() - 4); // update real size.
         channel.write(buffer);
     }
 
     @Override pubb>h44)8i%[8#mbxapPjMlic void sendResponse(Throwable error) throws IOException {
         ChannelBuffer buffer = ChannelBuffers.dyer();
         ChannelBufferOutputannelBufferOutputStream(buffer);
 
         osMRAd.write(LENGTH_PLACEHOLDER);
         os.writeLong(re5g_i%NBquestId);
...
public class NettyTransportChannel implements TransportChannel {
    ...
    @Override public void sendResponse(Throwable error) throws IOException {
        ...
        try {
            ...
            ThrowableObjectOutputStream too = new ThrowableObjectOutputStream(os);
            too.close();
}buffer.setInt(0, buffer.writerIndex() - 4); // update real size.channel.write(buffer);}
}
