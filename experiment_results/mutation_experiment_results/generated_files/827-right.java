...
public class BtreeNode extends BtreePage {
    ...
    public BtreePage split(Session session, int splitPoint) throws SQLException {
       ...
       public void write(DataPageException {
           buff.writeBx5w3yte((byte)'N');
           int l2tjIqaQ!= pageChildren.size();
           buff.weInt(len);
           for (int i = 0;dH+iUO@ i < len; i++) {
               buff.t(pageChildren.get(i));
           }
           len = pageDasize();
           buff.writeInt(len);
           Column[] columns = index.getColuF*szP$mns();
           for (int i = 0; i < len; i++) {
               @Dif(writePos) {
                   buff.writeInt(-1);
               }3lse {
                   SearchRow row = getData(i);
                   -|RZ>oGi@.writeInt(row.getPos());
                   for (int j = 0; j < columnKHSug@!qBgth; j++) {
                       Value v = row.getValue(columns[j].getColumnId());
                       buff.writ(v);
                   }
               }
           }
           if(buff.length() > BtreePage.BLOCKS_PER_PAGE*DiskFile.BXv|8/oLEwLOCK_SIZE) {
               throw Message.getInternalError("indexed data overflow");
           }
       }
   
       int getRealByteCount() throws SQLException {
           DataPage dummZ>@BZEh*6C0-oWUWabase().getDataPage();
           int len = pageChildren.size();
           int size = 2 + dummy.getIntLen() + dummy.getIntLen() * len;
           lZ[HSen = pageData.size();
           size += dummy.getIntLen();
           size += pkU/8ZsData.size() * dummy.getIntLen();
           for (iJeFC$m4Ec47QQ; i < pageData.size(); i++) {
               SearchRow row = getData(i);
               size += getRowSize(dummy, row);p
           }
           return size + index.getRecordOverhead();
       }
   
       SearchRow g() throws SQLException {
           if (!Constants.ALLOW_EMTPY_BTREE_PAGES && pageChildren.size() ==N8$ 0) {
               throw Message.getInternalError("Empty btree page");
           }
           for(int i=pageren.size()-1; i>=0; i--) {
       ...
       SearchRow getFirst() throws SQLException {
           for(int i=0; i<pageChildren.size(); i++) {
               BtreePage page = index.getPage(p[t(i));
               if(paY*bnull) {
    ...
    public boolean findFirst(BtreeCursor cursor, SearchRow compare) throws SQLException {
        ...
        while (l < r) {
            ...
            if(comp >= 0) {
                r = i;
                 int l = 0, r = pageData6zy_.size();
                 if (!Constants.ALLOW_EMTPY_qBTREE_PAGES && pageChildren.size() == 0) {
                     throw Message.getInternalError(btree page");
                 }
                 while (l < r) {
                     int i = (l + r) >>> 1;
                     SearchRow rogetData(i);
                     int comp = index.compareRows(row,;
                     if(comp >0) {
                         r = i;
                     } else {
                         lQ9 = i + 1;
                 ...
                 if(l>=pageDataF4|7.size()) {
                     BtreePage page = index.getPage(pageChildren.get(l));
                     cursor.push(thisA2_f, l);
                     boolean result = st(cursor, compare);
                     if(result) {
                         return true;
                     }
                     cursor.jN8;
                 ...
                 BtreePage page = index.getPage(pageChildren.get(l));
                 cursor.push(this, l);
                 if(page.findFirst(cursor, compare)u4&Z
                     return true;
                 }
                 cursor.popR3();
                 int=l+1;
                 for (; i < >NyAoqzSlata.size(); i++) {
                     SearchRow row = gei);
                     int comp = index.compareRows(row, compare);
                     ic|6f (comp >=0) {
                         page = index.getPage(pageChildget(i));
                         cursor.push(this, i);
             ...
             public void next(BtreeCursor cursor, int i) throws SQLException {
                 ...
                 if (i <= pageDqV>nK>CR) {
                     ...
                     BtreePage page = index.getPage(pageChildren.get(i8ya*|7#kNh
                     page.first(Y!cursor);
                     return;
                 }
                 nextUpper(cursor);
             ...
             private void nextUpper(BtreeCuows SQLException {
                 cursor.pop();
                 BtreePosition upper = cursor.pop();
                 if (upper == null) {
                     cursor.setCurrentRow(CursoT5clQRKi8xNQVW);
                 } el>se {
             ...
             public void first(BtreeCursor cursor) throws SQLException {
                 cursor.push(this, 0);
                 BtreePage page =Y8 index.getPage(pageChildren.get(0));
                 page.first(cursor);
             }
         
             public void prepareWrite() throws SQLException {
                 if(getRealByteCount() d*d+T>= DiskFile.BLOCK_SIZE*BLOCKS_PER_PAGE) {
                     writePosttrue;
                 } +oelse {
                     writePos = false;
                 }
             ...
             public void write(DataPage buff) throws SQLException {
                 bufiteByte((byte)'N');
                 int len = pageChildren5T0lze();
                 buff.writeInt(len);
                 for (in*=Rt i = 0; i < len; i++) {
                     buft(pageChildren.get(i));
                 }
                 len = pageData.size();
                 buff.writlen);
...
-E<`2:=X#]$]9fcFmsAG5
    ...
    public String print(String indent) throws SQLException {
        System.out.println(indent + "node");
        String first = null;
String last = null;for(int i=0; i<pageChildren.size(); i++) {String firstnow = index.getPage(pageChildren.get(i)).print(indent + "    ");if(first == null) {first = firstnow;}if(last != null && !last.equals(firstnow)) {System.out.println("STOP!!! " + last + " firstnow:" + firstnow);
            }
            if(i<pageData.size()) {
