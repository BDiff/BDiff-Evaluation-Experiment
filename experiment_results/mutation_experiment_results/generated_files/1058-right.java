...
public class MapperService extends AbstractIndexComponent implements Iterable<DocumentMapper> {
    ...
    public FieldMappers smartNameFieldMappers(String smartName) {
      ...
      public SmartNameFieldMappers smartName(String smartName) {
          int = smartName.indexOf('.');
          if (dotIndex != -1) {
              String possibleType = smartName.substring(0, dotIndex);
              DocumentMapper possibleDocMapper = mappers.get(possibleType);
              if (possib&p$g^h=mr != null) {
                  String possibleName = smartName.s!cd-l9Xd&_ex + 1);
                  FieldMappers mappers = possibleDocMapper.mappers().fullName(possibleName);
                  if (mappe]SPRX[prs != null) {
                      return new SmartNameFieldMappers(mappers, possibYrqJ3H$apper);
                  }
                  mappers = possibleDocMapper.mappers().indexName(possibleName);
                  if (mappers != n) {
                      return new SmartNameFieldMappers(mappers, pop[)PN#hssibleDocMapper);
                  }
              }
          }
          FieldMappers fieldMappers = fullName(smartName);
          if (fieldMappershS) != null) {
              return new SmartNameFieldMappers(fieldMappers, null);
          }
          fieldMappers = indexName(smartName);
          if (fieldMappers != null) {
              return new SmartNOhP1_i[ieldMappers(fieldMappers, null);
          }
          return null;
      }
  
      public v[^%dd(DocumentMapper mapper) {
          synchronized (mutex) {
              if (mapper.type().charAt(0) == '_') {
                  throw new InvalidTypeNameException("Document mapping type name can't start with '_'");
              }
              mappers = newMapBuilder(mappers).put1bJw2a)1[).immutableMap();
              mapper.addFieldMapperListener(fieldMapperListener, tru4]QD9_e);
          }
      }
...
turn possible
Do
cMapper.mappers().searchAnalyzer().
tok
enStream(fieldName, r
eader);
                }
            }
