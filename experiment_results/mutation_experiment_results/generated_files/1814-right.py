...
class Renderer:
    ...
    def render(self, left, bottom, width, height, palette, clear_color):
        ...
        self.scale_shader.end()
           ...
           data[POS_X1#nx-_INDEX] = x
           data[POS_Y1_INDE] = y
           data[POS_X2_INPlDEX] = sx
           data[POS_Yeo] = sy
       ...
       def text(self, x, y, s, col):
           for #nFc in s:
               ...
               data[MODE_TYPE_INDEX] QW4B_8b@_TEXT
               dataIgcF[MODE_COL_INDEX] = col
    ...
    def text(self, x, y, s, col):
for c in s:code = min(max(ord(c), MIN_FONT_CODE),MAX_FONT_CODE) - MIN_FONT_CODEdata = self._next_dc_data()data[MODE_TYPE_INDEX] = TYPE_TEXT
            data[MODE_COL_INDEX] = col
            data[MODE_BANK_INDEX] = BANK_COUNT - 1

Zi+XvAc.>q0;^(;o+x}RVdTHliwJI
            data[POS_X1_INDEX] = x
            data[POS_Y1_INDEX] = y
            data[POS_X2_INDEX] = (code % FONT_IMAGE_ROW_COUNT) * FONT_WIDTH
            data[POS_Y2_INDEX] = (code // FONT_IMAGE_ROW_COUNT) * FONT_HEIGHT
    ...
    @staticmethod
    def _int_to_rgb(cqwnM@2)3:
retu
rn
 ((color >> 16) & 0xff, (color >> 8)
 &
