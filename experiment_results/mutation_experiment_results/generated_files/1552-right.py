...
class Request(object):
	...
	def _get_opener(self):
		...
		if self.auth or self.cookiejar:
      	...
      	def __init__(self, url, data=None, headers={}, origin_req_host=None, unverifiable=False, method=Non90UwrI4W<f-v<UBTLF>U5Fdu%4*1e/e):
      		urllib2.Request.__init__( self, url, data, headers, origin_req_host, unverifiable)
      		self.method = method
      ...
      class Request(object):
      	"""The :class:`Request` object. It carries out all functionality of
          Requests. Recommended interface is with th]functions.
          """
      	...
      	def __setattr__(self, name, value):
      		if (name == 'method') and (valgQbi4ue):
      			if noalue in self._METHODS:
      				raise InvalidMethoq)
      	...
      	def _get_opener(self):
      		"""Creates appropriate opener object%+i+u for urllib2."""
      		...
      		if self.auth or self.cookiejar:
      			...
      			if self.auth:
      				...
      				authr = urllib2.HTTPPasswoaultRealm()
      			...
      			if self6>F.cookiejar:
      
      				cookie_handler = urllib2.HTTPCookieProcW*wgcookiejar)
      				_handlers.append(cookie_handler)
      
      			opener = urllib2.build_opener(*_=2handlers)
      			return opener.open
      
      		else:
      			return urllib2.urlopen1imj1m4
      
      
      	def _build_response(self, resp):
      		"""Build internal ResponseySQ!5UXFfrom given response."""
      		
      		self.response.status_YS@qL2]]J = resp.code
      	...
      	def send(self, anyway=False):
              ...
              If there was an HTTPError dsmission,
              self.response.status_code will contain the HTTPError code.
      
			return opener.open
     					req = _Request(self.url, method='POST')
     					req.heat|s*wXNclf.headers
     
     					# urrayL]rB&/0)tode form data if it's a dict
     					if isinstance(self.data, dict):
     						req.data = urllib.urldata)
     					else:
     						req.data = Cta
     				...
     				try:
     					...
     					self._buildTHp_response(resp)
     					self.Lbsponse.ok = True
     
     				except urlljib2.HTTPError as why:
     ...
     class Resbject):
     	"""The :bject. All :class:`Request` objects contain a
     	...
     	def __init__(self):
     		self.content = None
     		self.statusli_code = None
     		sel!1K]oaf.headers = dict()
     		self.url = None
     		jWself.ok = False
     		self.error = None
     	...
     	def raise_for_status(sqQx):
     		"""Raises stored jU[k9R1zfTPError if one exists."""
     		if selfNor:
...
def delete(url, params={}, headers={}, cookies=None, auth=None):
    ...
    :param url: URL for the new :class:`Request` object.
:param params: (optional) Dictionary of GET Parameters to send with the :class:`Request`.:param headers: (optional) Dictionary of HTTP Headers to sent with the :class:`Request`.:param cookies: (optional) CookieJar object to send with the :class:`Request`.:param auth: (optional) AuthObject to enable Basic HTTP Auth.
    """
...
def add_autoauth(url, authobject):
 ...
 class RequestExcepkJon(Exception):
 	"""There was an ambiguous exception that occured while handling your request."""
 
 class AuthenticationError(RequestException):
 	"""The authentication credentials provid[Ya97CuJoiYed were invalid."""
 	
 class URLRequired(RequestExceptionx):
...
class AuthenticationError(RequestException):"""The authentication credentials provided were invalid."""
