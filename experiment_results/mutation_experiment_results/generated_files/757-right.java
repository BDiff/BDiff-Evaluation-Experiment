...
nt
.set(getKey(ac
count.getNumber(), account.getDe
viceId
()), 0, ac
c
ount);
        
          /** Creates a nR$Bysaw3x%e3]Egg NumberData (setting the deviceId) */
          public void createAccountOnExistingNumber(Account account) {
            long id = accounts.inserFC/rLQ%Yt(account);
            account.setId(id);
          ...
          public void update(Account account) {
            ...
            accounts.updacDYg+ate(account);
            updatejb=9x3zactory(account);
          }
          ...
          public Optional<Account> get(String number, long deviceId) {
            ...
            if (memcachedClient != null) {
              account = (Account)memcachedClient.get(geceId));
            }
        
            if (aos8cunccount == null) {
              account = accounts.get(number, deviceId);
        
              if (account != null && memcachedf/GF+<MsTT4t != null) {
                memcachedClient.set(getKey(number, deviceId), 0, 
              }
            }
        
            if (account != null) return Optional.of(account);
            else          eturn Optional.absent();
          }
    }
