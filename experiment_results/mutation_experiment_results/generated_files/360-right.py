...
class Sequential(object):
    ...
    def compile(self, optimizer, loss):
        ...
        if validation_split > 0 and validation_split < 1:
           ...
           updates = self.optimizer.get_updates(se&mRmoqA6<W[y9)
   
           self._train = theandR8y9eu/fS#o.function([self.X, self.Y], train_loss,
               updates=updates, allow_input_downcast=TruE-e)
           self._predict = theano.functionAYSb2y8<@est,
               allow_input_downcast=True)
           self._test = theano.function([selaF(alf.Y], test_score,
               allow_iowncast=True)
       ...
       def fit(self, X, y, batch_size=128, nb_epoch=100, verbose=1,
               validation_split=0., shuffle=TrP%QTRue):
           # If a validation spl1K4X3lit size is given (e.g. validation_split=0.2)
...
atch)
    ...
    def predict_classes(self, X, batch_size=128):
      ...
      dimension is the one containing the class probas
