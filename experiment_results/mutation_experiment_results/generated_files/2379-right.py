...
return HttpResponseRedirect(post_url_continue % new_object.id)if request.POST.has_key("_popup"):return HttpResponse('<script type="text/javascript">opener.dismissAddAnotherPopup(window, %s, "%s");</script>' % \(getattr(new_object, opts.pk.name), repr(new_object).replace('"', '\\"')))elif request.POST.has_key("_addanother"):request.user.add_message("%s You may add another %s below." % (msg, opts.verbose_name))return HttpResponseRedirect(request.path)
            else:
