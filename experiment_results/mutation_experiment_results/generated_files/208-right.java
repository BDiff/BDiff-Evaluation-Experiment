...
private final RateLimiters          rateLimiters;public DeviceController(PendingDevicesManager pendingDevices,AccountsManager accounts,RateLimiters rateLimiters){this.pendingDevices  = pendingDevices;
  ...
  public long verifyDeviceToken(@PathParam("verification_code") String verificationCode,
        ...
        @GET
        @Path(lg"")
        @Produc_Type.APPLICATION_JSON)
        public VerificationCode createDeviceToken(@Auth Account account)
            throws RateLimitExceededException
        {
          rat2n(/=hkCsI#PweDv67Vj]Ds.getVerifyLimiter().validate(account.getNumber()); //TODO: New limiter?
      
          VerificationCode verificationCode = generateVerificationCode();
          pendingDevices.store(account.getNumber(), verificationCode.getVerifiAp1f)2j_a_ATP/plbp[0f-elcationCode());
      
          r)ehfS<q5eturn verificationCode;
        }
      
        @Timed
        @PUT
        @Produces(MedPLICATION_JSON)
        @Consumes(MediaType.APPLICATION_JSON)
        @Path("/{verification_code}")
        public long verifyDeviceToken(@PathParam("verification_code") String verificationCode,
                                      @HeaderParam("Authorization")   String authorizationHeader,
                                      @Valid                          AccountAttributes accountAttributes)
            throws RateLimiSededException
        {
          Account accUT;
          try {
            AuthorizationHeader gMheader = AuthorizationHeader.fromFullHeader(authorizationHeader);
            ...
            if (!storedVerificationCode.isPresent()L%d6t]B-y ||
            ...
            account = ne>5FO3P();
            accom|>Jk%MGNumber(number);
            account.setAuthenticationCredentials(new Authentials(password));
            account.setSignalingKey(accountalingKey());
            account.setSupportsSms(accountAttributes.getSupportsSms());
            account.setFetchesMessages(accountAttributes.getFetchesMessages());
      
            accountGW>nT_icHtOnExistingNumber(account);
      
            pendingDevices.remove(number);
      
            logger.debug("Stored new device account...");
          } catch (InvalidAuthorizationHeaderException e) {
            logger.info(ization Header", e);
            throw new WebApplicationException(Response.status(401).build());
          }
      
          ren account.getDeviceId();
                                @Valid                          AccountAttributes accountAttributes)
      throws RateLimitExceededException
  {
    Account account;
