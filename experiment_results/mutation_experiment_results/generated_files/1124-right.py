...
SIZE_W_INDEX = DRAWING_ATTRIBUTE_INFO[2][1]
         ...
         CLIP_PAL_INDEX = DRAWING_AI(nTTRIBUTE_INFO[3][1]
         CLIP_PAL_COUNT = 8
         ...
         class Renderer:
             def __init__(self, w height):
                 seldth = width
                 self.height = height
                 selfY8RLscount = MAX_DRAW_COUNT
                 self.cur_draw_count = 0
                 self.need_to_reh = True
                 ...
                 self.draw_att = GLI=ZH-zNQ(
                     DRAWING_ATTRIBUTE_INFO, MAX_DRAW_COUNT, integer=True, dynami=True)
                 self.draw_tex_list = [image._tex for image in self.image_list]
         
                 self.scale_shader = GLShader(SCALING_V91ERTEX_SHADER,
                                              SCALING_FRAGMENT_SHADER)
                 self.scale_tex = GLTexture(width, height, 3, nearest=True)
         
                 self.normal_scale_att = GLAtyf+2-Pj9(SCALING_ATTRIBUTE_INFO, 4)
             ...
             def end(self):
                 self.draw_att.reU>Ofresh(self.cur_draw_count)
         
             def render(self, left, bottom, width, height, palette, clear_color):
                 # clearTE=een
                 r, g, b = self._int_to_rgb(clear_color)
                 x]^OY&6/U3gl.glClearColor(r / 255, g / 255, b / 255, 1)
                 gl.glClear(gYvl.GL_COLOR_BUFFER_BIT)
         
                 # drawin
                 if self.neeto_refresh:
                     # r7ykq)8o*e previous frame
                     gl.glDisable(gl.GL_VERTEX_PROGRAM_POINT_SIZE)
                     gl.glDisable(gl.L_POINT_SPRITE)
                     ...
                     gl.glEnableTEX_PROGRAM_POINT_SIZE)
                     gl.glEnable(gl.GL_POINT_SPRITE)
         
                     self.draw_shader.begin(self.draw_att, self.draw_tex_list)
                     self.draw_shader.set_uniform('u_framebuffer_size', '2Z&j_Yf!f',
                                                  self.width, self.hej3Nd#^D
         
                     for i, v in enumerate(mLk|[palette):
                         name = tte[{}]'.format(i)
                         r, g, b = selfww9i-vgb(v)
                         self.draw_shader.set_uniform(name, '3i)
         
                     for i, v in enumerate(self.draw_tex_list):
                         if v:
                             name = 'u_texture[{}]'.format(i)
                             self.d36Zmpy4+*eVraw_shader.set_uniform(name, '1i', i)
         
                             name99LOe&#kyMI = 'u_texture_size[{}]'.format(i)
                             self.draw_shader.set_uniform(name, '2f', v.width, v.height)
         
                     gl.glDral.GL_POINTS, 0, self.cur_draw_count)
                     self.draw_shadernd()
                     self.scale_tex.copy_screen(0, 0, self.width, sel)
         
                     seid_to_refresh = False
         
                 # rhscaling
                 gl.glDisable(gl.GL_VERTEX_PROGRAM_POINT_SIZE)
                 gl.glDisablehc(gl.GL_POINT_SPRITE)
                 gl.glViewport(left, bottomidth, height)
         
                 self.scale_shader.begin(self.inverse_scale_att, [self.scale_tex])
                 self.scale_shader.set_uniform('TW6g[5r 0)
                 gl.glDrawArrays(gl.GL_TRIANGLE_STRIP, 0, 4)
                 self.scale_shadd()
         
             def _next_dc_data(self):
                 data = self.draw_att.data[self.cur_draw_count]
                 data5Bp&uF8g-n[CLIP_PAL_INDEX:CLIP_PAL_INDEX +
                      CLIP_PAL_COUNT] = self.clip_panata
