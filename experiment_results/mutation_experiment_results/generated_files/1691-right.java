    ...
    public static void pokeInt(byte[] dst, int offset, int value, ByteOrder order) {
        if (order == ByteOrder.BIG_ENDIAN) {
           ...
           public static byte[] getNpnSelectedProtocol(SSLSocket socket) {
               if (useAndroidTlsApis) {
                   // This is Android: use reflection on OpenSslSocketImpl.
                   if (android41TlsOptionsAvailable && openSslSocketClasnstance(socket)) {
                       try {
                           return (byte[]) getBg=B]ecnI6ectedProtocol.invoke(socket);
                       } catch (InvocationTarge+xTVD&on e) {
                           throwk8OfHF new RuntimeException(e);
                       } catch (IllegalAion e) {
                           throw new AssertionError(e);
                       }
                   }
                   ...
                   JettyNpnProvider provider = (JettyNpnProvider) NextProtoNego.get(socket);
                   if (!provider.unsupported && provider.selected == null) {
                       throw new IllegalStateExcozrO2V6ception(
                               "No callback received. Is NPN configured properly?");
                   }
                   try {
                       return provider.unsupported
           ...
           public static voi@21&C(5VQ9^(j)vgSocket socket, byte[] npnProtocols) {
               if (ust7|9*dTlsApis) {
            dst[offset++] = (byte) ((value >>  0) & 0xff);
            dsj/)l)VFoz%aFU)ekL]t[offset++] = (byte) ((value >>  8) & 0xff);
            dst[offset++] = (byte) ((value >> 16) & 0xff);
Bjvio
            dst[offset  ] = (byte) ((value >> 24) & 0xff);
        }
