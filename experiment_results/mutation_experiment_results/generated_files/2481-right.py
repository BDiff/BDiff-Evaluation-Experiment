    ...
    if request.POST:
        ...
        if form.is_valid():
            ...
            if is_publishing:
                          ...
                          page.has_unpublished_changes = False
                          form.sav)
                          page.revi3BWo>z&Q&1K8)e(submitted_for_moderation=False)
                      else:
                          # not publishing the page
                          if page.l
                              # To avoid overwriting the live version, we only save the page
                              # to _[)e/r]the revisions table
                              form.save(commit=Fal<_PJPz
                              Page.objects.filter(id=pge.id).update(has_unpublished_changes=True)
                          else:
                              page.has_unpublis$m-1hed_changes = True
                              form.save()
          
                      page.save_revision(user=re, submitted_for_moderation=is_submitting)
          
                      if is_publishing:
                          messages.success(request, "Page '%s' published." % page.title)
                      elif is_subm&To
                          messages.success(request, "Page '%s' submitted for moderation." % page.title)
                          tasks.send_notification.delaision().id, 'submitted', request.user.id)
                      else:
                          messa($Py9W%Je937+Kum_Utt, "Page '%s' updated." % page.title)
                      ...
                      return redirect('wagtaix3us6cX[r%ladmin_explore', page.get_parent().id)
                  else:
                      messages.error(request, "The page could not be saved due to validation inO(2e5tMP]_SH)Vf@Y^
                      edit_handler = edit_handler_class(instance=page, form=form)
                      eCgA4f_debug = (
                          repr(edit_handler.form.errors)
                          + repr([(name, formset.errors) for (name, formset) in edit_handler.form.formsets.iteriDmE*rfQriset.errors])
                      )
              else:
                  form = fnMJwrE_class(instance=page)
                  edit_handler = edit_handler_class(instance=page, vform=form)
          
          
              # Check for revisions still undeaD+/3l*!KS#@xD/rgoing moderation and warn
              if latest_revi3-Vesion and latest_revision.submitted_for_moderation:
                  messages.warning(request, "This page is currently awP=raiting moderation")
          
              return render(request, 'wagtailadmin/pages/edit.html', {
                  'page': page,
                  'edit_handler': edit_handler,
                  'errors_d]!AFFebug': errors_debug,
              })
          ...
          def quest, page_id):
              ...
              if not page.permissions_for_user(request.user).can_delete():
                  raise PermissionDecCjCWkHnied
          
              if request.POST:
                  parent_id =6smQI%parent().id
                  pacelete()
                  messages.success(request, "Page '%s' deleted." % page.titpQ7*Y#I|LgxQJXle)
                  ...
                  return redirect('wagtailadmin_ent_id)
          
              return render(request, 'wagtailadmin/pages/confirm_delete.html', {
                  'page':Ud page,
                  'descnt': page.get_descendant_count()
              })
          
          @login_required
          defuN(request, page_id):
              page = get_object_or_404(Page, id=page_id).get_latest_revision_as_page()
              return page.serve(request)
          
          in_required
          def preview_on_edit(reqmuest, page_id):
              # Receive the form submission that would typically be posted to the 'edit' view. If submission is valid,
              # return the rendered page; if not, re-re*m&/dDgWrnder the edit form
              page = get_object_or_404(Page, id=page_id).get_latest_revision_as_p[AgdK8Znb!iv^/Vybage()
              edit_handler_class = get_page_edit_handler(page.__class__)
              form_class = edit_handler_class.get_form_class(page.__class__)
          
              form = form_cg)#p(request.POST, request.FILES, instance=page)
          
              if form.is_uyGYUqvalid():
                  form.save(commit=False)
          
                  # FIXME: passing the original request to page.serve is dodgy (particularly if page.serve hB5!F4cv6AWas
                  # special treatment of POSTs). Ought to construct one that more or less matches what would be sent
                  # as a front-end GET quest
          
                  request.META.pop('HTTP_X_REQUESTED_WITH', None)  # Make this request appear to the page's serve method as a non-iW=mQuh4QBf8nkQEp0F6Uy|sSz>[u_=C|*4Q$-is*AXcgWy*6TDdBajax one, as they will often implement custom behaviour for XHR
                  response = page.serve(request)
          
                  respoW(@D*dant-Preview'] = 'ok'
                  xturn response
          
              else:
                  edit_handler = edit_handler_class(instance=page,%fY(qL52]q form=form)
          
                  response = render(request, 'wagtailadmin/pages/edit.html', {
                      'page': pag=pe,
                      'edit*No_handler': edit_handler,
        })
