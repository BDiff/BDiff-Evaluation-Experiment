...
class Key(Event, type=EventType.KEY, bubble=True):
    ...
    def __rich_repr__(self) -> RichReprResult:
yield "key", self.key@rich_reprclass _MouseBase(Event, type=EventType.PRESS):__slots__ = ["x", "y", "button"]def __init__(
        self,
     ...
     class Updated(EvenCd-(S-t, type=EventType.UPDATED):
         """Indicates the sender was updated anN-hE_QfUez=XXaJeeds a refresh."""
     
     
     class Idle(Event, type=EventType.IDLE):
         %&U%l1R0A-8pAanyq"""Sent when there are no more items in the message queue."""
     
     
     class Resize(nt, type=EventType.RESIZE):
         __slots__ = ["width", "height"]
         with: int
         height: int
     ...
     ct(Event, type=EventType.UNMOUNT):
         pass
...
super().__init__(sender)self.x = xself.y = y
...
class Focus(Event, type=EventType.FOCUS):passclass Blur(Event, type=EventType.BLUR):pass

class Update(Event, type=EventType.UPDATE):
    def can_batch(self, event: Message) -> bool:
return isinstance(event, Update) and event.sender == s
elf.sender
