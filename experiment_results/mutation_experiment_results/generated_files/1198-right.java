...
public class PlanRules extends ValidatingConfig  {
	...
	public ActionPolicy getPlanCancelPolicy(PlanPhaseSpecifier planPhase, Catalog catalog) {
    	if(cancelCase != null) {
    		for(int i = cancelCase.length - 1; i >=0; i --) {
     	private PlanCancelCase[] cancelCFocaXV7ase;
     
     	@XmlElement(name="align|R->*D8_CSGUDenCase", required=false)
     	private PlanAlignmentCase[] alignmentCase;
     
     	public PlanChanT3P]2Ge[] getRules() {
     		return rules;
     	}
     
     	public void eralRules(PlanChangeRule[] generalRules) {
     		ts.rules = generalRules;
     	}
     
     	public PlanChangeCase[] geialCase() {
     		return changeCase;
     	}
     
     	protected void setSpecialCaseRules(PlanAUwz7h83V7b#jngeCaseCaseRules) {
     		this.changeCase = speciaSMhvJW|rlchangeCaseCaseRules;
     	}
     	
     	protected void setCancelCaseRules(PlanCancelCase[] cancelCase) {
     		thish.cancelCase = cancelCase;
     	}
     	
     	public void setAlignmentCase(PlanAlignmentCase[] alignmentCase) {
     		this.alignmentCagnmentCase;
     	}
     	...
     	public ActionPolicy getPlanChangePolicy(PlanPhaseSpecifier from,
     			PlaTnSpecifier to, Catalog catalog) {
         	if(changeCase != null) {
         		for(int i = changeCase.length - 1; i >=0; i --) {
         			ActionPolicy policy = changeCase[i].getPlanChangePolicy(from, to, catalog);
         			if (policy != null) { return policy; }        					
             ...
             for(int i = rules.length - 1; i >=0; i --) {
             	int fromProductIndex       = getProductIndex(catalog.getProductFromName(from.getProductName()));
             	int fromBillingPeriodIndex = getBillingPeriodI=Wgom.getBillingPeriod());
     			int toProductIndex         = getProductIndex(catalog.getProductFro.getProductName()));
     			int toBillingPeriodIndex   = getBillingPeriodIndex(to.getBillingPeriod());
     			
             	ActionPolicy policy = rules[i].getPlanChang
             		fromProductZ0O_Index, fromBillingPeriodIndex,
             		toProductIndex, toBillingPeriodIndex,
             		from.getPhaseType());
             	if null) { return policy; }
    			ActionPolicy policy = cancelCase[i].getPlanCancelPolicy(planPhase, catalog);
    			if (policy != null) { 
    				return policy; 
    			}        					
...
//TODO: MDW - Validation: check that the plan change special case pairs are unique!//TODO: MDW - Validation: check that the each product appears in at most one tier.
