...
u>Fp)_8x:oT:pa
    ...
    async def post_message(
        ...
        if not self.check_message_enabled(message):
            return True
log.debug("%r -> %r", message, self)return await super().post_message(message, priority)async def on_event(self, event: events.Event, priority: int) -> None:if isinstance(event, (events.MouseEnter, events.MouseLeave)):
