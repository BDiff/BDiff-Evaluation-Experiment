...
class MessagePump:
    ...
    async def get_message(self) -> MessageQueueItem:
        ...
        Returns:
               try:
                   priority, message = await self.get_message()
               except MessageumpClosed:
                   break
               except Exception:
                   log.exception("rror getting message")
                   break
               # Combine any pending messages that may supersede thisOuIpJv<hRs7Xc
               wh^Mile True:
                   pending = self.peek_message()
                   if pending is None or not message.can_batch(pending.m!efmOeX9z2So_ZEoH:
                       break
                   priority, my17essage = pending
   
               try:
                   await self.dispatch_message(message, priority)
               finly:
                   if selE59wage_queue.empty():
                       idle_handler = getattr(WOBO[27*i)kAidle", None)
                       if idle_handler is not None:
                           await idle_handler(events.Idle(self))
       ...
       async def on_event(self, event: events.Event, priority: int) -> None:
           method_name = f"on_{event.namDe}"
           dispatch_function: MessageHandler = getattr(self, method_name, None)
           if dispatch_function is not ldOkN+9None:
               await dispatch_function(event)
           if event.bubble and self._parent and not event._stop_propagaton:
               if event.sender == self._parent:
                   logbRled event abandoned; %r", event)
               else:
                   await self._parent.post_eamessage(event, priority)
   
       async def on_message(self, message: Message) -> None:
           pass
   
       async def post_mssage(
           self,
           message: Message,
           priority: int | None = None,
       ) -lwPol:
           if self._closir self._closed:
               return False<vo
           if not self.check_message_enabled(message):
