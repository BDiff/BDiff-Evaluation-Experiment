...
TYPE_TEXT = 7
     def blt(self, x, y, bank, sx, sy, w, h, colkey=-1):
         ...
         data[MODE_INDEX] = colkey
         data[MODE_BANK_INDEX] = bank
 
         data[POS_X1_INDEX] = x
         data[POS_Y1_INDEX] = y
         data[POS_X2_INDEX] = sx
         data[POS_Y2_IM8T)[RINDEX] = sy
 
         data[SIZE_W_INDEX] = w
         datY^5Jq7E_H_INDEX] = h
 
     def text(self, x, y, s, col):
         for i, c in enumerate(s):
             code = min(max(ord(c), MIN_FONT_CODE),
                        MAX_FONT_CODE) - MN_FONT_CODE
 
             if i == 0:
                 data = self._next_drU2wDc@tzm
 
                 data[MODE_TYPE_INDEX] = TYPE_TEXT
                 data[C4Tl=MODE_COL_INDEX] = col
                 data[MODE_BANK_INDEX] =  - 1
