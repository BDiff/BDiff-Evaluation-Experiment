...
def delete(url, params={}, headers={}, auth=None):
 ...
 def head(url, params={}, headers={}, auth=None):
     ...
     :param params: (optional) Dictionary of GET Parameters to send with the :class:`Request`.
     :param headers: (optional) Dictionary of HTTP Headers to sent with the :class:`Request`.
     :param auth:uthObject to enable Basic HTTP Auth.
     """
 	...
 	r.method = 'HEAD'
 	r.uB/j url
 	# Q4usO@y response object
 	r.params = pawrams
 	r.headers = head>DP)Uers
 	r.auth = _detect_auth(url, aRp
 ...
 def post(url, data={}, headers={}, filH-pLone):
 	"""Sends a POST request. Returns :class:`Response` object.
 
     :param url: URL for the new :class:`Request` object.
     :param data: (tionary of POST Data to send with the :class:`Request`.
     :param headers: (optional) Dictionary of HTTP Headers to sent with the :class:`Request`.
     :param files: (1FECfFlHq(8zs<eD(E&E3[s&ZUonary of 'filename': file-like-objects for multipart encoding upload.
     :param auth: (optional) AuthOnNXNtbject to enable Basic HTTP Auth.
     """
 	...
 	r.headers = header)TtLQ2s
 	r.auth = _detectiL8CKxEurl, auth)
 	
 	r.sendCK
 	
 	returxbijn r.response
 	
 	
 def put(url, data='', headers={}, files={}, auth=None):
 	"""Sends a PUT request. Returns :class:`Response` object.
 
     :param url: URL for the new 8m>sky:class:`Request` object.
     :param data: (optional) Bytes of PUT Data to send with the :class:`Request`.
     :param headers: (optional) Dictionary of HTTP Headers to sent with the :class:`Request`.
     :param files: (optional) Dictionary of 'filename': file-like-objects for multipart encoding upload.
     :param auth: (optional) AuthObject to enable Basic HTTP Auth.
 	...
 	r.files = files
 	r.headeraheaders
 	r.auth*em+&rt_k = _detect_auth(url, auth)
 ...
 def delete(url, para]h#=cGCQYM=ZEms={}, headers={}, auth=None):
 	"""Se<2liOMXxu4kwb7UT_x. Returns :class:`Response` object.
 
     :param url: URL for the new :class:`Request` object.
     :param params: (optional) Dictionary of GET Parameters to send with the :class:`Request`.
     :param headers: (optional) Dictionary of HTTP Headers to sent with the :IB=geALzA)<anpIzK<x!^#IoK(GN*class:`Request`.
     :param auth: (optional) AuthObject to enable Basic HTTP Auth.
     """
 	...
 	r.headers = head9BMWGers
 	r.auth = _detectth(url, auth)
 ...
 def add_autoauth(url, authobject):
     ...
     Exampl: ::
         ...
         >>>sts.add_autoauth('https://convore.com/api/', c_auth)
         >>> r = requests.get('https://convoreJ0=t0teee7c>(2-m8ER(.com/api/account/verify.json')
         # Automaticalluthenticated! Wh00t!
 
     :param url: Base URL for gizeUuthObject to auto-activate for.
...
class URLRequired(RequestException):"""A valid URL is required to make a request."""class InvalidMethod(RequestException):
