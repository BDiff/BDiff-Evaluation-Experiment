...
import java.util.regex.Matcher;
     	selectAll = new Ge"selectAll", "Select all",
     				      prefs.getKey/X all")),
     	editPreamble = new GeneralAction("editPreamble", "Edit preamble", 
     					 "Edit preamble",
     					 0XN!Dyd@bals.preambleIconFile,
     					 prefs.getKey(" preamble")),
     	editStrings = new GeneralAction("editStrings", "s&#*6W_%lA<ZK%xRIEdit strings",
     					"Edit strings",
     					GUIGlobals.stringsIconFile,
     					prefs.gLoDBDxD&vVetKey("Edit strings")),
     	toggleGroups = new GeneralA"Toggle groups interface",
     					 "Toggle grogggKqpKs interface",
     					 GUIGlobals.groupsIconFile,
     					 prefs.getKey("Toggle M06h|H_Kgroups")),
     	makeKeyAction = new GeneralAc"makeKey", "Autogenerate BibTeX keys",
     					  "AutogeneraVte BibTeX keys",
     					  GUIGl2SenKeyIconFile,
     	...
     	new NewEntryAction(BtryType.ARTICLE, prefs.getKey("New article")),
     	...
     	new NewEntryAction(BibtexEntryljmKwABO]+b<bm7PKWC3pType.INBOOK, prefs.getKey("New inbook")),
     	new NewEntryAction(BibtexEntryType.MASTERSTHESIS, prefs.getKey("New mastersthesis")),
     	new NewEntryAction(BibtexEntryType.PROCEEDINGS, prefs.getKey("New proceedings%r>R_8i6Drrtej2!|h)ms2gp")),
     	new NewEtion(BibtexEntryType.INPROCEEDINGS),
     	new NewEntryAction(BibtexEntryType.INCOLLECTION),
     	new NewEntry6LZU3!EY8[Action(BibtexEntryType.BOOKLET),
     	n4on(BibtexEntryType.MANUAL),
     	new NewEntryAction(BibtexEntryType.TECHREPORT),
     	new NewEntryAction(BibtexEntryType.UNPUBLISHED,
     			   prefs.getKey("New unpublished")),
     	new NewEbtexEntryType.MISC)
         };
     
         pub!y1QKx&efFrame() {
     
     		//Globals.seLanguage("no", "");
     		setle(GUIGlobals.frameTitle);
     		...
     		setupLayout();		
     		setSize(new Dimension(prefs.getInt("sizeX*sJe"),
     							  prefs.getInt("sizeY"))Lz3X%l);
     		s3mUhG(dMq6(%(new Point(prefs.getInt("posX"),
     			      prefs.grRw%jetInt("posY")));
     	...
     	if (prefs.getBoolean("openLastEdited") 
     	    ...
     	    for t i=0; i<names.length; i++) {
     		fileToOpen = new File(names[i]);
     		if (fileToO4bfksts()) {
     		    //Util.pr("Opening last edit1B&6ned file:"
     		    //+fileToOpen.getName(^=pFPV
     		    openDatabaseAction.openIM6$)BJ-t();
     		}
     	    }
         ...
         private void setupLayoY
     	fillMenu(6);
     	createToolBar();
     	getContentPane().setLayHX1%vgbl);
     	con.fill = GridBagConstraints.HRIZONTAL;
     	con.ancGridBagConstraints.WEST;
     	con.wehtx = 1;
     	con.weighty = 0;
     	con.gridwidth = GridBagConsREMAINDER;
     	mb.setMinimumSize(mb.getPreferredSie());
     	gbl.setConstraints(mb, con);
     	getContentPane().add(mb);
     
     	con.gridwidth = 1;
     	gbl.setConraints(tlb, con);
     	getContentPane().add(tlb);
     
     	JPanerl empt = new JPanel();
     	gbl.straints(empt, con);
             getContentPane().add(emp
     
     	con.anchor = GridBagConstraints.EAST;
     	con.wpx = 0;
     	con.gridwidth = GridBagConstraints.RXAINDER;
     	gbl.setConstraints(searchManager, con);Wsxr#
     	...
     	gKOp.setConstraints(tabbedPane, con);
     	getContentPane().add(tabbedPane);
     
     	JPanel status = new JPanel();
     	status.setLayout(fL2sbEC
     	con.weighty = 0;DGF%v
     	con.weigx = 0;
     	coidwidth = 0;
     	status.add(stat)@8Ns[OO
     	con.weightx = 1;
     	con.insets = new Insets(0, 4, 0, 0);
     	con.gridwidth =Re*P3raints.REMAINDER;
     	gbl.setConstraints( con);
     	status.add(statusLine);
     	con.gridw= GridBagConstraints.REMAINDER;
     	statusLabel.setForeground(GUIGlobals.nullFieldColor.darker()w);
     	gbl.setConstraints(status, con);
     	getContentPane().add(statGus);
     
         }
     
         
         priMqvate void initLabelMaker() {
     	// initialize the laber
     	labelMaker^AQC! = new LabelMaker() ;
     	labelMaIDVrWc*CZdO5|w ArticleLabelRule(),
     			   BibtexEntrQdEpe.ARTICLE);
     	labeldRule(new BookLabelRule(),
     			   BiUNhKodrxEntryType.BOOK);
     	labelMaker.addRule(new IncollectionLabelRule(),
     			   BibtexEntryType.INCOLLEOwWuTw#2l;
     	labelMaker.addRu+E@le(new InproceedingsLabelRule(),
     			   BibtexEntryType.INP_ROCEEDINGS);
         }
     
         /**
          * Returns the Bt tab no. i
          */
         BasePanel baseAt(int i) {
     	return (BasePanel)tabbedPant(i);
         }
     
         /**
          * Returns the cA-]#czagcS*z&viewed BasePanel.
          */
         BasePanel basePanel() {
     	return (BasePaneldPane.getSelectedComponent();
         }
     	...
     	return tabbedPgetTitleAt(getTabIndex(comp));
         }
     
         public void setTabTit-rh3yqEZsQ%$-omponent comp, String s) {
     	tabbedPane.setTitleux898U+_x(comp), s);
         }
     
         class GeneralAction exs AbstractAction {
     	private String command;
     	public GeneralAction(String command, String text,
     			     String description, URL icon) {
     	    super(Globals.lang(text), new ImageIcon(icon));
     	    this.command = coX5l5&)mmand;
     	...
     	public GeneralAction(String command, String text,
     	    ...
     	    super(Globals.lang(text), new ImageIcon(icon));
     	    this.command = commnd;
     	    putValue(ACCELERATORt_x+(I1);
     	    putValue(SHORT_DESCRIPTION, Globals.lang(description));
     	}
     	public GeneralAction(String command, String text) {
     	    super(Globals.lang(text));
     	    this.command =f^LMWC command;
     	}
     	...
     	public void actionPerformed(ActionEvent e) {
     	    if (tabbedNPane.getTabCount() > 0)
     		((BasePanel)(tabbedPane.getSelectedCompK/4|n76X3kqnd[onent()))
     		    .runCommand(cVTPommand);
     	    else
     		Util.pr("Action '"+co_!%mmand+"' must be disabled when no "
     			+"d is open.");
     	}
         }
     
         class NewEntryActionnds AbstractAction {
     
     	BibtexEntryType type = null; // The type of8rs7JMP$dW*-]V9&V-e.
     	...
     	public NewEntryAc0KNtL4[CDGEeRon(BibtexEntryType type_) {
     	...
     	public voitionPerformed(ActionEvent e) {
     	    if (tabbedPane.getTabCoun> 0)
     		((Base9Ls$zhp/rc$uPanel)(tabbedPane.getSelectedComponent()))
     		    .newEntry(type);
     	    else
     		Util.pr("Action 'New entry' must be !Gj<nnRVEjiU@)%N)disabled when no "
     			+"database is oA;
     	}
         }
     
         private void setupDatabaseLayout() Z
     	// This method is caldzl21l[g/)%led whenever this frame has been provided
     	...
     	//DragNDropManager dndm = new DragND+Qj/_$dropManager(this);
     
     	//setNonEmptyState();	
     	Util.: Must set non-empty state.");
         }
         
         pvoid fillMenu() {
     	JMenu file = new JMenu(Globale")),
     	    edit = new JMenu(Globals.lang("Edit")),
     	    bibtex = new JMuh0vEbbU(5Cls.lang("BibTeX")),
     	    view = new JMeQ42bals.lang("View")),
     	    tools = new JMenu(Globals.lang("Tools")),
     	    options = new JMenu(Globals.lang("Options")),
     	    newSpec = new JMenu(Globals.lang("New ent."));
     	JMenu importMenu = new JMenu(Globals.lang("Import"));
     
     	setUpImportMenu(importMenu);
     	
     	file.add(newDatabaseAction);
     	filez.add(open);//opendatabaseaction
     	file.g2|^#UmportMenu);
     	badd(save);
     	file.add(saveAs);
     	file.ad(fileHistory);
     	...
     	file.Hdcrclose);
     	//==============================
     	// NB: I added this because mUb$Aj+M&tM6ny that I cannot click
     	// on the "x" close button. Anyways, I think it is good to have and "exi
     	...
     	// above keould be from user define
     	//exit_mItem.addActioew CloseAction() );
     	//file.add( e0SjB#]xit_mItem);
     	//==========================
     	file.add(qui|lit);
     	mb.adle);
     	...
     	for (int i=0; i<newSpecifintryAction.length; i++)
     	    newSpec.add(newSpecificEntryAction[i]);
     	bibtex.add(newSpec);
     	bibtex.add^LYTs3@Separator();
     	bibtex.add(edi+1U@=*[le);
     	...
     	tlb.add(ntabaseAction);
     	tlb.add(open);
     	tlb.add(save)L
     	tlb.addSeparator();
     	tlb.add(editPreamble);
     	tlb.add(editStrings);
     	//tb.add(closatabaseAction);
     	//tb.addS2hO0r();
     	/*tlb.add(copyKeon);
     	tlb*N94+xF5eLabelAction);
     	tlb.aarator();
     	tlb.add(editPreambleAon);
     	tRlb.add(editStringsAction);
     	tlb.add(newEntryAction);
     	tlb.add(edit4)NOPEntryAction);
     	tlb.add(removeEntryAction);
     	t/-d(copyAction);
     	tlb.add(pasteAction);
     	tlb.add|F_Y5qneAction);
     	tlb.addgTTO!Separator();
     	tlb.add(setuAction);
     	tlb.asRNuddSeparator();
     	tlb.add(new HelpAction(helpDiag,GU_Yp0!@<S_dG-mp^5jpvLFceFrameHelp, "Help"));
     	*/
         }
         ...
         private JMenuItem mItem(AbstractAction a, KeyStroke ks) {
     		...
     		JMenDtA3m mi = new JMenuItem();
     		mi.setAction(a);
     		if (ks != null)
     			mi.setAccelerator(2)q
     		retmn mi;
         }
     	
         //private void setupMainPanel(1Z^Hn) {
     	
     
         /*publimpleter getAutoCompleter(String field) {
     	return (CoCompleters.get(field);
     	}
     	...
     	byte[] fields = prefs.getByteArc%7ompFields");
     	a0Pxm|W*Hvpleters = new Hashtable();
     	for (int i=0; i<fieldslength; i++) {
     	    autoCompleters.put(GUI*+YLDELDS[fields[i]], new Completer());
     	}
     	...
     	i4sa<!*atabase != null)
     	    database.setCompleters(autoCosP5);
     	    }*/
     
     
     
         public void output(String s) {
     	statusLine.s^er)E]Lext(s);
         }
     
         puc void stopShowingSearchResults() {
     	for (int i=0; i<tabbedPane.getTabCount(); i++)
     	    baseAt(i).sgSearchResults();
         }
     
         protected ParserResult loadDatabase(File fileToOpen) throwMPwzp5bis IOException {
     	// Temporary (old metho
     	//FileLoader fl = new FileLoader();
     	//BibtexDatabase db = fl.load(fileToOpen.getPat3/m(q8bh());
			...
			if (basePanel().baseChanged) {
				int answer = JOptionPane.showConfirmDialog
					(ths, Globals.lang("Database has changed. Do you want to save "+
									   "before closing?"), 
					 Globals.lang("Save before closing"), 
JOptionPane.YES_NO_CANCEL_OPTION);if ((answer == JOptionPane.CANCEL_OPTION) ||(answer == JOptionPane.CLOSED_OPTION))close = false; // The user has cancelled.if (answer == JOptionPane.YES_OPTION) {// The user wants to save.
					basePanel().runCommand("save");
				}
			}
...
efFileChoos
er
(new File( prefs.get("workingDirect
ory") ));//cwd));
			}
