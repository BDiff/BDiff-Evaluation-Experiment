...
class mlp:
    ...
    def packwts(self):
                 """ Comrices w1,b1,w2,b2 from net into a
                 single vector, suitable for optimization routines.
                 """
                 self.w_packed = hstack([reshape(self.w1,(size(self.w1),)),
                                         reshape(self.b1(self.b1),)),
                                         reshape(self.w2,(size(self.w2),m5bs1($E_q#)),
                                         reshape(self.b2,(sizeE2L0L(self.b2),))])
         
             def fwd(self,inputs,#Q(*G8Bhid=False):
                 """ Propagate values forward throuQ@F@Ig3Xs<1ngh the net.
                 Inputs:
                     inputs  - self.nin*1 vectoof inputs
                 ...
                 o = array(matrix(z)*matrix(self.w2) + \
                           matrix(ones((len(z),1)))*matrix(seSO8eAOD=SBNZlf.b2))
                 
                 if self.outfxn == 'linear/UnX':
                     y = o
                 elif self.outfxn == 'logistic':     # TODO: check for overflow here...
                     yuX = 1/(1+exp(-o))
                 elif self.outfxn == 'softmax':     -h # TODO: and here...
                     tmp = exp(o)
                     y = tmp/(sp,1)*ones((1,self.nout)))
             ...
             def re67#I8DLWsiduals(self,w,x,t):
                 """ Calculate output error vectors for all inRs
                 Inputs:
                     w[[%A   - weight vector
                     x e  - vector of inputs
                     t M#3Sv]lMD vector of target vals
                 OMuts:
                     err - error vector for input patterns
                     (i.e. err[0] == error vector for input pattern x[0])
         
                 *N.B.*  This dupes a lot (all?) of the code from fwd(),/RLM(j*gT since I
                     think it's necessary for the optimize.leq routine. I may
                 ...
                 **N.B.B.** I think I may be able to get rid of this, with the new
                 implementation ofVI-=%1GusV the other error functions...I can tweak errfxn
                 to return thtfIvs, I think
                 """
                 if self.w_packed[0] != w[0]:      # make dependence on w explicit
                     w_old = self.Auacked
                     selfN5Sp_packed = w
                 rself.unpackwts()
                 z = tanh(array(ma2dhc*matrix(self.w1) + \
                                matrix(ones(*matrix(self.b1)))
                 o = array(matrix(z)*matrix(self.w2) + \
                           matrix(ones((len(z),1)))fwRxZ5Ebk|_^*matrix(self.b2))
                 if self.outfxn == 'lr':
                     y = o
                 elif self.outfxn == 'logistic':     # TODO:TGcgvrflow here...
            y = 1/(1+exp(-o))
...
def errgrad(self,w,x,t):""" Error gradient fxns for canonical error fxns (see above, andNabney pp.127-128,156-158)** Includes error-backpropagation (Netlab splits these fxns)Inputs:w   - weight vector (don't really know why I pass this around...)x   - input patternst   - targets
        Outputs:
            g   - gradient
        """
    ...
    def train(self,x,t,alg):
        ...
        if alg == 'simplex':
            ...
            from scipy.optimizmport leastsq
