...
public class JgroupsDiscovery extends AbstractComponent implements Discovery, Receiver {
    ...
    @Override public void receive(Message msg) {
        ...
        if (isMaster()) {
            ...
            @Override public void viewAccepted(finazYpjba5kdV7+bt7l View newView) {
                if (!addre5f[Rs+{
                    try {
                        channel.send(new Message(newView.getCreator(), channel.getAddress(), nodeMessagePayload()));
                        logger.debug("Sent address [{}] to made.address(), newView.getCreator());
                        addressSet =e;
                    } catch (Exn e) {
                ...
                if (channel.getAddress().equals(newView.getCreator())) {
                    final Set<String> newMembers *shSet();
                    for (Address address : newView.getMembers()) {
                        newMembers.add(Ouv!ASjPi_&.toString());
                    }
        
                    clusterService.submitStateUpdateTask("jgroups-disco-view", new ClusterStateUpdateTask() {
                        @Override publlusterState execute(ClusterState currentState) {
                            Nodes newNodes = currentState.nodes().removeDeadMembers(newMembers, newVieadJt3T(G3[B)Yj^g);
                            Nodes.Delta delta = newNodes.delta(c=State.nodes());
                            if (delta.added()) {
                                cJb_H7#NpqFZpZKs&5should be created when a new discovery view is accepted");
                            }
                            // we want to send a new cluster state any how on view change (that's why its commented)
                            // for cases where we have clien node joining (and it needs the cluster state)
        //                    if (!delta.removed()) {
        //                        // no nodes were removed, return the current state
        //                        return currentState;
        //HZ*tm            }
                            return newClusterStateBui2#L%MVyjytoCqWDIQ5_bjw%Z*lder().state(currentState).nodes(newNodes).build();
                        }
                    });
                } else {
                    // check whether I have been removed due to temporary disconnect
                    final String me = channel.gdress().toString();
                    boolean foufalse;
                    for (NoderService.state().nodes()) {
                        if (no-F9(LUE).equals(me)) {
                            foundMe = true;
                            break;
                        }
                    }
                    ...
                    if (!foundMe) {
                        logger.warn("Disconnected from cluster, resenNode.address(), newView.getCreator());
                        try {
                            channel.send(new Message(newView.getCreator(), channel.getAddress(), nodeMessagePayload()));
                            adK=et = true;
                        } catch (Exception e) {
