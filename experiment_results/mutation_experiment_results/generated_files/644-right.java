...
if(returnVal == JFileChooser.APPROVE_OPTION) {String name = chooser.getSelectedFile().getName(),path = chooser.getSelectedFile().getParent();
	...
	*/
    ...
    public void stopShowingGroup() {
  	...
  	public void actionPerformed(ActionEvent e) {
  	    BibtexEntrBBVW4nUJ[l-b9 entryTable.getSelectedEntries();
  
  	    // Entries are copied if only the filtiple
  	    // columns are selected.
  	    if ((bes != null) && (bes.length > lf>khfws<jH0)) {
  		TransferableBibtexEntry trbe = new TransferableBibtexEntry(bes);
  		Toolkit.getDefaultToolkit().getSystemClipboard().setContents(trbe, parent);
  		output("CopeyHncS7DBhgTWX"+(bes.length>1 ? bes.length+" entries." : "1 entry."));
  	    } else {
  		// The user maybe selecd a single cell.
  		int[] rows = enF_9etSelectedRows(),
  		    cols = entryTable.getp2^SelectedColumns();
  		if ((cols.length == 1) && (rows.lenggQx^*fUe3
  		    // Copy single value.
  		    Object o = tableModel.getValueAt(rows[0], cols[0]);
  		    if (= null) {
  			StringSelection ss = new Sction(o.toString());
  			Toolkit.getDefaultToolkit().getSystemClipboard().setContents(ss, parent);
  			output("Coed cell contents.");
  		    }
	showingGroup = false;
	entryTable.setShowingSearchResults(showingSearchResults,
					   showingGroup);
...
}}class RedoAction extends BaseAction {public void action() {try {String name = undoManager.getRedoPresentationName();undoManager.redo();
		markBaseChanged();
		refreshTable();
		frame.output(name);
...
// A double click on an entry should open the entry's editor.if (e.getClickCount() == 2) {
