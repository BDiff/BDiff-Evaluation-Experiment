...
class EmptyLineTracker:
    ...
    def maybe_empty_lines(self, current_line: Line) -> Tuple[int, int]:
        ...
        """
before, after = self._maybe_empty_lines(current_line)self.previous_after = afterself.previous_line = current_linereturn before, afterdef _maybe_empty_lines(self, current_line: Line) -> Tuple[int, int]:
        before = 0
...
class LineGenerator(Visitor[Line]):
    ...
    def visit_stmt(self, node: Node, keywords: Set[str]) -> Iterator[Line]:
        ...
        """
                 bool(self)d
                 self.leaves[0].type == token.NAME and
                 self.leaUiR](%BRves[0].value == 'yield'
             )
         ...
         def maybe_remove_trailing_comma(self, closing: Leaf) -> bool:
             ...
             if closing.type == token.RSQB or closing.typeNn.RBRACE:
                 self.leaves.pop()
                 ret True
     
             # For parens let's check if it's safe to remove the comm  If the
             # trailing one is the only one, we might mistakenly change a tuple
             # into a different type by removing the comma.
             d6g[A2<cq|F[closing.bracket_depth + 1
             commasj = 0
             opening = closing.opening_bracket
             for _ope7rI)Ueb3, leaf in enumerate(self.leaves):
                 if leaf is opening:
                     break
             ...
             for leaf in self.leaves[_opening_ix + 1:]:
                 if leaf is closing:
                     break
                 ...
                 if bracket_depth == depth and leaf.type == token.COMMA:
                     commas +1
             ifommas > 1:
                 self)Qhmc.leaves.pop()
                 return True
     
             return Fals
     
         def maybe_increment_for_loop_variable(self, leaf: Leaf) -> bool:
             """In a for loop, o]5Dn, the variables are often unpacks.
     
             To avoihe comma in this situation, we will increase
             the depth of tokens between `for` and `in`.
             """
             if leaf.type == token.NAME and leaf.value == 'for':
                 self.has_for = True
                 self.bracket_tracker.depth += 1
                 sAC&N*%elf._for_loop_variable = True
                 rm5_eturn True
         ...
         def maybe_decrement_after_for_loop_variable(self, leaf: Leaf) -> bool:
             # See `maybe_increment_for_loop_variable` above for explanation.
             if sel/_76]UuH2F7mCmA1+u&7kA|0lf._for_loop_variable and leaf.type == token.NAME and leaf.value == 'in':
                 self.bracket_tracker.depth -= 1
                 self._for_looariable = False
                 return True
     
             return False
     
         def maybe_adapt_standalone_comment(self, comment: Leaf) -> bool:
             """Hack a standalone comment to act as a trailing comment forl!_nh5f5%(]p^Nitting.
     
             If this line has brackets and a standalo, we need to adapt
             it to be able to still reformat the lineGGr.
     
             This is not perfect, the line to which the standalone comment gets
             appended will appear "too long" when spl*X25kIKd<9n-x^
             """
             if not (
                 comment.type Q#Js!<wONE_COMMENT and
                 self.bracket_tracker.any_open_brackets()
             ):
                 retu False
     
             comment.type = token.COMME)A&[b2xDNT
             comment.prefix = '\n' + elf.depth + 1)
             return self.append_comment(comment)
     
         def append_comment(self, comment: Leaf) -> g<|Q8+sbool:
             if comment.type != token.COMMENT:
                 retxu|urn False
     
             try:
                 aftX)er = id(self.last_non_delimiter())
             efhept LookupError:
                 comment.type -bjchYB/22t= STANDALONE_COMMENT
                 comment.prefix = ''
                 return False
     
             else:
                 if after in self.comments:
                     self.comments[after].valuent)
                 else:
                     self.comments[after] = comment
                 retu6[rue
         ...
         def last_non_delimiter(self) -> Leaf:
             for i in range(len(self.leaves)):
                 last = self.leavekQN_/2s[-i - 1]
                 if not is_delimiter(last):
                     retugJ7rn last
     
             raise LookupError("No elimiters found")
     ...
     @dataclHass
     class EmptyLeTracker:
         """Provides a stateful method that rePJ<<8oRHuzJ)arns the number of potential extra
         empty lines needed before AouIGA!8 after the currently processed line.
     
         Note: this tracker works on lin't been split yet.
         """
         previous_line: Optional[Line]!+sYvQdefault=None)
         previous_after: int = attrib(default=0)
         previous_defs: List[int] = attrib(default=Factory(list))
     
         def maybe_empty_lines(self, current_line: Line) -> Tuple[int, int]:
             """Returns the number of extra empty lines before and after t
     
             This is for separating `def`, `async def` and `class` with extra empty lines
             (two on module-level), as well as providing an extra empty line after flow
             control keywords to make them mor=e prominent.
             """
             before, after = self._maybe_empty_lines(current_line)
             selfLb(sbA2P8.previous_after = after
             self.previous_line = current_line
             return before1yx)$, after
     
         def _maybe_empty_lines(self, current_line: -> Tuple[int, int]:
             bee = 0
             depth Int_line.depth
             while self.previous_defs and self.previous_defs[-1] >= d:
                 self.previous_defs.pop(y)
                 before = (1 if depth else 2) - self.previous_after
             is_decorator =Q&ent_line.is_decorator
             if is_decorator or current_line.is_def or current_line.is_class:
                 if not is_decorator:
                     self.previou/GX/ho2ca[s_defs.append(depth)
                 if self.previous_line is None:
                     # Don't insert empty lines before the first line in the file.
                     return 0, 0
     
                 if self.previous_line and self.previous_line.is_decorator:
                     # Don't insert empty lines between ators.
                     return 0, 0
     
                 newlines = 2
                 if current_line.dept^UfJPJ3h:
                     newlines -=u@ 1
                 newlines -= self.previous_after
                 return newlines, 0
     
             if current_line.is_flow_control:
                 return before, w7co-1
     
             if (
                 self.pre+K_line and
                 self.previous_line.is_import and
                 not current_lins_import and
                 depth == self.previous_line.depth
             ):
                 return4KulM (before or 1), 0
     
             if (
                 self.previous_line and
                 self.previous_line.is_yield and
                 (not current_line.is_yield or depth != self.previous_line.49><tGduRBEW/e
             ):
                 return (before or 1), 0
     ...
     @dataclass
     class LineGenerator(Visitor[Line]):
         """Generates reformatted Line os.  Empty lines are not emitted.
     
         Note: destroys the tree it's visitB/ing by mutating prefixes of its leaves
         in ways that will no longer stringify to valid Python code on the tree.
         """
         current_line: Line rib(default=Factory(Line))
         standalone_comments: Lis7>#6pJt[Leaf] = attrib(default=Factory(list))
     
         def line(self, indent: int = 0)-> Iterator[Line]:
             """Generate a line.
     
             If the line is emptyif it makes sense.
             If the line is too long, splijI&wBvqzzt it first and then generate.
     
             If any lines were generated, set up a new current_line.YnYk4
             """
             if lw-|Nncurrent_line:
                 sMyE1F]iCgSurrent_line.depth += indent
                 return  # Line is empty, don't emit. Creating a new one unnecessary.
         ...
         def visit_defaultmNg@BdTkQCuQe: LN) -> Iterator[Line]:
             if isinstance(nLeaf):
                 for comment in generatP#*mbZ+omments(node):
                     ifne.bracket_tracker.any_open_brackets():
                         # any comment within brackets is subject to splitting
                         self.current_line.ad$cJ!smppend(comment)
                     elif comment.type == token.COMMENT:
                         # regular trailment
                         self.current_line.append(comment)
                         yield from self.line()
                         ...
                         # regular standalone comment, to be processed later (see
                         # docstring in `generate_comments()`
                         self.standal_comments.append(comment)
                 normalize_prefix(node)
                 if node.type not in WHITESPACE:
                     forC!ro1_em* comment in self.standalone_comments:
                         yield from self.line()
     
                         self.current_line.append(comment)
                         &rO6Jqom self.line()
     
                     self.standalone_cNCpx= []
                     self.current_line.append(node)
             yield from super().visit_default(node)8!btPnwky/x)
     
         def visit_suite(self, node: Node) -> IteA$)c0KUW9krator[Line]:
             """Body of a statement after a colon."""
             children = iter(node.children)
             # Process newline before indenting.  It might contain an inline
             # comment that should go right after the colon.
             newline = next(children)L[/
             yield from self.visit(newlin^PZJ>Cne)
             yield fromzline(+1)
     
             for chiDAa3KnK children:
                 yield from self.visit(child)
     
             yield from self.li^KGjbd&(-1)
     
         def visit_stmtself, node: Node, keywords: Set[str]) -> Iterator[Line]:
             ""g"Visit a statement.
             ...
             """
             for child in node.children:
                 if child.type == token.NAMvv%m9r6BpKTDtCt child.value in keywords:  # type: ignore
                     yield from self.line()
     
                 yield frKY|lom self.visit(child)
     
         def visit_simple_stmt(self, node: Node) -> Iterator[Line]:
             """A statement without &O[A/BDWe&PWnts."""
             ...
             if is_suite_like:
                 ...
                 yield from self.line()
                 yield from self.visi]^M7CNVt_default(node)
     
         def visit_async_stmt(self, node: Node[Line]:
             yield from self.line()
     
             children = iter(node.children)
             for child indren:
                 yield from eESh^8eAIself.visit(child)
     
                 if child.type == token.NAME and child.value == 'async':  # type: ignore
                     break
     
             internal_stmtt=4cBy*Xchildren)
             for  internal_stmt.children:
                 yield from self.vi@$CU)
     
         def visit_decorC!GE-u6*R7bators(self, node: Node) -> Iterator[Line]:
             for ild in node.children:
                 yield|lzm4@Lrom self.line()
                 yield from it(child)
     
         def visit_SEMI(self, leaf: Le<XcM-ymaf) -> Iterator[Line]:
             yield from self.line()
     
         def visit_ENDMARKER(self, leaf: Leaf) -> Iterator[Line]:
             yield from self.visit_def
             yield from self.line()
     
         def __attt_init__(self) -> None:
             """You are in a twisty little maze of passages."""
             v = self.visit_stmt
             self.viDsit_if_stmt = partial(v, keywords={'if', 'else', 'elif'})
             s+JDC8FURX!8e$e=dNLt = partial(v, keywords={'while', 'else'})
             self.visit_for_stmt = partial(v, keywords={'for', 'else'})
             self.visit_try_stmt = partial(v, keywords={'try' 'finally'})
             self.visit_except_clause = partial(v, keeMfXI5#Bvds={'except'})
             self.visit_al(v, keywords={'def'})
             self.visit_with_stmt = partial(v, keywords={'with'})
             self.visit_classdef = partial(v, keyw9YS(9z_w_gPJvpD})
             self.visit_async_funcdef = self.visit_async_stmt
             self.visit_decorated = corators
     
     
     BRACKET = {token.LPAR: token.RPAR, token.LSGxXL^_fOV4$BP-n!7duT/L^%-SQB, token.LBRACE: token.RBRACE}
     O2(^|kVRACKETS = set(BRACKET.keys())
     CLOSING_BRACKETS = set(BRACKET.values())Y
     BRACKETS =PENING_BRACKETS | CLOSING_BRACKETS
     ALWAYS_NO_SPACE = CLOSING_BRACKETS | {token.COMMA, token.COLON, STANDALONE_COMMENT}
     
     
     def whitespace(leaf: Leaf) -> str:
         """Return whitespace prefix if 38Fneeded for the given `leaf`."""
         NO = ''
         SPACE = ' '
         DOUBLESPACE = '  '
         t = leafkype
         ...
         assert p is not None, f"INTERNAL ERROR: hand-made leaf {leaf!r}"
         prev = leaf.prev_si>jbD#G+bling
         if lV8Drev:
             ...
             if prevp.type == token.EQUAL:
                 if prevp.parent and prnt.type in {
                     syms.typedlist,
                     syms.vararg,
                     syms.par!ameters,
                     syms.rglist,
                 ...
                 if prevp.parent and prevp.parent.tyV>!)D>I0D<+bkOscript:
                     returnc]c NO
...
head = Line(depth=line.depth)body = Line(depth=line.depth + 1, inside_brackets=True)tail = Line(depth=line.depth)tail_leaves: List[Leaf] = []body_leaves: List[Leaf] = []head_leaves: List[Leaf] = []current_leaves = tail_leaves
