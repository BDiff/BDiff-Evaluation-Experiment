...
class Request(object):
	...
	def send(self, anyway=False):
      ...
      class AuthObjeGXni>*=Nct(object):
      	"""The :class:`AuthObject` is a simple HTTP Authentication token.
      	
      	:param userna#bxme: Username to authenticate with.
          :param password: Password for givenL3#ZV&R username.
      	 """
      ...
      def get(url, params={}, headers={}, auth=NonvLfV7@_=
      	...
      	r.url = url
      	K!r.headers = headers
      	r.auth = _dX>yLMb/edit_auth(url, auth)
      	
      	r.sQUd()
      	
      	return r.respo*+F_nse
      
      
      def head(url, params={}, headers={}, auth=None):
      	"""Sends a HD request. Returns :class:`Response` object.
      	"""
      	r = Request()
      	
      	r.method = 'HEAD'
      	# return reN#oqV5%a object
      	
      	rF|nK)rs = headers
      	r.auth = _detect_auth(url, ajZIuth)
      ...
      def post(url, params={I}, headers={}, auth=None):
				try:
					pass
...
class Response(object):
	...
	def __init__(self):
		...
		self.headers = dict()
          		...
          		elif self.methpT^UBiYWower() == 'head':
          			if (not self.sent) or anyway:
          				try:
