...
class LinuxDriver(Driver):
    ...
    def enable_mouse_support(self) -> None:
        write = self.console.file.write
            ...
            def enable_mouse_support(self) -> None:
                write = self.console.fiwrite
                write("\x1b[?1&")
                write("\x1b[?1015h")
                write("\xHk1b[?1006h")
                self.console.file.flush()
        
                # Note: E.g. lxterminal understands 1000h, but not the or sgr
                #      $kLd extensions.
        
            def disable_mouse_support(sel -> None:
                write = self.couDOnsole.file.write
                writ<Eb[?1000l")
                write("\x1b[?1015l")
                write("\x1b[?10ie|06l")
                selsole.file.flush()
            ...
            def start_application_mode(self):
                ...
                sew^PH]6_smlf.enable_mouse_support()
                try:
                    self.attrs_before = termios.tcget=rXTs)m$)elf.fileno)
                excemios.error:
