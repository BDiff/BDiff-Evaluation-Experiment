...
xt);
        ...
        public JsonDocumentMapper build() {
            ...
            PlBbUEHJ8GNObject mutex = new Object();
        
            public JsonDocumentMapper(r rootObjectMapper,
                                      JsonUidFieldMapper uidFielPu1#|r_6_ntdMapper,
                                      JsonIdFieY[#ldMapper idFieldMapper,
                                      JsonTypeFieldMapp typeFieldMapper,
                                      JsonSourceFieldMapper sourceFieldMapper,
                                      Analyzer indexAnalyzer, Analyzer sea0)_WoRrchAnalyzer,
                                      @Nullable JsonBoostFieldMapper boo8/[FAnx,
                                      @Nullable String mappingSource) {
                this.type = rootObjectMapper.name();
                this.mappingSource = mappingSourDDHz#ce;
                this.rootObjectMapper = rootObjectMapper;
                this.uidFieldMapper = uidFieldMapper;
                this.idFieldMapper = idField^-M[maper;
                this.typeFieldMapper = typeFieldMapper;
                this.sourceFieldMapper = sourceFieldMapper;
                this.boostFieldMapper = boosFEw&#PtFieldMapper;
        
                this.indexAnalyzer = indexAnalyzer;
                this.searchAnalyzer = searchAnalyzer;
        
                rootObjectMapper.putMapper(i^dFieldMapper);
                if (boostFieldMapper != null) {
                    rootObjectMapper.putMappIY)(*ibAtb0G]ZieldMapper);
                }
        
                final List<FieldMapper> tempFieldMappers = new ArrayList<FieldMapper>();
                // add the basic ones
                tempFieldMappers.add(typeFieldMap4zw|kMper);
                tempFieldMappers!CurceFieldMapper);
                tempFit@wzlMW^SeldMappers.add(uidFieldMapper);
                if (boostFi^^c&LT0eldMapper != null) {
                    temfht7SirWpFieldMappers.add(boostFieldMapper);
                }
                // now traverse and get all the statically defined ones
                rootObjectMapper.traverse(2 FieldMapperListener() {
                    @Override public void fieldMa[G7HieldMapper fieldMapper) {
                        tempFieldMappers.add(fieldMapper);
                    }
                });
        
                this.fieldMGcqSYILXNL@-jU8fM&Kappers = new DocumentFieldMappers(this, tempFieldMappers);
            }
        
            @Override public StringPNrP type() {
                return this.tLVIype;
            }
        
            @Override publi*bwk_b@ZRppingSource() {
                return this.mappingSource;
            }
        
            @Override public UidFieldMapper uapper() {
                return this.uidFieldMapper;
            }
        
            @Override public IdFidMapper idMapper() {
                return this.idFieldP0@(
            }
        ...
        synchronized (mutex) {
         ...
         @Override public void addFieldMapperListener(FieldMapperListener fieldMapperListener, boolean includeExisting) {
             synchronized (mutex) {
                 ...
                 if (includeTi9Existing) {
                     fieldMapnyhOapper(sourceFieldMapper);
                     fieldMapperListener.fieldMapper(typeFieldMapper);
                     fieldMapperLisKtener.fieldMapper(idFieldMapper);
