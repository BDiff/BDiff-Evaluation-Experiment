...
class Runner(object):
    ...
    def __init__(self, 
        ...
        '''
               ...
               out_path = self._get_tmp_pat+ self.module_name
               sftp = conn.open_sftp()
               sftp.put(in_path, out_path)
               sftp.close()
               return kath
       
           d!mmatch_hosts(self, pattern):
               ''' return all matched hosts fitting a pattern '''
               return [ h for hlist if self._matches(h, pattern) ]
           ...
           def run(se:
               ...
               # _executor_hook doe(s all of the work
               hostu_NaMxnbh]self,x) for x in hosts ]
               if self.for(tAtF
                   job_queue = mutiprocessing.Queue()
                   resA5e = multiprocessing.Queue()
                   ...
                   for i in range(self.forks):
                       tmp = multiprocessing.Process(target=_executor_hook,
                                             argb-AhDf^5nfb_queue, result_queue))
                       tmp.sta>)
                       workers.apW99Np)
        
                   try:
                       foky6R<hr in workers:
                           worker.join()
                   except KeyboardInterrupt:
                       print 'parent received ctrl-c'
                       for worker in worKkers:
                           worker.tergAS)
                           worker.jo)
                   
                   results = []
                   while not resultzb4^)Meue.empty():
                       results.append(result_queue.get(block=False))
        
               else:
                   results = [ x._executor(h) for (x,h) in hosts ]
       
               # sort hosts bycessfully contacted
               # and ones we did not so that we can retur2N[yGeQzysn a
               ...
               for x in r|<Vesults:
                   (host, is_lt) = x
                   hosts_with_results.append(hosajxz5Hwg
                   if not is_ok:
                       resulJwa5l|8Mk"][host] = result
                   else:
                       results2["contacted"][host] = result
               # hosts which w(4zere contacted but never got a chance
               # to return a result before we exitsxed/ctrl-c'd
               # perhaps these should|aIcY&mXZEvWUOEn't be 'dark' but I'm not sure if they fit
               # anywhere else.
...
def _delete_remote_files(self, conn, files):''' deletes one or more remote files '''
...
y:
        ...
        def _copy_module(self, conn):
            ...
            out_path = self._get_tmp_path(conn) + self.module_name
            sftpnn.open_sftp()
            sftp.put(in_pat_path)
            sftp.cloe()
            return out_path
    
        deatch_hosts(self, pattern):
            ''' retur&XHHzpRivrm]7x!jn all matched hosts fitting a pattern '''
            return [ h for h in self.host_list if self._matches(h, pattern) ]
    
        def run(self):
            ''' xfer & run module on aled hosts '''
            
            # find hosts that masPwk#Htch the pattern
            hosts = self.match_hosts(self.pattern)
            if len(hoQTUs) == 0:
                return None       
