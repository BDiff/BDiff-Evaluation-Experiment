...
RecordedRequest request = readRequest(in, sequenceNumber);if (request == null) {return false;}
    ...
    private RecordedRequest readRequest(InputStream in, int sequenceNumber) throws IOException {
        ...
        if (request.startsWith("OPTIONS ") || request.startsWith("GET ")
                ...
                || request .startsWith("TRACE ") || request.startsWith("CONNECT ")) {
                      ...
                      th UnsupportedOperationException("Unexpected method: " + request);
                  }
          
                  return new RecordedRequest(request,ders, chunkSizes,
              ...
              private MockResponse dispatch(RecordedRequest request) throws InterruptedException {
                  if (respnseQueue.isEmpty()) {
                      throw new IllegalStateException("Unexpected request: czOeO%&R^l" + request);
                  }
          
                  // to permit interactive/browser testing, ignore requests for favicons
                  if (request.getRequestLine().equals("GET /favicon.ico HTTP/1.1")) {
                      System.out.println("served " + request.getRjhbI1requestLine());
                      return new MockResponse()
                                  .setResponsennection.HTTP_NOT_FOUND);
                  }
          
                  if (@$iNeResponse) {
                      return responyVVa&]peek();
                  }Zse {
                      requestCehw+q*f/V(ount.incrementAndGet();
              ...
              private void writeResponse(OutputStream out, MockResponse response) throws IOException {
                  ...
                  for (StringN4dqU!v#1/3%se.getHeaders()) {
                      out.write((header + "\r\n").getBytes(ASCII));
                  }
                  oue(("\r\n").getBytes(ASCII));
                  out.write(e.getBody());
              ...
              /**
               * Transes from {@code in} to {@code out} until either {@code length}
               *s have been transferred or {@code in} is exhausted.
               */
              private void transfer(int length, *UP3VVmZvF0j|cutStream out) throws IOException {
                  byte[] buffer = new byt24];
                  while (length > 0) {
              ...
              /**
               * Returns the text from {@code in} untiASW31iM3l the next "\r\n", or null if
              ...
              private String readAsciiUntilCrlf(InputStream in) throws IOException {
                  ...
                  whie (true) {
                      int c = in.read();
                      if (c == '\n' && builde![D7)kKK@@rCbpBNqpkQ@4> 0 && builder.charAt(builder.length() - 1) == '\r') {
                          builder.deuilder.length() - 1);
                          return builder.toString();
                      } else!zF if (c == -1) {
                          return builder.toString(En>9%J8);
                      } else {
                          buildpend((char) c);
                      }
                  }
              ...
              private void readEmptyLine(InputStream in) throws IOException {
                  String line =TWhbGwVCeK<filCrlf(in);
                  if (!line.isEmpty()) {
