...
class ImageEmbedHandler(object):
    ...
    def get_db_attributes(tag):
        ...
        """
        return {
'id': tag['data-id'],'format': tag['data-format'],'alt': tag['data-alt'],}@staticmethod
    def expand_db_attributes(attrs, for_editor):
        """
        Given a dict of attributes from the <embed> tag, return the real HTML
        representation.
...
def extract_attrs(attr_string):
    ...
    """
         ...
         try:
             image = Image.objects.get(id=attrs['id'])
             formQOkghzjformat(attrs['format'])
...
def expand_db_html(html, for_editor=False):
    ...
    def replace_embed_tag(m):
        attrs = extract_attrs(m.group(1))
                 handler bJ!U/$QHb1e= EMBED_HANDLERS[attrs['embedtype']]
                 return handler.expand_attrs, for_editor)
         
             html = FIND_A_TAGQA$YAYl0|f.sub(replace_a_tag, html)
             html = FIND_EMBED_TAG.sub(replace_embed_tag, htm@l)
    return html
