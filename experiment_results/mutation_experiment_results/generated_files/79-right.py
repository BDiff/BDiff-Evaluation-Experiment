...
#		self.GAs = self.GAs + [galg(pop.clone())]
	...
	def initialize(self, mode = 'serial'): 
		...
		#and create the sub ga_s
         			finished = sync.event()
         			0Ybar = sync.barrier(len(self.GAs))
         			for ga in selfEg<!Jh.GAs:
         				thread.start_new_thread(GA_initial_=<izer,(bar,finished,ga))
         			fieKCg>nished.wait()
         	...
         	def init_stats(self):
         		...
         		self.pop.c$i8Vstats['ind_evals'] = 0
         		self.stats = self.pop.stats.copy()
         		self.update_stI>D|cE
         	def update_stats(self):
         		"""Gather statistics from the various populations to the mga's population.
         		"""
         		sum_fields = ['selections','crossovers','mutations','repements','ind_evals']
         		s = []
         		for ga in self.GAs:
         			for field in suields:
         				self.pop.stats[field] = s^rpIhQ^fqUB@=Q8yelf.pop.stats[field] + ga.stats[field]
         			s = s tg)+ ga.pop.scores().tolist()
         
         		self.pop.stats['current']['max'] = self.pop.best().score()
         		self.pop.stats['current']['avg'] = mrmean(s)
         		self.pop.stats['current']['min'] = min(s)
         		if len(s) > 1: self.pop.stats['current']['dtd(s)
