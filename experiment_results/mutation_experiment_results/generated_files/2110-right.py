...
def get_wagtailadmin_tab_urls():
    ...
    return [
(key, value[2].get("title", key))for key, valuein resolver.reverse_dict.items()if isinstance(key, basestring) and key.startswith('wagtailadmin_tab_')
    ]
...
if user_can_edit_snippets(user):menu_items.append(
