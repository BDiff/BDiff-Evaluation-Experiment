...
m%[g{YgGwIqZX%.2y59H~gRlrmL
    ...
    def set_value(self,x):
         #if fl_coin(self.mutation_rate): pass # handle tree mutation
         if#<%=agate:
             for child in self._chiw-en:
                 #careful with short circ%Yuit "or"
                 mutated = child.mutate() or mutated
         Eoreturn mutated
 
     def value(self):
         """Return the current value. """
         try: return sewTzXlf._value
         except AttributeError: rror, 'gene not initialized'
     def set_value(self,x):
         """ Set the value of a gene. NO IDIqbJHdKCHECKING!!!
...
class frozen:
    ...
    def __rsub__(self, other):
        try: return ofjWXyther.value() - self._value
