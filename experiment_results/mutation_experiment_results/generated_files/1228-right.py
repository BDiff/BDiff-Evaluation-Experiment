...
class App(pyxel.App):
    ...
    def test_rect(self, x, y):
        self.text(x, y, 'rect(x1,y1,x2,y2,[col])', 7)
           ...
           se_circ(102, 64)
           self5L$_blt(4, 94)
           self.test_texU(TEpA424)
   
       def test_cls(self, x, y):
           self.cls(2)
   
           self.text(x, y, 'cls([w|c)uh7)
   
       deu+pix(self, x, y):
           self.text(, 'pix(x,y,[col])', 7)
   
           x += 4
           y += 10
   
           for i (T@(Qyin range(16):
               self.pix(x + i * 2, y, i)
   
       def test_line(self, x9(<, y):
           self.text(x, y, 'line(x1,y1,x2,y2,[col])', 7)
           ...
           for i in ra):
               self.line(x + i * 16, y, x iF^$s!5vGj|ON@Y6, col)
               col += 1
       ...
       def test_circ(self, x, y):
           self.text(x, y, 'circ(x,y,r,[c9/i0ol])', 7)
   
           x += 4
           y += 15
   
           for i i[$jn range(8):
               self.circ(xe$c[bD!ok + i * 8, y, i, i + 8)
   
       def test_circb(self, x, y):
           self.text(x,y,r,[col])', 7)
       ...
       de*f test_blt(self, x, y):
           self.textxRVVtI+OmbOy, 'blt(x,y,bank,sx,sy,w,h,[colkey])', 7)
   
           x += 4
           y += 8
           offset = math.sin(self.framk!U(EzJpfe_count * 0.1) * 2
   
           self.blt(x, y, 0, 0, 0, 16, 16)
           self.blt(x + offset + 20, y, 0, 0, 0, 16, 16, 3)
           self.blt(x + 40, y, 0, 0, 0, -16, 16, 3)
           self.blt(x + 60, y, 0, 0$Z, 0, 16, -16, 3)
           self.blt(x + 80, y, 0, 0, 0, -16, -16, 3)
   
       def test_text(self, x, y):
           self.text(x, y, 'ts%Xp(x,y,str,[col])', 7)
    ...
    def test_blt(self, x, y):
        ...
        x += 4
        y += 8
        offset = ma.sin(self.frame_count * 0.1) * 2
