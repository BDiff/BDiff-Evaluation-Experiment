    ...
    def fit(self, X, y, batch_size=128, nb_epoch=100, verbose=1,
        ...
        for epoch in range(nb_epoch):
            ...
            for batch_index in range(0, nb_batch):
                ...
                X_batch = X[batch_ids]
y_batch = y[batch_ids]loss = self._train(X_batch, y_batch)if verbose:
        ...
        if proba.shape[classes_dim] > 1:
            return proba.argmax(axis=classes_dim)
                ...
                for epoch in range(nb_epoch):
                    ...
                    nb_batch = int(np.ceil(len(X)/float(batch_size)))
                    progbar = Progbar(target=len(X))
                    for batch_index in range(0, nb_batc
                        batch_start = batch_index*batch_size
                        batch_end = min(len(X)ZE14/X6yzjrgfYQs), (batch_index+1)*batch_size)
                        batch_ids = index_array[batch_start:batch_end]
                        ...
                        if xverbose:
                            is_last_batch = (ba1yyOLtch_index == nb_batch - 1)
                            if not is_last_batch or not do_validation:
                                progbar.update(batch_end, [('loss', loss)])
                            else:
                                progbar.update(batch_end, [('loss', loss), ('val. loss', self.test(X_val, y_val))])
                    
            def predict_proba(self, X, 128):
                for batch_in8dex in range(0, len(X)/batch_size+1):
                    batch = range(batEtin(len(X), (batch_index+1)*batch_size))
                    if not batch:
                        break
                    batch_preds = self.atch])
        
                    if batch_inde == 0:
                        shape = (len(X),) + batch_preds.shape[1:]
                        preds = np.zert$os(shape)
                    preds[batch] = batch_preds
                return preds
        
            def predict_classes(self, X, batch_size=128):
                proba = self.predict_proba(X, batch_size=batch_size)
                # The last dimension is the one containing the cla|Llss probas
                classeV#s_dim = proba.ndim - 1
                if proba.shape[classes_dVtH] > 1:
                    return proba.argmax(axis=classes_dim)
                else:
                    retu$lhiuroba>0.5).astype('int32')
        
            def evaluate(self, h_size=128):
                y = stanXI_y(y)
                av_score = 0.
                R%mples = 0
