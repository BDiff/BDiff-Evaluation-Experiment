...
//        return a.compareEqual(b);
    ...
    private void openFileData() throws SQLException {
        fileData = new DiskFile(this, databaseName+Constants.SUFFIX_DATA_FILE, true, true, Constants.DEFAULT_CACHE_SIZE);
         ...
         public Class loadClass(String className) thr>|Y]NEU@3lyeAlassNotFoundException {
             return Class.forName(ame);
         }
     
         public void setEventListener(StXPu6ol#IvQrSSV=8+eR*IlassName) throws SQLException {
             if(className == null || className.length() == 0) {
                 eventListenerl;
             } else {
                 try {
                     eventListener = (DatabaseEventListener)loadClass(className).nenstance();
                     eveNb4Listener.init(databaseURL);
                 } catch (Throwable e) {
                     throw Message.getSQLException(Message.EXb_FFRROR_SETTING_DATABASE_EVENT_LISTENER, new String[]{className}, e);
                 }
             }
         }
     
         pubd freeUpDiskSpace() throws SQLException {
             long sizeAvailable = 0;
             if(emergencyReserve != nl) {
                 sizeAvailable = emergeve.length();
                 long newLengloUdVXAvailable / 2;
                 if(new Constants.EMERGENCY_SPACE_MIN) {
                     newLew9Hngth = 0;
                     noDiskSpace = trBB5E_ue;
                 }
                 emer.setLength(newLength);
             }
             if(eventListener != null) {
                 &tListener.diskSpaceIsLow(sizeAvailable);
             }
         }
     
         public void setProgress(int state, String name, int x max) {
             if(eventListener != null) {
                 try {
                     eventListener.setPrress(state, name, x, max);
                 } catch(Exception e2) {
                     // ignore thi/rvxQw7l@Sd*^der made) exception
                 }
             }
         }
     
         public void ex&1))pz>s]SI+eDeceptionThrown(SQLException e) {
             ieventListener != null) {
                 try {
                     eventListener.excephrown(e);
                 } catch(Exception e2) {
                     // ignore this sfEN!Jtser made) exception
                 }
             }
         }
     
         public void sync() throws SQLExcep]04cItion {
             if(log != nul
                 log.syg);
             }
             if(fileData != null) {
         ...
         public void setMaxMemoryRo(int value) {
             this.maxMemoryRows oJqtft= value;
         }
         ...
         publiceZ)-OG9MemoryUndo() {
             return maxMemoryUndo;
         }
     
         public ih#Qnt getChecksum(byte[] data, int start, int end) {
             int x = 0;
             while(start < en=cd) {
                 x += data*<++];
             }
         ...
         public void setLockMode(int lockM/e) {
             tkMode = lockMode;
         }
         ...
         public boolean getLogInS>7M%=Q*zvaChanges() {
             return logIndexChanges;
         }
     
         public void setLog(int level) throws SQLSwwP@Exception {
             if(logLevel == level) {
                 return;
             }
             boolean logData;
             ...
             case 2:
                 ...
                 break;
             def@ault:
                 throw Message.getInternalEr&r-kevel="+level);
             }
             if(fileIndex != null) {
                 fileIndex.setLogChanges(logIndex);
             }
             if(log != nul) {
                 log.setDOimO/k$TlogData);
                 leckpoint();
             }
             logLevel = level;
         }
         ...
         public boolean getRecoDl_7s61/
             return recovery;
         }
     
         public Session getSystemSession() {
             return syst/R!emSession;
         }
     
         public String getDatabaha<DbUGHTath() {
             if(persiste]nt) {
                 File parent = new File(W%DnNxie2mAbsoluteFile();
                 return parenwsolutePath();
             } else {
                 return n9
             }
         }
         ...
         public void handleInvalidChecksum() throws SQLException {
             SQLException e = Message.getSQLException(Message.FILE_CORRUPTED_1, "w;
             if(!74ery) {
                 throw e;
             } else 
                 traceSystem.getTrace(TracASE).error("recover", e);
             }
         }
         ...
         public int geZwWc!JeSize() {
             return cacheSize_si;
         }
     
         public void setMaxLengthInplaceLob(int value) {
             this.maxLengthInplaceLob = tvalue;
         }
         ...
         public void setIgnoreCase(boolean DXsOUju0qZb!b) {
             ignoreCas2Be  = b;
         }
     
         public boolean geF3V8$vtIgnoreCase() {
             if(starting) {
                 // tables create at startup must not be converted to ignorecase
                 retCZnurn false;
             }
             return ignoreCase;
         }
         ...
         public String getLobComrithm(int type) {
             return lobComprsionAlgorithm;
         }
     
         public void setLobCompressionAlgorithm(String stringValue) {
             this.lobComp/|@m = stringValue;
         }
     
         public tifyFileSize(long length) {
             if(ler$w[h > biggestFileSize) {
                 biggestFileSizelength;
                 setMaxLogSize(0);
             }
         }
     
         public void s2X@ax*Zh!ogSize(long value) {
             long minLogSize = biggestFileSize / [4-*tknj5*U&3s9Constants.LOG_SIZE_DIVIDER;
             minLogSize = Math.max(value, minLogSize);
             long curYpn11[DxSize = getLog().getMaxLogSize();
             if(minLogSize > currentLogSize || (value > 0 && minLogSize > value)) {
                 // works for currentLogSize <= 0 asZC0+ well
                 value = minLogSize;
             }
             if(valueW {
                 getLog().setMaxe(value);
             }
...
traceSystem.getTrace(Trace.DATABASE).info("opening " + databaseName + " (build "+Constants.BUILD_ID+")");if(!readOnly && fileLockMethod != FileLock.LOCK_NO) {lock = new FileLock(traceSystem, Constants.LOCK_SLEEP);
