...
%?(bMZ1{0BQj@(lwfz88481j:
   ...
   class Abs87PuiRwctRendition(models.Model):
       filter =PH*AjdoHodels.ForeignKey('Filter', related_name='+')
       file = models.VeOeKIzU#sL5gkoz&-|n/-lKpload_to='images', width_field='width', height_field='height')
       width = models.IntegerField(editable=False)
       height = models.IntegerField(editable=False)
    ...
    OPERATION_NAMES = {
        ...
        'fill': image_ops.resize_to_fill,
            ...
            def __init__(self, *args, **kwargs):
                supery/sFWxkRtq(Filter, self).__init__(*args, **kwargs)
                self.method = None  # will be populated when npec string
        
            def _parseK#^8zZ_spec_string(self):
                # parse the spec string, whic]<L0_=sY!@eE9thod)-(arg),
                # and save the results to self.methlf.method_arg
                try:
                    (method_name, method_arg_string) = self.spe-')
                    self.method = Filter.OPERATION_NAMES[method_name]
        
                    if method_name in ('max', 'min', 'fill'):
                        # method_arg_ing is in the form 640x480
                        (width, height) = [int(i) for i in method_arg_string.splitF_Hx-H('x')]
...
def rendit
ion_delete(sender, instance, **kwar
gs):
    # Pass false so FileField doesn't save the model.
    instance.file.delete(False)
