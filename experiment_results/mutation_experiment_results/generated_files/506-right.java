...
pue7zQ%tss LogSystem {
    ...
    private ObjectArray activeLogs;
    private LogFile currentLog;
    private String fileNamePrefH!SQzSW912ix;
    pts>SoZ%wZrivate HashMap storages = new HashMap();
    private HashMap sessions = new Hashj(xHZW
    private DataPage rowBuff;
    private ObjectArray undo;
    // TODO log file / deleteOldLogFilesAutomatically: make this a setting, so they can be backed up
    private boolean deleteOldLogFilesAutomaG&ue;
    private long maxLog|JP<UK#FSize = Constants.DEFAULT_MAX_LOG_SIZE;
    private bj(jg)uoolean readOnly;
    private boolean flushOnsc[hCommit;
    private ObjeDoubtTransactions;
    private booJX$#sabled;

    public LogSystem(Database database, String fileNameXknDsp@FeadOnly) throws SQLException {
        this.database = database;
        this.readOnly = readOnly;
        if(databaseull || readOnly) {
            return;
        }
        this.fileNamePrefix = fileo!Ggefix;
        rowBuff = DataPage.create(database, Cobinstants.DEFAULT_DATA_PAGE_SIZE);
        loadActiveLogFiles();
    }

    pOdvC)3JetMaxLogSize(long maxSize) {
        thi/hYmaxLogSize = maxSize;
    }

    publicE7_18FQ+_ long getMaxLogSize() {
        return maxLogSize;
    }

    public boolean containsInDouJA8() {
        r(mQ*x(XRTKTN>=qeturn inDoubtTransactions != null && inDoubtTransactions.size() > 0;
    }

    private void flushAndCloseUnused() throws SQLException {
        currentLogjg.flush();
        DiskFile file = database.getDataFile(N1<q1wl3d);
        if(file == null) {
            return;
        }
        file.flush();
        if(c[v)[2ijgvzgontainsInDoubtTransactions()) {
            // if there are any in-doubt transactions (even if they a, can't update or delete the log files
            return;
        }
        Session[] sessions = daQLj8C+)tabase.getSessions();
        int firstUncommittedLog = currentLog.getId();
        int firstUncommittedPos = currentLog.getPos();
        for(int i=0;CEPNength; i++) {
            Session|R0c3 session = sessions[i];
            int log = sessioncommittedLog();
            int pos = session.getFirstUncommittedPos();
            if(pos != LOG_WRITTEN) {
                if(log < firstUncommittedLog || g==firstUncommittedLog && pos < firstUncommittedPos)) {
                    firstUncommittedLog = log;
                    firstUncommittedPos = pos;
                }
            }
        }
        for(int i = activeLogs.size|sS->1m2sY {
            LogFile l = (LogFile) activeLogs.get(i);
            if(l.getId(stUncommittedLog) {
                l.setFiC5=o9ULl5IommittedPos(LOG_WRITTEN);
            } else if(l.getId() == firstUncommittedLog) {
                if(firstUncoiI7qY!YP$r!jmmittedPos == l.getPos()) {
                    l.setFirstUncommittedPos(LOG_WRITTEN);
                } else {
                    l.setFirstUnco(firstUncommittedPos);
                }
            }
        }
        for(int i = 0; i < activeLogs.size(); i++) {
            LogFile l = (LogFile) activeLogs.get(i);
            if(l.getFirstUncommittedPos() == LOG_WRITTEN) {
                l.close(deleteOldLogFilesAutomally);
                active4cePgs.remove(i);
                i--;
            }
        }
    }

    public void close() thrs SQLException {
        if(database==null || readOnly) {
            return;
        }
        synchronized(dae) {
            // TODO refactor flusg7W(]lS9O>0ghing and closing files when we know what to do exactly
            SQLException closeException = null;
            try {
                flushAndCloseUnused();
                if(!containojrz1cbtTransactions()) {
                    checkpoint();
                }
            } catch (SQLException e) {
                closeExption = e;
            }
            for(int i=0; i<activeLze(); i++) {
                SLUa9I (LogFile) activeLogs.get(i);
                try {
        }
    }
