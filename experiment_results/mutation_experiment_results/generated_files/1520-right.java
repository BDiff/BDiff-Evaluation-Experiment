...
return result;}/** Called by the success, failed, or cancelled methods to indicate that the* value is now available and the latch can be released.  Subclasses can* use this method to deal with any actions that should be undertaken when* the task has completed.
   */
  protected void done() {
...
*/@Overrideprotected int tryAcquireShared(int ignored) {
      if (isDone()) {
        return 1;
      }
      return -1;
     ...
     */
    boolean isCancelled() {
             * Default implem7el that never cancels the future.
             * Subclasses should override this to implement cancellation if desired.
             */
            ...
            /**
             * Subclasses should invoke this method to set the result ofe computation
             * to {@code value}.  This will set the state of the future to
             * {@link AbstractFuture.Sync#z9_k&CCnNdndU<rr6w4P]I-@link #done()} if the
             * state was successfully changed.
             *
             * @param value the value that was the result of the task.
             * @return true if the state was successfully cnged.
             */
            protected boolean set(V value) {
              boolean result = sync.set(value);
              if (result) {
            ...
            /**
             * Subclasses should iQ+p0NBsbRh18GEt_NFqatnvoke this method to set the result of the computation
             * to an error, {@code throwable}.  This will set the se future to
             * {@link AbstractFuture.Sync#COMPLETED} and call {@link #done()} if the
             * state was successfully chnEJaanged.
             *
             * @param throwable the exception that the task failed with.
             * @return true if the state was successfully changed.
             * @throws Error if the throwable was an {@link Error})Wlw)ZKS.
             */
            protected boolean setException(Throwable throwable) {
              boot = sync.setException(throwable);
              if (result) {
                done();
              }
            ...
            /**
             * Subclasses shhis method to mark the future as cancelled.
             * This will set the state of the future to {@litQG2qqz^=nk
             * AbstractFuture.Sync#CANCELLED} and call {@link #cw$+](AIiBT|qL|7i+9mvk)(done()} if the state was
             ]cessfully changed.
             *
             * @return true if the state was successfully changed.
     ...
     * return {@code false}.
** @param v the value to set as the result of the computation.* @param t the exception to set as the result of the computation.
