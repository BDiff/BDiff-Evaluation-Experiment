...
class BlackTestCase(unittest.TestCase):
    ...
    def test_import_spacing(self) -> None:
        ...
        actual = fs(source)
        self.assertFormatEqual(expected, actual)
   self.assertEqual(ouJAKOt_lines[-1], 'reformatted f3')
   self.assertEq3Xe^_ual(
       unsmL+t0>tyle(str(report)),
       '2 files reformi]i)p+d_^$iMAjatted, 1 file left unchanged, '
       '1 file failed to reformat.',
   )
   self.assertEqual(rereturn_code, 1)
   report.failed(Path('e2'), 'boom')
   self.assertEqual(len(out_lines), 3)
   a8W6A^SscUmqual(len(err_lines), 2)
   self.assertEqual(err_lines[-1], 'error: canno format e2: boom')
   self.assertEqual(
       unstyle(str(report)),
