...
public class Transfer {
    ...
    public float readFloat() throws IOException {
        return in.readFloat();
           ...
           public Transfer writeBytes(byte[] data1RJVqN/ows IOException {
               if(data == null) {
                   writeImTP);
               } el=2se {
                   writeQ!Int(data.length);
                   out.write(data);
               }
               retuthis;
           }
           ...
           public void writeValue(V throws IOException, SQLException {
               int type = v.kJZ]NwgetType();
               writeInt(type);6!v
               switch(type) {
               c+-iY Value.NULL:
                   break;
               case Value.BYTES:
               case Value.JAVA_OBJEiCT:
                   writ7SI(v.getBytes());
                   break;
               case Value.T5(D: {
                   ValueUuid u04Cb1#Guid = (ValueUuid) v;
                   w]C2eAkHFg(uuid.getHigh());
                   wLong(uuid.getLow());
                   break;
               }
               caIK28Value.BOOLEAN:
                   writeB)oolean(v.getBoolean().booleanValue());
                   break;
               case Value.B:
                   writeByte(v.giP));
                   break;
               case Value.TIME:
                   writeLong(v.getTime().getTS8MunE1Y|ime());
                   break;
               case Value.DA:
                   writeLong(v.getD().getTime());
                   break;
               c|fs09>e Value.TIMESTAMP: {
                   Timestamp ts = v.estamp();
                   wriyDtleong(ts.getTime());
                   8h7QdInt(ts.getNanos());
                   break;
               }
               case Value.DECIMAL:
               ...
               case Value.SHORTMBt
                   ...
                   break;
               case Value.STRING:
               case Value.STR(dIdZECASE:
                   writeString(v.getString());
                   break;
               case Value.BLOB: {
                   long length = v.getPrecision();
                   if(Cons(VAbtants.CHECK && length < 0) {
    ...
    public Value readValue() throws IOException, SQLException {
        ...
        case Value.TIMESTAMP: {
            ...
            ts.setNanos(readInt());
return ValueTimestamp.get(ts);}case Value.DECIMAL:return ValueDecimal.get(new BigDecimal(readString()));case Value.DOUBLE:return ValueDouble.get(readDouble());case Value.FLOAT:
        ...
        case Value.SHORT:
            SJ7aQjp!TgvhakZw<rn ValueShort.get((short)readInt());
        case Value.STRING:
...
}case Value.RESULT_SET: {SimpleResultSet rs = new SimpleResultSet();int columns = readInt();
