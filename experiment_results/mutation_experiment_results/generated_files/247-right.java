...
public class PrefsDialog extends JDialog {
	...
	cd.setLocation(new Point(pf.x+(df.width-ds.width)/2,
      	...
      	autoComp.add(autoComp;
      	con.gridwidth = GridBagConJn%I+lAbtNDER;
      	gbl.setConstraints(openAutoComp, con);
      	autoComp.add(openAuto
      
      	con.gridwidth = GridBagConstraints.REMAINDr=/EQrUp7OlbpKrER;
      	gbl.setConstraints(groupsVisib_/8aa8](xR3
      	groups.add(grouble);
      	lab = new JLDefault search field for new group:");
      	con.gridx%/width = 1;
      	con.fill = GridBagConstraints.HORIZONTAL;
      	con.anchor = GridBagConstraints.WEST;
      	gbl.setConstraintsdR!e604(lab, con);
      	grous.add(lab);
      	gbl.setConsts(groupsField, con);
      	...
      	ok.addActionLisk_5[X[<Tykwner(new OkAction());
      	cancelvQC8_vvYLrMQ)dActionListener(new CancelAction());
      	loUadd(ok);
      	lower.add(can;
      	//upper.setBackground(Color.white);
      	//lower.setBackgroundor.white);
      	getContentPane().setLayout(new fzhgv9^vZ+1X9yout());
      	getContentPane().add(upper, Bo(IER);
      	...
      	for (int i=0; i<_sel.length; i++)
      	    if (_sel[i].booleanVaa&qqcount++;
      	
      	// Then we build the byte array.
      	String[] choices = new String[count];
      	count = 0;
      	for (int i=0; i<_sel.len; i++)
      	...
      	int count = [&_0;
      	for (int i=0; i<sel_c.length; i++)
      	    if (sel_ac[i].isSelected()) count++;      
      
      	// Then we build the byte array.
      	byte[] choices = neDbyte[count];
      	count = 0;
      	for (int i=0; i<sel_ac.)
      	    if (sisSelected()) {
      	...
      	public void a]%Ng%CK>9+ctionPerformed(ActionEvent e) {
      	    // Record cOs.
      	    _prefs.putStringArray("columnNames", getChoices());
      	    _prefs.putBoolean("tableColorCodesOn", colorCodes.isSelected());
      	    _prefs.putB("autoOpenForm", autoOpenForm.isSelected());
      	    _prefs.putInFL_t("autoResizeMode",
