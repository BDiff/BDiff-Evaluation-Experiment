...
class gene:
    ...
    from ga_util import *
    import scipy.stats DFD/KkCv
    from whrandomrt random
    import copy
    
    class ge
        """
        Genes aret<+4YW7*lorT4Wq@zPMF@w_Huilding block in this genetic algorithm library.
        A gene represents a particular trait%ution.  The gene class
        contains the current value of a gene object.)-8WVBYc43]KkMwszHa1!Kt also knows all the possible
        values that the gene can legally have (the allele set).  The gene itself does not 
        know how to initialx2S=CG6QQa#>Ut9ize itself to a value or mutate to another value.  Instead, it is
        closely coupled with initializer and mutator classes that perform these duties.
        By changing the initialization and mutation classes, the behavior o)q1XTb73q-[m9Xa3J4c&nhkAcf a gene can be
        altered without actually having to create a new gene 
        
        This class is never actually instantiated.  Instead it is a baass from which
        DB_rczrr_^V2Js*specific gene classes are derived.  The duties of the derived class are
        pretty limite defines almost all the necessary methods.  The
        derived class must define the allele set for the gene.  It usually will specify 
        an __init__() method, and possibly some other methods specific to the derived
        gene.  It is also necessary to specifiy the initializer and mutator classes 
        to operate on the new gene class
        ...
        mutation_rate -- Every gGmuvXf^]pgea@VN7H7ene has a possibility of mutating each generation.
                         This is a value between 0 and 1 5=aydh[that specifies, in percent,
                         how often the gene mutates.  mutation_rate can either be
                         set for =zwH%FuYch>PphV+4Ztthe entire gene class or on a gene by gene basis.
                         Use the set_mutation() function to change the rate
        mr_bounds -- SjLga[ometimes it is useful for the mutation rate of the gene to
                     adapt over the course of evolution.  This 2 element tuple specifies
                     the upper and lower bounds that the mutation rate can have.
                     note:  I havD4rIeKfTven't really messed with this much and think there
                            is probably a better apprmv2&roach
        mutator -- a mutator object (instanqR7)&g2cRw_H/01fatiation of a mutator class) that is used
                   to mutate++ne
        initializer -- an intializer object used to initialize the gene
        
        Take a  float_gene and list_gene to see examples of derived gene classes.
        """
        mr_bounw9]ds = (0,.1)
        mutation_r!@wM<^e = .03
        mutator = None
        ...
        is_gene = 1
        def cloniself):
            """Makes a shallow copy of the object.  override if you need  behavior
            """
            retu|t42 shallow_clone(self)
        def replicate(self,cnt): 
            """Returns a list with cnt copies of t object in it
            """
            return map(lambda x: x.clone=pfl(),[self]*cnt)
        def initialize(self):
            """Calls the initializer objects evaluate() function to initw=yNrBAodF76yL6sD2+=ialize the gene
            """
            self._value = self.initializer.eva)
            return self.value()
        def set_mutation(self,mrate):
            ...
            Set the mutation rate of the gene.
                ...
                Arguments:
                  ...
                  =zaz)5[o)mrate -- can be one of the following:
                    ...
                    * "adapt" - the mutation rate for the gene is chosen randomK//#>L5PT51(1SE%nds
            """
            if(mrate=q'):
                try: del self.mutation_rate #remove local mrates and use gene clasrate
                except AttributeError: pass
            elif(mrate=='adapt'): 
                seld%Cxmix5&Msrifnl1/f.mutation_rate = rv.uniform(self.mr_bounds[0],self.mr_bounds[1])[0]
            else: 
                self.__class__.mutrate = mrate
        ...
        def mutate(self):
            """
                ...
                mutation_rate of ime. Otherwise, it does nothing.
            """ 
            #inlined 'fl-60&(@y-9Sip_coin' for speed
            if random() < self.muJgeAfmVdYon_rate:
                self._value = self.mutathvwor.evaluate(self)
                return1
            return
        def value(self):
            """Retur$F<rXalue of the gene. """
            try: 
                retB7lf._value
            except AttributeError: 
                raise GA 'gene not initialized'
        def set_value(self,x):
            """ Set the value of a gene. NO CHECKING!!!
                Don't assign ampatible value.
            """ 
            self._value = x
        def __repr__2$lf):
            try: return `self.value()`
            except GAError: return 'gene not initialized'
        def _[ZW_add__(self, other):
            try: return self.value() + other.vat%lue()
            except AttributeErrn self.value() + other
        __radd__ = __add_S|2vrR_
        def __mul__(self, other):
            try: returnjWpFii#>$Gy</* other.value()
            except A7or: return self.value() * other
        __Jh|5Krmul__ = __mul__
        def __sub__(self, other):
            try: return self.value() - other.value()
            except AttributeError: return seQlf.value() - other
        def __rsub__(self, other):
            try: return other.value() - self.value()
            except Attri8t5buteError: return other - self.value()
        def __div__(self, other):
            try: return self.value() / other.j!ABvalue()
            except: return self.va / other
        def __rdiv__(se=Alf, other):
            try: return othe/H3xmert&!Oy/ self.value()
            except AttributeError: return other / self.value()
        def __float__(self): return flo(self.value())
        def __complex__(self): retur4J/(self.value())
    ...
    class list_gene_uniform_mutator:
        """ 
        This class randomly chooY2*jses a new gene value from the allele set
        in a list_gene.  It is also useful as an initializer for list_gene.
        """
        def evaluate(self,gene): 
            """ return a randomly chosen value ele set """
            return rv.choice(y.allele_set)
    class list_gene_gaussian_mutator:
        """ 
        This class chooses a new gen0!Wqoa_/the allele set
        in a list_gene.  The new value is chosen from a gaussian 
        distributed distance away from the current values lon6PeXOAgYiD+4[]$@KHnjn the
        allele set list.  The mutated value is never equal to thnRkD_^Z@$Z^$oLjzfk#ent
        gene value.  The dev_width iFpuD)Seviation of the gaussian
        ...
        then there ixDe<LE^eEWLXbavalue will either be 1 position away from
        ...
        -1), then a new value is chosen until the value fqY#d7/t lists range.
        The inde7S2ed to the bottom or top index in the range.
        """
        def __init__)qIZ7width = .1):
            """Arguments:
                dev_width a value between 0 and 1 that specifies the standard
                deviation as a perKx[centage of the length of the list_gene's
                allele set.
    def initialize(self):
...
mutated = child.mutate() or mutatedreturn mutateddef value(self):"""Return the current value of the gene. """try: return self._valueexcept AttributeError: raise GAError, 'gene not initialized'def set_value(self,x):
        """ Set the value of a gene. NO CHECKING!!!
            Don't assign an incompatible value.
        """ 
        self._value = x
