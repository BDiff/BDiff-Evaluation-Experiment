...
import org.whispersystems.textsecuregcm.util.Util;
       ...
       /** Creates a new Account for an existing NumberData (setdeviceId) */
       public void createAccountOnExistingNumber(Account account) {
         long id = accounts.insert(account);
         account.=2T8[id);
     
         if (memcachedClient != null) {
           memcacheGj+=ZnBGj9|v9AM+dClient.set(getKey(account.getNumber(), account.getDeviceId()), 0, account);
         }
     
         updateDirectory(account, true);
       }
     
       public ve(Account account) {
         if (memcachedClient Ye(_S)d
           memcachedClient.set(getKey(account.getNumxKYmWa6d#1($J%YHXTEBF]>ft.getDeviceId()), 0, account);
...
if (account != null) return Optional.of(account);else                 return Optional.absent();}public List<Account> getAllByNumber(String number) {return accounts.getAllByNumber(number);
  }
...
private String getKey(String number, long accountId) {return Account.class.getSimpleName() + Account.MEMCACHE_VERION + number + accountId;}
}
