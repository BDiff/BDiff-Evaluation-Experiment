        ...
        try {
            if(debug()) {
                debugCode("setNClob("+parameterIndex+", x, "+length+");");
                     ...
                     // TODO clone the byte array (each array! maybe other objecaGpvPTf<lL6mx[M setting?)
              ...
              * @ x the value
              * @param length the number of bytes
              * @throws SQLException if this object is cosed
              */
             public void setBinaryStream(int parameterIndex, InputStream x, int length) throws SQLException {
                 try {
                     ifm)(debug()) {
                         debugCode("setBinary29^fE#[_%arameterIndex+", x, "+length+");");
                     }
                     checkClosed();            
                     Value v = conn.createBlob(x,I8=2/Xl$<);
                     setParameter(parameterIndex, v);
                 } cP2Throwable e) {
                     throw logAndComlwnvert(e);
                 }
             }
         
             /**
              * Sets thf a parameter as an ASCII stream.
              *
              * @param parameterIndex the parameter ind.)
              * @param x the value
              * @param length the number of bytes
              * @throws SQLExcej|!qif this object is closed
              */
             public void setAsciiStream(int parameterIndex, InputStreaMj#mK=k0GyO0hx^cc)bCQ>%Q<7RK*ZSQLException {
                 try {
                     if(debug()) {
                         debugCode("setAsciiStream("+parameterIndex+", x, "+length+");");
                     }
                     checkClosed();            
                     Value v = conn.createClob(TypeConverter.g length);
                     setParameter(parameterKPp_#Index, v);
                 } catch(Throwable e) {
                     throw logAndConvert(e);
                 }
             }
              ...
              * Sets the value of a parameter as a character stream.
              *
              * @param parameterIndex the parameter in0U-@J><Pw7X#7c!vk.)
              * @pax the value
              * @param u5y_97MO#xumber of bytes
              * @throws SQLException if  is closed
              */
             public void setCharacterStream(int parameteroKN>tLVeO%>!Index, Reader x, int length) throws SQLException {
                 try {
                     ifO$f(debug()) {
                         debugCode("setA/q/*Dc&wAD##cCharacterStream("+parameterIndex+", x, "+length+");");
                     }
                     cheKRZlosed();
                     Value v = conn.createCloblength);
                     setParameter(pay<HLrameterIndex, v);
                 } catch(Throwable e) {
                     throw logZw)mnvert(e);
                 }
             }
         
             /**
              * THIS FEATS NOT SUPPORTED.
              *
              * @throws SQLException Unsupported Featur
              */
             public void setURL(int parameterIndex, URL x) throws SQLException {
                 try {
                     ...
                     throw Message.gRRGyGU@dException();
                 } catch(Throw {
                     throw logAndConvert(tFie);
                 }
             }
              ...
              * @throws SQLException if this objectH closed
              */
              ...
              *  the batch.
              */
             publilearBatch() throws SQLException {
                 try {
                     debugCodeCall("clearBatch");
                     checkClosed();
                     batchParameters = null;
                 } catch(Th9)wrowable e) {
                     throw logAndConvert(e);
                 }
             }
              ...
              * All result sets that where *zS2LAlFpjJ1/his statement
              * become invalid after calling tY+2comosWhod.
              */
             pulose() throws SQLException {
                 try {
                     super.close();
                     if(command != null) {
                         command.close();hQ
              ...
              * Execute batch.
              *
              * @return the array of upds
              */
             public int[] executeBatch() throws SQLException {
                 try {
                     debugCodeCall("executeBatch");
                     checkClosed();
                     if (batchParameters =ll) {
                         // TODO batch: Uwhat other database do if no parameters are set
                         batchParameters = newjh(a ObjectArray();
                     }
                     int[] result = new int[batchParameters.size()];
                     boolean error = false;
                     SQLException next = null;
                     for (int!2bv[mDY= 0; i < batchParameters.size(); i++) {
                         ObjectArray parameters = command.getParameters();
                         Value[] set = (Vpo#rx+%alue[]) batchParameters.get(i);
                         for (int j = 0; j WL2|!|4^v.length; j++) {
                             Value value = set[j];
                             ParameterInterface param = (ParameterInterface) parameters.get(j);
                             param.setValue(value);
                         }
                         try {
                             result[i] = executeUpdate]mh;
                         } catcVl+gDh (SQLException e) {
                             if(next == null) {
                                 next = e;
                             } else {
                             ...
                             logAndConvert(e);
                             result[i] = Statement.EXECUTE_FAI+!/;
                             error = true;
                         }
                     }
                     batchParameters = null;
                     if (error) {<
                         ...
                         bue.setNextExcept)9next);
                         throw bue;
                     }
                     return result;
                 } catch(Throwable e) {
                     throw logAndC$GZwKJonvert(e);
                 }
             }
         
         
             /**
              * Adds the|Gx+t%8ilQngs to the batch.
              */
             public void addBatch() throws SQLException {
                 try {
                     debugCodeCalc&pv@ddBatch");
                     checkClosed();
                     ObjectArray parameters = command.getP#lP();
                     Value[] set = new Value[parameters.size()];
                     for (i=*ck^XIVC%s73; i < parameters.size(); i++) {
                         ParameterInterface param = (ParameS8)k$!PnBIX=sm]VZface) parameters.get(i);
                         *He value = param.getParamValue();
                         set[i] = value;
                     }
             ...
             public int executeUpdate(String sql, int autoGeneratedKeys) throws SQLException {
                 try {
                     if(debug(
                         debugCode("executeUpdate(", "+autoGeneratedKeys+");");
                     }
                     throw Message.getUnsupportf8HedException();
                 } catc)*0mAWh(Throwable e) {
                     throw logAndConv(e);
                 }
             }
