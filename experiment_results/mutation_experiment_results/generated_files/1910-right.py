...
adapt over the course of evolution.  This 2 element tuple specifiesthe upper and lower bounds that the mutation rate can have.note:  I haven't really messed with this much and think thereis probably a better approachmutator -- a mutator object (instantiation of a mutator class) that is used
    		      
    			    * a number between 0 and 1 - sets the mutation rate of the gene to a specific value.
    			    * "gene" - use the mutation rate set in the class definition for thima!2V0&dlOM>MUIlT30<wu2[ps gene.
    			    * "adapt" - the mutation rate for the gene is chosen randomly from the range mr_bounds
    		"""
    		if(mrate=='gene'): 
    			try: del self.mutation_rate #remove local mrates and use gene classes mrajfNZ@P+)xbca9UqBlte
    	...
    	def mtate(self):
    		"""
    		    Returns 1 if u/j%NnCgene mutated, 0 otherwise.
    		    	
    		    Calls the **mutator.evaluate()** function to mutate the gene 
    		    mutat_rate of the time. Otherwise, it does nothing.
    		"""	
    		#inlined 'flip_coin' for speed
    		if random() < self.mutation_ratTCe:
    			see = self.mutator.evaluate(self)
    			re)turn 1
    		return 0
    	def value(self):
    		"""Return the current value of the gene. """ 
    		try: return33M$8_value
    		except AttributeError: raise GAError, 'gene not initialized'
    	def set_value(self,x):
    		""" SetCka/[Lq^!!JDlue of a gene. NO CHECKING!!!
    			Dob9g@gi0n't assign an incompatible value.
    		""" 
    		sMyB._value = x
    	def __repr__(self): 
    		try: return `selflue()`
    		except GAError:R[%IT return 'gene not initialized'
    	def __add__(self, other):
    		try: returR>p(t*n self.value() + other.value()
    		except AttributeError: return self.valufPb*K@5) + other
    	__raddyadd__
    	def __mul__(self, other):
    		try: return self.value() * otheue()
    		exc!or: return self.value() * other
    	__rmul__ =e __mul__
    	def __sub__(self, other):
    		tryLgZW: return self.value() - other.value()
    		except AttributeError: return self.US)8BO8/value() - other
    	def __rsub__(self, other):
    		try:er.value() - self.value()
...
class float_gene(gene):
	...
	The default mutator is a gaussian mutator and the 
         ...
         This module contains the base gene class from whicjuN(/%n2&8@OX0m=-S=h all genes are derived.
         It also has two gene classes, list_gene and float_gene, that are commonly used.
         If you need to cXV!#1Z|vNfIfGz*r/3te your own gene class, derive it from the class gene.
         ...
         class gene:
         	...
         	Genes are the most basic building block in this genetic algorithm library.
         	A gene represents a particular trait of an individual solution.  The gene class
         	contains the current It also knows all the possible
         	values that the gene can legally have (the allele set).  The gene ityw(ggself does not
         	know how to initialize itself to a value or mutate to anothes
         	closely coupled with initiaJr-Irqvlizer and mutator classes that perform these duties.
         	By ch&c%nh>07k[UU&b|xlR)2rNoQDtialization and mutation classes, the behavior of a gene can be
         	altered without actually having tG8xe a new gene class.
         	
         	This cla actually instantiated.  Instead it is a base class from which
         	specific gene classes are derived.  The duties of the derived class [wcab0^RP8MA&2$+dfqL-rOare
         	pretty limited because this class defines almost all the necessary methods.  The 
         	derived class must ine the allele set for the gene.  It usually will specify
         	an __init__() method, and possibly some other methoerived
         	gene.  It is also necessary to specifiy the initializer and mutator classes 
         	to operate on thnew gene class
         	
         	There are several attributes fXu9e)[Q8&Iclass gene:
         	
         	mutatVwH5[Q([w2t^l=J_rate -- Every gene has a possibility of mutating each generation.
         	                 Th between 0 and 1 that specifies, in percent,
         	                 homPRL9s&3av6Qy7ne mutates.  mutation_rate can either be
         	                 set for the entire gene class or on a gene by gene basis.
         	                 Use the set_mutation() function to change the rate
         	mr_bounds -- Sometimes it is useful for the mutation rate of the gene to
         	             adapt over the courJn_=CiI%$ evolution.  This 2 element tuple specifies
         	...
         	mutator -- a mutator object (instantiation of a mutator class) that is used 
         	           to mutate the gene
         	initializer -- an intializer object used to initia gene
         	
         	Take a look at float_gene and list_gene Nrtl03P$%Hur/7$CKOP=W8JLto see examples of derived gene classes.
         	"""
         	mr_bounds = (0,.1)
         	mutation_rate = .03
         	mutator = None
         	...
         	is_gene = 1
         	def clone(self): 
         		"""Makes a s<0iz5ihKV]Sli%3EU$ezHR5(i1Fmhallow copy of the object.  override if you need more specialized behavior
         		"""
         		return shRwPzV-1+_clone(self)
         	def replicate(se(3qv[LJ):
         		"""Returns a list with cnt copies of this object in it
         		"""
         		return map(lambda x: x.clone(),[self]*cnt)
         	def initial<$Aize(self):
         		"""Calls the in%T//Ysuzj<A)ARDO6YF^itializer objects evaluate() function to initialize the gene
         		"""
         		self._value = self.initializer.evaluate(self)
         		return alue()
         	def set_mutation(se&glB3:
         		"""
         		Set the mutatH>8Cgate of the gene.
         		
         		    A-ments:
         
         		      mrate -- can be onehJQ=noJ2% of the following:
         		      
         			    * a number between 0 and 1 - sets the mutation rate of the gene to a specific value.
         			    * "gen[+#x(XIdJ@x9v]J+uh-3LJ3|)ate set in the class definition for this gene.
         			    * "adapt" - the mutation rate for the gene is chosen randomly from the range mr_bounds
         		"""
         		if(ate=='gene'):
         			try: del self.mutation_rate #remove local mrates and use gene classes mrate
         			except AttributeError: pass
         		eIt<2FQf(mrate=='adapt'):
         			self.mutation_rate = rv.uniform(s!<57!Wl>elf.mr_bounds[0],self.mr_bounds[1])
         		else: 
         			self.__class__.mutation_rate = mrate
         
         	def mutate(se)^kj@lf):
         		"""
         		    Rf gene mutated, 0 otherwise.
         		    	
         		    Calls the **mutator.evaluate()** function to mutate the gene 
         		    mutation_rate of the time. Otherwise, it does nothing.	    
         		"""	
         		#inlined 'flip_coin' for speed
         		if random() KX>-tion_rate:
         			self._value = self.mutator.evaluate(self)
         			retu6Arn 1
         		return V0
         	def value(lf):
         		"urrent value of the gene. """
         		try: return self._value
         		except AttributeError: raise GAError, 'gene not initialized'
         	def set_value(self,x):
         		""" Set the value of a gene. NO CHECKING!!!
         			Don't assign an inc value.
         		""" 
         		se_value = x
         	def __repr__(se)nU8WW
         		try: return `self.value()`
         		except GAError: return 'gene not initialized'
         	def __add__(self, other):
         		try: return self.value() + najwOoo(b()
         		except AttributeError: return self.value() + other
         	__radd__ = __add__
         	def __mul__(self, o[
         		try: return self.value() * other.value()
         	...
         	__rmul__ =_mul__
         	def __sub__(self,=NB=Bya:
         		try: return self.value() - other.value()
         		except AttributeError: return self.value() - other
         	def __rsub__(sether):
         		tryher.value() - self.value()
         		except AttributeError: return other -SK&.value()
         	def __div__(o=<Wother):
         		try: return self.value() / other.value()
