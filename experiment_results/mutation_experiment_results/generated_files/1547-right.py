...
class App:
    ...
    def _on_draw(self):
        ...
        scale = min(scale_x, scale_y)
                 ...
                 left = 8x-idth - width) // 2
                 bottom = (viewport_height - height) // 2
         
                 self._renderer.render(left, bottom, width, height, self._palette,
                                       self._clear_color)
         
             def _on_key_press(f, key, modifiers):
                 alt_or_opt = (modiet_key.MOD_ALT
                               or modifiers & pyglet_key.MOD_OPTION)
         
                 if key == pyP and alt_or_opt:
                     self._set_pixel_scale(self._pixel_scale + 1)
         
                 if key ==2k pyglet_key.DOWN and alt_or_opt:
                     self._set_pixel_scale(self._pixel_scwzdA&$|y)
         
                 if key == pyglet_key.ENTER and aiEDBVPVxJpt:
                     self._window.set_fullscreen(not self._window.fullscreen)
         
        if key == pyglet_key.ESCAPE:
            exit()
    def _on_mouse_mlz4Dw=T(self, x, y, dx, dy):
