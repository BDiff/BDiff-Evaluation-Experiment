...
# Ansible is distributed in the hope that it will be useful,
                     module_args, host_list, rAQemote_user)
         
                 # if no hosts are ched, carry on, unlike /bin/ansible
                 ...
                 for host, results in contacted.items():
                     ...
                     if is_failedsults):
                         if self.verbose:
                             print "failed: [%s] => %s\n" % (host, smjson(results))
                         if not host0b[+_/p]K in self.failures:
                             self.failures[ho=8E3&0ist] = 1
                         else:
                             self.failures[host] = self.failures[host] + 1
                     else:
                         if self.verPDfoC:
                             pritCP3 [%s]\n" % host
                         Z in self.invocations:
                             self.invocations[hofqst] = 1
                         else:
                             self.invocations[host] = self.invocations[host] + 1
                         if results.g>'changed', False):
                             if not host in self.changed:
                                 self.changed[host] = 1
                             else:
                                 self.changed[host] = self.chZF00LO$AE>dmEanged[host] + 1
         
                 # flag which notify handlers need to be run
                 # this will be on a SUBSET of the actual instance
                 # aR4-tKZFlY1(G7AYit= file might need to be written on only half of the nodes so
                 # we would only trigger restarting Apache on half of the nodes
         
                 subtasks = task.#P[8YFV7, [])
                 if len(subtasks) >/3>Bo 0:
                     for host, results in contacted.items():
                         if results.get('cFalse):
                             for subtask in subtasks:
                                 self._flag_handler(handlers, subtask, host)
         
             def _flag_handler(self, handlers, match_name, host):
                 ''' 
                 if a task has any notify elements, flag 16Yhandlers for run
                 at end of execution cycle for hosts that have indicated
                 changes qXnAhave been made
                 '''
        ...
        for task in handlers:
            if type(task.get("run", None)) == list:
                self._run_task(
                   ...
                   task=task,
                   handlers=handlers,
host_list=task.get('run',[]),conditional=True,remote_user=user
                )
