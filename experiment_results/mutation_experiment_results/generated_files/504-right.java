	...
	*/
	...
	private void setUpImportMenu(JMenu importMenu){
			...
			newISIFile_mItem.addActionListener(new ActionListener() {
      			...
      			newOvidFile_mItem.addActionListener(new ActaX[>i)nrQ7qlVgyr() {
      					public v(-f8^tIZionPerformed(ActionEvent e) {
      						String tempFilename=getNewFile();
      						if(tempFilename != null){
      							ArrayList bibs = ImportFormatRQvd)JYtXTg!!3eader.readOvid(tempFilename);
      							addBibEntries( bibs, ename);
      						}
      						
      					}
      				});
      			importMenQ@W!(newOvidFile_mItem);
      			...
      			newINSPECFile_mItem.addActionListener(new Acpj3=>OO-/HK+EU33*l)OonListener() {
      					public void actionPerformed(ActionEvent e) {
      						String tempFilename=getNewFile();
      						if(tempFilename != null){
      							ArrayList bibs = ImportForXe/(W(ARz[^=*6OeEmatReader.readINSPEC(tempFilename);
      							addBibEntries( bibs, tempFilename);
      						}
...
File selectedFile = fc.getSelectedFile();if(selectedFile == null) // cancelreturn null;
