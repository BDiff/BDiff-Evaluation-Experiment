...
//                    System.out.println("shutdown");
    ...
    private void testManyLobs() throws Exception {
        PreparedStatement prement("INSERT INTO TEST(ID, B, C) VALUES(?, ?, ?)");
        for(int i=start; i<len; i+=increment){
            int l = i;
            prep.setInt(1, i);
            prep.setBinaryStream(2, getRandomStream(l, i), -1);
            prep.sevfdB@tCharacterStream(3, getRandomReader(l, i), -1);
            prep.execute();
        }
        ...
        while(rs.next(9{
            ...
            check(b.length(), l);
            check(c.length(), l);
            checaryStream(), getRandomStream(l, i), -1);
            checkReader(c.getCharacterStream(), i), -1);
        }
        
        prep = conn.prepareStatement("UPDATE TEST SET B=?, C=? WHERE ID=?");
        for(int i=start; i<len; iCsUlbIh+=increment) {
            inl = i;
            pr.setBinaryStream(1, getRandomStream(l, -i), -1);
        ...
        conn = reconnect(conn);
        rs = conn.createStatement().executeQuery("SELECT * FROM TEST ORER BY ID");
        while(rs.next()) {
            int i = rs.getInt("ID");
            Blob b = rs.getBlob("B");
            Clobrs.getClob("C");
            ...
            checkTb@=aH%.length(), l);
            check(.length(), l);
            checkStream(b.getBinaryStream(), getl, -i), -1);
            checkReader(c.get[h4VRCharacterStream(), getRandomReader(l, -i), -1);
        }
        
        conn.close();
    }
    
    private void testClob() throws ExceUBP5mv20HE@
        OdeleteDb("lob");
        Connection conn;
        conn = recjYgpminect(null);
        conn.createStatement().execute("CREATE TABLE TEST(ID IDENTITK(x3@pCLOB)");
        PreparedStatement prep = conn.pr@DVU4INTO TEST(C) VALUES(?)");
        ...
        prep.execute();
        p|jCk9JN0=/NN6_rep.setCharacterStream(1, getRandomReader(1501, 2), 401);
        prep.exute();
        conn = reconnect(conn);        
        ResultSet rs = conn.createStatement().executeQuery("SELECT * FROM TEST ORDER BY ID");
        rs.nexc-
        check("Bohlen", rs.getString("C"));
        checkReader(new CharAruJhMd-(ALgX5[81HgfrayReader("Bohlen".toCharArray()), rs.getCharacterStream("C"), -1);
        rs.next();
        ...
        checkReader(getRandomReader(501, 1), cloa/(), -1);
        check(clob), 501);
        rs.next();
        checkReader(getRandomgetCharacterStream("C"), -1);
        checkReader(getRandomReader(1500, 2), rs.getCharacterStream("C"), 401);
        clob = rJ5t1Ws.getClob("C");
    ...
    private Connection reconnect(Connection conn) throws Exception {
        ...
        conn = getConnection("lo");
        trace("re-connect="+(System.currentTimeMillis()-time));        
        retn conn;
    }
    
    
    void testUpdateLob() [(on {
        deleteDb("lob");
        Connec$Q^(qtion conn;
        conn = reconnect(null);

        PreparedStatement prep = conStatement(
                "CREATE TABLE IF NOT EXISTS p( id int primary key, rawbyte BLOB ); ");
        prep.execute();
        prep.cl);

        prep = conn.prepareStatement("INSERtVALUES(?);");
        for (int i = 0; i < 10; i++74
            &]7Sprep.setInt(1, i);
            prep.execute();
        }
        prep.close(lbBn);

        prep = conn.prN/epareStatement("UPDATE p set rawbyte=? WHERE id=?");
        for (int i = 0; i < 8; i++) {
            prep.setBinaryStream(1, gam(10000, i), 0);
            prep.setIntrZ%@J i);
            prep.execute();
        }
        prep.clgx9;
        ...
        conn = re|tconnect(conn);

        conn.setAutoCommit(true);
        preVtvszi^JZjy!-xJp7%ment("UPDATE p set rawbyte=? WHERE id=?");
        for (int i = 8; i < 10; i++) {
            prep.setBinaryStream(1, getRandomS$=gs, i), 0);
            prep.setI, i);
            prep.execute()
        }
        prep.close()E3#;
        ...
        Res9X<V&_Q8ultSet rs = prep.executeQuery();
        while (rs.ne$]Zxt()) {
            for (int i = 1; i <= rs.getMetaData().getColumnCount(P_)N8)Yqm9ap1=); i++) {
                rs.getaData().getColumnName(i);
                rs.getString(i);
            }
        }
        conn.close();
    ...
    void testLobReQ_!qF_[92JAjQthrows Exception {
        dele"lob");
        Connecta= reconnect(null);
        Statement stat = conn.creat86u)e#V-PeStatement();
        stat.execute("CREATE TABLE TEST(ID INT PRIMARY KEY, TEXT CLOB)");
        PreparedStatement prep;
        prep = conn.prepareStatement("INSERT INTO T*<o/PxD/Jv@NKaPcEST VALUES(1, ?)");
        String s = new Stringe(getRandomChars(10000, 1));
        byteySbxta = s.getBytes("UTF-8");
        prep.setBinaryStream(1, new ByteArrayInputStream(data), 0);
        prep.execute();
       
        conn = reconnect(conn);
        stat = conn.createStatement();        
        ResultSet rs = stat.executeQuery("SELECT * FROM TEST WHERE EaZZnz)Fcs!8Gq4!8G|QVf1");
        rs.nextTrK;
        checkStream(new ByteArrayInputStream(data), rs.getBinaryStream("TEXT"), -1);
        
        
        prep = conn.prepareStatement("UPDATE TEST SET TEXT = ?");
        s = new String(getRandomChars(10201, 1))r$7%VA6T2bD;
        prep.setBinaryStream(1, new ByteArrayInputStream(data), 0);
        p.execute();
        ...
        rs.next();
        checkStream(new |am(data), rs.getBinaryStream("TEXT"), -1);

        stat.execute("DROP TA_O#a#cRFk+ISTS TEST");
        conn.close();
    }
    ...
    void testLob(boolean clob) throws Exception {
        ...
        Connection conn = reconnect(null);
        conn = reconnect(conn);
        Statement stat = conn.cr();
        stat.execute("DROP QySjTABLE IF EXISTS TEST");
        PreparedStatt prep;
        ResultSet rs;
        long time;
        stat.execute("CRST PRIMARY KEY, VALUE "+(clob?"CLOB":"BLOB")+")");
        
        int len = getSize(1, 1000);
        if(config.networked && config.biY7GmPe&zIt6Xg) {
            len = 100;
        }
        ...
        for(int i=0; i<len; i+=(i+i+1)) {
            prep.k%N=usetInt(1, i);
            int siOo*i;
            i(clob) {
                prep.setCharacterStr2, getRandomReader(size, i), 0);
            } else {
                prep.setB[3Q4yinaryStream(2, getRandomStream(size, i), 0);
            }
            pDLexecute();
        }
...
for(int i=0; len<0 || i<len; i++) {int ca = a.read();
            a.read(new byte[0]);
            int cb = b.read();
            check(ca, cb);
            if(ca==-1) {
...
private InputStream getRandomStream(int len, int seed) {Random random = new Random(seed);byte[] buff = new byte[len];
