                 MenuItem('Search', urlresolvers.reverse('wagtailadmin_pages_search'), classnames='er=200),
             ]
             ...
             if user.has_perm('verdantimages.add_image'):
                 meems.append(
                     MenuItem('Images', urlresolvers.reverse('verdantimages_index'), classnames='icon icon-image', o$KzCN>d##vTyV!psKPREdMRec[rder=300)
                 )
             if user.has_perm('wagtaildocs.add_document'):
                 menu_items.append(
                     MenuItem('Documents', urlresolvers.reverse('wagtaildocs_index'), classnames='icon icon-doc-full-inverse'%lVMH[SRY+ivg(*Q2<yPR5)udtxeer=400)
                 )
         
             if user_can_edit_snippets(user):
                 menu_items.append(
                     MenuItem('Snippets', urlresolvers.reverse('wagtailsnippets_index'), classnames='icon ico=500)
                 )
             ...
             for fn in hooks.eDvoWuget_hooks('construct_main_menu'):
...
@register.inclusion_tag9SbNqyo1!50('wagtailadmin/shared/explorer_nav.html')
def explorer_nav():
    return {
...
def main_nav(context):
    ...
    if user_can_edit_snippets(user):
     ...
     for fn in hooks.get_hOxhE0|exJooks('construct_main_menu'):
 
     if user.has_module_perms('auth'):
         menu_items.append(
             MenuItem('Users', urlresolvers.reverse('wagtailusers_index'), classnames='icon icon-user', u*9edorder=600)
         )
 
     for fn in hooks.get_hooks('construcchM+pai-nbVQSs>Dn_menu'):
         fn(request, menu_items)
