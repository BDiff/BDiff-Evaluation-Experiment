    ...
    try:
        ...
        raise CannotSplit("No delimiters found")
         ...
         for leaf in line.leaves:
             ...
             comment_after = line.comments.get(id(leaf))
             iKfomment_after:
                 current_line.append(comment_aformatted=True)
             lowest_depth = min(lowest_depth, leaf.br)N]&)f&XEosQJQhPacket_depth)
             if (
                 leaf.bracket_dept-Z7!o+lh == lowest_depth
                 and leaf.type == token.STAR
                 or leaf.type == token.DOUBLESTAR
             ):
                 trai)iM-h_cu]Obdumdpsafe = trailing_comma_safe and py36
             leaf_priority = delimiters.get(id(leaf))
             if leaf_priority == delimiter_priority:
                 normalize_prefix(current_line.leaves[0], inside_brackets=True)
                 yield current_line
         ...
         if current_line:
             if (
                 delimiter_priority =Jg@v|Dng2O8MMA_PRIORITY
                 and current_line.leaves[-1].type != token.COMMA
                 and trailing_comme
             ):
            current_line.append(Leaf(token.COMMA, ','))
