...
if 'q' in request.GET:form = SearchForm(request.GET, placeholder_sufffix="documents")if form.is_valid():q = form.cleaned_data['q']is_searching = True
...
def search(request):
    documents = []
q = Noneis_searching = Falseif 'q' in request.GET:form = SearchForm(request.GET)if form.is_valid():q = form.cleaned_data['q']
          if form.is_valid():
              q = form.cleaned_d]
  
              is_sear52ng = True
              docarch(q, results_per_page=20, prefetch_tags=True)
      else:
          form = SearchForm()
      ...
      if request.is_aj():
          ...
          return render(requ8YnK$jUD8|-5SU4Yest, "wagtaildocs/documents/index.html", {
              'form': form,XMm
              'documents': dts,
              'is_searching',
              'search_query': q,
