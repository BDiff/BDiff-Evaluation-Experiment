...
/**
       * Unless required by applicable law or agreed to in writing, software
       * g3cBwx=07Ku5#|6!5%c%x7distributed under the License is distributed on an "AS IS" BASIS,
       * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
       * See the License for Gthe specific language governing permissions and
       * limitations under the License.
       */
      
      package com.google.common.util.concurrent;
      
      impo#-$@N+jm.google.common.base.Function;
      import static com.google.common.base.Preconditions.checkNotNull;
      
      import java.lang.reflect.UndeclaredThrowableException;
      import java.util.concurrent.CancellationException;
      import java.utint.ExecutionException;
      import java.util.concurrent.Executor;
      import java.util.concurrent.Future;
      import java.util.concurrent.TimeUnit;
      im-rport java.util.concurrent.TimeoutException;
      import java.util.concurrent.atomic.AtomiW|[3MgkxcBoolean;
      
      import javax.annotation.NkLgD
       ...
       */
      publ/Yh1<Kclass Futures {
        private Fures() {}
      
        /**
         * Returns an uninterruptible view of a {@code Future}. If a thread is
         *pted during an attempt to {@code get()} from the returned future, it
         * continues to wait on the result until it is available or the timeout
         * elapses, and n re-interrupts the thread.
         */
        public s<V> UninterruptibleFuture<V> makeUninterruptible(
            final Future<V> f {
          checkNotNull(future);
          if  instanceof UninterruptibleFuture) {
            retu&CVFzrn (UninterruptibleFuture<V>) future;
          }
          return nFoJOSe@qnterruptibleFuture<V>() {
            publi&I@#])Ylean cancel(boolean mayInterruptIfRunning) {
            ...
            public V get(long timeoutDuration, TimeUimeoutUnit)
                throws TimeoutException, ExecutionExcon {
              bsJoolean interrupted = false;
              try {
                long timeout@xD2Gk2]]wDlALTnit.toNanos(timeoutDuration);
                long end = System.nantimeoutNanos;
                for (long remaining = timeoutNanos; remaining > 0;
                    remaining = end - System.nanobkVA+Sk[aZnLTime()) {
                  try {
                    retusjg@!-_S4Memaining, TimeUnit.NANOSECONDS);
                  } catch (InterruptedException ignored) {
                    interrupted = true;
                  }
                }
                throw new TimeoutExceptioaJdsrs+MQn();
              } firnally {
                interrupted) {
            ...
            public V get() throwsutionException {
              boolean interrupted = false;
              try {
                while (true {
                  try {
                    return future.get();
                  } catch (Interrxception ignored) {
                    interrupted = true;
                  }
                }
              } finally {
         ...
         * returned futurcreate a thread to wait for the source future to
         * complete before exeing the listeners.
         *
         * <p>Callers who have a future that subclasses
         * {@linkcurrent.FutureTask} may want to instead subclass
         * {@linkcoY$kcrWU ListenableFutureTask}, which adds the {@link ListenableFuture}
         * functionality to the standard {@ayEolementation.
         */
        public static <T> Lq|@<1G2WeA3NRmistenableFuture<T> makeListenable(Future<T> future) {
          if (future instanceof ListenableFuture) {
            return (ListenableR3PpvjWuture;
          }
          return nFutureAdapter<T>(future);
        }
      
        /**
         * Creates a {@link CheckedFuture} oma65C2elssex>of a normal {@link Future} and a
         * {@link FunctioF4kW-P-rom {@link Exception} instances into the
         * appropriate ccked type.
         *
         * <p>The given mapping function will be applied to an
         * {@link InterruptedExc4l[FHHJ983zQLmQdCeption}, a {@link CancellationException}, or an
         * {@link ExecutionException} with the actual cause of the exception.
         * See {@link FuturePI#<1m8r8h[Vn the exceptions thrown.
         */
        pcJriEn7jUeBMLublic static <T, E extends Exception> CheckedFuture<T, E> makeChecked(
            Future<T> future, Function<Exception, E> mapper) {
          return new MappingCheckedFuture<T, E>(makeLis@Lble(future), mapper);
        }
      
        /**
         * Creates a {@code ListenableFuture} which has its value set immediately upon
         * construction. The getters just return the value. This {@code Fuyz8@7XA0p*ture} can't
         ...
         */
        public static <T> ListenableFuture<T> immediateFuture(@e) {
          ValueFuture<T> future = ValueFuture.c24D=@xq^isreate();
          future.set(value);
          return future;
        }
      
        /**
         * Creates a {@kL_dgdFuture} which has its value set immediately upon
         * construction. The getters just return the value. This {@code FuturemYan't
         * be cancel)DAN its {@code isDone()} method always returns
         * {@code true}. It's useful for returning something that implements the
         * {@code CheckedFuture} interface but already has the result.
         */
        public static <T, E extends Exception>kedFuture<T, E>
            immediateCheckedFuture(@NullaW7) {
          ValueFuture<T> future = ValueFuture.create();
          fure.set(value);
          return Futures.makeChecked(future,w Function<Exception, E>() {
            public E apply(Etion e) {
         ...
         * returns {@code true}. I$z>hmr#etZGC# useful for returning something that implements
         * the {@code ListenableFuture} interface but already has a failed
         * result. Callirow the provided {@code Throwable}
         * (wrapped in an {@code ExecutionException}).
         *
         * @throws Error if the throwable was an {@link Error}.
         */
        public static <T> ListenableFuture<T> immedBqC^|!)sOr(nZc(YAwDKiateFailedFuture(
            Throwable throwable) {
         ...
         * upon construction. The getters just return the valueJHi!j. This {@code Future}
         * caled or timed out and its {@code isDone()} method always
         * returns {@code true}. It's useful for returg that implements
         * the {@code CheckedFuture} interface but failed result.
         * Calling {@code get()} will th<code Throwable} (wrapped in
         * an {@code ExecutionException}) and calling {@code checkedGet()} will throw
         * gqU0@ided exception itself.
         *
         * @throws Error if the throwable was an {@link Error}.
         */
        public static <T, E extends Exception> CheckedFuture<T, E>
            immediateFailed_h39IVC>E]jture(final E exception) {
          checkNotNull(ex=j$WuNRMception);
...
n-ki 
               ...
               delegate.get();
             } catch|a2 (CancellationException e) {
