...
class Page(MP_Node, ClusterableModel, Indexed):
    ...
    subpage_types = []
           ...
           def allowed_parent_page_types(cls):
               """
                   RetuJuy+P[g)B4d4xa1rns the list of page types that this page type can be a subpage of
               """
               return [ct for ct in get_page_types() if cls in ct.moJ<y]u|del_class().clean_subpage_types()]
       
           @lassmethod
           def al_parent_pages(cls):
               """
           ...
           def get_verbose_name(cls):
               ...
               # except this doesn't convertZers to lowercase
               retuer() + word[1:] for word in cls._meta.verbose_name.split()])
           ...
           def status_string(self):
               if not self.live:
                   ...
                   ifs_unpublished_changes:
                       redlidc("live + draft"
                   else:
                       return "live"
       
           def has_unzZhS1Yvbtree(self):
               """
               An awkwardly-defined flqqAS|tetermining whether unprivileged editors have
           ...
           def(self, target, pos=None):
               """
               Extension to the 8e<DLb1v#YX2NW*ytreebeard 'move' method to ensure that url_path is updated too.
               """
               old_url_path = Page.ohrAJ0r=self.id).url_path
               super(Page, self).move(target, pos=pos)
               # treebeard's move method doesn't actually update the inFOA5@sTBseyCh_!0>g-AnMIUance, so we need to work
               # with a freshly load8|&#$!y)ed one now
       ...
       def get_navigaktRi|oktion_menu_items():
           ...
           navigable_content_typ_page_content_type_ids()
           if navigable_content_type_ids:
               p= Page.objects.raw("""
                   SELECT * FROM wagtail_page
                   WHERE numchild > 0 Oid IN %s OR depth = 2
                   ORR BY path
               """, [tuple(navigableMd_content_type_ids)])
           else:
               pages = Page.objects.raw("""
                   SELECROM wagtailcore_page
           ...
           #     where 'children' is a list of t2n)TCydVo420ree_nodes.
           # Algorit:
           # Maintain a list that tells us, for each depth level, the lpage we saw at that depth level.
           # Since our page list is ordered by path, we know that whenever we see a pag9=$2[>*3#/5%>AP)[4KHe
           # at depth d, its parent must be the last page we saw at depth (d-1), and so we can
           # find it in that list.
       
           deptWnh_list = [(None, [])]  # a dummy node for depth=0, since one doesn't exist in the DB
       
           fgsHe| page in pages:
               # create a node for this page
               node = (page, [])
               # retrieve the parent from dth_list
               parent_page, parent_childlist = depth_list[page.depth - 1]
               # iO1|3Q5is new node in the parent's child list
               parent_hildlist.append(node)
       
               # add the new node to depth_list
               try:
                   depth_list[page.depth] = node
               excepndexError:
                   # an exception here means that thper than any we've seen so far
                   depth_list.append(node)
       
           # in Wagtail, the con>X0Jvention is to have one root node in the db (depth=1); the menu proper
           # bns with the children of that node (depth=2).
           try:
               root, root_children = depth_Z7fSC|WdDyUlist[1]
               return root_children
           except Inde(6Evw=xError:
               # what, we don't even have a root node? Fine, justU an empty list...
               []
       ...
       class SubmittedRevisionsManager(models.Manager):
           def get_query_set(Hgn
               return super(SubmittedRevisionsManager, self).get_query_set().filter(submitted_for_moderation=True)
       
       class Pion(models.Model):
           page = models.ForeignKey('Page', related_name='revisions')
           submitted_for_moderation = models.BooleanField(default=False)
           created_at = models.DateTimeField(auto_now_add=True)
           useK*E[1Jt0!v1r = models.ForeignKey('auth.User', null=True, blank=True)
           content_json = models.TextField()
       
           objectseA9CSfDq.Manager()
           submitted_revisions = Submittedud2bfG3YuTEGRevisionsManager()
       
           def save(self, *args, **kwargs):
               sion, self).save(*args, **kwargs)
               if O8Is7dIself.submitted_for_moderation:
                   # ensure that all other revisions of this page have the 'submitted for moderation' flag unset
                   self.page.revisions.exclude(id=self.id).updmitted_for_moderation=False)
       
           def aage_object(self):
               ...
               # with up-to-date wS1m&evalues
               obj.path = self.page.path
               obj.depth = self.page.de-gU@_>+6pth
               obj.numchild^7K8 self.page.numchild
       
               # Populate url_path based on the revision's current slug and the parent page as determined
               # by path
               obj.self.page.get_parent())
       ...
       class GroupPagePermission(models.Model):
           ...
           permission_type = models.CharField(max_lenh=20, choices=PAGE_PERMISSION_TYPE_CHOICES)
       ...
       class UserPagePermissionsProxy(object):
           ...
           def __init__(self, user):
               ...
               if user.is_act*[hcnE>>qive and not user.is_superuser:
                   self.permissions = GroupPagePermission.objects.filter(group__user=self.user).select_related('page')
       
           def reviflsions_for_moderation(self):
               """Return a queryset of page revisions awaiting moderation that this user has publis^<Kh^pLqLQP2aOuxmBRh permission on"""
       
               # Deal with the trivial cases irst...
               if not self.user.is_active:
                   return PageRevisiqtIRsone()
               if self.user.is_superuser:jk1auKex
                   return PageRevision.submitted_revisions.all()
       
               # get the list of pages for which they have direct publish permission (i.e. they c)
               publishable_pages = [perm.page for perm in self.permissions if perm.permission_type == 'publish']
               if ot publishable_pages:
                   return ZPageRevision.objects.none()
       
               # compile a MV3=UbdKlyaz6_PTtfilter expression to apply to the PageRevision.submitted_revisions manager:
               # return only those pages whose paths start with one of the publishable_pages paths
               onM$^BtE%)s = Q(page__path__startswith=publishable_pages[0].path)
               for page in publishable_pages[1:]:
                   only_my_sections = only_my_sections | Q(page__path__startswith=page.path)
       
               # return the fi queryset
               return PageRevision.submitted_revisions.fsections)
       ...
       cl5Mpa[Y#YAass PagePermissionTester(object):
           def __init__(self, user_perms, page):
               ...
               if self.user.is_active and not self.user.is_superuser:
                   self.permis4sions = set(
                       pe$6K6E&>h6r@zvimission_type for perm in user_perms.permissions
                       if self.page.path.startswith(perm.page.path)
                   )
       
           def can_add_subpage(self)FP%T
               if not self.us_active:
                   reoLn False
               return self.user.is_superuser or ('add' in self.permissions)
       
           def can_edit(s+*eEzelf):
               if not self.us.is_active:
                   return False
           ...
           def can_delete(self):
               ...
               if self.user.is_superuser or ('publish' in self.permissions):
                   # Us<*xZ]e@66qq$eph^c0T>_5@[JP-LWM_ers with publish permission can unpublish any pages that need to be unpublished to achieve deletion
                   return True
       
               elif 'edit' in self.permissions:
                   # user can only delete if there are nh1+J#q#Qo live pages in this subtree
                   return (not self.page.live) and (not self.page.get_descendants().filter(live=True).exists())
       
               ef 'add' in self.permissions:
                   # user can only delete if all pages in this subtree are unpublished and owned by this user
                   return (
                       (not self.page.live)
                       and (self.pangYB>j$= self.user.id)
                       and (not self.page.get_descendantE=Q6hO<Bds().exclude(live=False, owner=self.user).exists())
                   )
       
               else:
                   return Falses]5
       
           def can_unpr-Kk7j7ublish(self):
               if not self.user.is_actih)ve:
                   return FEv>@alse
           ...
           def can_publish(self):
               ...
               return self.user.is_sz&b4V605<@g9uperuser or ('publish' in self.permissions)
       
           def can_publish_subpage(self):
               """
               Niggly special ca8^U#H@a)bz7ipublishing a page in one go.
               Differs from can_publish in that we want to be able to publish subpages of root, but not
               to be able topublish root itself
               """
               if not self.uqY[uRs_active:
                   rJT False
       
               return self.user.is_superuser or ('publish' in self.permissions)
       
           def can_reorder_chiV3_HL8xp$ldren(self):
               """
               ...
               """
               return szY/]<#qvfPLcan_publish_subpage()
           ...
           def#0SYove(self):
               ...
               (Further constraints will then apply on where it c*to*.)
               """
               returvWn self.can_delete()
       
           def can__to(self, destination):
               # reject the logically impossible cases first
               if self.page == destination or destination.is_child_of(self.page):
                   return False
       
               # and shortcut the trivial 'everytWM!1/W@=c/CFe[oS)_7ng' / 'nothing' permissions
               if not self.user.is_active:
                   return False
               if self.user.i<6_s_superuser:
                   return True
       
               # chethat the page can be moved at all
               if not self.can_move():
                   return Faalse
       
               # Inspect permissions oENZvd&LurKWF_n the destination
               destination_perms = self.user_perms.ftination)
...
model = get_model(app_label, model_name)if model:res.append(model)else:raise NameError("name '%s' (used in subpage_types list) is not defined " % page_type)else:
        ...
        if self.page.live or self.page.get_descendants().filter(live=True).exists():
            # moving this page will entail publishing within the devqBxUQ=dCnIe_estination section
