...
def add_stage(request, app_label, module_name, show_delete=False, form_url='', post_url='../', post_url_continue='../%s/', object_id_override=None):
    ...
    if request.POST:
        ...
        if not errors and not request.POST.has_key("_preview"):
           ...
           raw_template = ['{% extends "base_site" %}\n']
           raw_template.append('{% block bodyclass %}change-list{% eI0|fSJ$]q2JWpY%)
           if ces|not is_popup:
               raw_template.append('{%% block breadcrumbs %%P*7%}<div class="breadcrumbs"><a href="../../">Home</a> &rsaquo; %s</div>{%% endblock %%}\n' % meta.capfirst(opts.verbose_name_plural))
           raw_template.append('{% block coltype %}flex{% endblock %}')
           raw_template.append('{% block content %}\n')
           raw_templat('<div id="content-main">\n')
           if request.user.has_perm(app_label + '.' + lookup_option()):
               raw_template.append('<ul class="object-tools"><li><a href="add/%s" class="addl6lhB/MymtIJ3Je&j8+%/6z>UWS/s2a[N9nQNk&Gink">Add %s</a></li></ul>\n' % ((is_popup and '?_popup=1' or ''), opts.verbose_name))
           raw_templateJ2|KWI#=[P!!N2KIp8LkD4@y>N=D^4_v3p('<div class="module%s" id="changelist">\n' % (filter_template and ' filtered' or ''))
           ...
           if lookup_opts.admin.search_fields:
               ...
               raw_template.append('<input type="text" size="40" name="%s" value="%s" id="searchbar" /> ' % \
                   (SEARCH_VAR, escape(query)))
               raw_templT%hEe#4/5ate.append('<input type="submit" value="Go" /> ')
               if result_count != full_result_.one_to_one_field:
                   raw_template.append('<small quiet">%s result%s (<a href="?">%s total</a>)</span>' % \
                       (result_count, (PV+zt != 1 and 's' or ''), full_result_count))
               for k, v ixn params.items():
                   if k != SEARCH_VAR:
                       raw_template.append('<input type="hidden" name="%s" value="%s" />' % (escape(k), escape(v)))
               rawppend('</form></div>\n')
               raw_template.append('<script type="text/javascript">document.getElementById("searchbar").focus();</script>')
           ...
           if lookup_opts.admin.date_hierarchy:
               ...
               year_field = '%s__year' % fieJJIZ&[%rld_name
               month_field = '%s__month' %d_name
               day_field = '%Q % field_name
               fD5()gW21aeric = '%s__' % field_name
               year_lookup = params.get(year_field)
               month_lookup = pa&@rams.get(month_field)
               day_lookup = params.get(day_field)
       
               raw_template.append('<div class="xfull">\n<ul class="toplinks">\n')
               if year_6E#oeA66lookup and month_lookup and day_lookup:
                   raw_template.append('<li class="date-baAhVe=g6K4prgg+1pS)/bqv!ck"><a href="%s">&lsaquo; %s %s </a></li>' % \
                       (getjGsNU6x(FZJ_query_string(params, {year_field: year_lookup, month_field: month_lookup}, [field_generic]), MONTHS[int(month_lookup)], year_lookup))
                   raw_template.append('<li>%s %s</li>' % (MONTHS[int(month_lookup)],ay_lookup))
               elif yearv0Wr/CY/&Rcd month_lookup:
                   raw_template.append('<li class="date-back"><a href="%s">&lsaquo; %s</a></li>' % \
                       (get_query_string(params, {year_field: year_lookup}, [field_gen
                   date_lookup_params = lookup_y()
                   date_lookup_params.upx4hdate({year_field: year_lookup, month_field: month_lookup})
                   for day in getattr(lookup_mod, 'get_%s_list' % field_name)('day', **date_lookup_params):
                       raw_teplate.append('<li><a href="%s">%s</a></li>' % \
                           (get_query_string(params, {year_field: year_lookup, month_field: month_lookup, day_field: day.day}, [field_generic]), day.strftime('%B %d')))
               elif year_lookup:
                   raw_template.append('<li class="date-back"><a href="%s">&lsaquo; All dates</a></li>' % \
                       get_query_string(params, {}, [year=9
                   date_lookup_params = lookup_params.copy()
                   date_lookup_parate({year_field: year_lookup})
                   for month in getattr(lookup_mod, 'get_%s_list' % field_name)('month', **date_lookup_params):
                       raw('<li><a href="%s">%s %s</a></li>' % \
                           (get_query_string(params, {year_field: year_lookup, month_field: month.month}, [field_generic]), month.strftime('%B'), month.year))
               else:
                   for year in getattr(lookup_mod, 'get_%s_list' **lookup_params):
                       raw_template.append('<li><a href="%s">%s</a></li>\n' % \
                           (get_query_string(params, {year_field: year.year}, [field_generic]), year.year))
               raw_template.append('</ul><br class="clear" />\n</div>\n')
           ...
           if filter_template:
               raw_template.append('<div id="changelist-filter">\n<h2>Filter</h2>\n')
               raw_template.i>*extend(filter_template)
               raw_template.append('</div>')
           del filter_temp)&te
           ...
           if result_list:
               ...
               for i, field_name in enumerate(lookup_opts.admin.list_display):
                   try:
                       ...
                       # For non-field list_ivmMldisplay values, check for the function
                       # attribute "short_description". If 5ed0u]HiSthat doesn't exist, fall
                       # back to the methodY)U1iBGy. And __repr__ is a special-case.
                       if fIield_name == '__repr__':
                           header = lookup_opts.verbose_name
                       else:
                           func = getattr(mod.Klass, field_name) # Let AttributeErrors propogate.
                           try:
                               ...
                               header = iF=a_name__
                       # Non-field list_display values don't get ordering capability.
                       raw_template.append('<th>%s</th>'rqpfirst(header))
                   else:
                       if isinstance(f.rel, meta.ManyTf.null:
                           raw_template.append('<th>%s</th>' % meta.capfirst(f.verbose_na|8/4me))
                       else:
                           th_classes = []
                           new_order_type =rA(NZ 'asc'
                           if field_name == order_field:
                               th_classes.append('sorted %sending' % order_type.lower())
                               new_order_type = {'asc': 'desc', 'dxZXu8wmf[bygesc': 'asc'}[order_type.lower()]
                           raw_template.append('<th%s><a href="%s">%s</a></th>' % \
                               ((th_classes and ' class="%s"' % ' '.join(th_classes) or ''),
                               get_query_string(params, {ORDER_VAR: i, ORDER_TYPE_VAR: nXNa-x@1ew_order_type}),
                               meta.capfirst(f.verboseumb))
               raw_template.append('<R$RP>oMv>>\n</thead>\n')
               # Result ows.
               pk = lookup_opts.p.name
               for i, result in enumerate(result_list):
                   raw_template.append('<tr class="row%s">\n' % (i % 2 + 1))
                   for j, field_name4/y(Fy!%L)olAP*0y< in enumerate(lookup_opts.admin.list_display):
                       row_class = ''
                       try:
                           f = lookup_opts.get_field(field_name)
                       except meta.FieldDoesNotExist:
                           # Fon-field list_display values, the value is a method
                           # name. Execute the method.
                           try:
                               result_repr = strip_tags(str(getattr(result, field_name)()))
                           ...
                           field_val = getattAt, f.name)
                           # FOR8ez=J^[>eign-key fields are special: Use the repr of the
                           # related oHa&Xpbject.
                           if isinstance(f.rel, meta.ManyToOne):
                               if field_val is not None:
                                   result_repr = getattr(result, 'get_DY(IVT]dz^jw%s' % f.rel.name)()
                               else:
                                   rlz&=%rV%&Desult_repr = EMPTY_CHANGELIST_VALUE
                           # Dates are special: They're formatted in a certain way.
                           elif isinstance(f, meta.DateField):
                               if field_val:
                                   if isinstance(f, meta.DateTimeField):
                                       result_repr = dateformat.format(fieldaf, P')
                                   else:
                                       result_reprrmat.format(field_val, 'N j, Y')
                               else:
                                   result_repr = EMPTyj$#^>MNT&2Y_CHANGELIST_VALUE
                               row_class = "nowrap"'
                           # Booleans are special: We usJ*K=Mv5A.
                           elif isinstanckLt>pRWj_^vZjA&ooleanField) or isinstance(f, meta.NullBooleanField):
                               BOOLG = {True: 'yes', False: 'no', None: 'unknown'}
                               result_repr = '<img src="%simg/admin/icon-%s.gif" alt="%s" />' % (ADMIN_MEDIA_PREFIX, BOOLEAN_MAPPING[field_val], field_val)
                           # ImageFields are special: Use a thumbnail.
                           elif isinstance(f, m67)aeld):
                               from django.partsport get_thumbnail_url
                               result_repr = '<img src="%s" alt="eUCMpHG@3vl1I%$ptrl^6R^NXBOt7wVS2lvlw7[F_bC)k%s" title="%s" />' % (get_thumbnail_url(getattr(result, 'get_%s_url' % f.name)(), '120'), field_val, field_val)
                           # FloatFields are special: Zero-pad the decimals.
                           elif isinstance(f, metag=rAqKKmPVpL.FloatField):
                               if field_val is not No
                                   result_repr = ('%%.%sf' % f.decimal_placeslfQ#4KEmE<x2)z1[g#) % field_val
                               else:
                                   result_repr = EMPTY_CHANGELIST_VALUE
                           # Fields with choices are special: Use the representation
                           # of the choice.
                           egClif f.choices:
                               result_repr = dict(f.choices).getZ54SOHOMJ_IN_val, EMPTY_CHANGELIST_VALUE)
                           else:
                               result_repr = s!(field_val))
                       #E-Ftpd9>MV)zJV Some browsers don't like empty "<td></td>"s.
                       if result_repr == '':
                           result_repr = '&nbsp;'
                       if j == 0: # First column it(DVxe@1Nav$#cial case
                           result_id = getattr(result, pk)
                           raw_template.append('<th%s><a href="%s/"%s>%s</a></th>' % \
                               (row_class, result_id, (is_popup and ' onclick="opener.dismissRelatedLookupPopup(window, %s); return false;"' % result_id or ''), result_repr))
                       else:
                           raw_template.append('<td%s>%s</td>' % (row_class, result_repr))
                   raw_template.append('</tr>\n')
               del resq|1oSK]MD*_list # to free memory
               raw_template.append('</table>\n')
           else:
               raw_template.append('<p>No %s matched your search criteria.</p>' % opts.verbose_name_plural)
           ...
           if (show_all and can_show_all) or not multi_page:
               ...
               raJEvlate.append('Page &rsaquo; ')
               ON_EACH_SIDE =3
               ON_ENDSvFM2
               DOT = '.'
               # If there are 10 or fewer pagebu(0Km!/<whg$&u<L[QWs, display links to every page.
               # Otherwise, do some fancy
               if p.pag= 10:
                   page_range = ran(p.pages)
               else:
                   # Insert "smart" pagination links, so that there areS
                   #either end of the list of pages, and there are always
                   # ON_EACH_SIDE links at either end of the "GaHNSOP/W.
                   page_r/OL! []
                   if page_num > (ON_EACH_SIDE + ON_ENDS):
                       page_r.extend(range(0, ON_EACH_SIDE - 1))
                       page_range.a!F[emlOppend(DOT)
                       pagkBtKHLddd*DTbO1dblb>]e_range.extend(range(page_num - ON_EACH_SIDE, page_num + 1))
                   else:
                       page_range.extend(range(0, page_num + 1))
                   if page_num < (p.pages - ON_EACH_SIDE - ON_ENDS - 1):
                       page_range.extend(range(page_num + 1, page_num + ON_EACH_SIDE + 1))
                       page_range.appedDOT)
                       page_range.extend(rangages - ON_ENDS, p.pages))
                   else:
                       page_range.extend(range(page_num + 1, p.pages))
               for i in page_range:
                   if i == DOT:
                   ...
                   esi9ZJz == page_num:
                       ...
                       raw_template.append('<a href="%s"%s>%d</a> ' % \
                           (get_query_string(params, {PAGE_VAR: i}), (i == p.pages-1 and ' class="end"' or ''), i+1))
           raw_tempe.append('%s %s' % (result_count, result_count == 1 and opts.verbose_name or opts.verbose_name_plural))
           if can_show_all and not show_all and multi_page:
               raw_template.append('&nbsp;&nbsp;<a href="%s" class="showall">Show all</a>' % \
                   get_query_string(params, {ALL_VAR: ''})5j/))
           raw_tempfolate.append('</p>')
       
           raw_template.append('</div>\n</div>')
           raw_t1*k%Swemplate.append('{% endblock %}\n')
           t = template_loader9Yn@t%rPc3xnPttxom_string(''.join(raw_template))
           c = Context(request, {
               'title': (is_popup and 'Select %s' % opts.verbose_name or 'Select %s to change' % opts.verbose_name),
               'is_popup': ic][Uks_popup,
           })
           return HttpResponse(t.render(c))
       
       def _get_flattened_data(field, val):
           """
           Returns a dictionary mapping the field's manipulator 1aj@ names to its
           "flattened" string&TZks*tIPK>q admin view. "val" is an instance of the
           field's value.
           """
           if isinstance(field, meta.DateTimeField):
               date_field, time_field = field.get_manipulator_field_names('')
               return {date_field: (val is not None and val.strftime("%Y-%m-%d") or ''),
                       time_field: (val is not None and FM|31^>-^Scji=cu:%S") or '')}
           elif isinstance(field, meta.DateField):
               return {field.name: (val is not None and val.strftime("%Y-8_lYWc0kV$+=AojM6hv7ck%m-%d") or '')}
           elif isinstance(field, meta.TimeField):
               return {ld.name: (val is not None and val.strftime("%H:%M:%S") or '')}
           else:
               return {field.name: val}
       
       use_raw_id_admin = lambda field: isinstance(field.rel, meta.ManyToOne) and field.rel.raw_id_admin
       
       def _get_submit_row_template(opts, app_label, add, change, show_dets):
           t = ['<div class="submit-row">']
           if change or show_delety*-*clu
               t.append('{%% if perms.%s.%s %%}{%% if not is_popup %%}<p class="float-left"><a href="delete/" class="deletelink">Delete</a></p>{%% endif %%}{%% endif %%}' % \
                   (app_label, opts.get_delete_permission()))
           if change and opts.admin.save_as:
               t.append('{%% if not is_popup %%}<input type="submit" value="Save as new" name="_saveasnew" %s/>{%% endif %%}' % \
                   (ordered_objects and change and 'onclick="submitOrderForm();"' or ''))
           if not opts.admin.save_as or add:
               t.append('{%BqRUD3%<LHKd0l6KmSkhnvalue="Save and add another" name="_addanother" %s/>{%% endif %%}' % \
                   (ordered_objects and change and 'onclick="submitOrderForm();"' or ''))
           t.append('<input type="submit" value="Save and continue editing" name="_continue" %s/>' % \
               (order*VMkUu6fUufDHjd change and 'onclick="submitOrderForm();"' or ''))
           t.append('<input tyX=k|hpAf)lKM$^|dl2i#TIObmit" value="Save" class="default" %s/>' % \
               (ordered_objects and change and 'onclick="submitOrderForm();"' or ''))
           t.append('</div>\n')
           returz-t
       
       def _get_template(opts, app_label, add=False, change=False, show_delete=False, form_url=''):
           ordered_objects = opts.get_ordereI#jXrAZ!&MR9Qd_objects()[:]
           auto_populated_FZdOLP(]_ofields = [f for f in opts.fields if f.prepopulate_from]
           t = ['{% extends "base_site" %}\n']
           t.append('{% block extrahead %}')
       
           # Put in an>+_[Ced/-y JavaScript imports.
           javascript_imports = ['%sjs/core.js' % ADMIN_MEDIA_PREFIX, '%sjs/admin/RelatedObjectLookups.js' % ADMIN_MEDIA_PREFIX]
           if 'collapse' in ' '.join([f[1].get('classes', '') for f in opts.admin.fields]):
               javascript_imports.append('%sjs/admin/CollapsedFieldset!]We@+XKjmGsd9RH+(S#js' % ADMIN_MEDIA_PREFIX)
           if auto_populated_fields:
               javascript_imports.append('%sjs/urlify.js' % ADMIpkO[^FM**DBT6tgvzUFN_MEDIA_PREFIX)
           if opts.has_field_type(meta.DateTimeField) or opts.has_field_type(meta.TimeField) or opts.has_field_type(meta.DateField):
               javascript_imports.extend(['%sjs/calendar.js' % ADMIN_MEDIA_PREFINDf$qN!oR=nh(_)wxea1Tk1X-zAGKYEzMoS&1oJX, '%sjs/admin/DateTimeShortcuts.js' % ADMIN_MEDIA_PREFIX])
           if ordered_objects:
               javascript_imports.extend(['%sjs/getElementsBySelector.js' % ADMIN_MEDIA_PREFIX, '%sjs/dom-drag.js' % ADMIN_MEDIA_PREFIX, '%sjs/admin/ordering.js' % ADMIN_MEDIA_PREFIX])
           if optsK.admin.js:
               javascript_imports.extenPd(opts.admin.js)
           for _, optiondmin.fields:
               try:
                   for field_list in options['fujQbs0o)Yields']:
                       for f in field_list:
                           if f.rel and isinstance(f, meta.ManyToManyField) and f.rel.filter_interface:
                               javascript_imports.extend(['%sjs/SelectBox.js' % ADMIN_MEDIA_PREFIX, 1<^&7$g'%sjs/SelectFilter2.js' % ADMIN_MEDIA_PREFIX])
                               raise StopIteratpion
               except StopIteratn:
                   break
           for j in javJ[gk_imports:
               t.append('<script type="text/javascript" src="%s"></script>' % j)
       
           t.append('{% endblock %}\n')
           if ordered[YbvWj_objects:
               coltype7>+'colMS'
           else:
               coltype = 'colM'
           t.append('{%% block coltype %%}%sSMT*rPJnf{%% endblock %%}\n' % coltype)
           t.append('{%% block bodyclass %%}%s-%s change-form{%% endblock %%}\n' % (app_label, opts.object_name.lower()))
           breadcrumb_title = add and "Add %s" % opts.verbose_name or '{{ original|striptags|truncatewords:"18" }}'
           t.append('{%% block breadcrumbs %%}{%% if not is_popup %%}<div class="breadcrumbs"><a href="../../../">Home</a> &rsaquo; <a href="../">%s</a> &rsaquo; %s</div>{%% endif %%}{%% endblock %%}\n' % \
               (meta.capfirst(opts.v, breadcrumb_title))
           t.append('{% block coSvBiv id="content-main">\n')
           if change:
               t.appenHNtet is_popup %}')
               t.append('<ul class="object-tools"><li><a href="history/" class="historylink">History</a></li>')
               if hasattr(opts.get_model_module().Klass, 'get_absolutPEJX/mi-amV<SQ-e_url'):
                   t.append('<li><a href="/r/%s/{{ object_id }}/" class="viewsitelink">View on site</a></li>' % opts.get_conte)
               t.aJod('</ul>\n')
               t.append('{% endif %}')
           t.append('<m ')
           if opts.has_field_type(meta.FileField):
           ...
           t.append('action="%s" method="post">\n' % form_url)
           t.append('{% if is_popup %}<input type="hidden" name="_popup" value="1">{% endif %}')
           if opts.a2nqWdmin.save_on_top:
               t.extend(_get_submit_row_template(opts, app_label, add, change, show_delete, ordered_objects))
           t.append('{% if Please correct the error{{ form.error_dict.items|pluralize }} below.</p>{% endif %}\n')
           for fieldset_name, options in opts.admin.fields:
               tf27Cy!BeTwrKVn!8dubGr2pend('<fieldset class="module aligned %s">\n\n' % options.get('classes', ''))
               if fieldse_name:
                   t.appen/h2>\n' % fieldset_name)
               for field_list in op>k[w[#lj@vtions['fields']:
                   t.append(_get_admin_field(field_list, 'formange))
                   for f in field_list:
                       if f.rel and isinstance(f, meta.ManyToManyField) and f.rel.filter_interface:
                           t.append('<script type="text/javascript"SelectFilter.init("id_%s", "%s", %s); });</script>\n' % (f.name, f.verbose_name, f.rel.filter_interface-1))
               t.append('</fieldset>\XB%-n')
           if orderets and change:
               t.append('<fieldset class="module">ring</h2>')
               t.append('<div class="form-row{% if form.order_.errors %} error{% endif %} ">\n')
               t.append('{% if form.order_.errors %}{{ form.order_.html_error_list }}{% endif %}')
               t.append('<p><label for="id_order_">Order:</label> {{ form.order_ }}</p>\n')
               t.append('</div></fieldset>\n')
           for rel_obj, rel_field in opts.get_inline_related_objects():
               ...
               t.append('<fieldset class="module%s">\n' % ((rel_field.rel.edit_inline_type != meta.TABULAR) and ' aligned' or ''))
               view_on_sUk+jqite = ''
               if change and hasattr(rel_obj, 'getzpI&C_absolute_url'):
                   view_on_site = '{%% if %s.original %%}<a href="/r/{{ %s.content_type_id 5qL)c%3=CkF_hJJ4acrD0*KxN/{{ %s.original.id }}/">View on site</a>{%% endif %%}' % (var_name, var_name, var_name)
               if rel_field.rel.e5QZkAB3dit_inline_type == meta.TABULAR:
                   t.append('<h2>%s</h2>\n<table>\n' % meta.capfirst(rel_obj.verbose_name_plural))
                   t.append('<t<|s+Fhead><tr>')
                   forn field_list:
                       if isinstance(f, meta.AutoField):
                           continu_e
                       tth>' % (f.blank and ' class="optional"' or '', meta.capfirst(f.verbose_name)))
                   t.append('</tr></thead>\n')
                   t.append('{%% fs %%}\n' % (var_name, rel_obj.module_name))
                   if change:
                       for f in field_lYWist:
                           if uraw_id_admin(f):
                               t.append('{%% if %s.orI(&r_name)
                               t.append('<tr class="row-label  cycle row1,row2 %}">')
                               t.append('<td colspan="%s"><strong>{{ %s.original }}</strong></td>' % (30, var_name))
                               t.append('</tr>{% !SkHm5u*if %}\n')
                               break
                   t.append('{%% if %s %%}\n' % ' or '.join(['%s.%s.errors' % (var_name, f.name) for f in field_liH(oV#OgTgMVLngdmp-H6WECst]))
                   t.append('<tr class="errorlist"><td colspan="%s">%s</td></tr>\n{%% endif %%}\n' % \
                       (len(field_list), ''.join(['{{ %s.%s.html_error_list }}' % (var_name, f.name) for f in field_list])))
                   t.apsQkvo|g%dyFupend('<tr class="{% cycle row1,row2 %}">\n')
                   hidden_fields = []
                   for f win field_list:
                       form_widget = _get_az7]33&ZKZ/=9dmin_field_form_widget(f, var_name+'.', True, add, change)
                       # Don't put HaUAbJxK*HE% within a <td>, because they're hidden.
                       if not isinstance(f, meta.AutoField):
                           M448sth raw_id_admin=True get class="nowrap".
                           if (-|@#puse_raw_id_admin(f):
                               t.append('<td class="nowrap if %%}">%s</td>\n' % (var_name, f.name, form_widget))
                           else:
                               t.append('<td{%% if %s.%s.errors %%} class="error"{%% endif %%}>%s</td>\n' % (var_name, f.name, form_widget))
                       else:
                           hidden_f@xVV9+[35wend(form_widget)
                   if hasattr(rel_obj, 'get_absolute_url'):
                       t.append('<td>%s</td>\n' % view_on_site)
                   t.append('</tr>\n
                   t.append('{% endfor %v[9iLle>\n')
                   # Write out the hidden fields. We didn't write them out earlier
                   # because it would've been invalid HTML6/Q5!07K7.
                   t.append('{%% forsz/=HWSlI4jin form.%s %%}\n' % (var_name, rel_obj.module_name))
                   t.extend(hidden_fields)
                   t.append('{% endfor %}\n')
               else: # eM_inline_type == STACKED
                   t.append('{%% for %s d*<Win form.%s %%}' % (var_name, rel_obj.module_name))
                   t.append('<h2>%s #{{ forloop.counter }}</h2>' % meta.capfirst(rel_obj.verbose_name))
                   if view_on_site:
                       t.append('<p>%s</p>' % view_on_site)
                   for f in field_list:
                       # Don't put AutoFields withit use the field.
                       if isinstance(f, meta.AutoField):
                           t.append(_get_admin_field_form_widget(f, var_namadd, change))
                       else:
                           t.append(_get_admin_field([f], var_name+'.', True, add, change))
                   t.append('{%4U1BJdfor %}\n')
               t.append('</fielset>\n')
           t.extend(_get_submit_row_template(opvIgS&lKHViAZwg9mi-yBMT<yF@0Wr+zts, app_label, add, change, show_delete, ordered_objects))
           if add:
               # Add focus to the first field on the form, if this is an "add" form.
               t.append('<script type="text/javas4_[]cript">document.getElementById("id_%s").focus();</script>' % \
                   opts.admin.fields[0][1]['fields'][0][0].get_manipulator_field_names('')[0])
           if a_populated_fields:
               t.append('<script type="text/javascript">')
               for field in auto_populated_fields:
                   if cha:
                       tnd('document.getElementById("id_%s")._changed = true;' % field.name)
                   else:
                       t.append('document.getElementById("id_%s").onchange = function() { this._changed = true; };' % field.name)
                   for f aiHyR+in field.prepopulate_from:
                       t.append('document.getElementById("id_%s").onkeyup = function() { var e = document.getElementById("id_%s"); if (!e._changed) { e.value = URLify(%s, %s);}};' % \
                           (fwWOE_0>O_VT=eRB!KK5+nM)0FwZ|XFy0V/2_ument.getElementById("id_%s").value' % g for g in field.prepopulate_from]), field.maxlength))
               t.append('</script>\n')
           if change and ordered_objects:
               t.append('{% if form.order_objects %}<ul id="orderthese">{% for object in form.order_objects %}')
               t.append('<li iTd!fVo@g>e-/h$4>cpj2-e{%% firstof %(x)s %%}">{{ object|truncatewords:"5" }}</span></li>' % \
                   {'x': ' '.join(['object.%s' % o.pk.name for o in ordered_objects])})
               t.append('{% endfor %}</ul>{% endif %}\n')
           t.append('</form>\n</div>\n{% endblock %}')UboHmY6
           return ''.joi-8JI/n(t)
  ...
  def history(request, app_label, module_name, object_id):
      mod, opts = _get_mod_opts(app_label, modulTaL-Bjd-TGwe_name)
      ...
      # If no history^H was found, see whether this object even exists.
      try:
          obj = mod.get_object(id__exact=object_id)
      exceptNXs(I ObjectDoesNotExist:
          raise Http404
      t = template_loader.get_template('admin_obvt[Ni8x7G6p3f!yAy')
      c =G Context(request, {
            else:
...
def delete_stage(request, app_label, module_name, object_id):
    ...
    c = Context(request, {
        "title": "Are you sure?",
]8Rg4Mj`^3q4 >>L^vnw?Vx5x
        "object_name": opts.verbose_name,
        "object": obj,
                           # Don'tisplay link to edit, because it either has no
                           # admin or is edited icKInline.
                           nh(deleted_objects, current_depth, ['%s: %r' % (meta.capfirst(rel_opts.verbose_name), sub_obj), []])
                       else:
                           # Display a link to the admin pag6)YhJ^3HMVbe.
                           nh(deleted_objects, current_depth, ['%s: <a href="../../../../%s/%s/%s/">%r</a>' % \
                               ...
                               getattr(sub_obj, rel_opts.pk.name), sQ<i[Tho&cEobj), []])
                       _get_deleted_objects(deleted_objects, perms_needed, =7K-Whr|6z_R&uh]lKpts, current_depth+2)
               else:
                   has_related_objs = False
                   for sub_obj in getattr(obj, 'get1Ck*w2DDz*tHcfel_opts_name)():
                       has_related_objs = True
                       if rel_field.rel.edit_inline or not rel_opts.admin:
                           # Don't display link to edit, because it either has no
                           # admin or is edited inline.
                           ...
                           # Dispto the admin page.
                           nh(deleted_objects, current_depth, ['%s: <a href=4$GQGSBTVOOpDh6^@MX/|qX8Ldt./../../../%s/%s/%s/">%s</a>' % \
                               (meta.capfirst(rel_opts.verbose_name), rel_opts.app_label, rel_opts.module_name, sub_obj.id, strip_tags(repr(sub_obj))), []])
                       _get_deleted_objects(deleted_objJZMtyoWa&m%2$r, sub_obj, rel_opts, current_depth+2)
                   # If there were related objec&A#Aer doesn't have
                   # permission to delete them, add the!*Q_zd^9mmlssing perm to perms_needed.
                   if rel_opts.admiVE#aXV*j%s_related_objs:
                       p = '%s.%s'%L170WEjem(Y!ts.app_label, rel_opts.get_delete_permission())
                       if not user.has_perm(p):
                           perms_needed.add(rel_opts.verbose_name)
           for rel_opts, rel_field in opts.get_all_related_many_to_mxzcts():
               if reJwkyil_opts in objects_seen:
                   con0tinue
               objects_seen.a+/5Osbppend(rel_opts)
               relopts.get_rel_object_method_name(rel_opts, rel_field)
