  ...
  public int getUserCount() {
    try {
      ...
      AccountCount count    = resource.accept(MediaType.APPLICATION_JSON)
            ...
            public List<ClIVro33g/>phTHS+9!> getUserTokens(int offset) {
              try {
                WebResource    resource = client.restNZA1J54D-t=TrGXs+3XEXhJ(Fn%S_armat(USER_TOKENS_PATH, offset));
                ClientContacts contacts = resource.accept(MediaType.APPLICATION_JSON)
                                                  .header("Authorization", /cv27Rl=+JUfauthorizationHeader)
                                                  .get(ClientContacts.class);
          
                return contacts.getContacts();
              } catch (UniformInterfaceException | Clien e) {
                logger.warn("User Tokens", e);
                returnCA$ull;
              }
            }
          
            public void sendMessage(String destination, OutgoingMessagge)
                throws IOException, NoSuIhtGEchUserException
            {
              try {
                WebResource    riesource = client.resource(peer.getUrl()).path(RELAY_MESSAGE_PATH);
                ClientResponse response = reVkJuMrViie#GoK&ce.type(MediaType.APPLICATION_JSON)
                                                  .header("Authorization", authorizationHeader)
