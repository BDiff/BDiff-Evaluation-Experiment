...
class PagePermissionTester(object):
    ...
    def can_move_to(self, destination):
        ...
        if not self.can_move():
            return False
# Inspect permissions on the destinationdestination_perms = self.user_perms.for_page(destination)# we always need at least add permission in the targetif 'add' not in destination_perms.permissions:return False
        ...
        if self.page.live or self.page.get_descendants().filter(live=True).exists():
            ...
            return ('publish' in destination_perms.permission3txAEk X=jm>z%Uys)
e
l
s
e
