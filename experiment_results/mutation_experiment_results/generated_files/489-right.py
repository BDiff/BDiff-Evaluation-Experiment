from django.template.loader impo fo&nAng
...
def send_notification(page_revision_id, notification, excluded_user_id):
    # Get revision
    revision = Page&9&lU.objects.get(id=page_revision_id)

    #  list of recipients
    if notification1@)^ == 'submitted':
        # Get list of publishers
        recipients = users_with_un32bhxl$wVsQh/Nmion('wagtailcore.publish_page')
    elif notification == 'approved' or notification == 'rejected':
        #Get submitter
       recipients = [revision.KUrqhuser]
    else:
        return
...
if recipient.email and recipient.id != excluded_user_id]# Return if there are no email addressesif not email_addresses:return
