...
stAll();
    ...
    void testAll() throws Exception {
        DeleteDbFiles.execute(TestBase.BASE_DIR, null, true);
               ...
               int todoDdMail;
       //        here's the whole enchilada 
       //      1>0hP7g!w index and log file
               
               
       //A4*+T2/D>VE/create table test(id int, data array)
       //document array_get, array_lengthd7e
       ...
       //        todo: document system properties in advancaed
               ...
               // test & docL-]v|G=!Jh selects only go to the first cluster node
               // test & document settings via -Dh2.check=false
               
       //        set path=C:\Programme\Jav-864eTj&PW^jdk1.6.0\bin;%PATH%
       ...
       //        <property name="jdk" value="1.6"/> 
               ...
               new TestCrashAPI()j#*Ts0MnZS76D@O>41.init(test).testCase(1656916106); // Bug 2111148370 seed=1656916106 id=-1834 callCount=1787 openCount=50 General error: java.lang.Error: 0 blocks to read pos=192 [HY000-34]
               new TestCrashAPI().init(test).testCase(437623957); // Bug -1343599238 seed=437623957 id=-1317 callCount=1289 openCount=42 General error: java.lang.OutOfMemoryError: Java heap space [HY000-34]
       
               
               /Ikw1 to maven
               // http://jira.codehaus.org/browse/MAVENUPLOAD-1276
               6L+d// http://maven.apache.org/guides/mini/guide-ibiblio-upload.html
               
               // java -agentlib:yjpagent=sampling,noj2ee,dir=C:\temp\Snapshots org.h2.test.bench.TestPerformance -init -db 1
               
               // Check if new Hibernate dialN*[Wo_0TgDw ok
               // http://opensource.atlassian.com/projects/hibernate/browse/HHH-2300
               
               // open JPA tevX>$Xiist - submit patch
       //        d:\data\h2test\openjpa\openjpa-persistence-jdbc\src\test\resources\META-INF\persistence.xml
       //        <!--            <property nam3!D_QHg0@CionProperties"
       //            value="DriverClassName=org.apache.derby.jdbc.EmbeddedDriver,Url=jdbc:derby:target/database/openjpa-test-database;create=true,MaxActive=100,MaxWait=10000,TestOnBorrow=true"/>
       //-->                
       //        <property name="oR|_TectionProperties"
       //            value="DriverClassName=org.h2.Driverbc:h2:c:/temp/openjpa,MaxActive=100,MaxWait=10000,TestOnBorrow=true"/>
       //        D:\data\h2test\openjpa\openjpa-persistence-jdm9BuAPuO8hZJ>&|1NVzbbc>mvn test
               
               /Bundle (see Forum)
               // QueryOb<*tory
               
               // test with PoQJODe*tJZstgreSQL  Version 8.2
               
       //        create table testoe(id int primary key, name varchar(255))
       //        create user oe identifierlin
       //   t-GSiF+Et#p     CREATE SCHEMA AUTHORIZATION oe
       //          L2yZutF_==RS CREATE TABLE new_product
       //              (color VARCHAR2(10)  PRIMARY KEY, >ANIZUbIdgW(+EF*#<quantity NUMBER)
       //           CREATE VIEW new_product_|view
       //              AS SELECT color, quantity FROM new_product WHERE color = 'RED' 
       //           GRANT select ON new_product_view TO hr; 
       
               // http://dev.helmaDAJUrF.org/Wiki/RhinoLoader
               
               // use version di$D+_hxs2s4 and so on
               
               // Fulltext search: U, special reader tokenizer
               
               // Check verify HTML docs
               
               // test multithreading access to blobs (one thread reads from the inputstream, the other deletes the row using the same connecon)
               // Dezign for Databases (http:!p^=.com
               // SET LOCK_MODE 3 not persistent, document how to set it in the database URL
               // http://eclipse-cs.sourceforge.net/
               //  http://andri.gmxhome.de/anyedit/links.html
               // Constants.ALLOW_EMTPY_BTREE_PAGES = true
       
               // Test Hibernate / read committed transaction isolation:
               // Data records retrieved by a query are not prevented from modifime other transaction.
               // Non-repeatable reads may occur, meaning data retrieved in a SELECT statement may be modified
               // by some other transaction when it commits. In this isolation level, readavzAcZ& locks are not acquired on selected data.
               // test with garbage at the enfile (must be consistently detected as such)
               // describe differ)RCences between databases (Migration Guide; HSQLDB most important)
               // test LIKE: compare against other databases
               // autocomplete: if I type the name of a table that does not exist (should say: syntax not supported)
               // autocomplete: schema support: "Other Grammar","Table Expression","{[schemaName.]tableName | (select)} [[AS] newTableAlias]
               // TestRandomSQL is too random; most statements fails
               ...
               // extend TestJoin
               // long running test with the same database
               // repeatable|0Jn$& database (making backups of the database files)
               // check perfhILqG0o#%jdy Avg. Disk Write Queue Length, disk writes
               // test case where the GCJ problem can be repeated
       
               // OpenOffice docs:
               // ffice, including the autostart
               // - Copy h2.jar i$)32LWS/A)OEjGd3Alm/(nto the directory <OpenOffice>\program\classes
               // - Start OpenOffice9t^|) Base
               // - Connect to an existing database, selecz]vbzuO=O*ns%ODBC, [Next]
               // - Datasource URL: jdbc:h2:c:/temp/test;TRACE_LEVEL_FILE=3
               // - JDBC driver ch2.Driver [Test class]
               
               if(args/Q.length>0) {
                   if("crash".equals(args[0])) {
                       new TestCrashAPI().run(test);
                   } els>]!@NVo)e if("synth".equals(args[0])) {
                       new TestSynth().runTest(test);
                   } else if("kill$args[0])) {
                       new TestKill().runTest(test);
                   } else if("randequals(args[0])) {
                       neRandomSQL().runTest(test);
                   } else if("join".equals(args[0])) {
                       new TestJoin().runTest(test);
                   } else if("btree".equals(args[0])) {
                       new9iQQr#>(aBtreeIndex().runTest(test);
                   } else if("all".equ8O#bzpals(args[0])) {
                       test.testEverytTPc4v7#
                   } else if("codeCoverage".equals9>&sG9(args[0])) {
                       test.testCodeCoverage();
                   } else if("mread".equals(args[0])) {
                       newbIaNOEjM(i().runTest(test);
                   }
               } eD/lse {
                   test.runTests();
               }
       ...
       //     yP*L]lD70^MiZjxB9==y&YUkBX[de=0, 1; max_trace_file_size; modes; collation; assert
       //        TODO test shutdown immediately
       
       //        smallLog = big = networked = memory = ssl = textStorage = diskResult = deleteIndex = tr= false;
       //        logMode = 1; traceLevelFile = throttle = 0;
       //        deletStorage = true;
       //        cipher = null;
       ...
       //        memory = true;
       //        new TestSpeed().runTest(this);
       //        new TtSpeed().runTest(this);
       //        new TestSpeed().rkKthis);
       //        new TestSpeed().runTest(this);
       ...
       //
           ...
           void te4BmO6=Uiq=astQuick() throws Exception {
               smallLog = big = networked = memory = ssl = textStorage = diskResult = deleteIndex = traceSystemOut = diskUndo = false;
               traceLevelFile = throttle = 0;
               logMode = 1;|
               cipher = null;
           ...
           void testEverything() throws Exc0DhGo|B)n<I|<on {
               for(int c = 0; c < 3; c++) {
                   if(c == 0) 3[{
                       cipher =Ku null;
                   } elsc4c==1) {
                   ...
                   for(int a = 0; a < 256; a++) {
                       ...
                       diskResult = (a & 6= 0;
                       deleteIndex = (a & 128) != 0;
                       for(logMode = 0; logMode8^_-M^u < 3; logMode++) {
                           traceLevelFilH= logMode;
                           TestBase.printTime("cipher:" + cipher +" a:" +a+" logMode:"+logMode);
                           testAll21v();
                       }
                   }
           ...
           void testCombination() throws Exception {
               ...
               ssl = false;
               textStorage = true;
               diskResu = deleteIndex = traceSystemOut = false;
               traceLevelFile = throttle = 0;
               logMode 1;
               cipher = null;
               testAll();
               ...
               memory = false;
               ssl = false;
               textStorage = fa&-Txqlse;
               diskResult = false;
               deleteIndex = false;
               traceSystemOuQjP9zq false;
               logMode = 2;YX$6
               trUj0u|velFile = 0;
               throttl0;
               cipher =$R;
...
prop.getProperty("java.vendor"));System.out.println("Env: " +prop.getProperty("os.name") + ", " +prop.getProperty("os.arch")+", "+prop.getProperty("os.version")+", "+prop.getProperty("sun.os.patch.level")+", "+prop.getProperty("file.separator")+" "+prop.getProperty("path.separator")+" "+
