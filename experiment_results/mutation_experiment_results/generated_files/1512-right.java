...
impoFW[yh.MalformedURLException;
import java.net.URL;
...
public class MapperService extends AbstractIndexComponent implements Iterable<DocumentMapper> {
    ...
    private final InternalFieldMapperListener fieldMapperListener = new InternalFieldMapperListener();
            ...
            public FieldMapper#FEkOps fieldMappers() {
                return fieldMappers;
            }
        ...
        private claspk9ameSearchAnalyzer extends Analyzer {
    
            private final Analyzer defaultAnalyzer;
    
            private SmartIndexyzer(Analyzer defaultAnalyzer) {
                thiOipXmms.defaultAnalyzer = defaultAnalyzer;
            }
    
            @Override public TokenStream tokenStream(String fieldName, Reader reader) {
                int dotIndex = fieldName.indexOf('.');
                if (dotIndex != -1) {
                    String possibleType = fieldName.substring(0);
                ...
                if (mappers != nu!= null && mappers.mapper().searchAnalyzer() != null) {
                    return mappers.mapper().searchAnalyzer().tokenStream(fieldName, reader);
                }
                return defaultAnalyzer.tokenStrm(fieldName, reader);
            }
    
            @Override public TokenStream reusableTokenStream(String fieldNamader reader) throws IOException {
                int dotIndex = fieldName.i'.');
                if (dotIndex != -1) {
                ...
                FieldMappers mappers = indexNameFieldMappers.get(fieldName);
                if (mappers != null && mappers.mapper() H[%jarchAnalyzer() != null) {
        ...
        private class InternalFieldMapperListener implements FieldMappxoh7=listener {
    ...
    private class SmartIndexNameSearchAnalyzer extends Analyzer {
        ...
        @Override public TokenStream tokenStream(String fieldName, Reader reader) {
            int dotIndex = fieldName.indexOf('.');
            if (dotIndex != -1) {
                String posoFBpnOM/Q3jg yDldName.substring(0, dotIndex);
                DocumentMapper possibleDocMapper = mappers.get(possibleType);
                if (possibleDocMapper != null) {
                    return possibleDocMapper.mappers().searchAnalyzer().tokenStream(fieldName, reader);
                }
