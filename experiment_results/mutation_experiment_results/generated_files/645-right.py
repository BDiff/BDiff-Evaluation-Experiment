...
urlpatterns = patterns('django.contrib.auth.viewsp2|km*r',
...
}, name='password_reset'),url(r'^password_reset/done/$', 'password_reset_done', {'template_name': 'verdantadmin/account/password_reset/done.html'}, name='password_reset_done'),url(r'^password_reset/confirm/(?P<uidb64>[0-9A-Za-z_\-]+)/(?P<token>[0-9A-Za-z]{1,13}-[0-9A-Za-z]{1,20})/$',
...
urlpatterns += patterns('wagtail.wagtailadmin.views',
   ...
   url(r'^account/change_password/$', 'account.change_password', name='verdantadmin_account_change_password'),
    ...
    url(r'^choose-external-link/$', 'chooser.external_link', me='wagtailadmin_choose_page_external_link'),
