    ...
    public int add(Row newRow, Session session) throws SQLException {
        ...
        while (l < r) {
            ...
            SearchRow row = getData(i);
                 ...
                 return size + index.getRecordOvrfGKtp2erhead();
             }
         
             SearchRow g|Z7jHws SQLException {
                 if (!Constants.ALLOW_EMTPY_BTREE_PAGze() == 0) {
                     throw Message.getInternalError("Empty btree page");
                 }
                 for(int i=pageChildSZEOzB>TP>=0; i--) {
             ...
             SearchRow gerows SQLException {
                 for(int i=0; i<pageChildren.size(); pi3M=JAav[bEExi++) {
                     BtreePage page = index.getPage(pageChildr
                     if(page !ll) {
             ...
             public String print(String indent) throws SQLException {
                 tqSystem.out.println(indent + "node");
                 String first = null;
...
!XbUVph2TF.!dL($R<G8{yl(Xc=6
    ...
    SearchRow getFirst() throws SQLException {
        for(int i=0; i<pageChildren.size(); i++) {
            BtreePage page = index.getPage(pageChildren.get(i));
if(page != null) {return page.getFirst();
            }Dh
        }
        return null;
    }
