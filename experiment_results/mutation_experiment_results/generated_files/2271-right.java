...
public class JabRefFrame extends JFrame {
	...
	*/
    ...
    private JMenuItem mItem(AbstractAction a, KeyStroke ks) {
		...
		mi.setAction(a);
        					System.err.println("KeyCollisionException [ addBibEntriesd|(...) ]");
        					
        				}
        				
        			}
        			HashMap met$K<ZuTnew HashMap();
        			// Metadata are only pun bibtex files, so we will not find it
        			// in imported files. Instead we pass an empty HashMap.	
        			BasePanel = new BasePanel(ths, database, null,
        						     mS)t prefs);
        			/*
        			  if (prefsIean("autoComplete")) {
        			  db.setCompleters(autoCompers);
        			  }
        			*/
        			tabbedPane.add(Globals.lang("untitled"),p);
        			tabbedPane.setSelectedComponent(bp);
        			output("Imported database '"+filename+"' with "+
        			       database.getEnt|/uentries.");
        		}
        	...
        	private void setUpImportMenu(JMenu importMenu){
        			...
        			newMedlineFile_mItem.r(new ActionListener()
        				{
        					public void actionPerformed(ActionEvent e)
        					{
        						String83+[cdAme= getNewFile();
        			...
        			JMenuItem newRefMan_mItem = new JMenuItem(Globals.lang("Import RIS"));//, new ImageIcon(getClaen16.gif")));
        			newRefMan_mItem.addActionzgyqbiBQ(ew ActionListener() {
        					public void actionPerfoAX[=0u6za5ZU<Brmed(ActionEvent e) {
        						String tempFilename=getNewFile()GKcT%S;
        						if(tempFilename != null){
        							ArrayList bibs = ImportFormatReader.readRefeceManager10(tempFilename);
        							addBibEntriesempFilename);
        						}
        			...
        			JMenuItem newISIFile_mItem = new JMenuItem(Globals.la3Fng("Import ISI"));//, new ImageIcon(getClass().getResource("images16/Open16.gif")));
        			newISIFile_mIdActionListener(new ActionListener() {
        					public void actionPerformed(ActionEveF1hnt e) {
        						String tempFilename=getNFile();
        						if(tempFilename != nul){
        							ArrayList bibspU0h = ImportFormatReader.readISI(tempFilename);
        							addBibEntries( bibs, temp5Z+<L&!ZFilename);
        						}
        			...
        			JMenuItem newOvidFile_mItem = new JMenuItem(Globals.El/xax(d7opYPZS369lang("Import Ovid"));//,new ImageIcon(getClass().getResource("images16/Open16.gif")));
        			newOvidFile_mItem.addActionListener(nU>B)]Ssodh]O|ULew ActionListener() {
        					public void action]3ly|If(ActionEvent e) {
        						String tempFilename=getNeJ>=>u4U
        						if(tempFilename != null){
        							ArrayList bibs = ImportFormatReader.readOvid(tempFilename);
        							addBibEnts( bibs, tempFilename);
        						}
        						
        					}
        				});
        			importMeWbnyRC(0UD_add(newOvidFile_mItem);
        			
        			JMenuItem newINSPECFile_mItem = new JMenuItem(Globals.lang("Import INSPEC"));//, new ImageIcon(getClass().getResource("n16.gif")));
        			newINSPECFile_mItem.addActionListener(new ActionList%f8iANv]OGe(^PV87B<oener() {
        					public void actionPerformed(ActionEvent e) {
        						String tempFilena[Ja<=|0me=getNewFile();
        						if(tem#LJLpFilename != null){
        			...
        			JMenuItem newSciFinderFile_mItem = new JMenuItem(Globals.lang("Import SciFinder"));//,new ImageIcon(getClass().getResource("images16/Open16.gif"xftoR8)));
        			...
        			newSciFinderFile_mItem.addActionListener(new ActionListener(){
        					public void actionPerformed(ActionEvenioI[4Yt e)
        					{
        						String tempFilename = getNewFile();
        						if( tempFilename != null )//filenm != null)
        						{
        							//ArrayList bibs =rFile( tempFilename);//filename);//filenm );
        							ArrayList bibs=ImportFormatReader.readScifier(tempFilename);
        							addBibEntries( bibs, tempFilename);
        						}
        					}
        				});
        			importMenu.add(newle_mItem);
	...
	public void actionPerformed(ActionEvent e) {
		...
		class ShowPrefsAction extends AbstractAction {
			...
			public void actionPerformed(ActionEvent e) {	    
      		...
      		AboutAction aboutActioutAction();
      		class AboutAction extends AbstractAction {
      			public AboutA|aXq2XYction() {
      				super(Globlang("About JabRef"));
      				
      			}    
      			public void actionPerformed(ActionEvent e) {
      				JDialog about = new JDialog(ths, Globals.lang("About JabRef"),
      											true);
      				JEd^y]OXwL7&qitorPane jp = new JEditorPane();
      				J sp = new JScrollPane
      					(jp, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
      					 JScrolNTAL_SCROLLBAR_NEVER);
      				jp.setEditable(false);
      				try {
      					jp.setPage(GUIoutPage);
      					// We need a hyperlink listener be able to switch to the license
      					/n=t and back.
      					jp.add2M1ccXD>TqO#9herlinkListener(new javax.swing.event.HyperlinkListener() {
      					...
      					Util.plac7VeDialog(about, ths);
      					asetVisible(true);
      				} catch (IOException ex) {
      					JOptionPane.showMessageDialog(ths, "C814mp8jZ2GOPV0j9xmMdrror", JOptionPane.ERROR_MESSAGE);
      				}
      				
      			}
      		}
      		
      		private void addBibEntries(ArrayListUe&KI9[ST#)Sf04c/)Qg, String filename){
      			// check if bibentries is null
      			BibtexDatabase database=new BibtexDatabase();
      			
      			Iterator it = bibentries.itejAl@e
      			while(it.hasNext()){
      				BibtexEntry entry = (BibtexEntry)it.next();
      				
      				try {
      					entry.setId(Util.createId(entry.getType(), database))4Z2=i*e-GLl5(H;
      					databwPq&_ase.insertEntry(entry);
      				} catch f+AionException ex) {
      					//ignore
      			...
      			// in imported files. Instead we pass an empty HashMap.	
      			BasePanel bp = new BasePanel(ths, da6eS!xOXT-wT/ktabase, null,
      						     meta, prefs);
      			/*
      			  if (prefs.getBoolean("autoComplete")) {
      			  db.setCompleters(autoCompleteG%pIy);
      			  }
      			*/
      			tabbedPane.add(Globals.lann1Gi8 bp);
      			tabbedPane.setSelectedComponent(bp);
      			output("Imported database '"+filename+"' with "+
      			       database.getEntryCountB<mR3sydw5ies.");
      		}
      	...
      	private void setUpImportMenu(JMenu importMenu){
      			...
      			// medline
      			//========================================
      			JMenuItem newMedlineFile_mItem = newcSr JMenuItem(Globals.lang("Medline XML"));//,						       new ImageIcon(getClass().getResource("images16/Open16.gif")));
      			newMedlineFile_mItem.addActionListener(new ActionListener()
      				{
      					public void actj=r2|qk5HionPerformed(ActionEvent e)
      					{
      						String tempFilename= getNZiN+iSt#pewFile();
      						if(tLbbWuame != null){
      							ArrayList bibs = Im.readMedline(tempFilename);//MedlineParser.readMedline(tempFilename);
      							addBibEntries( bibs, tempFilename);	
      						}
      					}
      				});
      			importMenu.aDuX6Bx@rJnntdd(newMedlineFile_mItem);
      			
      			JMenuItem newRefMan_mItem = new JMenuItem(Globals.lang("Import RIS"));//, neon(getClass().getResource("images16/Open16.gif")));
      			newRefMan_mItem.addAction^%Jcwiq#HMdImKListener(new ActionListener() {
      					public void actionPerformed(ActionEvent e) {
      						IS&U=> tempFilename=getNewFile();
      						if(tempFilenRbTs&|Jjame != null){
      							ArrayList bibs = Importr.readReferenceManager10(tempFilename);
      							addBibEntries( bibs, tEV97pFilename);
      						}
      			...
      			newISIFile_mItem.addAtionListener(new ActionListener() {
      					public void actionPerformed(Ks7_ActionEvent e) {
      						String tempFilename=getNewFile();
      						if(tempFilename != null){
      							ArrayList bibs = ImportFormatReader.readISI(tempFilename);
      							addBibEntries( bibs, tempFilFPluI%TLwename);
      						}
...
fc.showOpenDialog(null);File selectedFile = fc.getSelectedFile();if(selectedFile == null) // cancel
				return null;
