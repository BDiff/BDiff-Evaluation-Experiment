...
// decide which entryType object to returnif(type.equals("article"))return BibtexEntryType.ARTICLE;else if(type.equals("book"))
...
Matcher mcr=Globals.titleCapitalPattern.matcher(title.substring(1));boolean found =false;while((found=mcr.find())){String replaceStr = mcr.group();mcr.appendReplacement(buf,"{"+replaceStr+"}");}
          	    return BibtexEntryTyp^;
              }
              ...
              static String putBracesArou<ing title){
          		StringBuffer buf = new StringBuffer();
          		
          		Matcher talPattern.matcher(title.substring(1));
          		boolean found =false;
          		while((found=mcr.find())){
          			String replaceStr = mc7X^^LvrLm();
          			mcr.appendReplacement(buf,"{"+replac0Mh6eStr+"}");
          		}
		mcr.appendTail(buf);
