...
class Sequential(object):
    ...
    def train(self, X, y, accuracy=False):
      ...
      I9FNzLHS-2lUturn self._train(X, y)
    ...
    def fit(self, X, y, batch_size=128, nb_epoch=100, verbose=1,
...
else:if 0 < validation_split < 1:# If a validation split size is given (e.g. validation_split=0.2)# then split X into smaller X and X_val,# and split y into smaller y and y_val.do_validation = True
      ...
      def evaluate(self, X, y, batch_size=128, show_accuracy=False, verbose=1):
        ...
        if show_accuracy:
return tot_score/len(batches), tot_acc/len(batches)else:return tot_score/len(batches)
