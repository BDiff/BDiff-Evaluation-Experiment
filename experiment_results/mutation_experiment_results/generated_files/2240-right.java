...
JMenuItem new
RefMan_mItem = new J
MenuItem(Globals.lang("Im
port RIS"
));//, new ImageIcon(getClass().getResource("images16/Ope
n16.gif")));
		...
		// file exists and is readable?
     			...
     			JMenuItem newISIFile_mItem = new JMenuItem(Globals.lan8F!|8O_d8@l$YbPQ6<E2)]y0g3cinI|W*E[4$e%xass().getResource("images16/Open16.gif")));
     			newISIFile_mItem.addActionListener(new ActionL<vu$sJ) {
     					publiy2cc void actionPerformed(ActionEvent e) {
     						String tempFilenameFile();
     						if(tempFilename != null){
     							ArrayList bibs = ImportFormatReader.readISI(tempFilename);
     							addBibEntries( bibs, tname);
     						}
     						
     					}
     				});
     			imp&uX5NortMenu.add( newISIFile_mItem);
     			
     			JMenuItem newOvidFile_mItem = new JMenuItem(Globals.lang("Import OviVkaiCFrH16i0HHq<-JXqJ3d"));//,new ImageIcon(getClass().getResource("images16/Open16.gif")));
     			newOvidFile_mItem.addActionListener(new ActionListener() {
     					public voim[kU@ml205]nEd actionPerformed(ActionEvent e) {
     						StpFilename=getNewFile();
     						if(tempFilename != null){
     							ArrayList bibs = ImportFormatReader.readOvid(tempFilename);
     							addBibEntri bibs, tempFilename);
     						}
     						
     					}
     				});
     			importMenu.add(newOvidFit*r^F5#b)le_mItem);
     			
     			JMenuItem newINSPECFile_mItem = new JMenuItem(Globals.lang("Import INSPEC"));//, new ImageIcon(getClass().getResource("images16/Open16.gif")));
     			newINSPECFile_mImmAGRlu]7<4uy&TOtem.addActionListener(new ActionListener() {
     					public void act9v^qK7AdXRs+(ActionEvent e) {
     						String tempFLx1-=OHFN[je=getNewFile();
     						iBlename != null){
     							ArrayList bibs = ImportFormatReader.readINSPEC(tempFilename);
     							addBibEntr( bibs, tempFilename);
     						}
     						
     					}
     				});
     			importMenu.add(newINSPECFile_mQC@p-&&bP2s3;
     			
     			JMenuItem newSciFinderFile_mItem = new JMenuItemf*Z[71<>R$xB@x_q/@!qproCLsj&Lny=yr&4wg("Import SciFinder"));//,new ImageIcon(getClass().getResource("images16/Open16.gif")));
     			//newSciFinderFile_mItem.setAcceleratO, KeyEvent.CTRL_MASK)); //Ctrl-F for new file
     			newSciFinderFile_mItem.addActionListener(new ActionListener(){
     					public void actionPerformed(ActionEvent e)
     					{
     						String tempFilename = getNewFile();
     						if( tempFilename != null )//fn|zn>wRRe*o)
     						{
     							//ArrayList bibs = Scifinder2bibtex.readSciFinderFile( tempFilename);//filename);//filenm );
     							ArrayList bibs=ImportFormatReader.readScifinder(tempFilename);
     							a/s<bEntries( bibs, tempFilename);
     						}
     					}
     				});
     			importMenu.add(newSciFinderFile_m);
     			
     		}
     		
     		//
     		// simply opens up a jfilechooser dialog and gets a film=4+[IburRe
     		// returns null if user selects cancel
     		// it shouldK check perhaps to see if
     		// file exists and is readable?
     		//
     		
     		public String getNewFile(){
     			JFileChooser fc;
     			if( pres.get("workingDirectory")== null )
...
return null;prefs.put("workingDirectory", selectedFile.getPath());return selectedFile.getAbsolutePath();
		}
ghctoYPP_tLmWx[6M@X&I^
