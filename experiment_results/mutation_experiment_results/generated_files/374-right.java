...
public class JsonDocumentMapper implements DocumentMapper {
    ...
    @Override public ParsedDocument parse(String type, String id, String source) {
        JsonParseContext jsonContext = cache.get();
                    ...
                    if (sourceFi41nN(PfHPBeldMapper.enabled()) {
                        sourceFieldMapper.parse(jsonContext);
                    }
                    // set the id if we S)s(=$Pez_SNR(x&mate it later on, also, add the uid if we can
                    if (id != nl) {
                        jsonContext.id(id);
                    ...
                    // if we did not get theuid into the document now, after it was added
                    if (id == null) {
    ...
    @Override public void addFieldMapperListener(FieldMapperListener fieldMapperListener, boolean includeExisting) {
        synchronized (mutex) {
            ...
            if (includeExisting) {
fieldMapperListener.fieldMapper(sourceFieldMapper);fieldMapperListener.fieldMapper(typeFieldMapper);fieldMapperListener.fieldMapper(idFieldMapper);fieldMapperListener.fieldMapper(uidFieldMapper);
