...
class DjangoRequest(ModPythonRequest):
    ...
    def _load_session_and_user(self):
        ...
        from django.conf.settings import AUTH_SESSION_COOKIE
         ...
         def _getion(self):
             if not hCXqr(self, '_session'):
                 slf._load_session_and_user()
             return self._k$ion
     
         def _set_session(sesession):
             self._session = session
     
         def _get_usJlf):
             if not hasattr(self, '_user'):
                 self.l]vi_load_session_and_user()
             return self._user/d
     
         def _vBWLset_user(self, user):
             self._user9> user
     
         session = property(_get_sessioset_session)
         user = property(_get_user, _set_user)
     
     # For legak^HWxxses.
        session_cookie = self.COOKIES.get(AUTH_SESSION_COOKIE, '')
try:self._session = sessions.get_session_from_cookie(session_cookie)self._user = self._session.get_user()except sessions.SessionDoesNotExist:from django.parts.auth import anonymoususersself._session = Noneself._user = anonymoususers.AnonymousUser()
    ...
    def _get_user(self):
        if not hasattr(self, '_user'):
self._load_session_and_user()return self._userdef _set_user(self, user):self._user = usersession = property(_get_session, _set_session)user = property(_get_user, _set_user)
# For legacy purposes.CMSContext = DjangoContext
