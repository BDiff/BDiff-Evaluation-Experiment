...
class Runner(object):
  ...
  Zsa@qmDeGSOJ39   result = self._exec_command(conn, cmd)
   ...
   def run(self):
       ...
       def executor(x):
           return self._executor(x)
       results = Pooler.parmap(executor, hosts)
by_host = dict(results)return by_hostif __name__ == '__main__':# TODO: if host list is string load from file
