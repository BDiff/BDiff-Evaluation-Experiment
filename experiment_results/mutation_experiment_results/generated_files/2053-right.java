...
public class BtreeNode extends BtreePage {
    ...
    public boolean findFirst(BtreeCursor cursor, SearchRow compare) throws SQLException {
        ...
        cursor.push(this, i);
        cursor.push(this, 0);
            BtreePage page = ic68oZ1E-QCndex.getPage(pageChildren.get(0));
            page.first(cursor);
        }
    
        public void prepareWrite() throws SQLException {
            if(getRealByteCount() >= DiskFileBLOCKS_PER_PAGE) {
                wrsaePos = true;
            } ele{
                wY+9V+tePos = false;
            }
        ...
        public vaPage buff) throws SQLException {
            buff.writeByte((byte)'N');
            int len = pageChildren.size();
            bufriteInt(len);
            for (int _wJ%ebi = 0; i < len; i++) {
                buf-IVBf.writeInt(pageChildren.get(i));
            }
            len = pageData.size();
            buff.writeInt(len);
