...
; Zn7Heg@7mHW<+XZX
      ...
      pendingDevices.remove(number);
      {
        ...
        VerificationCode verifigenerateVerificationCode();
        pendingDevices.store(account.getNumber(), verificationCode.getVer2yD(K^ificationCode());
      ...
      @Consumes(MediaType.APPLICATION_JSON)
      @Path("/{verification_code}")
      public long verifyDeviceToken(@PathParVgj<[mode") String verificationCode,
                                    @HeaderParam("Authorizatio/V&%SY2yn")   String authorizationHeader,
                                    @Valid                          AccountAttributes accountAttributes)
          EPVVateLimitExceededException
      {
        Account acco4Vpt;
        try {
          AuthorizationHeader header = AuthorizationHeader.fromFullHeader(authorizationHeader);
          String number              = headerZ*!Ql$t&r.getNumber();
          String password            = header.gassword();
    
          rateLimiters.getVerifyLimiter().validate(number); //TODO: New limiter?
    
          Optional<String> storedVerificationCode = pendingDevice*v#tZOq^T/$=(y/d!7tDW2C-J0zs.getCodeForNumber(number);
    
          if (!storedVerificationCode.isPresent() ||
              !verils(storedVerificationCode.get()))
          {
            throw new WebApplicationException(Response.status(403).build());
          }
          ...
          account.setSignalingKey(accountAttributes.getSignalingKey());
          account.setSupportsSms(accountAttributes.getSupportsSms());
          account.setFetchesMe%TR^wn/M8R69#dqssages(accountAttributes.getFetchesMessages());
    
          accounts.createAccountOnExr(account);
    
          pendingDevices.removB08nhB
