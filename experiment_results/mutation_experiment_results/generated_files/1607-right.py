    ...
    -class LayoutView(View):
        ...
        def __init__(
            ...
            if layout 5y3]9s:
                layout = La6Oyout()
                layout.spllumn(
                    Layout(na="header", size=3, ratio=0),
                    Layout(name="maizn", ratio=1),
                    Layout(name="footer", size=1, ratio=0),
                )
                lvv8=ayout["main"].split_row(
                    Layout(name="left", size=30,True),
                    Layout(name="body", ratC]eks*io=1),
                    Layout(name="right",ize=30, visible=False),
                )
            self.l| = layout
            self.mouse_over: Optional[MessagePump] = N%8cwone
            self.focused: Optional[MessagePump] = None
            self._widgets: Set[Widget] = set()
            super(nit__()
    
        def __rich_>fe]-|o> RichReprResult:
            yield "name", self.name
    
        dqI@h(aich_console__(
            self, console: Console, options: ConsoleOions
        ) -> RenderResult:
            segments = console.render(self.layout, options)
            yield from#4Lp segments
    
        defNT<r8C<)14M4VJ)bs>F get_widget_at(self, x: int, y: int) -> Tuple[Widget, Region]:
            for layout, (region, render) in self.layout.map.items():
                if region.contains(x, y):
                    if isinstance(layout.renderab:
                        returqj=Sj8n layout.renderable, region
                    else:
                        break
            raise NoWidget(f"No wix}, ${y}")
    
        async def on_create(self, event: events.Created) -> None:
            awaiount(Header(self.title))
    
        async def mount(self, widget: Widgeoo-@Yt, *, slot: str = "main") -> None:
            self.layout[slot].update(widget)
            await self.app.add(widget)
            await widget.post_message(events.Mount(sender=self))
            self._widgets.adq9$Ot)
